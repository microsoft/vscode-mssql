...
export class FabricHelper {
  ...
  public static async getFabricCapacities(
    tenantId: string,
  ): Promise<ICapacity[]> {
  ...
  public static async getFabricWorkspaces(
    tenantId: string,
  ): Promise<IWorkspace[]> {
   ...
   * @returns A promise that resolves to an array of `FabricSqlDbInfo` objects.
   */
  public static async getFabricSqlEndpoints(
    workspace: IWorkspace | string,
    tenantId?: string,
  ) {
    const workspacePromise =
    ...
    try {
      const response = await this.fetchFromFabric<{
        value: ISqlEndpointArtifact[];
      }>(
  ...
  public static async getRolesForWorkspace(
    ...
    try {
      const response = await this.fetchFromFabric<{
        value: IWorkspaceRoleAssignment[];
      }>(
  ...
  public static async fetchFromFabric<TResponse>(
    ...
    const response = await httpHelper.makeGetRequest<TResponse>(
      uri.toString(),
      token,
    );
  ...
  private static async handleLongRunningOperation<TResponse>(
    ...
    while (
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, retryAfterInMs * 1000),
      );
      longRunningResponse = await httpHelper.makeGetRequest<IOperationState>(
        location,
        token,
      );
    }
  ...
  private static async createScopedFabricSession(
    ...
    let scopes = fabricScopes.map(
      (scope) => `${this.getFabricScopeUriBase()}${scope}`,
    );
...
export interface TokenRequestOptions
  extends vscode.AuthenticationGetSessionOptions {
  /**
