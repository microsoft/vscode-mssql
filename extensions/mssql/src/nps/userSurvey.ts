...
export class UserSurvey {
  ...
  private async promptUserForNPSFeedbackAsync(
    surveySource: string,
  ): Promise<void> {
  ...
  private async shouldPromptForFeedback(
    surveySource: string | undefined,
  ): Promise<boolean> {
    const globalState = this._context.globalState;
    ...
    const isCandidate =
      ...
      Math.random() < SELECTION_PROBABILITY;
    ...
    this.registerReducer("submit", async (state, payload) => {
      ...
      if (
        ...
        sendActionEvent(
          ...
          {
            response:
              response === Loc.UserSurvey.submitIssue
                ? "submitted"
                : "not submitted",
          },
        );
