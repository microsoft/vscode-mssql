/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt in the project root for license information.
 *--------------------------------------------------------------------------------------------*/

/**
 * Performance-harness instrumentation facade.
 *
 * This is a thin facade over the unified diagnostics core (src/diagnostics):
 * every Perf.marker call becomes a diagnostic event that
 * routes to whichever sinks are active — the PERF_MODE harness sink (the
 * established wire format, gated on PERF_MODE=1), the Debug Console live tail, and
 * the user-enabled Session Diag store. One emission path, several gates.
 *
 * The existing PERF_MODE wire contract is preserved by PerfModeSink. When no
 * sink is active a marker call costs one array-length check.
 */

import { diag } from "../diagnostics/diagnosticsCore";
import { PerfModeSink } from "../diagnostics/sinks";
import { DataClassification } from "../sharedInterfaces/diagnostics";

export type PerfMarkerPhase = "instant" | "begin" | "end" | "counter";

export interface PerfState {
    perfMode: boolean;
    activationState: "inactive" | "activating" | "activated" | "failed";
    extensionHostPid: number;
    stsPid?: number;
    markersQueued: number;
    markersDropped: number;
}

export interface IPerfTelemetry {
    readonly enabled: boolean;
    marker(
        name: string,
        phase?: PerfMarkerPhase,
        attrs?: Record<string, string | number | boolean | null>,
        correlationId?: string,
    ): void;
    webviewMark(
        mark: {
            name: string;
            timestampUnixNs: string;
            monotonicNs: string;
            attrs?: Record<string, string | number | boolean | null>;
        },
        webviewName: string,
    ): void;
    setActivationState(state: PerfState["activationState"]): void;
    setStsPid(pid: number | undefined): void;
    getState(): PerfState;
    flush(): void;
}

/**
 * Runtime-only feature bucketing. The generated registry remains the source
 * of truth in conformance tests, but is intentionally not imported here: it
 * is a large snapshot and instrumentation is loaded on every activation.
 */
export function featureFor(name: string): string {
    if (name.startsWith("mssql.activate") || name.startsWith("mssql.extension")) {
        return "activation";
    }
    if (name.startsWith("mssql.command")) return "shell";
    if (name.startsWith("mssql.connection")) return "connection";
    if (name.startsWith("mssql.sts")) return "stsLifecycle";
    if (name.startsWith("centralObservability.") || name.startsWith("mssql.central.")) {
        return "centralObservability";
    }
    if (name.startsWith("completions.")) return "completions";
    if (name.startsWith("mssql.queryStudio") || name.startsWith("queryStudio.")) {
        return "queryStudio";
    }
    if (name.startsWith("mssql.queryResults")) return "queryResults";
    if (name.startsWith("mssql.query")) return "query";
    if (name.startsWith("mssql.resultsGrid")) return "resultsGrid";
    if (name.startsWith("mssql.schemaCompare")) return "schemaCompare";
    if (name.startsWith("mssql.schemaDesigner")) return "schemaDesigner";
    if (name.startsWith("mssql.schemaVisualizer") || name.startsWith("schemaVisualizer.")) {
        return "schemaVisualizer";
    }
    if (name.startsWith("mssql.tableDesigner")) return "tableDesigner";
    if (name.startsWith("mssql.oe") || name.startsWith("objectExplorerV2.")) {
        return "objectExplorer";
    }
    if (name.startsWith("sqlDataPlane.")) return "sqlDataPlane";
    if (name.startsWith("sqlLanguage.") || name.startsWith("sqlScripting.")) {
        return "sqlLanguage";
    }
    if (
        name.startsWith("sessionDiag.") ||
        name.startsWith("debugConsole.") ||
        name.startsWith("system.rich.")
    ) {
        return "diagnostics";
    }
    if (name.startsWith("docker.")) return "deployment";
    if (name.startsWith("replay.")) return "replay";
    if (name.startsWith("selfTest.")) return "selfTest";
    if (
        name.startsWith("metadata.") ||
        name.startsWith("metadataStore.") ||
        name.startsWith("metadataCache.") ||
        name.startsWith("mssql.metadata.")
    ) {
        return "metadata";
    }
    if (name.startsWith("driver.")) return "harness";
    return "system";
}

/**
 * Attr-key classification for normal-use capture. Under PERF_MODE the
 * database is synthetic by harness contract, so attrs are diagnostic
 * metadata; in normal use, name-bearing keys are classified so the
 * redaction policy governs them.
 */
const STRUCTURAL_PERF_ATTRS = [
    "action",
    "activeTab",
    "ageMs",
    "appendMsTotal",
    "approxIncluded",
    "approxMs",
    "authKind",
    "availableRows",
    "backend",
    "batchCount",
    "batches",
    "batchId",
    "batchIndex",
    "buildMs",
    "bytes",
    "bytesRemoved",
    "cache",
    "cacheHit",
    "canceled",
    "candidateCells",
    "characters",
    "childCount",
    "clipboardAttempts",
    "clipboardMode",
    "clipboardMs",
    "columnCount",
    "columns",
    "columnStart",
    "connects",
    "copyRoute",
    "count",
    "dataLoss",
    "dataResultSets",
    "derivedBytes",
    "derivedRows",
    "deserializeMs",
    "differences",
    "dimensions",
    "dims",
    "dirsRemoved",
    "dirty",
    "done",
    "durationMs",
    "edgeCount",
    "enabled",
    "encoding",
    "encrypted",
    "errorCode",
    "errors",
    "exactMs",
    "executionKind",
    "expired",
    "failures",
    "features",
    "fetchDecodeMs",
    "fetchedColumns",
    "fingerprintChanged",
    "firstPageIndex",
    "format",
    "formatMs",
    "frames",
    "fromCache",
    "fromDerived",
    "fromSpill",
    "generationChanged",
    "gridPreview",
    "hasMore",
    "hasSelection",
    "hostRowFetches",
    "includeHeaders",
    "includes",
    "inputDelayMs",
    "intervalMs",
    "ioMs",
    "isEdit",
    "k",
    "layer",
    "layerClass",
    "layoutMode",
    "leaseCount",
    "leases",
    "longTasks",
    "matched",
    "materializedPages",
    "materializeMsTotal",
    "memoryBytesBefore",
    "memoryBytesPeak",
    "metadataSession",
    "mode",
    "mounted",
    "ms",
    "msFromWindowReceived",
    "msToAck",
    "msToTerminal",
    "nextIndex",
    "nodeCount",
    "nullCells",
    "nulls",
    "numberOfRows",
    "offline",
    "onError",
    "opCount",
    "operation",
    "operationClass",
    "operationCount",
    "outcome",
    "outputClass",
    "outputRows",
    "ownerKind",
    "p95FrameMs",
    "packedBytes",
    "pageCount",
    "pages",
    "pagesMissing",
    "pagesVisited",
    "partial",
    "partialReason",
    "payloadBudgetBytes",
    "payloadBytes",
    "pendingSpillBytesPeak",
    "phase",
    "planMs",
    "planResultSets",
    "plans",
    "preprocessMs",
    "projected",
    "purpose",
    "ranges",
    "renderedRows",
    "requestedCells",
    "residentPageBytes",
    "responseBytes",
    "resultColumns",
    "resultId",
    "resultRows",
    "resultSetCount",
    "resultSetId",
    "retainedRows",
    "retainedStores",
    "returned",
    "returnedCells",
    "returnedColumns",
    "returnedRows",
    "returnedValueCharacters",
    "rowBudget",
    "rowLimit",
    "rows",
    "rowsMatched",
    "rowsReturned",
    "rowsScanned",
    "rowStart",
    "rpcRequests",
    "rules",
    "sampledLines",
    "scanFree",
    "scannedBytes",
    "scope",
    "selectedCells",
    "selection",
    "sequence",
    "serializeMs",
    "sessions",
    "setvars",
    "shortWindow",
    "skipped",
    "snapshots",
    "source",
    "sourceKind",
    "sourceMode",
    "sourceRowsScanned",
    "sourceValueCharacters",
    "spatialColumns",
    "specDigest",
    "spillBytes",
    "spillDeserializeMsTotal",
    "spillEncoding",
    "spillFileRemoved",
    "spillReadMsTotal",
    "spillReads",
    "spillSerializeMsTotal",
    "spillWriteIoMsTotal",
    "spillWriteMsTotal",
    "spillWrites",
    "start",
    "startIndex",
    "status",
    "steps",
    "stillExecuting",
    "storeDisposed",
    "streamed",
    "streaming",
    "swept",
    "tableCount",
    "targetBytes",
    "terminal",
    "terminalKind",
    "textCharacters",
    "tier",
    "tiles",
    "total",
    "totalColumns",
    "totalRows",
    "transport",
    "transportUnavailableCells",
    "trigger",
    "unavailable",
    "urgent",
    "valueMode",
    "vectorColumns",
    "vertices",
    "visibleMessages",
    "visibleRows",
    "waitedForByRender",
    "windowCache",
    "windowCacheBypasses",
    "windowCacheBytes",
    "windowCacheEvictions",
    "windowCacheHits",
    "windowCacheMaxBytes",
    "windowCacheMisses",
    "windowCacheOversizeSkips",
    "windowCachePeakBytes",
    "windowRows",
    "workspace",
] as const;

export const PERF_ATTR_CLASSIFICATION: Readonly<Record<string, DataClassification>> = {
    ...Object.fromEntries(
        STRUCTURAL_PERF_ATTRS.map((attribute) => [attribute, "diagnostic.metadata"] as const),
    ),
    nodePath: "object.name",
    nodeType: "diagnostic.metadata",
    objects: "diagnostic.metadata",
    objectName: "object.name",
    documentUri: "source.path",
    uri: "source.path",
    messages: "user.text",
    error: "diagnostic.metadata",
    errorClass: "diagnostic.metadata",
    errorNumber: "diagnostic.metadata",
    failed: "diagnostic.metadata",
    commandId: "diagnostic.metadata",
    hasError: "diagnostic.metadata",
    pid: "diagnostic.metadata",
    rafThrottled: "diagnostic.metadata",
    reason: "diagnostic.metadata",
    resultSets: "diagnostic.metadata",
    rowCount: "diagnostic.metadata",
    waitedMs: "diagnostic.metadata",
};

/** Parse the harness repetition id without ever emitting NaN/null on the wire. */
export function parsePerfRepId(value: string | undefined): number {
    if (value === undefined || !/^\d+$/.test(value)) {
        return 0;
    }
    const parsed = Number(value);
    return Number.isSafeInteger(parsed) && parsed >= 0 ? parsed : 0;
}

class PerfFacade implements IPerfTelemetry {
    public readonly enabled: boolean;
    private readonly perfSink: PerfModeSink | undefined;
    private activationState: PerfState["activationState"] = "inactive";
    private stsPid: number | undefined;

    constructor() {
        this.enabled = process.env.PERF_MODE === "1";
        if (this.enabled) {
            const markerUrl = process.env.PERF_MARKER_URL;
            const token = process.env.PERF_CONTROL_TOKEN;
            if (markerUrl && token) {
                this.perfSink = new PerfModeSink(
                    markerUrl,
                    token,
                    process.env.PERF_RUN_ID ?? "unknown-run",
                    parsePerfRepId(process.env.PERF_REP_ID),
                    process.env.PERF_SCENARIO_ID ?? "unknown-scenario",
                );
                diag.addSink(this.perfSink);
            }
        }
    }

    public marker(
        name: string,
        phase: PerfMarkerPhase = "instant",
        attrs?: Record<string, string | number | boolean | null>,
        correlationId?: string,
    ): void {
        if (!diag.anySinkActive) {
            return;
        }
        try {
            const fields: Record<string, { raw: unknown; cls: DataClassification }> = {};
            if (attrs) {
                for (const [key, value] of Object.entries(attrs)) {
                    fields[key] = {
                        raw: value,
                        cls: this.enabled
                            ? "diagnostic.metadata"
                            : (PERF_ATTR_CLASSIFICATION[key] ?? "unknown"),
                    };
                }
            }
            const tags = ["perfMarker", `phase:${phase}`];
            if (correlationId) {
                tags.push("perfCorrelation");
            }
            diag.emit({
                feature: featureFor(name),
                kind: phase === "counter" ? "metric" : "event",
                type: name,
                status: attrs?.["error"] === true || attrs?.["hasError"] === true ? "error" : "ok",
                ...(correlationId ? { traceId: correlationId } : {}),
                ...(Object.keys(fields).length > 0 ? { fields } : {}),
                tags,
            });
        } catch {
            // Instrumentation must never surface into the product.
        }
    }

    public webviewMark(
        mark: {
            name: string;
            timestampUnixNs: string;
            monotonicNs: string;
            attrs?: Record<string, string | number | boolean | null>;
        },
        webviewName: string,
    ): void {
        if (!diag.anySinkActive) {
            return;
        }
        try {
            if (!/^[0-9]+$/.test(mark.timestampUnixNs) || !/^[0-9]+$/.test(mark.monotonicNs)) {
                return;
            }
            const fields: Record<string, { raw: unknown; cls: DataClassification }> = {};
            if (mark.attrs) {
                for (const [key, value] of Object.entries(mark.attrs)) {
                    fields[key] = {
                        raw: value,
                        cls: this.enabled
                            ? "diagnostic.metadata"
                            : (PERF_ATTR_CLASSIFICATION[key] ?? "unknown"),
                    };
                }
            }
            diag.emit({
                feature: featureFor(mark.name),
                kind: "event",
                type: mark.name,
                process: "webview",
                pid: 0,
                epochMs: Number(BigInt(mark.timestampUnixNs) / 1000000n),
                monotonicNs: mark.monotonicNs,
                ...(Object.keys(fields).length > 0 ? { fields } : {}),
                tags: ["perfMarker", "phase:instant", `webview:${webviewName}`],
            });
        } catch {
            // Instrumentation must never surface into the product.
        }
    }

    public setActivationState(state: PerfState["activationState"]): void {
        this.activationState = state;
    }

    public setStsPid(pid: number | undefined): void {
        this.stsPid = pid;
    }

    public getState(): PerfState {
        const state: PerfState = {
            perfMode: this.enabled,
            activationState: this.activationState,
            extensionHostPid: process.pid,
            markersQueued: this.perfSink?.queuedCount ?? 0,
            markersDropped: this.perfSink?.droppedCount ?? 0,
        };
        if (this.stsPid !== undefined) {
            state.stsPid = this.stsPid;
        }
        return state;
    }

    public flush(): void {
        diag.flushAll();
    }
}

/**
 * The singleton perf telemetry surface. `enabled` reflects PERF_MODE exactly
 * as before; marker emission also feeds the Debug Console and Session Diag
 * sinks when those are active.
 */
export const Perf: IPerfTelemetry = new PerfFacade();
