...
export class MssqlProtocolHandler {
  ...
  private async handleConnectCommand(uri: vscode.Uri) {
    ...
    const { profile: foundProfile, score } =
      await this.mainController.connectionManager.findMatchingProfile(
        parsedProfile,
      );
    ...
    if (
      ...
      this._logger.info(
        `Matching profile found for ${uri.query}; connecting...`,
      );
    ...
    await vscode.window.withProgress(
      {
        ...
        title: Loc.Connection.connectingToProfile(
  ...
  private openConnectionDialog(
    connProfile: IConnectionProfile | undefined,
  ): void {
  ...
  private async readProfileFromArgs(
    ...
    const connString = this.fillConnectionProperty(
      connectionInfo,
      args,
      "connectionString",
    );
