  ...
  public async getSchemaDesigner(
    ...
    if (treeNode) {
      ...
      const connectionDetails =
        await mainController.connectionManager.createConnectionDetails(
          connectionInfo,
        );

      await mainController.connectionManager.confirmEntraTokenValidity(
        connectionInfo,
      );
      treeNode.updateConnectionProfile(connectionInfo);

      connectionString =
        await mainController.connectionManager.getConnectionString(
          connectionDetails,
          true,
          true,
        );
    ...
    if (
      !this.schemaDesigners.has(key) ||
      this.schemaDesigners.get(key)?.isDisposed
    ) {
