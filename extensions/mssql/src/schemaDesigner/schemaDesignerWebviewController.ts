...
> {
  ...
  constructor(
    ...
    private schemaDesignerCache: Map<
      string,
  ...
  private setupRequestHandlers() {
      ...
      async () => {
        ...
        try {
          ...
          if (!this.schemaDesignerCache.has(this._key)) {
            ...
            // session is dirty
            sessionResponse = this.updateCacheItem(
              undefined,
              true,
    ...
    this.onRequest(
      SchemaDesigner.GetDefinitionRequest.type,
      async (payload) => {
        ...
        try {
          ...
          reportActivity.end(
            ActivityStatus.Succeeded,
            {
              ...
              possibleDataLoss:
                result.report?.dacReport?.possibleDataLoss?.toString(),
    ...
    this.onRequest(
      SchemaDesigner.PublishSessionRequest.type,
      async (payload) => {
    ...
    this.onNotification(
      SchemaDesigner.ExportToFileNotification.type,
      async (payload) => {
        ...
        sendActionEvent(
          TelemetryViews.SchemaDesigner,
          TelemetryActions.ExportToImage,
          {
        ...
        if (payload.format === "svg") {
          let fileContents = new Uint8Array(
            Buffer.from(
              decodeURIComponent(payload.fileContents.split(",")[1]),
              "utf8",
            ),
          );
    ...
    this.onNotification(
      SchemaDesigner.CopyToClipboardNotification.type,
      async (params) => {
        await vscode.env.clipboard.writeText(params.text);
        await vscode.window.showInformationMessage(
          LocConstants.scriptCopiedToClipboard,
        );
      },
    );

    this.onNotification(
      SchemaDesigner.OpenInEditorNotification.type,
      async () => {
    ...
    this.onNotification(
      SchemaDesigner.CloseSchemaDesignerNotification.type,
      () => {
  ...
  private updateCacheItem(
    ...
    schemaDesignerCacheItem.isDirty =
      isDirty ?? schemaDesignerCacheItem.isDirty;
