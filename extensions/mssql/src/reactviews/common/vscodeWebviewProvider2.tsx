 ...
 */
export function VscodeWebviewProvider2<State, Reducers>({
  children,
}: VscodeWebviewProvider2Props) {
  ...
  useEffect(() => {
    ...
    extensionRpc.onNotification<State>(
      StateChangeNotification.type<State>(),
      (params) => {
...
export function useVscodeWebview2<State, Reducers>() {
  ...
  if (!context) {
    throw new Error(
      "useVscodeWebview2 must be used within a VscodeWebviewProvider2",
    );
  }
