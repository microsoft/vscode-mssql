...
export function getCoreRPCs2<TReducers>(
 ...
 */
export function getNextFocusableElementOutside(
  ...
  const currentIndex = focusables.findIndex(
    (el) => el === active && container.contains(el),
  );
 ...
 */
export function getPreviousFocusableElementOutside(
  container: HTMLElement,
): HTMLElement | null {
  const focusables = getFocusableElements();
  const active = document.activeElement as HTMLElement | null;
