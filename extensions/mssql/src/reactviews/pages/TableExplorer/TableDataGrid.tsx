...
export const TableDataGrid = forwardRef<TableDataGridRef, TableDataGridProps>(
  (
    ...
    const previousResultSetRef = useRef<EditSubsetResult | undefined>(
      undefined,
    );
    ...
    useEffect(() => {
      ...
      const rowCountChanged =
        previousResultSet?.subset?.length !== resultSet.subset.length;
      ...
      // Scenario 3: Row count same - incremental updates only
        ...
        for (let i = 0; i < resultSet.subset.length; i++) {
          ...
          if (!oldRow || JSON.stringify(newRow) !== JSON.stringify(oldRow)) {
            const dataRow = convertRowToDataRow(newRow);
            const existingItem = reactGridRef.current.dataView.getItemById(
              dataRow.id,
            );
    ...
    function handleCellChange(_e: CustomEvent, args: any) {
      ...
      console.log(
        `Cell Changed - Row ID: ${rowId}, Column Index: ${columnIndex}`,
      );
    ...
    function handleContextMenuCommand(_e: any, args: any) {
      ...
      switch (command) {
        ...
        case "revert-cell":
          ...
          console.log(
            `Reverted cell for row ID ${rowId}, column ${columnIndex}`,
          );
    ...
    const isDarkMode =
      currentTheme === ColorThemeKind.Dark ||
      currentTheme === ColorThemeKind.HighContrast;
    ...
    return (
      ...
      >
        <SlickgridReact
          ...
          onCellChange={($event) =>
            handleCellChange($event, $event.detail.args)
          }
        />
      </div>
    );
