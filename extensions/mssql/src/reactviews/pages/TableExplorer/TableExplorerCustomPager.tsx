...
const TableExplorerCustomPager = React.forwardRef<TableExplorerCustomPagerRef>(
  (_, ref) => {
    const [currentPagination, setCurrentPagination] =
      useState<PaginationMetadata>({} as PaginationMetadata);
    const [isLeftPaginationDisabled, setIsLeftPaginationDisabled] =
      useState(false);
    const [isRightPaginationDisabled, setIsRightPaginationDisabled] =
      useState(false);
    ...
    const checkLeftPaginationDisabled = (
    ...
    const checkRightPaginationDisabled = (
      pagination: PaginationMetadata,
    ): boolean => {
      return (
    ...
    const init = (
      ...
      setIsLeftPaginationDisabled(
        checkLeftPaginationDisabled(currentPagination),
      );
      setIsRightPaginationDisabled(
        checkRightPaginationDisabled(currentPagination),
      );
      setSelectedPageSize(
        String(currentPagination.pageSize || DEFAULT_PAGE_SIZE),
      );
      ...
      const subscription = pubSubService.subscribe<PaginationMetadata>(
        ...
        (paginationChanges) => {
          ...
          setIsLeftPaginationDisabled(
            checkLeftPaginationDisabled(paginationChanges),
          );
          setIsRightPaginationDisabled(
            checkRightPaginationDisabled(paginationChanges),
          );
          setSelectedPageSize(
            String(paginationChanges.pageSize || DEFAULT_PAGE_SIZE),
          );
        },
      );
    ...
    const renderPagination = () => {
      if (paginationServiceRef.current) {
        const currentPagination =
          paginationServiceRef.current.getFullPagination();
        setCurrentPagination(currentPagination);
        setIsLeftPaginationDisabled(
          checkLeftPaginationDisabled(currentPagination),
        );
    ...
    return (
      <div
        className="table-explorer-custom-pagination"
