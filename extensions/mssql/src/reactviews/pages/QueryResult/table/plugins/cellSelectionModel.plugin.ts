  ...
  implements Slick.SelectionModel<T, Array<Slick.Range>>
{
  ...
  public init(grid: Slick.Grid<T>) {
    ...
    this._handler.subscribe(
      ...
      (e: Event, cell: Slick.Cell) =>
        this.handleBeforeCellRangeSelected(e, cell),
    );
  ...
  private removeInvalidRanges(ranges: Array<Slick.Range>): Array<Slick.Range> {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      if (
        ...
        result.push(
          new Slick.Range(r.fromRow, r.fromCell + 1, r.toRow, r.toCell),
        );
      }
    }
  ...
  private async handleCellRangeSelected(
    _e: Event,
    range: Slick.Range,
    append: boolean,
  ) {
  ...
  private async handleHeaderClick(
    ...
    if (e.target) {
      if (
        (e.target as EventTargetWithClassName).className ===
    ...
    if (!isUndefinedOrNull(args.column)) {
      ...
      if (this.options.hasRowSelector && columnIndex === 0) {
        ...
        if (newSelectedRange) {
          result = this.insertIntoSelections(
            rangesToBeMerged,
            newSelectedRange!,
          );
        } else {
  ...
  private async handleKeyDown(e: KeyboardEvent): Promise<void> {
    ...
    if (
      ...
      eventMatchesShortcut(
        e,
        keyBindings[WebviewAction.ResultGridExpandSelectionRight]
          .keyCombination,
      )
    ) {
      ...
      !isHandled &&
      ((e.shiftKey && keyCode === KeyCode.F10) ||
  ...
  public async updateSummaryText(ranges?: Slick.Range[]): Promise<void> {
    ...
    const actualRanges = convertDisplayedSelectionToActual(
      this.grid,
      simplifiedRanges,
    );
    await this.context.extensionRpc.sendNotification(
      SetSelectionSummaryRequest.type,
      {
