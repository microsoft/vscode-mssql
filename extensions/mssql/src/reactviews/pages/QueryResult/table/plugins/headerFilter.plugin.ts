...
export class HeaderMenu<T extends Slick.SlickData> {
  ...
  private _columnFilterButtonMapping: Map<string, HTMLElement> = new Map<
    string,
    HTMLElement
  >();
  ...
  private async showColumnMenuForColumn(
    column: FilterableColumn<T>,
  ): Promise<void> {
  ...
  public async openMenuForActiveColumn(): Promise<void> {
    ...
    const column = this._grid.getColumns()[
      activeCell.cell
    ] as FilterableColumn<T>;
    await this.showColumnMenuForColumn(column);
  }

  private onHeaderCellRendered(
    _e: Event,
    ...
    // Add sort indicator
    jQuery('<span class="slick-sort-indicator-icon"></span>').appendTo(
      args.node,
    );
      ...
      `
            <button id="anchor-btn"
                    aria-label="${locConstants.queryResult.showMenu(
                      this.keyBindings[WebviewAction.ResultGridOpenColumnMenu]
                        .label,
                    )}"
  ...
  private async showColumnMenu(menuButton: HTMLElement) {
    ...
    const initialSelected = (this._columnDef.filterValues ??
      []) as FilterValue[];
    ...
    const currentSort =
      this._columnSortStateMapping.get(columnId) ?? SortProperties.NONE;
  ...
  private async applySort(
    ...
    if (this._currentSortColumn && this._currentSortColumn !== columnId) {
      const prevColumn = this._grid
        .getColumns()
        .find(
          (col) => col.id === this._currentSortColumn,
        ) as FilterableColumn<T>;
      if (prevColumn) {
        this._columnSortStateMapping.set(
  ...
  private async handleClearSort(
    ...
    if (
      currentSortState === SortProperties.NONE ||
  ...
  public updateSortStateFromExternal(
    columnId: string,
    sort: SortProperties,
  ): void {
    if (!this._grid) {
  ...
  public clearSortState(): void {
    ...
    this.updateSortStateFromExternal(
      this._currentSortColumn,
      SortProperties.NONE,
    );
  }
  ...
  private async buildFilterItems(): Promise<FilterListItem[]> {
    ...
    otherEntries.sort((a, b) =>
      this.filterValueToSortString(a[0]).localeCompare(
        this.filterValueToSortString(b[0]),
      ),
    );
  ...
  private clearFilterValues(
    gridFiltersArray: ColumnFilterMap,
    columnId: string,
  ) {
  ...
  private async handleMenuItemClick(
    ...
    if (command === SortProperties.ASC || command === SortProperties.DESC) {
      this._grid.setSortColumn(
        columnDef.id as string,
        command === SortProperties.ASC,
      );
    }
  ...
  private getAllFilterValues(
    ...
    return Array.from(seen).sort((a, b) =>
      this.filterValueToSortString(a).localeCompare(
        this.filterValueToSortString(b),
      ),
    );
  }
  ...
  public resizeColumn(columnId: string, newWidth: number): void {
    ...
    void this.queryResultContext.extensionRpc.sendRequest(
      SetColumnWidthsRequest.type,
      {
        uri: this.uri,
        gridId: this.gridId,
