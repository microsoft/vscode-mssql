  ...
  implements Slick.Plugin<T>
{
  ...
  private resizeColumn(headerEl: JQuery, columnDef: Slick.Column<T>) {
    let headerWidth =
      this.getElementWidths([headerEl[0]])[0] +
      this._options.extraColumnHeaderWidth!;
    ...
    let autoSizeWidth = this.calculateOptimalColumnWidth(
      headerWidth,
      contentWidth,
    );
    ...
    texts.forEach((text, index) => {
      ...
      if (formatFun) {
        ...
        if (item) {
          template = jQuery(
            "<span>" +
              formatFun(index, colIndex, text, columnDef, item) +
              "</span>",
          );
