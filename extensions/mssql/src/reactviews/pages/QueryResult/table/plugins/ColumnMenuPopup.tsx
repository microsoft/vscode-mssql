...
export const ColumnMenuPopup: React.FC<ColumnMenuPopupProps> = ({
  ...
  const sortAscendingButtonRef = useRef<
  ...
  const firstFocusableRef = useRef<
    HTMLButtonElement | HTMLAnchorElement | null
  >(null);
  const lastFocusableRef = useRef<HTMLButtonElement | HTMLAnchorElement | null>(
    null,
  );
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) =>
      item.displayText.toLowerCase().includes(trimmed),
    );
  }, [items, search]);
  ...
  const updateSelection = useCallback(
    (value: FilterValue, checked: boolean) => {
        ...
        <div className={styles.section}>
          <Toolbar className={styles.sortButtons}>
            <Button
              ...
              appearance={
                currentSort === SortProperties.ASC ? "primary" : "subtle"
              }
              size="small"
            ...
            <Button
              appearance={
                currentSort === SortProperties.DESC ? "primary" : "subtle"
              }
              size="small"
