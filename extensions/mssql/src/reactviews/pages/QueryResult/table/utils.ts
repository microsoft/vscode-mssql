...
export function convertDisplayedSelectionToActual(
  ...
  for (const selection of selections) {
    ...
    for (
      let displayRow = selection.fromRow;
      displayRow <= selection.toRow;
      displayRow++
    ) {
...
export function selectEntireGrid<T extends Slick.SlickData>(
  grid: Slick.Grid<T>,
): ISlickRange[] {
...
export function getGridCount(
  summaries: Record<number, Record<number, ResultSetSummary>>,
): number {
  let count = 0;
