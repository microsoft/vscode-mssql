...
const useStyles = makeStyles({
  ...
  noResultsIcon: {
    ...
    background:
      "linear-gradient(135deg, rgba(0,120,212,.16), rgba(0,120,212,.06))",
  },
  resultErrorIcon: {
    width: "56px",
    height: "56px",
...
export const QueryResultPane = () => {
  ...
  const isExecutionPlan = useQueryResultSelector<boolean | undefined>(
    (s) => s.isExecutionPlan,
  );
  const executionPlanGraphs = useQueryResultSelector<
    ExecutionPlanGraph[] | undefined
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (
        ...
        eventMatchesShortcut(
          event,
          keyBindings[WebviewAction.QueryResultSwitchToMessagesTab]
            ?.keyCombination,
        )
      ) {
  ...
  const getWebviewLocation = async () => {
    const res = await context.extensionRpc.sendRequest(
      qr.GetWebviewLocationRequest.type,
      {
        uri: uri,
      },
    );
      ...
      <div className={classes.root}>
        <div className={classes.noResultsContainer}>
          <div className={classes.noResultsScrollablePane}>
            ...
            <Text className={classes.noResultMessage}>
              {initilizationError}
            </Text>
          </div>
        </div>
