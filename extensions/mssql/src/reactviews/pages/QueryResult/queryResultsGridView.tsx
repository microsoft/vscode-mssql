...
export const QueryResultsGridView = () => {
  ...
  const resultSetSummaries = useQueryResultSelector(
    (state) => state.resultSetSummaries,
  );
  const viewMode =
  ...
  const [gridViewContainerHeight, setGridViewContainerHeight] =
    useState<number>(0);
  const gridContainerRefs = useRef<
    Map<string, React.RefObject<HTMLDivElement>>
  >(new Map());
  const [maximizedGridKey, setMaximizedGridKey] = useState<string | undefined>(
    undefined,
  );
  ...
  const gridHeights: number[] = useMemo(() => {
    ...
    const preferredHeights = gridList.map((it) =>
      naturalHeight(
        resultSetSummaries?.[it.batchId]?.[it.resultId]?.rowCount ?? 0,
      ),
    );
    ...
    return preferredHeights.map(
      (preferredHeight) => preferredHeight + heightAdjustment,
    );
  ...
  const getActiveGrid = useCallback(():
    ...
    for (let i = 0; i < gridList.length; i++) {
      ...
      if (
        gridContainerDiv === activeElement ||
        gridContainerDiv?.contains(activeElement)
      ) {
        return {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (
        ...
        // Circular navigation
        const newIndex =
          (activeGrid.gridIndex - 1 + gridList.length) % gridList.length;
