...
export function FilterTablesButton() {
    ...
    if (selectedTables.length === 0) {
      ...
      edges.forEach((edge) => {
        ...
        if (
          sourceNode &&
          targetNode &&
          tablesToShow.includes(
            `${sourceNode.data.schema}.${sourceNode.data.name}`,
          ) &&
          tablesToShow.includes(
            `${targetNode.data.schema}.${targetNode.data.name}`,
          )
        ) {
          reactFlow.updateEdge(edge.id, {
            ...edge,
  ...
  function renderListItems() {
    const nodes = reactFlow.getNodes();
    const tableNames = nodes.map(
      (node) => `${node.data.schema}.${node.data.name}`,
    );
    tableNames.sort();
  ...
  return (
    <Menu
      ...
      onOpenChange={(_, data) => setIsFilterMenuOpen(data.open)}
    >
      <MenuTrigger>
        <Button
          size="small"
