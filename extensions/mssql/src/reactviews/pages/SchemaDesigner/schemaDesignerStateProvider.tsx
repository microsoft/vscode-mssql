...
const SchemaDesignerStateProvider: React.FC<SchemaDesignerProviderProps> = ({
  ...
  const [renderOnlyVisibleTables, setRenderOnlyVisibleTables] =
    ...
    if (visibleNodes.length === 0) {
      ...
      const bottomMostNode = visibleNodes
        ...
        .reduce((prev, current) => {
          ...
          const prevBottom =
            prev.position.y + flowUtils.getTableHeight(prev.data);
      ...
      nodeWithPosition.position = {
        ...
        y:
          ...
          flowUtils.getTableHeight(bottomMostNode.data) +
          50,
      };
    }
  ...
  const updateTable = async (updatedTable: SchemaDesigner.Table) => {
    ...
    const existingTableNode = existingNodes.find(
      (node) => node.id === updatedTable.id,
    );
    ...
    existingEdges = existingEdges.filter(
      (edge) => edge.source !== updatedTable.id,
    );
