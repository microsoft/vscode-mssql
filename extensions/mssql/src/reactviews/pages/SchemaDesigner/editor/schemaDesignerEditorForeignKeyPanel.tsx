...
const ForeignKeyCard = ({
  ...
  useEffect(() => {
    ...
    const warning =
      context.warnings[`${FOREIGN_KEY_ERROR_PREFIX}${foreignKey.id}`];
    if (warning) {
  ...
  return (
    <Card className={classes.cardStyle} key={`fk-card-${index}`}>
      <CardHeader
        header={
          <Text>{locConstants.schemaDesigner.foreignKeyIndex(index + 1)}</Text>
        }
        action={
          <Button
            appearance="subtle"
      ...
      {/* Warning Message */}
      {warningMessage && (
        <MessageBar intent="warning">{warningMessage}</MessageBar>
      )}
      ...
      <div className={classes.row}>
        <Field style={{ flex: 1 }} size="small">
          ...
          <Dropdown
            size="small"
            value={foreignKeyUtils.convertOnActionToString(
              foreignKey.onDeleteAction,
            )}
            selectedOptions={[
              foreignKeyUtils.convertOnActionToString(
                foreignKey.onDeleteAction,
              ),
            ]}
            multiselect={false}
