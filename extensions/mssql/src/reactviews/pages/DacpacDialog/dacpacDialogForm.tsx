...
export const DacpacDialogForm = () => {
  ...
  const initialOperationType = useDacpacDialogSelector(
    (state) => state.operationType,
  );
  const initialOwnerUri = useDacpacDialogSelector((state) => state.ownerUri);
  const initialServerName = useDacpacDialogSelector(
    (state) => state.serverName,
  );
  const initialDatabaseName = useDacpacDialogSelector(
    (state) => state.databaseName,
  );
  const initialSelectedProfileId = useDacpacDialogSelector(
    (state) => state.selectedProfileId,
  );

  // Local state
  const [operationType, setOperationType] =
    useState<dacpacDialog.DacPacDialogOperationType>(
  ...
  const handleServerChange = async (profileId: string) => {
    ...
    try {
      ...
      if (result?.isConnected && result.ownerUri) {
        ...
        const errorMsg =
          result?.errorMessage || locConstants.dacpacDialog.connectionFailed;
        setValidationMessages({
          connection: {
            message: errorMsg,
            severity: "error",
  ...
  const validateFilePath = async (
    ...
    try {
      const result = await context?.validateFilePath({
        filePath: path,
        shouldExist,
      });
      ...
      if (!result?.isValid) {
        setValidationMessages((prev) => ({
          ...
          filePath: {
            message:
              result?.errorMessage || locConstants.dacpacDialog.invalidFile,
            severity: "error",
          },
        }));
        return false;
    ...
    try {
      ...
      if (!result?.isValid) {
        setValidationMessages((prev) => ({
          ...
          databaseName: {
            message:
              result?.errorMessage || locConstants.dacpacDialog.invalidDatabase,
            severity: "error",
          },
        }));
        return false;
  ...
  const getValidationRequirements = (
    ...
    switch (opType) {
      case dacpacDialog.DacPacDialogOperationType.Deploy:
        return {
          filePathShouldExist: true,
          databaseShouldNotExist: isNewDatabase,
        };
  ...
  const validateOperationInputs = async (
    ...
    const filePathValid = await validateFilePath(
      filePath,
      requirements.filePathShouldExist,
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(
        error instanceof Error
          ? error.message
          : locConstants.dacpacDialog.unexpectedError,
      );
      setIsOperationInProgress(false);
    }
  };
  ...
  const isFormValid = () => {
    ...
    const hasErrors = Object.values(validationMessages).some(
