...
export interface DacpacDialogRpcMethods {
  ...
  listDatabases: (params: {
    ownerUri: string;
  }) => Promise<{ databases: string[] } | undefined>;
...
export const DacpacDialogContext = createContext<
  DacpacDialogReactProvider | undefined
>(undefined);
...
const DacpacDialogStateProvider: React.FC<DacpacDialogProviderProps> = ({
  children,
}) => {
  ...
  // Validation methods
  const validateFilePath = async (params: {
    filePath: string;
    shouldExist: boolean;
  }) => {
    return await extensionRpc?.sendRequest(
