...
export const FabricWorkspaceContentsList = ({
  ...
  const items = useMemo(() => {
    ...
    if (
      ...
      selectedWorkspace.databases.length > 0
    ) {
  ...
  const renderGridContent = (): React.ReactNode => {
    ...
    if (
      !selectedWorkspace ||
      selectedWorkspace.loadStatus.status === ApiStatus.NotStarted
    ) {
    ...
    return (
      <>
        ...
        <Label>
          FabricWorkspaceLoadStatus: {fabricWorkspacesLoadStatus.status}
        </Label>
        <Label>SelectedWorkspace: {selectedWorkspace?.id}</Label>
        <Label>
          SelectedWorkspace.Status: {selectedWorkspace?.loadStatus.status}
        </Label>
        <Label>
          SelectedWorkspace.Databases: {selectedWorkspace?.databases.length}
        </Label>
      </>
    );
  };
  ...
  const renderLoadingWorkspaces = () => (
    <div
      className={styles.workspaceContentMessageContainer}
      role="status"
      aria-live="polite"
    >
      <Spinner size="medium" />
  ...
  function renderRow(
    ...
    return (
      <DataGridRow<FabricSqlGridItem>
        key={rowId}
        className={
          selectedRowId === item.id ? styles.selectedDataGridRow : undefined
        }
        style={style}
        onClick={() => {
      ...
      >
        {({
          ...
          renderCell: (item: FabricSqlGridItem) => React.ReactNode;
        }) => <>{renderCell(item)}</>}
      </DataGridRow>
    );
  }
  ...
  function getItemIcon(artifactType: string, theme: ColorThemeKind): string {
    switch (artifactType) {
      ...
      default:
        console.error(
          `Unknown artifact type for getItemIcon(): ${artifactType}`,
        );
        return sqlDatabaseIcon(theme);
    }
  }
  ...
  return (
    <div className={styles.workspaceContentList}>{renderGridContent()}</div>
  );
};
...
export function getTypeDisplayName(artifactType: string): string {
  switch (artifactType) {
    ...
    default:
      console.error(
        `Unknown artifact type for getTypeDisplayName(): ${artifactType}`,
      );
      return artifactType;
  }
}
