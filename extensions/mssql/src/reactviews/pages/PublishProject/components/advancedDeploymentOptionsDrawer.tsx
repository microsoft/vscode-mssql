...
export const AdvancedDeploymentOptionsDrawer = ({
  ...
  const [localChanges, setLocalChanges] = useState<
    Array<{ optionName: string; value: boolean }>
  >([]);
  // Snapshot of localChanges when drawer opens (for cancel functionality)
  const [localChangesSnapshot, setLocalChangesSnapshot] =
    useState(localChanges);
  ...
  const getCurrentValue = useCallback(
    (optionName: string, baseValue: boolean): boolean => {
      const localChange = localChanges.find(
        (change) => change.optionName === optionName,
      );
  ...
  const optionGroups = useMemo(() => {
    ...
    if (state.deploymentOptions.objectTypesDictionary) {
      const baseExcludedTypes =
        state.deploymentOptions.excludeObjectTypes?.value || [];
      const excludeEntries = Object.entries(
        state.deploymentOptions.objectTypesDictionary,
      )
        .map(([key, displayName]) => {
  ...
  const renderOption = (option: {
    ...
    return (
      <Checkbox
        ...
        onChange={(_, data) =>
          handleOptionChange(option.key, data.checked === true)
        }
        label={
          option.description ? (
            <InfoLabel info={option.description}>
              {option.displayName}
            </InfoLabel>
          ) : (
            option.displayName
