...
export const SqlCmdVariablesSection: React.FC = () => {
  ...
  const sqlCmdVariables = usePublishDialogSelector(
    (s) => s.formState.sqlCmdVariables,
  );
  const sqlCmdComponent = usePublishDialogSelector(
    (s) => s.formComponents.sqlCmdVariables,
  );
  const defaultSqlCmdVariables = usePublishDialogSelector(
    (s) => s.defaultSqlCmdVariables,
  );
  ...
  const hasModifiedValues = useMemo(() => {
    ...
    for (const varName in sqlCmdVariables) {
      if (
        (sqlCmdVariables[varName] || "") !==
        (defaultSqlCmdVariables[varName] || "")
      ) {
        return true;
      }
    }
