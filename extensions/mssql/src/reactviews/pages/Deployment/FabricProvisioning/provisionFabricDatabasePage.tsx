...
export const ProvisionFabricDatabasePage: React.FC = () => {
  ...
  if (!context || !fabricProvisioningState || showPrevious === undefined)
    return undefined;
  ...
  const getStatusIcon = () => {
    ...
    if (status === ApiStatus.NotStarted) {
      return (
        <Circle20Regular style={{ color: tokens.colorNeutralStroke1Pressed }} />
      );
    }
  ...
  const getHeaderText = () => {
    ...
    if (fabricProvisioningState.provisionLoadState === ApiStatus.Error) {
      ...
      fabricProvisioningState.provisionLoadState !== ApiStatus.Loaded
    ) {
