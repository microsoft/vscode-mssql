...
const CONNECTION_SHARING_PERMISSIONS_KEY =
...
export class ConnectionSharingService
  implements mssql.IConnectionSharingService
{
    ...
    this._context.subscriptions.push(
      vscode.commands.registerCommand(
        ...
        async (extensionId?: string) =>
          this.editConnectionSharingPermissions(extensionId),
      ),
    );
  ...
  private async getStoredExtensionPermissions(): Promise<ExtensionPermissionsMap> {
    ...
    try {
      ...
      this._logger.error(
        "Failed to parse stored extension permissions:",
        error,
      );
      const emptyPermissions: ExtensionPermissionsMap = {};
      await this.storeExtensionPermissions(emptyPermissions);
  ...
  private async storeExtensionPermissions(
    permissions: ExtensionPermissionsMap,
  ): Promise<void> {
  ...
  private async setApprovedExtensions(
    extensions: ExtensionPermissionsMap,
  ): Promise<void> {
    await this._context.secrets.store(
      CONNECTION_SHARING_PERMISSIONS_KEY,
  ...
  private async requestConnectionSharingPermission(
    extensionId: string,
  ): Promise<boolean> {
    this._logger.info(
      `Requesting connection sharing permission for extension: ${extensionId}`,
    );
    ...
    if (currentPermission === "approved") {
      this._logger.info(
        `Connection sharing already approved for extension: ${extensionId}`,
      );
      return true;
    }
    ...
    if (currentPermission === "denied") {
      this._logger.info(
        `Connection sharing denied for extension: ${extensionId}`,
      );
      return false;
    }
    ...
    switch (userChoice) {
      case "Approve":
        this._logger.info(
          `User approved connection sharing for extension: ${extensionId}`,
        );
        await this.updateExtensionPermission(extensionId, "approved");
        return true;
      case "Deny":
        this._logger.info(
          `User denied connection sharing for extension: ${extensionId}`,
        );
  ...
  private async validateExtensionPermission(
    extensionId: string,
  ): Promise<void> {
    const hasPermission =
      await this.requestConnectionSharingPermission(extensionId);
  ...
  public async getActiveEditorConnectionId(
    extensionId: string,
    ...
    const connectionDetails =
      this._connectionManager.getConnectionInfoFromUri(activeEditorUri);
  ...
  public async connect(
    ...
    if (!targetConnection) {
      ...
      throw new ConnectionSharingError(
        ...
        LocalizedConstants.ConnectionSharing.connectionNotFoundError(
          connectionId,
        ),
        extensionId,
        connectionId,
    ...
    if (!connectionResult) {
      ...
      throw new ConnectionSharingError(
        ...
        LocalizedConstants.ConnectionSharing.failedToEstablishConnectionError(
          connectionId,
        ),
        extensionId,
        connectionId,
  ...
  public getServerInfo(connectionUri: string): mssql.IServerInfo {
    this._logger.info(
      `Retrieving server info for connection URI: ${connectionUri}`,
    );
  ...
  public async editConnectionSharingPermissions(
    ...
    if (!extensionId) {
      this._logger.info(
        "No extension ID provided, prompting user to select an extension.",
      );
      const extensionQuickPickItems: vscode.QuickPickItem[] =
      ...
      const selectedExtension = await vscode.window.showQuickPick(
        extensionQuickPickItems,
        {
          canPickMany: false,
          placeHolder:
            LocalizedConstants.ConnectionSharing.SelectAnExtensionToManage,
    ...
    const newPermission = await vscode.window.showQuickPick(
      ...
      {
        placeHolder:
          LocalizedConstants.ConnectionSharing.SelectNewPermission(
    ...
    const newApproval: ExtensionPermission =
      newPermission.detail as ExtensionPermission;
  ...
  public async getConnectionString(
    ...
    const connectionDetails =
      this._connectionManager.createConnectionDetails(targetConnection);
