...
> {
  ...
  private registerRpcHandlers() {
    ...
    this.registerReducer("loadFromConnectionString", async (state, payload) => {
      ...
      try {
        ...
        state.connectionProfile =
          await this.hydrateConnectionDetailsFromProfile(
            connDetails,
            state.connectionProfile,
          );
        ...
        this.logger.error(
          "Error parsing connection string: " + getErrorMessage(error),
        );
    ...
    this.registerReducer("openConnectionStringDialog", async (state) => {
      ...
      try {
        ...
        if (
          !shallowEqualObjects(state.connectionProfile, getDefaultConnection())
        ) {
          const cleanedConnection = this.cleanConnection(
            state.connectionProfile,
          );
        ...
        this.logger.error(
          "Error generating connection string: " + getErrorMessage(error),
        );
        state.dialog = {
          type: "loadFromConnectionString",
          connectionString: "",
    ...
    this.registerReducer("signIntoAzureForBrowse", async (state, payload) => {
      ...
      try {
        ...
        this.logger.error(
          "Error signing into Azure: " + getErrorMessage(error),
        );
        state.formMessage = {
    ...
    this.registerReducer("selectAzureAccount", async (state, payload) => {
      ...
      const azureAccount = await VsCodeAzureHelper.getAccountById(
        payload.accountId,
      );
      const tenants =
        await VsCodeAzureHelper.getTenantsForAccount(azureAccount);
      ...
      const homeTenantId =
        VsCodeAzureHelper.getHomeTenantIdForAccount(azureAccount);
    ...
    this.registerReducer("selectAzureTenant", async (state, payload) => {
      ...
      if (state.fabricWorkspaces.length <= FABRIC_WORKSPACE_AUTOLOAD_LIMIT) {
        ...
        for (const workspace of state.fabricWorkspaces) {
          promiseArray.push(
            this.loadFabricDatabasesForWorkspace(state, workspace),
          );
        }
    ...
    this.registerReducer("messageButtonClicked", async (state, payload) => {
      if (payload.buttonId === CLEAR_TOKEN_CACHE) {
        ...
        this.vscodeWrapper.showInformationMessage(
          LocAll.Accounts.clearedEntraTokenCache,
        );
        this.state.formMessage = undefined;
      } else {
        this.logger.error(
          `Unknown message button clicked: ${payload.buttonId}`,
        );
      }
  ...
  private async loadConnections(): Promise<{
    ...
    return {
      recentConnections: await Promise.all(
        processConnections(recentConnections, "recent"),
      ),
      savedConnections: await Promise.all(
        processConnections(savedConnections, "saved"),
      ),
    };
  }
    ...
    try {
      try {
        ...
        const connectionInfo =
          this._mainController.connectionManager?.getConnectionInfo(
            tempConnectionUri,
          );
        ...
        if (
          getErrorMessage(error).includes(
            AzureConstants.multiple_matching_tokens_error,
          )
        ) {
      ...
      async function saveConnectionAndCreateSession(
        ...
        const node =
          await self._mainController.createObjectExplorerSession(
            cleanedConnection,
          );
  ...
  private async handleConnectionErrorCodes(
    ...
    if (errorType === SqlConnectionErrorType.TrustServerCertificateNotEnabled) {
      ...
      this.state.formMessage = {
        message: `${result.errorNumber}: ${result.errorMessage}`,
      };
      this.state.dialog = {
        type: "changePassword",
        props: {
  ...
  private async getConnectionGroups(
  ...
  private async getAzureActionButtons(): Promise<FormItemActionButton[]> {
    ...
    if (
      ...
      const account = (
        ...
        (account) =>
          account.displayInfo.userId === this.state.connectionProfile.accountId,
      );
      if (account) {
  ...
  private async loadAzureSubscriptions(
    ...
    try {
      ...
      state.formMessage = {
        message: l10n.t("Error loading Azure subscriptions."),
      };
      state.loadingAzureSubscriptionsStatus = ApiStatus.Error;
  ...
  private async loadAllAzureServers(
    ...
    try {
      ...
      state.loadingAzureServersStatus = ApiStatus.Error;
      this.logger.error(
        state.formMessage.message + os.EOL + getErrorMessage(error),
      );
  ...
  private async loadFabricDatabasesForWorkspace(
    ...
    try {
      ...
      try {
        databases.push(
          ...(await FabricHelper.getFabricDatabases(
            workspace.id,
            workspace.tenantId,
          )),
        );
      ...
      try {
        databases.push(
          ...(await FabricHelper.getFabricSqlEndpoints(
            workspace.id,
            workspace.tenantId,
          )),
        );
  ...
  private async hydrateConnectionDetailsFromProfile(
    ...
    if (
      ...
      const matchingAccount = accounts.find(
        (a) => a.displayInfo.email === toProfile.user,
      );
