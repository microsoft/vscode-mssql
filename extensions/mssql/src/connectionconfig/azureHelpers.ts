...
export class VsCodeAzureHelper {
  ...
  public static async getAccounts(
    ...
    try {
      ...
      console.error(
        `Error fetching VS Code accounts: ${getErrorMessage(error)}`,
      );
    }
    ...
    try {
      account =
        typeof account === "string"
          ? await this.getAccountById(account)
          : account;
...
export async function constructAzureAccountForTenant(azureAccountInfo: {
  ...
  if (!sub) {
    throw new Error(
      Loc.errorLoadingAzureAccountInfoForTenantId(azureAccountInfo.tenantId),
    );
  }
...
export function buildServerUri(
  serverResource: GenericResourceExpanded,
): string {
