...
import VscodeWrapper, {
  ConfigurationTarget,
} from "../controllers/vscodeWrapper";
...
export class ConnectionConfig implements IConnectionConfig {
  ...
  public async addConnection(profile: IConnectionProfile): Promise<void> {
    ...
    let profiles = await this.getConnections(
  ...
  public async removeGroup(
    ...
    if (remainingGroups.length === groups.length) {
      this._logger.error(
        `Connection group with ID '${id}' not found when removing.`,
      );
      return false;
    }
  ...
  public async updateGroup(updatedGroup: IConnectionGroup): Promise<void> {
    ...
    if (index === -1) {
      throw Error(
        `Connection group with ID ${updatedGroup.id} not found when updating`,
      );
    } else {
