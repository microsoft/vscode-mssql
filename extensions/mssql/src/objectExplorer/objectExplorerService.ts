...
export class ObjectExplorerService {
   ...
   */
  private _pendingSessionCreations: Map<
    string,
    Deferred<SessionCreatedParameters>
  > = new Map<string, Deferred<SessionCreatedParameters>>();
  /**
   ...
   */
  public handleSessionCreatedNotification(
  ...
  public handleExpandNodeNotification(result: ExpandResponse): void {
    const promise = this._pendingExpands.get(
      `${result.sessionId}${result.nodePath}`,
    );
    if (promise) {
      promise.resolve(result);
    } else {
  ...
  public async expandNode(
    ...
    this._logger.verbose(
      `Expanding node ${node.label} with session ID ${sessionId}`,
    );
    try {
   ...
   */
  private getAddConnectionNodes(
    parent?: TreeNodeInfo,
  ): AddConnectionTreeNode[] {
  ...
  private async getRootNodes(): Promise<vscode.TreeItem[]> {
    ...
    for (const connection of savedConnections) {
      if (
        connection.groupId &&
        newConnectionGroupNodes.has(connection.groupId)
      ) {
  ...
  private async createSessionAndExpandNode(
    ...
    if (sessionResult?.shouldRetryOnFailure) {
      setTimeout(
        () => void this.reconnectProfile(element.connectionProfile),
        0,
      );
      return undefined;
    }
  ...
  public async createSession(
    ...
    const connectionProfile =
      await this.prepareConnectionProfile(connectionInfo);
    ...
    this._pendingSessionCreations.set(
  ...
  private async handleSessionCreationSuccess(
    ...
    await this._connectionManager.handlePasswordStorageOnConnect(
      connectionProfile,
    );
  ...
  private async handleSessionCreationFailure(
    ...
    const errorHandlingResult =
      await this._connectionManager.handleConnectionErrors(
        failureResponse,
        connectionProfile,
      );
  ...
  public async removeNode(
    ...
    if (showUserConfirmationPrompt) {
      const response = await vscode.window.showInformationMessage(
        LocalizedConstants.ObjectExplorer.NodeDeletionConfirmation(
  ...
  public async removeConnectionNodes(
    ...
    for (let conn of connections) {
      const node = this.getConnectionNodeFromProfile(
  ...
  private addConnectionNode(connectionNode: ConnectionNode): void {
    const oldNode = this._connectionNodes.get(
      connectionNode.connectionProfile.id,
    );
    ...
    if (oldNode) {
      this._connectionGroupNodes
        .get(oldNode.connectionProfile.groupId)
        ?.removeChild(oldNode);
    }

    this._connectionNodes.set(
      connectionNode.connectionProfile.id,
      connectionNode,
    );
    this._connectionGroupNodes
  ...
  public get rootNodeConnectionsOld(): IConnectionInfo[] {
    const connections = this._rootTreeNodeArray.map(
      (node) => node.connectionProfile,
    );
    return connections;
  }

  public get connections(): IConnectionProfile[] {
    return [...this._connectionNodes.values()].map(
      (node) => node.connectionProfile,
    );
  }

  public get connectionGroups(): IConnectionGroup[] {
    return [...this._connectionGroupNodes.values()].map(
      (node) => node.connectionGroup,
    );
  }
