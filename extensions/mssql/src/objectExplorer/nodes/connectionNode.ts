...
const createDisconnectedNodeContextValue = (
  ...
  if (connectionProfile.containerName)
    nodeSubType = disconnectedDockerContainer;
  return {
...
export class ConnectionNode extends TreeNodeInfo {
  constructor(connectionProfile: ConnectionProfile, parentNode?: TreeNodeInfo) {
    ...
    if (connectionProfile.containerName) {
      this.iconPath = ObjectExplorerUtils.iconPath(
        ICON_DOCKER_SERVER_DISCONNECTED,
      );
  ...
  private getConnectionTooltip(
    ...
    const connectionNodeDefaults: Partial<IConnectionProfile> =
    ...
    let lines = props.map((p) => {
      ...
      if (value !== p.defaultValue) {
        if (
          value === Constants.azureMfa ||
          value === Constants.integratedauth
        ) {
