...
import { canvasCompositeMatrix } from "./spatialMapExport";
...
/** Imperative surface for the pane's map context menu ("Copy image"). */
export interface SpatialMapHandle {
...
export const SpatialMap = React.forwardRef<SpatialMapHandle, SpatialMapProps>(
    function SpatialMap(props, ref): React.JSX.Element {
        ...
        const clusterLayerRef = React.useRef<VectorImageLayer<Cluster<Feature>> | undefined>(
            undefined,
        );
        ...
        React.useLayoutEffect(() => {
            ...
            const selected = new Style({
                ...
                image: new CircleStyle({
                    radius: 7,
                    fill: new Fill({
                        color: themeColor("--vscode-list-activeSelectionBackground"),
                    }),
                    stroke: new Stroke({ color: themeColor("--vscode-focusBorder"), width: 3 }),
                }),
            });
            ...
            if (sourceFeatures.length > 0) {
                ...
                if (!renderBeginRef.current) {
                    ...
                    perfMark("mssql.queryResults.spatial.render.begin", {
                        tier: tierRef.current,
                        offline:
                            typeof propsRef.current.basemapLayer === "string" ? "true" : "false",
                        layer: layerAttrOf(propsRef.current.basemapLayer),
                    });
                }
        ...
        // "Copy image" export: composite the OL layer canvases (including a
        // just-rendered WebGL buffer — renderSync inside rendercomplete keeps it
        // readable) onto one CSS-pixel canvas over the editor background.
            ...
            () => ({
                exportImage: () =>
                    new Promise<Blob>((resolve, reject) => {
                        ...
                        if (!map || !target) {
                            reject(new Error(locConstants.spatialResults.waiting));
                            return;
                        }
                        map.once("rendercomplete", () => {
                            try {
                                ...
                                out.width = Math.max(1, Math.round(size[0]));
                                out.height = Math.max(1, Math.round(size[1]));
                                const context = out.getContext("2d");
                                if (!context) {
                                    reject(new Error("canvas 2d context unavailable"));
                                    return;
                                }
                                context.fillStyle = themeColor("--vscode-editor-background");
                                context.fillRect(0, 0, out.width, out.height);
                                const canvases = target.querySelectorAll<HTMLCanvasElement>(
                ...
                <div
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            if (view)
                                view.animate({ zoom: (view.getZoom() ?? 2) + 1, duration: 120 });
                        }}>
