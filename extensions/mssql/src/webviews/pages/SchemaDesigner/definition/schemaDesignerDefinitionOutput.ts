...
export enum SchemaDesignerDefinitionKind {
    Sql = "sql",
    Prisma = "prisma",
    Sequelize = "sequelize",
    TypeOrm = "typeorm",
...
function renderHeader(kind: SchemaDesignerDefinitionKind): string {
    ...
    const lines = [
        `${commentPrefix} Generated from SQL Server schema by GitHub Copilot in VS Code MSSQL.`,
    ...
    for (const table of schema.tables) {
        ...
        for (const foreignKey of outgoingForeignKeys) {
            ...
            const referenceNames = foreignKey.targetColumns
                ...
                .join(", ");
            lines.push(
                `  ${relationName} ${targetModel}${isNullableForeignKey(foreignKey) ? "?" : ""} @relation(fields: [${fieldNames}], references: [${referenceNames}])`,
            );
        }
        ...
        for (const foreignKey of incomingForeignKeys) {
            ...
            lines.push(`  ${relationName} ${sourceModel}[]`);
        }
...
function renderTypeOrm(schema: SchemaDesigner.Schema): string {
    ...
    for (const table of schema.tables) {
        ...
        for (const column of table.columns) {
            ...
            } else if (column.isPrimaryKey && primaryKeys.length === 1) {
    ...
    for (const table of schema.tables) {
        ...
        for (const column of table.columns) {
            ...
            const decorator = column.isIdentity
                ? "@PrimaryGeneratedColumn"
                : column.isPrimaryKey && primaryKeys.length === 1
                  ? "@PrimaryColumn"
                  : "@Column";
...
function renderSqlAlchemy(schema: SchemaDesigner.Schema): string {
    ...
    for (const table of schema.tables) {
        ...
        for (const column of table.columns) {
            ...
            const pythonType = SQL_TO_PYTHON[getSqlType(column)] ?? "str";
            lines.push(
...
function renderEfCore(schema: SchemaDesigner.Schema): string {
    ...
    lines.push("", "namespace GeneratedSchema;", "");
