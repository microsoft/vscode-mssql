...
import { applyDabCommands, getDabToolStateFromConfig } from "../../../dab/dabCommandEngine";
...
export function registerSchemaDesignerDabToolHandlers(params: {
    ...
    const handleGetState = async (): Promise<Dab.GetDabToolStateResponse> => {
        ...
        const result = await getDabToolStateFromConfig(
            getCurrentDabConfig(),
            getCurrentSchemaTables(),
        );
        if (result.changed) {
            commitDabConfig(result.config);
        }
    ...
    const handleApplyChanges = async (
        ...
        const result = await applyDabCommands({
            baseConfig: getCurrentDabConfig(),
            schemaTables: getCurrentSchemaTables(),
            expectedVersion: request?.expectedVersion,
            commands: request?.changes,
            returnState: request?.options?.returnState,
            source: request?.options?.source ?? "ux",
        });
