...
> {
  ...
  constructor(
    ...
    sendActionEvent(
      ...
      TelemetryActions.PublishDialogOpened,
      {
  ...
  private isDatabaseExisting(
    ...
    if (
      ...
      if (databaseComponent?.options) {
        return databaseComponent.options.some(
  ...
  private async runDockerPrerequisiteChecks(): Promise<{
    ...
    for (
      let stepIndex = 0;
      stepIndex <= DockerStepOrder.checkDockerEngine;
      stepIndex++
    ) {
      const currentStep = dockerSteps[stepIndex];
      const args = currentStep.argNames.map(
        (argName) =>
          (dummyProfile as unknown as Record<string, unknown>)[argName],
      );
   ...
   */
  private async prepareContainerConfiguration(
    state: PublishDialogState,
  ): Promise<{
  ...
  private async createDockerContainer(
    ...
    for (
      ...
      const args = currentStep.argNames.map(
        (argName) =>
          (dockerProfile as unknown as Record<string, unknown>)[argName],
      );
      const result = await currentStep.stepAction(...args);
  ...
  private async initializeDialog(projectFilePath: string) {
    ...
    const tagComponent =
      this.state.formComponents[PublishFormFields.ContainerImageTag];
    if (tagComponent) {
      try {
        const tagOptions =
  ...
  private registerRpcHandlers(): void {
    this.registerReducer(
      ...
      async (state: PublishDialogState) => {
          ...
          try {
            ...
            this.state = {
              ...
              formState: {
                ...
                serverName:
                  parsedProfile.serverName || state.formState.serverName,
            ...
            if (parsedProfile.connectionString) {
              void this.connectAndPopulateDatabases(
    ...
    this.registerReducer(
      ...
      async (
        state: PublishDialogState,
        _payload: { publishProfileName: string },
      ) => {
        ...
        const projectName = state.projectFilePath
          ? path.basename(
              ...
              path.extname(state.projectFilePath),
            )
          : "project";
  ...
  private async handleSuccessfulConnection(
    event: ConnectionSuccessfulEvent,
  ): Promise<void> {
    try {
  ...
  public async afterSetFormProperty(
    ...
    if (propertyName === PublishFormFields.PublishTarget) {
      ...
      if (this.state.formState.publishTarget === PublishTarget.LocalContainer) {
        ...
        this.state.formState.publishTarget === PublishTarget.ExistingServer
      ) {
  ...
  protected async validateForm(
    ...
    const erroredInputs = await super.validateForm(
      formTarget,
      propertyName,
      updateValidation,
    );
    ...
    const sqlCmdVariablesValid = validateSqlCmdVariables(
      this.state.formState.sqlCmdVariables,
    );
