...
export async function getProjectTargetVersion(
  ...
  try {
    ...
    const result =
      await sqlProjectsService.getProjectProperties(projectFilePath);
    if (!result?.success) {
      return undefined;
    }
    ...
    if (
      !dsp ||
      !dsp.startsWith(constants.DSP_PREFIX) ||
      !dsp.endsWith(constants.DSP_SUFFIX)
    ) {
      return undefined;
    }
...
export async function readProjectProperties(
  ...
  try {
    ...
    const result = await sqlProjectsService.getProjectProperties(
      projectFilePath as string,
    );
    if (!result?.success) {
      return undefined;
    }
    const version = await getProjectTargetVersion(
      sqlProjectsService,
      projectFilePath,
    );
...
export function validateSqlServerPortNumber(port: number): boolean {
  return (
    Number.isInteger(port) && port >= 1 && port <= constants.MAX_PORT_NUMBER
  );
}
...
export async function getSqlServerContainerTagsForTargetVersion(
  ...
  try {
    ...
    dockerLogger.error(
      `Error filtering SQL Server container tags: ${getErrorMessage(e)}`,
    );
    return [];
  }
}
 ...
 */
export function readSqlCmdVariables(profileText: string): {
  [key: string]: string;
} {
  ...
  try {
    ...
    const sqlCmdVarElements =
      xmlDoc.documentElement.getElementsByTagName("SqlCmdVariable");
    ...
    for (let i = 0; i < sqlCmdVarElements.length; i++) {
      ...
      if (varName) {
        ...
        const defaultValueElements =
          sqlCmdVar.getElementsByTagName("DefaultValue");

        if (valueElements.length > 0 && valueElements[0].firstChild) {
          varValue = valueElements[0].firstChild.nodeValue || "";
        } else if (
          defaultValueElements.length > 0 &&
          defaultValueElements[0].firstChild
        ) {
 ...
 */
export function extractServerFromConnectionString(
  connectionString: string,
): string {
  if (!connectionString) {
    return "";
...
export async function parsePublishProfileXml(
  ...
  try {
    ...
    if (dacFxService) {
      try {
        const optionsResult =
          await dacFxService.getOptionsFromProfile(profilePath);
 ...
 */
export function validateSqlCmdVariables(sqlCmdVariables?: {
  [key: string]: string;
}): boolean {
  if (!sqlCmdVariables) {
  ...
  return Object.values(sqlCmdVariables).every(
    (v) => v !== "" && v !== undefined,
  );
}
