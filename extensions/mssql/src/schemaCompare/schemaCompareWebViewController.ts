...
> {
  ...
  private async resolveEndpointInfo(
    ...
    if (this.isTreeNodeInfoType(context)) {
      ...
      context.endsWith(".dacpac")
    ) {
      this.logger.verbose(
        `Using dacpac: ${context} - OperationId: ${this.operationId}`,
      );
      return this.getEndpointInfoFromDacpac(context as string);
    } else if (
      context &&
      typeof context === "string" &&
      context.endsWith(".sqlproj")
    ) {
      this.logger.verbose(
        `Using project: ${context} - OperationId: ${this.operationId}`,
      );
      ...
      this.logger.verbose(
        `No context provided - OperationId: ${this.operationId}`,
      );
      return undefined;
    }
  }
  ...
  private registerRpcHandlers(): void {
    ...
    this.registerReducer("listActiveServers", (state) => {
      this.logger.verbose(
        `Listing active SQL servers - OperationId: ${this.operationId}`,
      );
      const activeServers = this.getActiveServersList();
      ...
      sendActionEvent(
        TelemetryViews.SchemaCompare,
        TelemetryActions.ListingActiveServers,
        {
          operationId: this.operationId,
    ...
    this.registerReducer("selectFile", async (state, payload) => {
      ...
      if (filePath) {
        this.logger.info(
          `Selected file: ${filePath} - OperationId: ${this.operationId}`,
        );
    ...
    this.registerReducer("confirmSelectedSchema", async (state, payload) => {
      ...
      if (payload.endpointType === "source") {
        ...
        if (
          ...
          SchemaCompareEndpointType.Project
        ) {
          this.logger.info(
    ...
    this.registerReducer("confirmSelectedDatabase", (state, payload) => {
      ...
      const connection =
        this.connectionMgr.activeConnections[payload.serverConnectionUri];
      this.logger.verbose(
      ...
      if (payload.endpointType === "source") {
        this.logger.info(
          `Setting as source endpoint - OperationId: ${this.operationId}`,
        );
        state.sourceEndpointInfo = endpointInfo;
      } else {
        this.logger.info(
          `Setting as target endpoint - OperationId: ${this.operationId}`,
        );
    ...
    this.registerReducer("confirmSchemaOptions", async (state, payload) => {
      ...
      if (payload.optionsChanged) {
        ...
        this.logger.info(
          `No options were changed - OperationId: ${this.operationId}`,
        );
    ...
    this.registerReducer("switchEndpoints", async (state, payload) => {
      ...
      this.logger.info(
        `Successfully switched endpoints - OperationId: ${this.operationId}`,
      );
      endActivity.end(ActivityStatus.Succeeded, {
    ...
    this.registerReducer("generateScript", async (state, payload) => {
      ...
      this.logger.verbose(
        `Starting script generation - OperationId: ${this.operationId}`,
      );
      ...
      if (!result || !result.success) {
        ...
        vscode.window.showErrorMessage(
          locConstants.SchemaCompare.generateScriptErrorMessage(
            result?.errorMessage,
          ),
        );
      } else {
    ...
    this.registerReducer("publishChanges", async (state, payload) => {
      this.logger.info(
        `Publishing changes requested with operation ID: ${this.operationId}`,
      );
      this.logger.verbose(
      ...
      try {
        ...
        endActivity.endFailed(
          new Error(
            `Exception during publish operation: ${getErrorMessage(error)}`,
          ),
          true,
          undefined,
          undefined,
    ...
    this.registerReducer("publishDatabaseChanges", async (state, payload) => {
      this.logger.info(
        `Publishing database changes with operation ID: ${this.operationId}`,
      );
      ...
      try {
        ...
        if (result.success) {
          ...
          endActivity.endFailed(
            new Error(
              `Failed to publish database changes: ${result.errorMessage}`,
            ),
            true,
            undefined,
            undefined,
    ...
    this.registerReducer("publishProjectChanges", async (state, payload) => {
      this.logger.info(
        `Publishing project changes with operation ID: ${this.operationId}`,
      );
      this.logger.verbose(
      ...
      try {
        ...
        if (result.success) {
          ...
          endActivity.endFailed(
            new Error(
              `Failed to publish project changes: ${result.errorMessage}`,
            ),
            true,
            undefined,
            undefined,
    ...
    this.registerReducer("resetOptions", async (state) => {
      ...
      try {
        ...
        this.logger.info(
          `Reset options to defaults - OperationId: ${this.operationId}`,
        );
        this.updateState(state);
      } catch (error) {
        this.logger.error(
      ...
      try {
        ...
        if (result.success) {
          ...
          state.schemaCompareResult.differences =
            result.allIncludedOrExcludedDifferences;
    ...
    this.registerReducer("saveScmp", async (state) => {
      ...
      this.logger.verbose(
        `Calling saveScmp service - OperationId: ${this.operationId}`,
      );
    ...
    this.registerReducer("cancel", async (state) => {
      this.logger.info(
        `Cancelling schema comparison operation with ID: ${this.operationId}`,
      );
      ...
      try {
        ...
        if (!result || !result.success) {
          ...
          endActivity.endFailed(
            new Error(
              `Failed to cancel: ${result?.errorMessage || "unknown error"}`,
            ),
            true,
            undefined,
            undefined,
  ...
  private async autoSelectNewConnection(
    ...
    try {
      ...
      this.logger.verbose(
        `Resetting waiting state - OperationId: ${this.operationId}`,
      );
  ...
  private async schemaCompare(
    ...
    this.logger.info(
      `Starting schema comparison with operation ID: ${this.operationId}`,
    );
    this.logger.verbose(
    ...
    if (
      ...
      payload.sourceEndpointInfo.targetScripts =
        await this.getProjectScriptFiles(
    ...
    for (const key in generalOptionsDictionary) {
      if (generalOptionsDictionary.hasOwnProperty(key)) {
        booleanOptionsAsStrings[key] =
          generalOptionsDictionary[key].value.toString();
      }
    }
    ...
    endActivity.update({
      operationId: this.operationId,
      includeObjectTypesConfig: JSON.stringify(
        includedObjectTypesTelemetryDictionary,
      ),
    });

    this.logger.info(
      `Executing schema comparison with operation ID: ${this.operationId}`,
    );
    ...
    const diffTypeFrequencies = this.countTargetObjectTypeFrequencies(
      result.differences,
    );
   ...
   */
  private getSchemaUpdateActionString(
   ...
   */
  private countTargetObjectTypeFrequencies(differences: DiffEntry[]): {
    [key: string]: number;
  } {
