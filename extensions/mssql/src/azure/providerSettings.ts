...
interface MssqlEnvironment
  extends AzureEnvironments.Environment,
    MssqlEnvironmentAdditions {
...
function getCustomCloudProviderSettings(): IProviderSettings {
  ...
  const mssqlCustomCloud: MssqlEnvironmentAdditions =
  ...
  if (!customCloud.isCustomCloud) {
    throw new Error(
      "Attempted to read custom cloud, but got preconfigured one instead.",
    );
  }
...
export function getAzureEnvironment(
  ...
  switch (cloudId) {
    ...
    case CloudId.Custom:
      ...
      throw new Error(
        Loc.customCloudNotConfigured(configSovereignCloudCustomEnvironment),
      );
    default:
      throw new Error(`Unexpected cloud ID: '${cloud}'`);
  }
