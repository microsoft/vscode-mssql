...
export class AccountStore {
  ...
  public async getAccounts(): Promise<IAccount[]> {
    ...
    this._logger.info(
      `Retrieved ${accounts.length} Entra accounts from account store.`,
    );
    return accounts;
  }
  ...
  public async addAccount(account: IAccount): Promise<boolean> {
    if (!account) {
      this._logger.error(
        "Empty Entra Account cannot be added to account store.",
      );
      return false;
    }
  ...
  private isValidAccount(account: IAccount): boolean {
    return (
      account.key?.id !== undefined &&
      account.displayInfo?.displayName !== undefined
    );
  }
}
