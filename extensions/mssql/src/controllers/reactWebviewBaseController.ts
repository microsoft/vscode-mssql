...
class WebviewControllerMessageReader
...
class WebviewControllerMessageWriter
  ...
  implements MessageWriter
{
  ...
  write(msg: Message): Promise<void> {
    if (this._webview) {
      ...
      sendActionEvent(
        TelemetryViews.WebviewController,
        TelemetryActions.SentToWebview,
        {
          messageType: method ? "request" : "response",
          type: method,
          isError: error ? "true" : "false",
  ...
  implements vscode.Disposable
{
  ...
  constructor(
    ...
    this.connection = createMessageConnection(
      this._connectionReader,
      this._connectionWriter,
    );
    ...
    this.onRequest(ReducerRequest.type<Reducers>(), async (action) => {
      ...
      if (reducer) {
        ...
        reducerActivity.endFailed(
          new Error(
            `No reducer registered for action ${action.type as string}`,
          ),
          false,
        );
        throw new Error(
  ...
  public sendRequest<TParam, TResult, TError>(
    ...
    if (this._isDisposed) {
      return Promise.reject(
        new Error("Cannot send request on disposed controller"),
      );
    }
  ...
  public onNotification<TParams>(
    ...
    if (this._isDisposed) {
      throw new Error(
        "Cannot register notification handler on disposed controller",
      );
    }
