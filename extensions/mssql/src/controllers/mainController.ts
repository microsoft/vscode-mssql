...
import {
  ITableExplorerService,
  TableExplorerService,
} from "../services/tableExplorerService";
...
export default class MainController implements vscode.Disposable {
      ...
      this._event.on(
        ...
        async (treeNodeInfo: TreeNodeInfo) => {
          sendActionEvent(
            ...
            TelemetryActions.ChatWithDatabase,
          );
  ...
  public async initialize(): Promise<boolean> {
    ...
    if (activeTextEditor && this._vscodeWrapper.isEditingSqlFile) {
      await this.sqlDocumentService.onDidOpenTextDocument(
  ...
  public async onSearchObjects(node?: ConnectionNode): Promise<void> {
    try {
      ...
      const searchService = new DatabaseObjectSearchService(
        this._connectionMgr.client,
      );
      ...
      const runSearch = async (value: string, token: number) => {
        ...
        const result = await searchService.searchObjects(
          connectionUri,
          term,
          databaseName,
        );
        ...
        if (!result.success) {
          qp.items = [
            {
              label: LocalizedConstants.searchObjectsErrorWithDetail(
                result.error,
              ),
              description: "",
              detail: "",
      ...
      if (objType === "Table" || objType === "View") {
        ...
        objType === "Scalar Function" ||
        objType === "Table-valued Function"
      ) {
        operations.push(
          {
            label: LocalizedConstants.ObjectExplorer.ScriptSelectLabel,
      ...
      const selectedOp = await vscode.window.showQuickPick(
        ...
        {
          placeHolder:
            LocalizedConstants.ObjectExplorer.FetchingScriptLabel("…"),
          ignoreFocusOut: true,
        },
      );
      if (!selectedOp) {
      ...
      const op =
        operations.find((o) => o.label === selectedOp)?.op ??
      ...
      const serverInfo = this._connectionMgr.getServerInfo(
      ...
      const scriptingParams =
        this._scriptingService.createScriptingRequestParams(
          serverInfo,
          scriptingObject,
          connectionUri,
          op,
  ...
  private async initializeObjectExplorer(
    ...
    const connectParentNode = async (
      node: AccountSignInTreeNode | ConnectTreeNode,
    ) => {
    ...
    this._context.subscriptions.push(
      vscode.commands.registerCommand(
        ...
        async (node: AccountSignInTreeNode) => {
          ...
          switch (choice) {
            case LocalizedConstants.ObjectExplorer
              .FailedOEConnectionErrorUpdate:
              const connDialog = new ConnectionDialogWebviewController(
                this._context,
                this._vscodeWrapper,
                this,
    ...
    this._event.on(
      Constants.cmdConnectionGroupEdit,
    ...
    if (this.isRichExperiencesEnabled) {
        ...
        vscode.commands.registerCommand(
          ...
          async (
            ...
            if (args.length >= 2) {
              ...
              targetNode = args[1];
              runComparison =
                args.length > 2 && typeof args[2] === "boolean"
                  ? args[2]
                  : false;
            }
      ...
      this._context.subscriptions.push(
        vscode.commands.registerCommand(
          ...
          async (node: any) => this.onTableExplorer(node),
        ),
      );
    ...
    if (this.isExperimentalEnabled) {
      registerDacPacCommand(
        Constants.cmdDacpacDialog,
        DacPacDialogOperationType.Deploy,
      );
      registerDacPacCommand(
      ...
      registerDacPacCommand(
        ...
        DacPacDialogOperationType.Extract,
      );
      registerDacPacCommand(
        Constants.cmdImportBacpac,
        DacPacDialogOperationType.Import,
      );
      registerDacPacCommand(
        Constants.cmdExportBacpac,
        DacPacDialogOperationType.Export,
      );
    }
    ...
    this._context.subscriptions.push(
      vscode.commands.registerCommand(
        ...
        async (node: TreeNodeInfo) => {
          ...
          await stopContainer(containerName).then(
            async (stoppedSuccessfully) => {
  ...
  private async onToggleSqlCmd(): Promise<void> {
    ...
    if (queryRunner) {
      ...
      await this._outputContentProvider.createQueryRunner(
        this._statusview,
        uri,
        title,
      );
  ...
  public async onNewQueryWithConnection(
    ...
    const isSqlEditor =
      activeEditor?.document.languageId === Constants.languageId;
    const isConnected = activeUri
      ? this.connectionManager.isConnected(activeUri)
      : false;
  ...
  public async checkIsReadyToExecuteQuery(): Promise<boolean> {
    ...
    if (
      this._connectionMgr.isConnected(this._vscodeWrapper.activeTextEditorUri)
    ) {
      return true;
    }
   ...
   */
  public async onPublishDatabaseProject(
  ...
  private async onChangeGroupBySchemaConfig(
    e: vscode.ConfigurationChangeEvent,
  ): Promise<void> {
    if (
      !e.affectsConfiguration(Constants.cmdObjectExplorerGroupBySchemaFlagName)
    ) {
      return;
    }
   ...
   */
  private async onChangeConnectionConfig(
    e: vscode.ConfigurationChangeEvent,
  ): Promise<boolean> {
    if (
    ...
    let result = await this.handleRemovedConns(
      objectExplorerConnections,
      configConnections,
    );
    needsRefresh ||= result;

    result = await this.handleAddedConns(
      objectExplorerConnections,
      configConnections,
    );
    needsRefresh ||= result;
  ...
  private async handleAddedConns(
    ...
    let newConnections = configConnections.filter((userConn) => {
      return !oeConnections.some((oeConn) =>
    ...
    for (let conn of newConnections) {
      ...
      const uri =
        ObjectExplorerUtils.getNodeUriFromProfile(newConnectionProfile);
      if (
  ...
  private async isContainerReadyForCommands(
    node: TreeNodeInfo,
  ): Promise<boolean> {
    const containerName = node.connectionProfile?.containerName;
    return (
