...
export default class QueryRunner {
  ...
  private _startFailedEmitter: vscode.EventEmitter<string> =
    new vscode.EventEmitter<string>();
  public onStartFailed: vscode.Event<string> = this._startFailedEmitter.event;

  private _startEmitter: vscode.EventEmitter<string> =
    new vscode.EventEmitter<string>();
  ...
  public onBatchStart: vscode.Event<BatchSummary> =
    this._batchStartEmitter.event;

  private _batchCompleteEmitter: vscode.EventEmitter<BatchSummary> =
    new vscode.EventEmitter<BatchSummary>();
  public onBatchComplete: vscode.Event<BatchSummary> =
    this._batchCompleteEmitter.event;
  ...
  private _resultSetUpdatedEmitter: vscode.EventEmitter<ResultSetSummary> =
    new vscode.EventEmitter<ResultSetSummary>();
  public onResultSetUpdated: vscode.Event<ResultSetSummary> =
    this._resultSetUpdatedEmitter.event;
  ...
  public onExecutionPlan: vscode.Event<ExecutionPlanEvent> =
    this._executionPlanEmitter.event;
  ...
  public onComplete: vscode.Event<QueryExecutionCompleteEvent> =
    this._completeEmitter.event;

  private _onSummaryChangedEmitter: vscode.EventEmitter<SummaryChanged> =
    new vscode.EventEmitter<SummaryChanged>();
  public onSummaryChanged: vscode.Event<SummaryChanged> =
    this._onSummaryChangedEmitter.event;
  ...
  public handleQueryComplete(
    ...
    this._completeEmitter.fire({
      totalMilliseconds: Utils.parseNumAsTimeString(
        this._totalElapsedMilliseconds,
      ),
      hasError,
    });
    sendActionEvent(
  ...
  public handleResultSetUpdated(
    result: QueryExecuteResultSetUpdatedNotificationParams,
  ): void {
    let resultSet = result.resultSetSummary;
    let batchSet = this._batchSets[resultSet.batchId];
  ...
  public async handleResultSetComplete(
    ...
    if (
      resultSet.columnInfo?.[0]?.columnName === Constants.showPlanXmlColumnName
    ) {
      ...
      async (_progress, token) => {
        return new Promise<void>(async (resolve, reject) => {
          try {
            ...
            const selections: TableSelectionRange[] = selection.map(
              (range) => ({
  ...
  public async generateSelectionSummaryData(
    ...
    const sendCancelSummaryEvent = async () => {
      ...
      this._cancelConfirmation = undefined;
      await waitForUserToProceed(requestId, totalRows);
      await this.generateSelectionSummaryData(
        selections,
        batchId,
        resultId,
        false,
    ...
    try {
      ...
      const simpleSelections: TableSelectionRange[] = selections.map(
        (range) => ({
          fromRow: range.fromRow,
          toRow: range.toRow,
          fromColumn: range.fromCell,
      ...
      if (result.average !== undefined && result.average !== null) {
        ...
        tooltip =
          LocalizedConstants.QueryResult.nonNumericSelectionSummaryTooltip(
            result.count,
            result.distinctCount,
            result.nullCount,
          );
