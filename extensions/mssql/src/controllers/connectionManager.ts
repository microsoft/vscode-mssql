...
export default class ConnectionManager {
  ...
  public isConnecting(fileUri: string): boolean {
    return (
      fileUri in this._connections && this._connections[fileUri].connecting
    );
  }
   ...
   */
  public async parseConnectionString(
  ...
  public handleConnectionCompleteNotification(): NotificationHandler<ConnectionContracts.ConnectionCompleteParams> {
    ...
    return async (
      ...
      const completionPromise =
        self._uriToConnectionCompleteParamsMap.get(fileUri);
  ...
  public async confirmEntraTokenValidity(connectionInfo: IConnectionInfo) {
     ...
     * that case below.
     */
    const azureAccountToken = await new Promise<IToken | undefined>(
      (resolve) => {
   ...
   */
  public async handlePasswordStorageOnConnect(
    profile: IConnectionProfile,
  ): Promise<void> {
  ...
  public async connect(
    ...
    if (Utils.isNotEmpty(result.connectionId)) {
      ...
      if (shouldHandleErrors) {
        ...
        if (errorHandlingResult.isHandled) {
          return await this.connect(
            fileUri,
            errorHandlingResult.updatedCredentials,
            {
              connectionSource: connectionSource,
            },
          );
      ...
      this.statusView.setConnectionError(
        fileUri,
        connectionInfo.credentials,
        result,
      );
  ...
  public async prepareConnectionInfo(
    ...
    const passwordResult =
      await this.handlePasswordBasedCredentials(credentials);
    if (!passwordResult) {
      throw new Error(LocalizedConstants.cannotConnect);
    }
  ...
  private async handleConnectionSuccess(
    ...
    void this.statusView.connectSuccess(
      fileUri,
      newCredentials,
      connectionInfo.serverInfo,
    );
  ...
  public async handleConnectionErrors(
    ...
    if (errorType === SqlConnectionErrorType.PasswordExpired) {
      const result =
        await this._changePasswordService.handleChangePassword(credentials);
      if (result) {
      ...
      const wasCreated = await this.handleFirewallError(
        credentials,
        errorMessage,
      );
  ...
  public async removeAccount(prompter: IPrompter): Promise<void> {
    ...
    let azureAccountChoices = await ConnectionProfile.getAccountChoices(
      this._accountStore,
    );
  ...
  private async migrateLegacyConnection(
    ...
    try {
      ...
      if (
        connectionString.includes(ConnectionStore.CRED_CONNECTION_STRING_PREFIX)
      ) {
        const retrievedString = await this.connectionStore.lookupPassword(
          profile,
          true,
        );
  ...
  private async selectAccount(): Promise<IAccount> {
    ...
    const quickPickItems = this.createAccountQuickPickItems(
      accounts,
      currentAccountId,
    );
    ...
    const accountItems: AccountQuickPickItem[] = accounts.map((account) => ({
      label:
        account.key.id === currentAccountId
          ? LocalizedConstants.Connection.currentAccount(
  ...
  private async showAccountQuickPick(
    ...
    const account = await new Promise<IAccount | undefined>(
      (resolve, reject) => {
  ...
  private async showTenantQuickPick(
    items: TenantQuickPickItem[],
