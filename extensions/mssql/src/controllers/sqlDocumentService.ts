...
export default class SqlDocumentService implements vscode.Disposable {
   ...
   */
  public async handleNewQueryCommand(
    node?: TreeNodeInfo,
    content?: string,
  ): Promise<boolean> {
    ...
    const connectionResult =
      this._connectionMgr.getConnectionInfo(newEditorUri);
  ...
  public async onDidChangeActiveTextEditor(
    ...
    const connectionInfo =
      this._connectionMgr?.getConnectionInfo(activeDocumentUri);
    ...
    if (connectionInfo?.connectionId && !connectionInfo?.connecting) {
      this._lastActiveConnectionInfo = Utils.deepClone(
  ...
  private async onSuccessfulConnection(
    params: ConnectionSuccessfulEvent,
  ): Promise<void> {
    const activeEditorKey = getUriKey(
  ...
  private async createNewQueryDocument(
    options: NewQueryOptions,
  ): Promise<vscode.TextEditor> {
    // Create the document
    ...
    if (
      ...
      if (connectionResult) {
         ...
         */
        if (
          !this.objectExplorerService?.hasSession(
            connectionConfig.connectionInfo,
          )
        ) {
        ...
        if (options.connectionInfo && connectionConfig.connectionInfo) {
          const tokenUpdates = removeUndefinedProperties({
            azureAccountToken:
              connectionConfig.connectionInfo.azureAccountToken,
  ...
  private async resolveConnectionConfig(
    ...
    const strategy =
      options.connectionStrategy ?? ConnectionStrategy.CopyLastActive;
    ...
    switch (strategy) {
      ...
      case ConnectionStrategy.CopyFromUri:
        ...
        const resolvedConnectionInfo =
          this._connectionMgr.getConnectionInfoFromUri(options.sourceUri);
      ...
      default:
        ...
        return {
          shouldConnect: true,
          connectionInfo: Utils.deepClone(credentials),
        };
    }
  }
