...
} from "./deploymentWebviewController";
...
export async function initializeFabricProvisioningState(
  ...
  const defaultAccountId =
    azureAccountOptions.length > 0 ? azureAccountOptions[0].value : "";
  let defaultTenantId = "";
  let tenantOptions: FormItemOptions[] = [];
  if (defaultAccountId !== "") {
    const tenants =
      await VsCodeAzureHelper.getTenantsForAccount(defaultAccountId);
  ...
  const azureActionButtons = await getAzureActionButton(
...
export function registerFabricProvisioningReducers(
  ...
  deploymentController.registerReducer(
    ...
    async (state, _payload) => {
      ...
      fabricProvisioningState.formErrors =
        await deploymentController.validateDeploymentForm();
...
export async function getAzureActionButton(
  ...
  const state = deploymentController.state
    .deploymentTypeState as fp.FabricProvisioningState;
...
export async function loadComponentsAfterSignIn(
  ...
  const tenants = await VsCodeAzureHelper.getTenantsForAccount(
    state.formState.accountId,
  );
  ...
  if (tenantComponent) {
    ...
    if (
      ...
      state.formState.tenantId = getDefaultTenantId(
        state.formState.accountId,
        tenants,
      );
      const errors =
...
export async function getWorkspaces(
  ...
  let state = deploymentController.state
    .deploymentTypeState as fp.FabricProvisioningState;
...
export async function sortWorkspacesByPermission(
  ...
  if (workspaces.length < WORKSPACE_ROLE_REQUEST_LIMIT) {
    ...
    state.workspacesWithPermissions = Object.fromEntries(
...
export async function handleWorkspaceFormAction(
  ...
  if (workspace && !workspace.role) {
    ...
    if (
      hasWorkspacePermission(workspaceWithRole.role, WorkspaceRole.Contributor)
    ) {
      state.workspacesWithPermissions[workspaceWithRole.id] = workspaceWithRole;
    } else {
      state.workspacesWithoutPermissions[workspaceWithRole.id] =
        workspaceWithRole;
    }
  ...
  const workspaceValidation = workspaceComponent.validate(
    state,
    state.formState.workspace,
  );
...
export async function connectToDatabase(
  ...
  try {
    ...
    const databaseConnectionString =
      databaseDetails.properties.connectionString;
    ...
    const databaseConnectionProfile: IConnectionDialogProfile =
      await ConnectionCredentials.createConnectionInfo(
    ...
    await deploymentController.mainController.createObjectExplorerSession(
      profile,
    );
...
export function sendFabricProvisioningCloseEventTelemetry(
  state: fp.FabricProvisioningState,
): void {
  sendActionEvent(
    TelemetryViews.FabricProvisioning,
