...
export async function initializeLocalContainersState(
  ...
  state.formComponents = setLocalContainersFormComponents(
    versions,
    groupOptions,
  );
  state.formState = {
...
export function registerLocalContainersReducers(
  ...
  deploymentController.registerReducer(
    ...
    async (state, payload) => {
      const localContainersState =
        state.deploymentTypeState as lc.LocalContainersState;
      ...
      localContainersState.dockerSteps[currentStepNumber].loadState =
        ApiStatus.Loading;
  ...
  deploymentController.registerReducer(
    ...
    async (state, _payload) => {
      ...
      sendActionEvent(
        TelemetryViews.LocalContainers,
        TelemetryActions.RetryDockerStep,
        {
          dockerStep: lc.DockerStepOrder[currentStepNumber],
        },
      );
  ...
  deploymentController.registerReducer(
    "checkDockerProfile",
    async (state, _payload) => {
      let localContainersState =
        state.deploymentTypeState as lc.LocalContainersState;
      localContainersState.formValidationLoadState = ApiStatus.Loading;
      updateLocalContainersState(deploymentController, localContainersState);

      localContainersState =
        await validateDockerConnectionProfile(localContainersState);
      ...
      localContainersState.isDockerProfileValid =
        localContainersState.formErrors.length === 0;
...
export async function handleLocalContainersFormAction(
  ...
  return await validateDockerConnectionProfile(
    state,
...
export async function addContainerConnection(
  ...
  try {
    const profile =
      await mainController.connectionManager.connectionUI.saveProfile(
  ...
  return {
    version: createFormItem({
      ...
      validate(_state, value) {
        ...
        // Issue tracking this: https://github.com/microsoft/vscode-mssql/issues/20337
        const isArm64With2025 =
          arch() === "arm64" && value?.toString()?.includes("2025");
        return {
          isValid: !isArm64With2025,
          validationMessage: isArm64With2025
            ? LocalContainers.sqlServer2025ArmError
            : "",
        };
      },
    }),
    ...
    groupId: createFormItem(
      getGroupIdFormItem(groupOptions) as lc.LocalContainersFormItemSpec,
    ),
