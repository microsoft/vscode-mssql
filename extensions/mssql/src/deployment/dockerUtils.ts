 ...
 */
export async function pullSqlServerContainerImage(
  version: string,
...
export async function startSqlServerDockerContainer(
  ...
  try {
    ...
    dockerLogger.append(
      `SQL Server container ${containerName} started on port ${port}.`,
    );
    return {
      success: true,
      port,
...
export async function isDockerContainerRunning(name: string): Promise<boolean> {
  try {
    const output = await execDockerCommand(
      COMMANDS.CHECK_CONTAINER_RUNNING(name),
    );
...
export async function restartContainer(
  ...
  const containerReadyResult =
    await checkIfContainerIsReadyForConnections(containerName);
...
export async function checkIfContainerIsReadyForConnections(
  ...
  dockerLogger.appendLine(
    `Checking if container ${containerName} is ready for connections...`,
  );

  return new Promise((resolve) => {
    const interval = setInterval(async () => {
      try {
        const { dockerCmd, grepCmd } = COMMANDS.CHECK_LOGS(
          containerName,
          startTimestamp,
        );
 ...
 */
async function getUsedPortsFromContainers(
  containerIds: string[],
): Promise<Set<number>> {
...
export async function checkIfConnectionIsDockerContainer(
  ...
  if (!serverName.includes(localhost) && !serverName.includes(localhostIP))
    return "";
...
export async function getAllSqlServerContainerTags(): Promise<string[]> {
  try {
    const stdout = await execDockerCommand(
      COMMANDS.GET_SQL_SERVER_CONTAINER_VERSIONS(),
    );
    const parsed = JSON.parse(stdout);
    return (parsed.tags ?? []).filter((tag: string) => tag);
  } catch (e) {
    dockerLogger.appendLine(
      `Error fetching SQL Server container tags: ${getErrorMessage(e)}`,
    );
    return [];
  }
}
 ...
 */
export async function getSqlServerContainerVersions(): Promise<
  FormItemOptions[]
> {
  try {
...
export async function checkContainerExists(name: string): Promise<boolean> {
  try {
    ...
    dockerLogger.appendLine(
      `Error checking if container exists: ${getErrorMessage(e)}`,
    );
    return false;
  }
}
