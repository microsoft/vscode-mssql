...
> {
  ...
  constructor(
    ...
    context.subscriptions.push(
      vscode.window.onDidChangeActiveTextEditor((editor) => {
        ...
        if (hasWebviewViewState && !hasPanel) {
           ...
           */
          if (
            this.shouldAutoRevealResultsPanel &&
            editorViewColumn !== panelViewColumn
          ) {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        Constants.cmdHandleSummaryOperation,
        async (uri) => {
  ...
  private get shouldAutoRevealResultsPanel(): boolean {
    return this.vscodeWrapper
      .getConfiguration()
      .get(Constants.configAutoRevealResultsPanel);
  }
  ...
  public getNumExecutionPlanResultSets(
    ...
    Object.values(resultSetSummaries).forEach((batch) => {
      Object.values(batch).forEach((result) => {
        ...
        if (
          result.columnInfo[0].columnName === Constants.showPlanXmlColumnName
        ) {
          total++;
        }
      });
  ...
  public updateSelectionSummary() {
    let activeUri = Array.from(
      this._queryResultWebviewPanelControllerMap.keys(),
    ).find(
    ...
    if (state?.selectionSummary) {
      ...
      this._selectionSummaryStatusBarItem.tooltip =
        state.selectionSummary.tooltip;
      this._selectionSummaryStatusBarItem.command =
        state.selectionSummary.command;
