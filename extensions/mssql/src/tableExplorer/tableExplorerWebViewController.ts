...
> {
  ...
  private async initialize(): Promise<void> {
    ...
    if (!this._targetNode) {
      this.logger.error(
        `No target node provided - OperationId: ${this.operationId}`,
      );
    ...
    try {
      ...
      let connectionCreds = Object.assign(
        {},
        this._targetNode.connectionProfile,
      );
      ...
      endActivity.endFailed(
        new Error(
          `Failed to initialize table explorer: ${getErrorMessage(error)}`,
        ),
        true,
        undefined,
        undefined,
   ...
   */
  private async regenerateScriptIfVisible(
    state: TableExplorerWebViewState,
  ): Promise<void> {
  ...
  private registerRpcHandlers(): void {
    this.registerReducer("commitChanges", async (state) => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          LocConstants.TableExplorer.failedToSaveChanges(
    ...
    this.registerReducer("loadSubset", async (state, payload) => {
      ...
      try {
        ...
        const backendRowsOnly = subsetResult.subset.filter(
          (row) => !newRowIds.has(row.id),
        );
    ...
    this.registerReducer("createRow", async (state) => {
      ...
      try {
        const result = await this._tableExplorerService.createRow(
          state.ownerUri,
        );
        vscode.window.showInformationMessage(
        ...
        vscode.window.showErrorMessage(
          LocConstants.TableExplorer.failedToCreateNewRow(
            getErrorMessage(error),
          ),
        );
      }
    ...
    this.registerReducer("deleteRow", async (state, payload) => {
      ...
      try {
        await this._tableExplorerService.deleteRow(
          state.ownerUri,
          payload.rowId,
        );
        vscode.window.showInformationMessage(
    ...
    this.registerReducer("generateScript", async (state) => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          LocConstants.TableExplorer.failedToGenerateScript(
            getErrorMessage(error),
          ),
        );
      }
    ...
    this.registerReducer("openScriptInEditor", async (state) => {
      this.logger.info(
        `Opening script in SQL editor - OperationId: ${this.operationId}`,
      );
    ...
    this.registerReducer("copyScriptToClipboard", async (state) => {
      this.logger.info(
        `Copying script to clipboard - OperationId: ${this.operationId}`,
      );
  ...
  public override dispose(): void {
    if (this.state.ownerUri) {
      ...
      void this._tableExplorerService
        .dispose(this.state.ownerUri)
        .catch((error) => {
          this.logger.error(
