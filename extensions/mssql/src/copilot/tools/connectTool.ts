...
export class ConnectTool extends ToolBase<ConnectToolParams> {
  ...
  private async findConnectionWithProperties(
    ...
    if (profileId) {
      ...
      return {
        profile: undefined,
        matchType: ConnectionMatchType.ProfileNotFound,
      };
    }
    ...
    if (serverName) {
      ...
      if (database) {
        ...
        if (exactMatch) {
          return {
            profile: exactMatch,
            matchType: ConnectionMatchType.ExactMatch,
          };
        }
      }
      ...
      if (serverMatch) {
        return {
          profile: serverMatch,
          matchType: ConnectionMatchType.ServerMatch,
        };
      }
      ...
      return {
        profile: undefined,
        matchType: ConnectionMatchType.NoServerMatch,
      };
    }
  ...
  async call(
    ...
    const result = await this.findConnectionWithProperties(
      profileId,
      serverName,
      database,
    );
    ...
    try {
      ...
      message = success
        ? loc.connectToolSuccessMessage
        : loc.connectToolFailMessage;
    } catch (err) {
      success = false;
  ...
  async prepareInvocation(
    ...
    if (profileId) {
      confirmationText =
        loc.connectToolConfirmationMessageWithProfile(profileId);
      invocationMessage =
        loc.connectToolInvocationMessageWithProfile(profileId);
    } else {
      confirmationText = database
        ? loc.connectToolConfirmationMessageWithServerAndDatabase(
            serverName!,
            database,
          )
        : loc.connectToolConfirmationMessageWithServerOnly(serverName!);
      invocationMessage = database
        ? loc.connectToolInvocationMessageWithServerAndDatabase(
            serverName!,
            database,
          )
