...
export const createSqlAgentRequestHandler = (
  ...
  const handler: vscode.ChatRequestHandler = async (
    ...
    logger.logDebug(
      activeEditor ? "Active editor found." : "No active editor found.",
    );
    ...
    async function findEditorFromReferences(
      ...
      for (const reference of references) {
        ...
        if (referenceUri) {
          ...
          logger.info(
            "No matching visible editor found. Checking tab groups...",
          );
      ...
      logger.info(
        "No matching editor found in references. Checking tab groups...",
      );
      ...
      logger.logDebug(
        "No matching editor found in tab groups. Returning undefined.",
      );
      return undefined;
    }
    ...
    try {
      ...
      if (!connectionUri || !connection) {
        ...
        if (!commandSkipsConnectionLabels(request.command)) {
          ...
          stream.markdown(
            `${loc.connectionRequiredMessage(buttonInfo.label)}\n\n`,
          );
          stream.button({
  ...
  async function processToolCalls(
    ...
    for (const toolCall of sqlTools) {
      try {
        logger.logDebug(
          `Getting next message for conversationUri: ${conversationUri}`,
        );
        ...
        stream.markdown(
          `⚠️ ${loc.messageCouldNotBeProcessed}\n${copilotFeedbackUrl}`,
        );
    ...
    logger.logDebug(
      `Finished processing tool calls for conversationUri: ${conversationUri}`,
    );
    return result;
  }
  ...
  function prepareRequestMessages(
    ...
    const historyMessages = context.history
      .map((historyItem) => {
        if ("prompt" in historyItem) {
          ...
          return vscode.LanguageModelChatMessage.User(
            historyPrefix + historyItem.prompt,
          );
        } else if (
          "response" in historyItem &&
          Array.isArray(historyItem.response)
        ) {
      ...
      .filter(
        (msg): msg is vscode.LanguageModelChatMessage => msg !== undefined,
      );
    ...
    return tools.map((tool, index): vscode.LanguageModelChatTool => {
      try {
        ...
        if (!tool.functionName || typeof tool.functionName !== "string") {
          throw new Error(
            `Tool at index ${index} must have a valid functionName`,
          );
        }
    ...
    for await (const part of chatResponse.stream) {
      if (part instanceof vscode.LanguageModelTextPart) {
        ...
        const { sqlTool: tool, sqlToolParameters: parameters } =
          await processToolCall(tools, part, stream, correlationId, logger);
        if (tool) {
  ...
  async function handleRequestLLMMessage(
    ...
    const options: vscode.LanguageModelChatRequestOptions = {
      justification:
        "Azure SQL Copilot agent requires access to language model.",
      tools: [],
    };
    ...
    const messages = prepareRequestMessages(
      result,
      context,
      referenceTexts,
      logger,
  ...
  async function processToolCall(
    ...
    if (!tool) {
      logger.logDebug(
        `No tool was found for: ${part.name} - ${JSON.stringify(part.input)}}`,
      );
      if (copilotDebugLogging) {
        stream.markdown(
          loc.toolLookupFor(part.name, JSON.stringify(part.input)),
        );
      }
    ...
    try {
      ...
      throw new Error(
        loc.gotInvalidToolUseParameters(
  ...
  async function sendToDefaultLanguageModel(
    ...
    try {
      ...
      logger.error(
        "Error in fallback language model call:",
        getErrorMessage(err),
      );
...
export function provideFollowups(
  ...
  if (
    (result as ISqlChatResult).metadata?.command !== CHAT_COMMAND_NAMES.help
  ) {
    return [];
  }
