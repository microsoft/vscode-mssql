...
export class SqlOutputContentProvider {
  ...
  constructor(
    ...
    this._context.subscriptions.push(
      vscode.commands.registerCommand(
        Constants.cmdDisableActualPlan,
        async (context) => {
    ...
    this._context.subscriptions.push(
      vscode.commands.registerCommand(
        Constants.cmdrevealQueryResult,
        (uri: vscode.Uri) => {
  ...
  public async createQueryRunner(
    ...
    if (this._queryResultsMap.has(uri)) {
      ...
      const startListener = queryRunner.onStart(async (_panelUri) => {
        ...
        resultWebviewState.tabStates.resultPaneTab =
        ...
        sendActionEvent(
          ...
          {
            defaultLocation: isOpenQueryResultsInTabByDefaultEnabled()
              ? "tab"
              : "pane",
          },
        );
      ...
      const onExecutionPlanListener = queryRunner.onExecutionPlan(async (e) => {
        ...
        const resultWebviewState =
          this._queryResultWebviewController.getQueryResultState(e.uri);

        const existingGraphs =
          resultWebviewState.executionPlanState.executionPlanGraphs;
      ...
      const onSelectionSummaryListener = queryRunner.onSummaryChanged(
        async (e) => {
          const state = this._queryResultWebviewController.getQueryResultState(
            e.uri,
          );
          if (!state) {
            return;
  ...
  public async updateQueryRunnerUri(
    oldUri: string,
    newUri: string,
  ): Promise<void> {
  ...
  public onToggleActualPlan(isEnable: boolean): void {
    ...
    if (isEnable && !actualPlanStatuses.includes(uri)) {
      ...
      this._actualPlanStatuses = actualPlanStatuses.filter(
