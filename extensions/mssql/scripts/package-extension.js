...
function packageExtension(packageName = null) {
  ...
  try {
    ...
    logger.success(
      `Extension packaged${packageName ? `: ${packageName}` : ""}`,
    );
  } catch (error) {
...
async function packageOffline() {
  ...
  try {
    ...
    logger.info(
      `Creating offline packages for: ${packageJson.name} v${packageJson.version}`,
    );
    ...
    for (let i = 0; i < OFFLINE_PLATFORMS.length; i++) {
      ...
      logger.info(
        `[${i + 1}/${OFFLINE_PLATFORMS.length}] Processing ${platform.rid}...`,
      );
