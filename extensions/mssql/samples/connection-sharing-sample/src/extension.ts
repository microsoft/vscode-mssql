...
> {
  ...
  if (!connectionSharingService) {
    vscode.window.showErrorMessage(
      "Connection sharing service is not available",
    );
    return;
  }
...
async function connectionSharingWithCommands() {
  try {
    ...
    if (!activeConnectionId) {
      vscode.window.showErrorMessage(
        "No database connection found for the active editor",
      );
      return;
    }
    ...
    if (databaseForConnection) {
      console.log(
        `Database for connection ${activeConnectionId}: ${databaseForConnection}`,
      );
    } else {
      console.log(
        `No database configured for connection ${activeConnectionId}`,
      );
    }
    ...
    console.log(
      `Successfully connected. Connection URI: ${databaseConnectionUri}`,
    );
...
async function connectionSharingWithApis() {
  try {
    ...
    const activeDatabase =
      await connectionSharingService.getActiveDatabase(EXTENSION_ID);
    if (activeDatabase) {
      console.log(`Active database: ${activeDatabase}`);
      vscode.window.showInformationMessage(
        `Currently connected to database: ${activeDatabase}`,
    ...
    // New feature: Get database name for a specific connection ID
    const databaseForConnection =
      await connectionSharingService.getDatabaseForConnectionId(
        EXTENSION_ID,
        activeConnectionId,
      );
    if (databaseForConnection) {
    ...
    const serverInformation = await connectionSharingService.getServerInfo(
      databaseConnectionUri,
    );
    if (!serverInformation) {
      vscode.window.showErrorMessage(
        "Failed to retrieve database server information",
      );
      return;
    }
    ...
    const databaseListResults =
      await connectionSharingService.executeSimpleQuery(
        databaseConnectionUri,
        "SELECT TOP(10) name AS DatabaseName FROM sys.databases ORDER BY name",
      );
    ...
    vscode.window.showInformationMessage(
      "Connection sharing API demo completed successfully!",
    );
  } catch (error) {
  ...
  try {
    ...
    if (result === "approved") {
      ...
      vscode.window.showWarningMessage(
        "Connection sharing permissions were denied.",
      );
    } else {
      vscode.window.showInformationMessage(
    ...
    vscode.window.showErrorMessage(
      `Failed to open permissions dialog: ${error}`,
    );
  }
}
