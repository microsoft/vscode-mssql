...
suite("CloudAuthApplication Tests", () => {
  ...
  setup(() => {
    ...
    mockCachePluginProvider = sandbox.createStubInstance(
      MsalCachePluginProvider,
    );
  ...
  test("should initialize and create client application", async () => {
    ...
    sandbox
      .stub(providerSettings, "getCloudProviderSettings")
      .callsFake(getCloudSettingsStub);
    ...
    sandbox
      .stub(msalNode, "PublicClientApplication")
      .callsFake(publicClientApplicationStub);
    ...
    expect(getCloudSettingsStub).to.have.been.calledTwice.calledWithExactly(
      cloudId,
    );
  ...
  test("should create and cache MsalAzureCodeGrant instance", async () => {
    ...
    const codeGrantInstance1 = cloudAuthApp.msalAuthInstance(
      AzureAuthType.AuthCodeGrant,
    );
    const codeGrantInstance2 = cloudAuthApp.msalAuthInstance(
      AzureAuthType.AuthCodeGrant,
    );
    ...
    const deviceCodeInstance = cloudAuthApp.msalAuthInstance(
...
suite("MsalAzureController Tests", () => {
  ...
  setup(() => {
    ...
    mockSubscriptionClientFactory =
      sandbox.stub() as azureUtils.SubscriptionClientFactory;
  });
  ...
  test("clearTokenCache should clear cache for all cloud auth mappings", async () => {
    ...
    sandbox
      .stub(mockCloudAuth, "clientApplication")
      .get(() => mockClientApplication);

    controller["_cloudAuthMappings"] = new Map();
    controller["_cloudAuthMappings"].set(CloudId.AzureCloud, mockCloudAuth);

    const mockCachePluginProvider = sandbox.createStubInstance(
      MsalCachePluginProvider,
    );
    ...
    expect(mockCachePluginProvider.clearCacheEncryptionKeys).to.have.been
      .calledOnce;
  });
