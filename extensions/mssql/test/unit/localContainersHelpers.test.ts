...
suite("localContainers logic", () => {
  ...
  test("validateDockerConnectionProfile validates containerName and port", async () => {
    ...
    const validResult =
      await localContainersHelpers.validateDockerConnectionProfile(state);
  ...
  test("completeDockerStep updates state on successful step", async () => {
    ...
    localContainersHelpers.registerLocalContainersReducers(
      deploymentController,
    );
    const newState = await (deploymentController as any).completeDockerStep(
      state,
      {
        dockerStep: 0,
      },
  ...
  test("resetDockerStepState resets current step", async () => {
    ...
    const newState = await (deploymentController as any).resetDockerStepState(
      state,
      {},
    );
  ...
  test("checkDockerProfile validates form and sends telemetry", async () => {
    ...
    sandbox.stub(dockerUtils, "findAvailablePort").resolves(1433);
    sandbox
      .stub(localContainersHelpers, "validateDockerConnectionProfile")
      .resolves({
        formState: {
    ...
    // Call the reducer
    const newState = await (deploymentController as any).checkDockerProfile(
      state,
      {},
    );
  ...
  test("updateLocalContainersState updates state", async () => {
    await localContainersHelpers.updateLocalContainersState(
      deploymentController,
      {} as any,
    );
