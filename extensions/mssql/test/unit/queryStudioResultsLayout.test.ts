 ...
 * splits, fair-share growth with the 12-row minimum, and pane scrolling
 * once even the minimums overflow.
 */
...
suite("Query Studio results layout (sizing v2)", () => {
    ...
    test("all grids fit → each gets exactly its content height", () => {
    ...
    test("a grid never grows past its own content, even under the minimum", () => {
        // Fair share is below the minimum, but this grid only has 2 rows —
        // it gets exactly its content, not the 12-row floor.
    ...
    test("zero-row grids reserve one visual row", () => {
        ...
        expect(layout.sizing[0]).to.deep.equal({
            kind: "height",
            bodyPx: qsGridContentHeight(1, metrics),
        });
        expect(layout.paneScrolls).to.equal(false);
    });
});
