...
suite("ObjectExplorerDragAndDropController Tests", () => {
  ...
  setup(() => {
    ...
    getQualifiedNameStub = sandbox.stub(
      ObjectExplorerUtils,
      "getQualifiedName",
    );
    ...
    sandbox
      .stub(mockConnectionStore, "connectionConfig")
  ...
  suite("handleDrag", () => {
    ...
    test("should handle drag for ConnectionGroupNode", () => {
      ...
      expect(
        getQualifiedNameStub.notCalled,
        "getQualifiedName should not be called",
      ).to.be.true;
    });
  ...
  suite("handleDrop", () => {
    test("should handle drop of connection onto connection group successfully", async () => {
      ...
      (
        mockConnectionStore.connectionConfig
          .getConnectionById as sinon.SinonStub
      ).resolves(mockProfile);
      (
      ...
      expect(
        (
          mockConnectionStore.connectionConfig
            .updateConnection as sinon.SinonStub
        ).calledOnce,
    ...
    test("should handle drop of connection group onto another connection group successfully", async () => {
      ...
      // Mock connection store methods
      (
        mockConnectionStore.connectionConfig.getGroupById as sinon.SinonStub
      ).returns(mockSourceGroup);
      (
        mockConnectionStore.connectionConfig.updateGroup as sinon.SinonStub
      ).resolves();
