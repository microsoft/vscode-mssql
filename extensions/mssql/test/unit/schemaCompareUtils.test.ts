...
suite("Schema Compare Utils Tests", () => {
  ...
  test("publishProjectChanges should call schemaCompareService with project directory path", async () => {
    ...
    expect(mockSchemaCompareService.publishProjectChanges.calledOnce).to.be
      .true;
    expect(
      mockSchemaCompareService.publishProjectChanges.firstCall.args[0],
    ).to.equal(operationId);
    expect(
      mockSchemaCompareService.publishProjectChanges.firstCall.args[1],
    ).to.equal(projectDirectoryPath);
    expect(
      mockSchemaCompareService.publishProjectChanges.firstCall.args[2],
    ).to.equal(extractTarget);
    expect(
      mockSchemaCompareService.publishProjectChanges.firstCall.args[3],
    ).to.equal(taskExecutionMode);
  });
});
