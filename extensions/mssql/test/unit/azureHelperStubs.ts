...
export function stubIsSignedIn(sandbox: Sinon.SinonSandbox, result: boolean) {
  return sandbox
    .stub(AzureHelpers.VsCodeAzureHelper, "isSignedIn")
    .resolves(result);
}
...
export function stubVscodeAzureSignIn(sandbox: sinon.SinonSandbox) {
  return sandbox.stub(AzureHelpers.VsCodeAzureHelper, "signIn").resolves({
    ...
    getTenants: () =>
      Promise.resolve(
        mockTenants.filter((t) => t.account.id === mockAccounts[0].id),
      ),
...
export function stubPromptForAzureSubscriptionFilter(
  sandbox: Sinon.SinonSandbox,
  result: boolean,
) {
  return sandbox
    .stub(AzureHelpers, "promptForAzureSubscriptionFilter")
    .resolves(result);
}
