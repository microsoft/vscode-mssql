...
suite("ReactWebviewPanelController", () => {
  ...
  function createController<TResult = void>(
    options: Partial<MssqlWebviewPanelOptions> = {},
  ) {
    ...
    const controller = new TestReactWebviewPanelController<TResult>(
      mockContext,
      {
        ...defaultOptions,
        ...options,
  ...
  test("should show restore prompt when showRestorePromptAfterClose is true", async () => {
    ...
    const disposeHandler = (mockPanel.onDidDispose as sinon.SinonStub).firstCall
      .args[0];
  ...
  suite("DialogResult", () => {
    ...
    test("dialogResult should be undefined when disposed without setting dialogResult", async () => {
      ...
      assert.equal(
        isCompleted,
        false,
        "dialogResult should be an uncompleted promise",
      );
...
class TestReactWebviewPanelController<
  TResult,
> extends ReactWebviewPanelController<TestState, TestReducers, TResult> {
  constructor(
