...
suite("ReactWebviewController Tests", () => {
  ...
  test("Should register default request handlers", () => {
    ...
    assert.ok(
      onNotificationStub.calledWith(
        LoadStatsNotification.type,
        sinon.match.any,
      ),
  ...
  test("Should send initial keybindings notification", () => {
    ...
    assert.ok(
      ...
      "Configuration listener not registered",
    );
  });
  ...
  test("Should notify keybindings when configuration changes", () => {
    ...
    handler({
      affectsConfiguration: (section: string) =>
        section === Constants.configShortcuts,
  ...
  test("GetKeyBindingsConfigRequest returns current configuration", async () => {
    ...
    assert.ok(
      requestCall,
      "GetKeyBindingsConfigRequest handler not registered",
    );
  ...
  test("Should setup theming and handle theme change", () => {
    const originalOnChangeActiveColorTheme =
    ...
    try {
      ...
      (vscode.window.onDidChangeActiveColorTheme as any) =
        originalOnChangeActiveColorTheme;
    }
  });
