...
suite("SqlInlineCompletionProvider Tests", () => {
    ...
    let schemaContextService: sinon.SinonStubbedInstance<CompletionSchemaContextService>;
    ...
    test("honors the configured schema-context switch even when a profile enables it", async () => {
        useSchemaContextEnabled = false;
        ...
        expect(schemaContextService.getSchemaContext).to.not.have.been.called;
        const userMessageText = getMessageText(sendRequestStub.firstCall.args[0][1]);
        expect(userMessageText).to.include("<schema_context>\n-- unavailable\n</schema_context>");
    });
