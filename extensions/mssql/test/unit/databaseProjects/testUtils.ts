...
import { exists } from "../../../src/databaseProjects/common/utils";
...
export function initializeTestServices(): Promise<void> {
    serviceInitialization ??= (async () => {
        ...
        const controller = await vscode.commands.executeCommand<MainController>(
            "mssql.getControllerForTests",
        );
        initializeDatabaseProjectsServices(createMssqlInternalApi(controller), {
            defaultProjectSaveLocation: undefined,
...
export function getTemplatesRootPath(): string {
    return getExtensionResourcePath("resources", "databaseProjects", "templates");
}
