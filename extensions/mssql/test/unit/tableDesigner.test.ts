...
suite("TableDesignerWebviewController tests", () => {
  ...
  setup(async () => {
    ...
    mockConnectionManager.createConnectionDetails.returns(
      mockConnectionDetails as any,
    );
    mockConnectionManager.getConnectionString.resolves(
      mockConnectionDetails.connectionString,
    );
  ...
  test("should call processTableEdit in processTableEdit reducer", async () => {
    ...
    let processTableEditStub =
      mockTableDesignerService.processTableEdit.resolves(editResponse);
    ...
    expect(
      ...
      mockPayload.table,
      mockPayload.tableChangeInfo,
    );
    expect(
      result.tabStates.resultPaneTab,
      "State tab should be set to Script",
    ).to.equal(td.DesignerResultPaneTabs.Script);
  ...
  test("should call publishTable in publishTable reducer", async () => {
    ...
    let publishChangesStub =
      mockTableDesignerService.publishChanges.resolves(publishResponse);
    ...
    const errorMessage = "error message";
    sinon
      .stub(mockTableDesignerService, "publishChanges")
      .rejects(new Error(errorMessage));
    ...
    ).to.equal(`Error: ${errorMessage}`);

    expect(
      result.apiState.publishState,
      "State should contain correct status",
    ).to.equal(td.LoadState.Error);
  });
  ...
  test("should call generateScript in generateScript reducer", async () => {
    ...
    // First scenario: no issues, view is defined
    let scriptStub =
      mockTableDesignerService.generateScript.resolves(scriptResponse);
  ...
  test("should call generatePreviewReport in generatePreviewReport reducer", async () => {
    ...
    // Success scenario
    let result = await controller["_reducerHandlers"].get(
      "generatePreviewReport",
    )(callState, mockPreviewPayload);
  ...
  test("should set mainPaneTab in setTab reducer", async () => {
    ...
    const result = await controller["_reducerHandlers"].get("setTab")(
      state as any,
      { tabId },
    );
  ...
  test("should set resultPaneTab in setResultTab reducer", async () => {
    ...
    const result = await controller["_reducerHandlers"].get("setResultTab")(
      state as any,
      {
        tabId,
      },
  ...
  test("should set publishState and send telemetry in continueEditing reducer", async () => {
    ...
    await controller["_reducerHandlers"].get("continueEditing")(
      state,
      mockPayload,
    );
