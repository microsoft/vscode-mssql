...
suite("keyboardUtils Tests", () => {
  ...
  suite("parseWebviewKeyboardShortcutConfig", () => {
    test("should parse configuration with defaults", () => {
      ...
      expect(result[WebviewAction.ResultGridCopySelection].label).to.equal(
        "Ctrl+C",
      );
      expect(
        result[WebviewAction.ResultGridCopySelection].keyCombination.ctrlKey,
      ).to.equal(true);
    ...
    test("should override defaults", () => {
      ...
      expect(
        result[WebviewAction.ResultGridSelectAll].keyCombination.ctrlKey,
      ).to.equal(true);
      expect(
        result[WebviewAction.ResultGridSelectAll].keyCombination.shiftKey,
      ).to.equal(true);
    });
  });
  ...
  suite("eventMatchesShortcut", () => {
    ...
    test("should match special keys", () => {
      ...
      const escapeEvent = createKeyboardEvent({
        key: "Escape",
        code: "Escape",
      });
      const escapeCombo: WebviewKeyCombination = {
        key: "Escape",
        code: "Escape",
      };
    ...
    test("should treat undefined modifiers as must not be pressed", () => {
      ...
      const event3 = createKeyboardEvent({
        key: "c",
        code: "KeyC",
        shiftKey: true,
      });
      ...
      const event4 = createKeyboardEvent({
        key: "c",
        code: "KeyC",
        altKey: true,
      });
      ...
      const event5 = createKeyboardEvent({
        key: "c",
        code: "KeyC",
        metaKey: true,
      });
