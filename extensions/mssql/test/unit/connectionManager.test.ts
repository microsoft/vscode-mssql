...
suite("ConnectionManager Tests", () => {
    ...
    suite("refreshEntraTokenIfNeeded - MSAL account missing fallback", () => {
        setup(() => {
            connectionManager = new ConnectionManager(
                mockContext,
                mockStatusView,
            ...
            // Simulate account not found in MSAL store
            mockAccountStore.getAccount.resolves(undefined);

            // Token is expired so refresh is needed
            sandbox.stub(AzureController, "isTokenValid").returns(false);

            // Preview feature disabled so MSAL path is used
        ...
        function createAzureMfaConnectionInfo(overrides: Partial<IConnectionInfo> = {}) {
            return {
                server: "test-server",
                authenticationType: "AzureMFA",
                accountId: "vscode-oid.tenant-1",
                tenantId: "tenant-1",
                azureAccountToken: "expired-token",
                expiresOn: Math.floor(Date.now() / 1000) - 1000,
        ...
        test("falls back to VS Code accounts when MSAL account is not found", async () => {
            sandbox.stub(VscodeEntraMfaUtils, "acquireSqlAccessTokenFromVscodeAccount").resolves({
                account: {
                    id: "vscode-oid.tenant-1",
                ...
                tenantId: "tenant-1",
                token: {
                    key: "vscode-oid.tenant-1",
                    token: "vscode-fallback-token",
                    tokenType: "Bearer",
                    expiresOn: Math.floor(Date.now() / 1000) + 3600,
                },
            });
            ...
            expect(mockAzureController.refreshAccessToken).to.not.have.been.called;
            expect(connectionInfo.azureAccountToken).to.equal("vscode-fallback-token");
            expect(connectionInfo.accountId).to.equal("vscode-oid.tenant-1");
            expect(connectionInfo.tenantId).to.equal("tenant-1");
            expect(connectionInfo.user).to.equal("user@example.com");
            expect(connectionInfo.email).to.equal("user@example.com");
        });

        test("throws MissingEntraAuthAccountError when VS Code fallback also fails", async () => {
            sandbox
                .stub(VscodeEntraMfaUtils, "acquireSqlAccessTokenFromVscodeAccount")
                .rejects(new Error("No matching session found"));
            ...
            try {
                ...
                expect(error).to.be.instanceOf(VscodeEntraMfaUtils.MissingEntraAuthAccountError);
