...
suite("ConnectionManager Tests", () => {
  ...
  suite("Initialization Tests", () => {
    ...
    test("Initialization migrates legacy Connection String connections in credential store", async () => {
      ...
      mockConnectionStore.saveProfile.callsFake(
        async (profile: IConnectionProfile) => {
      ...
      expect(savedProfile, "Migrated profile should have been saved").to.not.be
        .undefined;

      expect(
        savedProfile,
        "Saved profile should have the expected properties",
      ).to.deep.equal({
        id: testConnectionId,
        server: testServer,
        database: testDatabase,
  ...
  suite("Functionality tests", () => {
    ...
    test("User is informed when legacy connection migration fails", async () => {
      ...
      const result =
        await connectionManager["migrateLegacyConnection"](erroringConnProfile);

      expect(
        result,
        "Migration should return that it errored instead of throwing",
      ).to.equal("error");
  ...
  suite("Token request handling", () => {
    ...
    test("should return cached token when valid", async () => {
      ...
      connectionManager["_keyVaultTokenCache"].set(
  ...
  suite("prepareConnectionInfo", () => {
    ...
    test("should handle Entra/Azure MFA authentication", async () => {
      ...
      expect(confirmEntraTokenValidityStub).to.have.been.calledOnceWith(
        mockConnectionInfo,
      );
