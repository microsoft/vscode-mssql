...
suite("SqlOutputProvider Tests using mocks", () => {
  ...
  setup(() => {
    ...
    sandbox
      .stub(vscodeWrapper, "onDidOpenTextDocument")
      .get(() => () => disposable);
    sandbox
      .stub(vscodeWrapper, "onDidChangeConfiguration")
      .get(() => () => disposable);
    ...
    contentProvider.setVscodeWrapper =
      vscodeWrapper as unknown as VscodeWrapper;
    ...
    mockContentProvider.onUntitledFileSaved.callsFake(
      async (oldUri, newUri) => {
        const entry = ensureRunnerState(oldUri);
        mockMap.delete(oldUri);
        mockMap.set(newUri, {
          ...entry,
          queryRunner: { isExecutingQuery: true },
        });
      },
    );

    mockContentProvider.onDidCloseTextDocument.callsFake(
  ...
  test("RunQuery properly sets up two queries to be run", async () => {
    ...
    await mockContentProvider.runQuery(
      statusViewInstance,
      uri,
      querySelection,
      title,
  ...
  test("cancelQuery should cancel the execution of a query by result pane URI", async () => {
    ...
    expect(statusView.cancelingQuery).to.have.been.calledOnceWithExactly(
      resultUri,
    );
    expect(mockMap.size).to.equal(1);
  });
  ...
  test("showWarningRequestHandler should call vscodeWrapper to show warning message", () => {
    ...
    expect(vscodeWrapper.showWarningMessage).to.have.been.calledOnceWithExactly(
      "test_warning",
    );
  });
