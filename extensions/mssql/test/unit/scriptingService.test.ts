...
suite("Scripting Service", () => {
  ...
  function configureRunQueryStub(): void {
    sqlOutputContentProvider.runQuery.callsFake(
      async (_statusView, _uri, _options, _title, _something, queryPromise) => {
        if (
          queryPromise &&
          typeof (queryPromise as any).resolve === "function"
        ) {
  ...
  suiteSetup(() => {
    ...
    registerCommandStub = sandbox
      ...
      .callsFake(
        (
          ...
          registeredCommands[commandId] = thisArg
    ...
    sqlOutputContentProvider = sandbox.createStubInstance(
  ...
  test("scriptTreeNode forwards scripting params to script", async () => {
    ...
    const scriptStub = sandbox
      .stub(scriptingService, "script")
      .resolves("SELECT 1");
  ...
  test("createScriptingRequestParams honors engine settings", () => {
    ...
    expect(params.scriptOptions.scriptCompatibilityOption).to.equal(
      "Script160Compat",
    );
  ...
  test("createScriptingRequestParams maps delete operation", () => {
    ...
    expect(params.scriptOptions.targetDatabaseEngineType).to.equal(
      "SingleInstance",
    );
  });
  ...
  test("getScriptCompatibility handles known versions", () => {
    expect(ScriptingService.getScriptCompatibility(15, 0)).to.equal(
      "Script150Compat",
    );
    expect(ScriptingService.getScriptCompatibility(10, 50)).to.equal(
      "Script105Compat",
    );
    expect(ScriptingService.getScriptCompatibility(999, 0)).to.equal(
      "Script140Compat",
    );
  });
  ...
  test("script sends cancellation request when token is cancelled", async () => {
    ...
    expect(
      (cancellationCalls[0] as { operationId: string }).operationId,
    ).to.equal("op-cancel");
  ...
  test("scriptNode commands use tree selection when node not provided", async () => {
    ...
    const scriptNodeStub = sandbox
      .stub(scriptingService, "scriptNode")
      .resolves();

    try {
      await createCommand(undefined);
      expect(scriptNodeStub).to.have.been.calledOnceWithExactly(
        node,
        ScriptOperation.Create,
      );
    } finally {
  ...
  test("scriptNode maps operations for other commands", async () => {
    ...
    try {
      ...
      expect(scriptNodeStub.getCall(0).args[1]).to.equal(
        ScriptOperation.Delete,
      );
      expect(scriptNodeStub.getCall(1).args[1]).to.equal(ScriptOperation.Alter);
      expect(scriptNodeStub.getCall(2).args[1]).to.equal(
