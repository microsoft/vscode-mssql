...
suite("PublishProjectWebViewController Tests", () => {
  ...
  test("constructor initializes state and derives database name", async () => {
    ...
    expect(controller.state.projectFilePath).to.equal(
      "c:/work/MySampleProject.sqlproj",
    );
    ...
    expect(components.databaseName, "databaseName component should exist").to
      .exist;
    expect(components.publishTarget, "publishTarget component should exist").to
      .exist;
  });
  ...
  test("reducer handlers are registered on construction", async () => {
    ...
    const reducerHandlers = controller["_reducerHandlers"] as Map<
      string,
      Function
    >;
    ...
    expect(
      reducerHandlers.has("publishNow"),
      "publishNow reducer should be registered",
    ).to.be.true;
    expect(
  ...
  test("getActiveFormComponents returns correct fields for EXISTING_SERVER target", async () => {
    ...
    const activeComponents = controller["getActiveFormComponents"](
      controller.state,
    );
  ...
  test("Azure SQL project shows Azure-specific labels", async () => {
    ...
    const publishTargetComponent =
      controller.state.formComponents.publishTarget;
    ...
    expect(containerOption?.displayName).to.equal(
  ...
  test("field validators enforce container and server requirements", () => {
    ...
    expect(validateSqlServerPortNumber(70000), "out-of-range port invalid").to
      .be.false;
    expect(validateSqlServerPortNumber(1.5), "decimal port invalid").to.be
      .false;
    expect(validateSqlServerPortNumber(-1), "negative port invalid").to.be
      .false;
    ...
    expect(
      validateSqlServerPassword("Abc123!@#"),
      "complex password valid",
    ).to.equal("");
    expect(
      validateSqlServerPassword("MyTest99"),
      "3 categories valid",
    ).to.equal("");
    expect(
      validateSqlServerPassword("alllower"),
      "simple lowercase invalid",
    ).to.not.equal("");
    expect(
      validateSqlServerPassword("ALLUPPER"),
      "simple uppercase invalid",
    ).to.not.equal("");
    expect(
      validateSqlServerPassword("Short1"),
      "too short invalid",
    ).to.not.equal("");
    expect(
      validateSqlServerPassword("Abc123!@#".repeat(20)),
      "too long invalid",
    ).to.not.equal("");
  });
  ...
  test("getSqlServerContainerTagsForTargetVersion filters versions correctly for SQL Server 2022", async () => {
    ...
    const result =
      await projectUtils.getSqlServerContainerTagsForTargetVersion("160");
...
<Project ToolsVersion="Current" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  test("selectPublishProfile reducer parses XML profile correctly", async () => {
    ...
    sandbox
      ...
      .resolves([vscode.Uri.file(profilePath)]);
    ...
    expect(newState.formState.publishProfilePath.replace(/\\/g, "/")).to.equal(
      profilePath,
    );
    expect(newState.formState.databaseName).to.equal("MyDatabase");
    expect(newState.formState.serverName).to.equal(
    ...
    expect(
      newState.deploymentOptions.booleanOptionsDictionary
  ...
  test("savePublishProfile reducer is invoked and triggers save file dialog", async () => {
    ...
    // Mock DacFx service
    mockDacFxService.savePublishProfile.resolves({
      success: true,
      errorMessage: "",
    });
    ...
    expect(deploymentOptions.excludeObjectTypes.value).to.deep.equal([
      "Users",
      "Permissions",
    ]);
    expect(
    ...
    expect(
      deploymentOptions.booleanOptionsDictionary.allowIncompatiblePlatform
        ?.value,
  ...
  test("server and database fields are initialized with correct default values", async () => {
    ...
    expect(serverComponent.required, "serverName component should be required")
      .to.be.true;
    ...
    expect(
      databaseComponent.required,
      "databaseName component should be required",
    ).to.be.true;
  ...
  test("formAction updates server and database names via user interaction", async () => {
    ...
    expect(controller.state.formState.databaseName).to.equal(
      "SelectedDatabase",
    );
  });
  //#endregion
  ...
  test("deployment options should have three groups: General, Ignore, and Exclude", async () => {
    ...
    const reducerHandlers = controller["_reducerHandlers"] as Map<
      ...
      Function
    >;
    const updateDeploymentOptions = reducerHandlers.get(
    ...
    expect(
      newState.deploymentOptions.booleanOptionsDictionary.ignoreTableOptions,
    ).to.exist;

    // Verify Exclude group - object types dictionary
    expect(newState.deploymentOptions.objectTypesDictionary.users).to.equal(
      "Users",
    );
  });
  ...
  test("updateDeploymentOptions reducer should save and collect options properly", async () => {
    ...
    expect(
      newState.deploymentOptions.booleanOptionsDictionary
        .allowDropBlockingAssemblies.value,
    ).to.be.false;
  ...
  test("deployment options reset restores default values", async () => {
    ...
    expect(
      state.deploymentOptions.booleanOptionsDictionary.ignoreTableOptions.value,
    ).to.be.true;
  ...
  test("SQLCMD variables are loaded from project during initialization", async () => {
    ...
    mockSqlProjectsService.getSqlCmdVariables.resolves({
      ...
      sqlCmdVariables: [
        {
          varName: "DatabaseName",
          value: "$(DatabaseName)",
          defaultValue: "MyTestDB",
        },
        {
          varName: "Environment",
          value: "$(Environment)",
  ...
  test("revertSqlCmdVariables reducer restores variables to default values", async () => {
    ...
    expect(
      ...
      "revertSqlCmdVariables reducer should be registered",
    ).to.exist;
  ...
  test("prepareContainerConfiguration generates unique container name and parses port", async () => {
    ...
    sandbox
      ...
      .resolves("sql-server-container-abc123");

    // Call the method
    const config = await controller["prepareContainerConfiguration"](
      controller.state,
    );
  ...
  test("publishNow with LocalContainer target runs full Docker workflow", async () => {
    ...
    sandbox
      .stub(
        controller,
        "runDockerPrerequisiteChecks" as keyof typeof controller,
      )
      .resolves({
        success: true,
      });
    ...
    sandbox
      .stub(
        controller,
        "prepareContainerConfiguration" as keyof typeof controller,
      )
      .resolves({
    ...
    sandbox
      .stub(controller, "createDockerContainer" as keyof typeof controller)
      .resolves({
        success: true,
    ...
    sandbox
      .stub(controller, "publishToDatabase" as keyof typeof controller)
      .resolves();
  ...
  function validateContainerPublishError(
    ...
    expect(state.inProgress, "inProgress should be false after error").to.be
      .false;
