...
/** Optional localhost smoke for the exact basic DACPAC extract/import repro.
 * It compiles through the registered product command, executes through the
 * normal local coordinator, verifies the zero-delta import, and removes only
 * the exact ownership-marked target database. */
...
const SOURCE_DATABASE = process.env.RBS2_DACPAC_SOURCE_DATABASE ?? "WideWorldImporters";
const TARGET_DATABASE = `WWI_Import_${randomBytes(5).toString("hex")}`;
const EXACT_INTENT =
    "Extract a wideworldimporter dacpac. Import the dacpac back into the server and name WWI_Import.";

suite("Runbook Studio basic DACPAC round trip live smoke (gated)", function () {
    ...
    test("compiles and runs the exact simple extract/import prompt", async () => {
        const parsed = parseSqlConnectionString(CONNECTION_STRING!);
        if ("error" in parsed) {
        ...
        const profile = {
            ...baseProfile,
            id: `rbs-basic-live-${suffix}`,
            profileName: `Runbook Studio basic live ${suffix}`,
            groupId: "ROOT",
        ...
        ).find((candidate) => candidate.profileName === `Runbook Studio basic live ${suffix}`);
        expect(persistedProfile, "the live connection profile was not persisted").not.to.equal(
            undefined,
        );
        const profileId = persistedProfile!.id;
        const tempDirectory = await fs.promises.mkdtemp(path.join(os.tmpdir(), "rbs-basic-live-"));
        const runbookPath = path.join(tempDirectory, `dacpac-basic-${suffix}.runbook.json`);
        const baseArtifact = createNewRunbookArtifact("New runbook", `rbs-basic-live-${suffix}`);
        ...
        try {
            ...
            expect(
                run.nodeStates?.find((node) => node.nodeId === "verify")?.outputCount,
            ).to.be.greaterThan(0);
            ...
            const objects = await api.connectionSharing.executeSimpleQuery(
                targetUri,
                "SELECT COUNT_BIG(*) AS object_count FROM sys.objects WHERE is_ms_shipped = 0;",
            );
            expect(Number(objects.rows?.[0]?.[0]?.displayValue ?? "0")).to.be.greaterThan(0);
        } finally {
            if (targetUri) {
