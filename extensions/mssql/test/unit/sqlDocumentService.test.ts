...
suite("SqlDocumentService Tests", () => {
  ...
  setup(async () => {
    ...
    let mockContext = {
      globalState: {
        get: sandbox
          .stub()
          .callsFake((_key: string, defaultValue?: any) => defaultValue),
  ...
  test("handleNewQueryCommand should create a new query and update recents", async () => {
    ...
    sandbox
      .stub(node, "connectionProfile")
      .get(() => ({}) as IConnectionProfile);
    ...
    expect((connectionManager as any).connectionStore.removeRecentlyUsed).to
      .have.been.calledOnce;
    expect(connectionManager.handlePasswordBasedCredentials).to.have.been
      .calledOnce;
  });
    ...
    const newQueryStub = sandbox
      ...
      .callsFake((opts: any) => {
        expect(opts.connectionStrategy).to.equal(
          ConnectionStrategy.CopyLastActive,
        );
  ...
  test("handleNewQueryCommand uses OE selection when exactly one node is selected", async () => {
    ...
    expect(
      connectionManager.handlePasswordBasedCredentials,
    ).to.have.been.calledOnceWith(nodeConnection);
    expect(newQueryStub).to.have.been.calledOnce;
    newQueryStub.restore();
  });
  ...
  test("handleNewQueryCommand refreshes Entra token info on source node", async () => {
    ...
    expect(nodeConnection.azureAccountToken).to.equal(
      oldToken.azureAccountToken,
    );
    expect(nodeConnection.expiresOn).to.equal(oldToken.expiresOn);

    await sqlDocumentService.handleNewQueryCommand(node, undefined);

    expect(node.updateEntraTokenInfo).to.have.been.calledOnceWith(
      nodeConnection,
    );
    expect(nodeConnection.azureAccountToken).to.equal(
      newToken.azureAccountToken,
    );
  ...
  test("onDidCloseTextDocument should call renamedDoc function when rename occurs", async () => {
    ...
    const mockTimer = {
      getDuration: sandbox
        .stub()
        .returns(Constants.renamedOpenTimeThreshold - 5), // Less than threshold
      end: sandbox.stub(),
    };
  ...
  test("onDidCloseTextDocument should propagate to the connectionManager even if a special event occurred before it", (done) => {
    ...
    const expiredTimer = {
      getDuration: sandbox
        .stub()
        .returns(Constants.untitledSaveTimeThreshold + 10), // Expired
      end: sandbox.stub(),
    };
  ...
  test("onDidOpenTextDocument should propagate the function to the connectionManager", async () => {
    ...
    expect(
      connectionManager.onDidOpenTextDocument,
    ).to.have.been.calledOnceWithExactly(document);
    expect(docUriCallback).to.equal(document.uri.toString());
  });
  ...
  test("onDidSaveTextDocument should propagate the function to the connectionManager", () => {
    ...
    expect(sqlDocumentService["_lastSavedUri"]).to.equal(
  ...
  test("newQuery should call the new query method", async () => {
    ...
    const mockCreateDocument = sandbox.stub(
      sqlDocumentService as any,
      "createDocument",
    );
  ...
  test("newQuery should copy connection from URI when copyConnectionFromUri is provided", async () => {
    ...
    const mockConnect = sandbox
      .stub()
      .callsFake(async (uri, connectionInfo, promise) => {
  ...
  test("external SQL files auto-connect using last active connection", async () => {
    ...
    const editor1: vscode.TextEditor = {
      document: script1,
    ...
    expect(connectStub).to.have.been.calledOnceWithExactly(
  ...
  test("onDidChangeActiveTextEditor should handle error cases gracefully", async () => {
    ...
    const editorWithDoc = {
      document: mockTextDocument("test.sql"),
    } as vscode.TextEditor;
    ...
    (connectionManager.getConnectionInfo as any).returns(
  ...
  suite("Connection Strategy Tests", () => {
    ...
    setup(() => {
      ...
      mockCreateDocument = sandbox.stub(
        sqlDocumentService as any,
        "createDocument",
      );
      mockCreateDocument.resolves(editor);
      ...
      mockGetConnectionInfoFromUri =
        mockConnectionManager.getConnectionInfoFromUri;
    ...
    test("ConnectionStrategy.CopyFromUri should copy connection from source URI when found", async () => {
      ...
      expect(mockGetConnectionInfoFromUri).to.have.been.calledOnceWith(
        "file:///source.sql",
      );
      expect(mockConnect).to.have.been.calledOnce;
      expect(mockOnNewConnection).to.not.have.been.called;
    ...
    test("should call createObjectExplorerSession when connection is established", async () => {
      ...
      expect(
        mainController.createObjectExplorerSession,
      ).to.have.been.calledWith(connectionInfo);
    });
    ...
    test("should set status view properties correctly", async () => {
      ...
      expect(mockSqlCmdModeChanged).to.have.been.calledOnceWith(
        sinon.match.string,
        false,
      );
    });
