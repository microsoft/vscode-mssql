...
suite("DacpacDialogWebviewController", () => {
  ...
  suite("Deployment Operations", () => {
    test("Publish DACPAC succeeds for new database", async () => {
      ...
      const requestHandler = requestHandlers.get(
        DeployDacpacWebviewRequest.type.method,
      );
  ...
  suite("Extract Operations", () => {
    test("extract DACPAC succeeds", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        ExtractDacpacWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
    ...
    test("extract DACPAC returns error on failure", async () => {
      ...
      expect(response.success).to.be.false;
      expect(response.errorMessage).to.equal(
        "Extraction failed: Database not found",
      );
    });
  });
  suite("Import Operations", () => {
    test("import BACPAC succeeds", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        ImportBacpacWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
    ...
    test("import BACPAC returns error on failure", async () => {
      ...
      expect(response.success).to.be.false;
      expect(response.errorMessage).to.equal(
        "Import failed: Corrupted BACPAC file",
      );
    });
  });
  suite("Export Operations", () => {
    test("export BACPAC succeeds", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        ExportBacpacWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
  ...
  suite("Browse File Operations", () => {
    ...
    test("browse input file returns file path when file is selected", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        BrowseInputFileWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
    ...
    test("browse output file returns file path when file is selected", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        BrowseOutputFileWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
    ...
    test("get suggested database name handles complex filename with multiple hyphens", async () => {
      ...
      expect(result.databaseName).to.equal(
        "My-Complex-Database-Name-2025-01-15-10-30",
      );
    });
    test("get suggested database name extracts only filename from full path", async () => {
      ...
      const testPath = path.join(
        path.sep === "\\"
          ? "C:\\very\\long\\path\\to\\files"
          : "/very/long/path/to/files",
        "TestDB.dacpac",
      );
      const result = await requestHandler!({
        filePath: testPath,
  ...
  suite("File Path Validation", () => {
    test("validates existing DACPAC file", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        ValidateFilePathWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
    ...
    test("rejects non-existent file when it should exist", async () => {
      ...
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.FileNotFound,
      );
    });
    test("rejects empty file path", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.FilePathRequired,
      );
    });
    test("rejects invalid file extension", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.InvalidFileExtension,
      );
    });
    test("validates output file path when directory exists", async () => {
      ...
      const testDir = path.join(
        path.sep === "\\"
          ? "C:\\database-test-folder"
          : "/database-test-folder",
      );
    ...
    test("warns when output file already exists", async () => {
      ...
      expect(response.isValid).to.be.true;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.FileAlreadyExists,
      );
    });
    test("rejects output file path when directory doesn't exist", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.DirectoryNotFound,
      );
    });
  });
  suite("Database Operations", () => {
    test("lists databases successfully", async () => {
      ...
      createController();
      const requestHandler = requestHandlers.get(
        ListDatabasesWebviewRequest.type.method,
      );
      expect(requestHandler, "Request handler was not registered").to.be.a(
        "function",
      );
  ...
  suite("Database Name Validation", () => {
    ...
    test("rejects non-existent database name for extract/export", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.DatabaseNotFound,
      );
    });
    test("rejects empty database name", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.DatabaseNameRequired,
      );
    });
    test("rejects database name with invalid characters", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.InvalidDatabaseName,
      );
    });
    test("rejects database name that is too long", async () => {
      ...
      expect(response.isValid).to.be.false;
      expect(response.errorMessage).to.equal(
        LocConstants.DacpacDialog.DatabaseNameTooLong,
      );
    });
    ...
    test("returns validation failed on error", async () => {
      ...
      expect(response.errorMessage).to.include(
        "Failed to validate database name",
      );
  ...
  suite("Deploy to Existing Database Confirmation", () => {
    test("confirmation dialog shows and returns confirmed=true when user clicks Deploy", async () => {
      ...
      expect(confirmHandler, "Confirm handler was not registered").to.be.a(
        "function",
      );
      ...
      expect(
        vscodeWrapperStub.showWarningMessageAdvanced,
  ...
  suite("Controller Initialization", () => {
    ...
    test("registers all request handlers", () => {
      createController();
      expect(requestHandlers.has(DeployDacpacWebviewRequest.type.method)).to.be
        .true;
      expect(requestHandlers.has(ExtractDacpacWebviewRequest.type.method)).to.be
        .true;
      expect(requestHandlers.has(ImportBacpacWebviewRequest.type.method)).to.be
        .true;
      expect(requestHandlers.has(ExportBacpacWebviewRequest.type.method)).to.be
        .true;
      expect(requestHandlers.has(ValidateFilePathWebviewRequest.type.method)).to
        .be.true;
      expect(requestHandlers.has(ListDatabasesWebviewRequest.type.method)).to.be
        .true;
      expect(
        requestHandlers.has(ValidateDatabaseNameWebviewRequest.type.method),
      ).to.be.true;
      expect(requestHandlers.has(ListConnectionsWebviewRequest.type.method)).to
        .be.true;
      expect(requestHandlers.has(ConnectToServerWebviewRequest.type.method)).to
        .be.true;
      expect(
        requestHandlers.has(ConfirmDeployToExistingWebviewRequest.type.method),
      ).to.be.true;
    });
  ...
  suite("Connection Operations", () => {
    ...
    test("lists connections successfully", async () => {
      ...
      const handler = requestHandlers.get(
        ListConnectionsWebviewRequest.type.method,
      );
    ...
    test("returns empty array when readAllConnections fails", async () => {
      connectionStoreStub.readAllConnections.rejects(
    ...
    test("connects to server successfully when not already connected", async () => {
      ...
      createController();
      const handler = requestHandlers.get(
        ConnectToServerWebviewRequest.type.method,
      );
      expect(handler).to.exist;
      ...
      expect(connectionManagerStub.getUriForConnection).to.have.been
        .calledTwice;
    ...
    test("retrieves ownerUri after successful connection when initially undefined", async () => {
      ...
      expect(connectionManagerStub.connect).to.have.been.calledWith(
        "",
        mockConnections[0],
      );
    });
    test("returns existing ownerUri when already connected", async () => {
      ...
      connectionManagerStub.isConnected
        .withArgs("existing-owner-uri")
        .returns(true);
  ...
  suite("Initialize Connection", () => {
    ...
    test("returns all connections when no initial state provided", async () => {
      ...
      connStoreStub.readAllConnections.resolves(recentConns);
      sandbox.stub(connectionManagerStub, "activeConnections").get(() => ({}));
      createController();
      // Execute
      const handler = requestHandlers.get(
        InitializeConnectionWebviewRequest.type.method,
      );
      expect(handler).to.exist;
      const result = await handler!({});
      // Verify
    ...
    test("matches connection by server and database names", async () => {
      ...
      connStoreStub.readAllConnections.resolves(recentConns);
      // Stub findMatchingProfile to return the matching connection
      connStoreStub.findMatchingProfile.resolves({
        profile: recentConns[1],
        score: 2, // ServerAndDatabase match
      });
      sandbox.stub(connectionManagerStub, "activeConnections").get(() => ({}));
      createController();
      // Execute
    ...
    test("fetches ownerUri for already active connection", async () => {
      ...
      expect(result.autoConnected).to.be.true; // connectToServer was called and returned success
    });
    test("handles exception during initialization gracefully", async () => {
      // Setup
      connStoreStub.readAllConnections.throws(new Error("Store error"));
      createController();
      // Execute
      const handler = requestHandlers.get(
        InitializeConnectionWebviewRequest.type.method,
      );
      expect(handler, "InitializeConnection handler should be registered").to
        .exist;
      const result = await handler!({
        initialProfileId: "profile1",
      });
