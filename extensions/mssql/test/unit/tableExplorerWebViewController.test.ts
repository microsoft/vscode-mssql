...
suite("TableExplorerWebViewController - Reducers", () => {
  ...
  setup(() => {
    ...
    const notificationHandler = (
      mockTableExplorerService.sqlToolsClient.onNotification as any
    ).firstCall.args[1];
    notificationHandler({
  ...
  suite("commitChanges reducer", () => {
    test("should commit changes successfully and clear newRows", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("commitChanges")(
        controller.state,
        {},
      );

      // Assert
      expect(mockTableExplorerService.commit.calledOnceWith("test-owner-uri"))
        .to.be.true;
      expect(
  ...
  suite("loadSubset reducer", () => {
    test("should load subset with specified row count", async () => {
      ...
      expect(
        mockTableExplorerService.subset.calledWith("test-owner-uri", 0, 100),
      ).to.be.true;
  ...
  suite("createRow reducer", () => {
    test("should create a new row and add it to resultSet", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("createRow")(
        controller.state,
        {},
      );
      ...
      // Assert
      expect(
        mockTableExplorerService.createRow.calledOnceWith("test-owner-uri"),
      ).to.be.true;
      expect(
        showInformationMessageStub.calledOnceWith(
  ...
  suite("deleteRow reducer", () => {
    test("should delete a row from resultSet", async () => {
      ...
      // Assert
      expect(
        mockTableExplorerService.deleteRow.calledOnceWith("test-owner-uri", 0),
      ).to.be.true;
      expect(
        showInformationMessageStub.calledOnceWith(
  ...
  suite("revertCell reducer", () => {
    test("should revert cell to original value", async () => {
      ...
      // Assert
      expect(
        mockTableExplorerService.revertCell.calledOnceWith(
          "test-owner-uri",
          0,
          1,
        ),
  ...
  suite("revertRow reducer", () => {
    test("should revert entire row to original values", async () => {
      ...
      // Assert
      expect(
        mockTableExplorerService.revertRow.calledOnceWith("test-owner-uri", 0),
      ).to.be.true;
      expect(
        controller.state.resultSet?.subset[0].cells[1].displayValue,
      ).to.equal("John");
  ...
  suite("generateScript reducer", () => {
    test("should generate script and show script pane", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("generateScript")(
        controller.state,
        {},
      );
      ...
      // Assert
      expect(
        mockTableExplorerService.generateScripts.calledOnceWith(
          "test-owner-uri",
        ),
      ).to.be.true;
  ...
  suite("openScriptInEditor reducer", () => {
    test("should open script in SQL editor", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("openScriptInEditor")(
        controller.state,
        {},
      );
  ...
  suite("copyScriptToClipboard reducer", () => {
    test("should copy script to clipboard", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("copyScriptToClipboard")(
        controller.state,
        {},
      );

      // Assert
      expect(writeTextStub.calledOnceWith("SELECT * FROM TestTable;")).to.be
        .true;
      expect(
        showInformationMessageStub.calledOnceWith(
  ...
  suite("toggleScriptPane reducer", () => {
    test("should toggle script pane from false to true", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("toggleScriptPane")(
        controller.state,
        {},
      );
  ...
  suite("setCurrentPage reducer", () => {
    test("should set current page number", async () => {
      ...
      // Act
      await controller["_reducerHandlers"].get("setCurrentPage")(
        controller.state,
        {
          pageNumber: 5,
        },
