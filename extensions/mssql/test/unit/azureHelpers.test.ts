...
suite("Azure Helpers", () => {
  ...
  suite("VsCodeAzureHelpers", () => {
    test("getAccounts", async () => {
      ...
      expect(
        accounts,
        "Only signed-in accounts should be returned",
      ).to.deep.equal(mockAccounts);
    });
    ...
    test("signIn", async () => {
      ...
      let result = await azureHelpers.VsCodeAzureHelper.signIn(
        false /* forceSignInPrompt */,
      );

      expect(result).to.not.be.undefined;
      expect(signInStub.calledOnce, "signIn should be called once").to.be.true;
      expect(isSignedInStub.calledOnce, "isSignedIn should be called once").to
        .be.true;
      ...
      result = await azureHelpers.VsCodeAzureHelper.signIn(
        false /* forceSignInPrompt */,
      );
    ...
    test("getTenantsForAccount", async () => {
      ...
      const tenants =
        await azureHelpers.VsCodeAzureHelper.getTenantsForAccount(account);
  ...
  test("getTenants handles error cases", async () => {
    const getAccountStub =
      mockAzureAccountService.getAccount as sinon.SinonStub;
    // undefined tenants
    ...
    expect(
      (mockLogger.error as sinon.SinonStub).calledWithMatch(
        "undefined tenants",
      ),
    ...
    result = await azureHelpers.getTenants(
      mockAzureAccountService,
      "test-user-id",
      mockLogger,
    );
  ...
  test("fetchServersFromAzure", async () => {
    ...
    expect(servers[0].server).to.equal(
      mockAzureResources.azureSqlDbServer.name,
    );
