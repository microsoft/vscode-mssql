...
suite("Query Runner tests", () => {
  ...
  setup(() => {
    ...
    testQueryNotificationHandler = sandbox.createStubInstance(
    ...
    (testVscodeWrapper.showInformationMessage as sinon.SinonStub).returns(
      undefined,
    );
    (testVscodeWrapper.logToOutputChannel as sinon.SinonStub).returns(
      undefined,
    );
    (testVscodeWrapper.openTextDocument as sinon.SinonStub).resolves(
      {} as vscode.TextDocument,
    );
    (testVscodeWrapper.showTextDocument as sinon.SinonStub).resolves(
  ...
  test("Handles Query Request Result Properly", async () => {
    ...
    expect(
      testQueryNotificationHandler.registerRunner.firstCall.args[1],
    ).to.equal(standardUri);

    // ... The VS Code status should be updated
    expect(testStatusView.executingQuery).to.have.been.calledOnceWithExactly(
      standardUri,
    );
    expect(testVscodeWrapper.logToOutputChannel as sinon.SinonStub).to.have.been
      .calledOnce;
  ...
  test("Handles Query Request Error Properly", async () => {
    ...
    try {
      ...
      expect(testVscodeWrapper.logToOutputChannel as sinon.SinonStub).to.have
        .been.calledOnce;
      expect(testStatusView.executingQuery).to.have.been.calledOnceWithExactly(
        standardUri,
      );
  ...
  test("Notification - Query complete", () => {
    ...
    expect(testStatusView.executedQuery).to.have.been.calledOnceWithExactly(
      standardUri,
    );
    expect(testStatusView.setExecutionTime).to.have.been.calledOnce;
    expect(testStatusView.setExecutionTime.firstCall.args[0]).to.equal(
      standardUri,
    );
  ...
  test("Correctly handles error from subset request", async () => {
    ...
    expect(testVscodeWrapper.showErrorMessage as sinon.SinonStub).to.have.been
      .calledOnce;
  });
  ...
  function setupWorkspaceConfig(configResult: { [key: string]: any }): void {
    ...
    (testVscodeWrapper.getConfiguration as sinon.SinonStub).callsFake(
      () => config,
    );
  }
});
