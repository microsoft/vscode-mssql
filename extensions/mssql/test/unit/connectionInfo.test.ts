...
suite("getConnectionDisplayString", () => {
  ...
  setup(() => {
    ...
    sandbox
      .stub(LocalizedConstants, "defaultDatabaseLabel")
      .value("default-db");
  });
  ...
  test("returns formatted string with server, database and user", () => {
    ...
    // Assert
    expect(result).to.equal(
      `${mockServer} : $(database) ${mockDatabase} : ${mockUser}`,
    );
  });
  ...
  test("uses default database label when database is not provided", () => {
    ...
    // Assert
    expect(result).to.equal(
      `${mockServer} : $(database) default-db : ${mockUser}`,
    );
  });
...
suite("fixupConnectionCredentials", () => {
  ...
  test("Azure SQL Database specific fixups", () => {
    ...
    expect(
      connCreds.encrypt,
      "encrypt should be set to Mandatory for Azure SQL",
    ).to.equal(EncryptOptions.Mandatory);
    expect(
      connCreds.connectTimeout,
    ...
    expect(
      connCreds.encrypt,
      "encrypt should remain Strict for Azure SQL",
    ).to.equal(EncryptOptions.Strict);
  });
});
...
test("getServerTypes", () => {
  const testCases = [
    {
      input: "test.database.windows.net",
      expected: [ServerType.Azure, ServerType.Sql],
    },
    {
    ...
    {
      input: "test.database.fabric.microsoft.com,1433",
      expected: [ServerType.Fabric, ServerType.Sql],
    },
    {
      input: "test.ppe-database.fabric.microsoft.com,1433",
      expected: [ServerType.Fabric, ServerType.Sql],
    },
    {
