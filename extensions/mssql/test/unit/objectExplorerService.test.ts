...
suite("OE Service Tests", () => {
  suite("rootNodeConnections", () => {
    ...
    setup(() => {
      ...
      sandbox
        .stub(mockConnectionStore, "rootGroupId")
        .get(() => TEST_ROOT_GROUP_ID);
    ...
    test("rootNodeConnections should return empty array when no root nodes exist", () => {
      ...
      expect(result, "Result should be an empty array").to.be.an("array").that
        .is.empty;
    });
    ...
    test("rootNodeConnections should return connection profiles from root nodes", () => {
      ...
      expect(
        result[0],
        "First result should match mock profile 0",
      ).to.deep.equal(mockProfiles[0]);
      expect(
        result[1],
        "Second result should match mock profile 1",
      ).to.deep.equal(mockProfiles[1]);
    });
    ...
    test("_rootTreeNodeArray should contain correct hierarchy of groups and connections", () => {
      ...
      const rootLevelNodes = {
        groups: rootTreeNodeArray.filter(
          (node) => node instanceof ConnectionGroupNode,
        ),
        connections: rootTreeNodeArray.filter(
          (node) => node instanceof ConnectionNode,
        ),
      };
      ...
      expect(rootGroupIds).to.have.members([
        topLevelGroups[0].id,
        topLevelGroups[1].id,
      ]);
      ...
      expect(rootConnection.connectionProfile.id).to.equal(
        rootConnections[0].id,
      );
    });
  });
  ...
  suite("expandNode", () => {
    ...
    test("expandNode should handle standard node expansion successfully", async () => {
      sandbox
        .stub(ObjectExplorerUtils, "iconPath")
        .callsFake((_: string) => undefined);
      ...
      expect(result!.length, "Expand node should return 2 children").to.equal(
        2,
      );

      // Verify telemetry was started correctly; it should have been called for the expand but also on constructor initialization
      expect(startActivityStub.calledTwice, "Telemetry should be started once")
        .to.be.true;
      expect(
      ...
      expect(
        ...
        "Telemetry action should be ExpandNode",
      ).to.equal(TelemetryActions.ExpandNode);

      // Verify logging
      expect(
        mockLogger.verbose.called,
        "Logger should be called once for verbose",
      ).to.be.true;

      // Verify the request was sent correctly
      expect(
        mockClient.sendRequest.calledOnce,
        "Send request should be called once",
      ).to.be.true;
      expect(
      ...
      const mappedChildren = (
        objectExplorerService as any
      )._treeNodeToChildrenMap.get(mockNode);
      expect(mappedChildren, "Mapped children should exist").to.exist;
      expect(
        mappedChildren.length,
        "Mapped children length should be 2",
      ).to.equal(2);
      expect(
        mappedChildren[0].label,
        "First mapped child label should be child1",
      ).to.equal("child1");
      expect(
        mappedChildren[1].label,
        "Second mapped child label should be child2",
      ).to.equal("child2");
      ...
      expect(
        endStub.args[0][0],
        "Telemetry status should be Succeeded",
      ).to.equal(ActivityStatus.Succeeded);
      expect(
      ...
      expect(result, "Result should match mapped children").to.equal(
        mappedChildren,
      );

      // Verify shouldRefresh was reset
      expect(mockNode.shouldRefresh, "Node shouldRefresh should be false").to.be
        .false;
    });
    ...
    test("expandNode should handle error response from SQL Tools Service", async () => {
      ...
      expect(result, "Expand node should return error node").to.be.not
        .undefined;
      expect(result!.length, "Expand node should return 1 error node").to.equal(
        1,
      );
      ...
      expect(
        mappedChildren.length,
        "Mapped children length should be 1",
      ).to.equal(1);
      expect(
        mappedChildren[0],
      ...
      expect(endFailedStub.calledOnce, "Telemetry should be ended with failure")
        .to.be.true;
      expect(
      ...
      expect(
        result![0],
        "Result child should be an ExpandErrorNode",
      ).to.be.instanceOf(ExpandErrorNode);
      expect(
  ...
  suite("handleSessionCreationFailure", () => {
    ...
    test("handleSessionCreationFailure should handle basic error without error number", async () => {
      ...
      expect(updateStub.calledOnce, "Telemetry should be updated once").to.be
        .true;
      expect(
      ...
      expect(
        updateStub.args[0][0].isFixed,
        "Is fixed should be false",
      ).to.equal("false");
      expect(
        updateStub.args[0][0].errorHandled,
    ...
    test("handleSessionCreationFailure should update telemetry when error number is present", async () => {
      ...
      expect(updateStub.calledTwice, "Telemetry should be updated twice").to.be
        .true;
      expect(
        updateStub.args[0][0].connectionType,
      ...
      expect(
        updateStub.args[0][1].errorNumber,
    ...
    test("handleSessionCreationFailure returns true and updates connection when error is handled", async () => {
      ...
      expect(
        connectionNode.updateConnectionProfile.calledOnceWithExactly(
          updatedProfile,
        ),
  ...
  suite("handleSessionCreationSuccess", () => {
    ...
    test("handleSessionCreationSuccess should return undefined when success is false", async () => {
      ...
      const result = await (
        objectExplorerService as any
      ).handleSessionCreationSuccess(failedResponse, connectionProfile);
    ...
    test("handleSessionCreationSuccess should create a new connection node when none exists", async () => {
      ...
      const result = await (
        objectExplorerService as any
      ).handleSessionCreationSuccess(successResponse, connectionProfile);
      ...
      const newNode: ConnectionNode = (objectExplorerService as any)
        ._rootTreeNodeArray[0];
      expect(
        newNode,
        "New node should be an instance of ConnectionNode",
      ).to.be.instanceOf(ConnectionNode);
      expect(
        newNode.connectionProfile,
      ...
      expect(
        newNode.nodeStatus,
        "New node status should be Connected",
      ).to.be.equal("Connected");

      // Verify connect was called
      expect(
        mockConnectionManager.connect.calledOnce,
    ...
    test("handleSessionCreationSuccess should update existing connection node", async () => {
      ...
      expect(result.connectionNode, "Connection node should exist").to.equal(
        existingNode,
      );
      ...
      expect(
        updateProfileSpy.calledOnce,
        "Update connection profile should be called once",
      ).to.be.true;
      expect(
        updateProfileSpy.args[0][0],
        "Connection profile should match",
      ).to.equal(connectionProfile);

      // Verify updateToConnectedState was called
      expect(
        updateStateSpy.calledOnce,
        "Update to connected state should be called once",
      ).to.be.true;
      expect(
        updateStateSpy.args[0][0].nodeInfo,
        "Node info should match",
      ).to.equal(successResponse.rootNode);
      expect(
        updateStateSpy.args[0][0].sessionId,
        "Session ID should match",
      ).to.equal(successResponse.sessionId);
      expect(
  ...
  suite("getNodeIdentifier", () => {
    ...
    setup(() => {
      ...
      const mockConnectionManager =
        sandbox.createStubInstance(ConnectionManager);
    ...
    test("getNodeIdentifier should return the correct identifier for a given node with session", () => {
      ...
      // Verify the result
      expect(result, "Node identifier should match").to.equal(
        "server1_db1_profile1",
      );
    });
  ...
  suite("ObjectExplorerService - createSession Tests", () => {
    ...
    test("createSession should return undefined if prepareConnectionProfile returns undefined", async () => {
      ...
      (objectExplorerService as any).prepareConnectionProfile.resolves(
        undefined,
      );
      ...
      expect(startActivityStub.calledTwice, "Telemetry should be started twice")
        .to.be.true;
      expect(
        startActivityStub.args[1][0],
        "Telemetry view should match",
      ).to.equal(TelemetryViews.ObjectExplorer);
      expect(
        startActivityStub.args[1][1],
        "Telemetry action should match",
    ...
    test("createSession should call client to get session ID and create session", async () => {
      ...
      (objectExplorerService as any).handleSessionCreationSuccess =
        sandbox.stub();
      ...
      const promise = (
        objectExplorerService as any
      )._pendingSessionCreations.get("test-session-id");
      ...
      expect(
        result,
        "Result should match session creation success response",
      ).to.equal(sessionCreationSuccessResponse);
      ...
      expect(
        mockClient.sendRequest.calledTwice,
        "Client should send two requests",
      ).to.be.true;
      expect(
      ...
      expect(
        createConnectionStub.calledOnce,
        "Connection details should be created once",
      ).to.be.true;
      expect(
        createConnectionStub.args[0][0],
        "Connection profile should match",
      ).to.equal(connectionProfile);
    ...
    test("createSession should handle session creation failure", async () => {
      ...
      (objectExplorerService as any).handleSessionCreationFailure =
        sandbox.stub();
      (objectExplorerService as any).handleSessionCreationFailure.resolves(
        true,
      );
      ...
      expect(
        endFailedStub.calledOnce,
        "Telemetry should record session creation failure",
      ).to.be.true;
    });
    ...
    test("createSession should return undefined if CreateSessionResponse is false", async () => {
      ...
      expect(
        mockClient.sendRequest.calledTwice,
        "Client requests should be sent twice",
      ).to.be.true;
    ...
    test("createSession should generate telemetry with correct connection type", async () => {
      ...
      expect(
        ...
        "Telemetry should be started with 'newConnection'",
      ).to.be.true;
      expect(
    ...
    test("createSession should handle unexpected session creation notification", async () => {
      ...
      // Verify the result
      expect(result, "Result should match success result").to.equal(
        successResult,
      );

      // Verify only the correct session was cleaned up
      expect(
        (objectExplorerService as any)._pendingSessionCreations.has(
          "test-session-id",
        ),
    ...
    test("createSession should use new connection profile when none is provided", async () => {
      ...
      expect(
        createConnectionDetails.calledOnce,
        "Connection details should be created once",
      ).to.be.true;
      expect(
      ...
      expect(endStub.calledOnce, "Telemetry end should be called once").to.be
        .true;
      expect(
        endStub.args[0][1].connectionType,
  ...
  suite("getRootNodes test", () => {
    ...
    test("getRootNodes should return AddConnectionNodes when no saved connections exist", async () => {
      ...
      (objectExplorerService as any).getAddConnectionNodes.returns(
      ...
      expect(
        startActivityStub.calledThrice,
        "Telemetry start should be called 3 times",
      ).to.be.true;
      expect(
        startActivityStub.args[1][0],
        "Telemetry view should be ObjectExplorer",
      ...
      expect(
        startActivityStub.args[1][3].nodeType,
        "Node type should be root",
      ).to.equal("root");

      // Verify activity ended with success
      expect(endStub.calledTwice, "Telemetry end should be called twice").to.be
        .true;
      expect(
        endStub.args[1][0],
        "Telemetry end status should be Succeeded",
      ).to.equal(ActivityStatus.Succeeded);
      expect(
    ...
    test("getRootNodes should create connection nodes from saved profiles", async () => {
      ...
      expect(endStub.calledOnce, "Telemetry end should be called once").to.be
        .true;
      expect(
        endStub.args[0][2].nodeCount,
        "Telemetry end node count should be 2",
      ).to.equal(2);
    });
    ...
    test("getRootNodes should handle error in connection store", async () => {
      ...
      try {
        ...
        expect(
          startActivityStub.calledTwice,
          "Telemetry start should be called twice",
        ).to.be.true;
        expect(endStub.called, "Telemetry end should not be called").to.be
          .false;
        expect(
          endFailedStub.called,
          "Telemetry end failed should not be called",
        ).to.be.false; // We're letting the error propagate
      }
    });
    ...
    test("getRootNodes should return groups and connections in correct order", async () => {
      ...
      expect(
        result.length,
        "Should have 3 root nodes (2 groups + 1 connection)",
      ).to.equal(3);
      ...
      expect(resultGroupIds).to.have.members([
        rootGroups[0].id,
        rootGroups[1].id,
      ]);

      const resultConnection = result[2] as ConnectionNode;
      expect(resultConnection.connectionProfile.id).to.equal(
        rootConnections[0].id,
      );
    });
    ...
    test("getRootNodes should handle nested group hierarchy correctly", async () => {
      ...
      const groupConnections = createMockConnectionProfiles(
        1,
        topLevelGroup.id,
      );
      ...
      const connectionNodes = (objectExplorerService as any)
        ._connectionNodes as Map<string, ConnectionNode>;
      ...
      expect(
        topLevelGroupNode.connectionGroup.parentId,
        "Parent ID should match",
      ).to.equal(topLevelGroup.parentId);
      expect(
        topLevelGroupNode.parentNode,
        "parent of a top-level node should be undefined",
      ).to.be.undefined;
      expect(
      ...
      expect(rootNode.children.length, "Root should have 2 children").to.equal(
        2,
      );
      ...
      expect(
        (childGroup.parentNode as ConnectionGroupNode)?.connectionGroup.id,
      ).to.equal(topLevelGroup.id);
      ...
      expect(
        topLevelConnection.connectionProfile.groupId,
  ...
  suite("Miscellaneous", () => {
    ...
    setup(() => {
      ...
      mockConnectionUI = sandbox.createStubInstance(ConnectionUI);
      sandbox
        .stub(mockConnectionManager, "connectionUI")
        .get(() => mockConnectionUI);
      mockAccountStore = sandbox.createStubInstance(AccountStore);
      sandbox
        .stub(mockConnectionManager, "accountStore")
        .get(() => mockAccountStore);
    ...
    test("getConnectionNodeFromProfile should return the correct node for a given profile", () => {
      ...
      const result = (
        objectExplorerService as any
      ).getConnectionNodeFromProfile(mockProfiles[0]);

      // Verify the result
      expect(result, "Result should be a ConnectionNode").to.be.instanceOf(
        ConnectionNode,
      );
      expect(
      ...
      const resultNonExistent = (
        objectExplorerService as any
      ).getConnectionNodeFromProfile(nonExistentProfile);
    ...
    test("closeSession should call closeSession on client, disconnectNode and cleanNodeChildren", async () => {
      ...
      const mockNode = (objectExplorerService as any)._connectionNodes.get(
        mockProfile.id,
      );
      mockNode.sessionId = "session1";

      (objectExplorerService as any)._treeNodeToChildrenMap = new Map();
      (objectExplorerService as any)._treeNodeToChildrenMap.set(
        mockNode,
        nodeChildren,
      );
      ...
      expect(
        mockClient.sendRequest.calledOnce,
        "Client closeSession should be called once",
      ).to.be.true;
      expect(
        mockClient.sendRequest.firstCall.args[0],
      ...
      expect(
        (objectExplorerService as any)._treeNodeToChildrenMap.has(
          nodeChildren[0],
        ),
...
function setUpOETreeRoot(
  ...
  (objectExplorerService as any)._connectionGroupNodes = new Map<
    string,
    ConnectionGroupNode
  >([[rootNode.connectionGroup.id, rootNode]]);
  (objectExplorerService as any)._connectionNodes = new Map<
    string,
    ConnectionNode
  >();
  ...
  for (const group of groups) {
    ...
    if (parentNode) {
      ...
      (objectExplorerService as any)._connectionGroupNodes.set(
        group.id,
        groupNode,
      );
  ...
  for (const profile of profiles) {
    ...
    (objectExplorerService as any)._connectionNodes.set(
      profile.id,
      connectionNode,
    );
