...
import ResultsSerializer, {
  SaveAsRequestParams,
} from "../../src/models/resultsSerializer";
...
suite("save results tests", () => {
  ...
  function configureSuccess(
    ...
    (vscodeWrapper.showInformationMessage as sinon.SinonStub).returns(
      undefined,
    );
  ...
  test("check if filepath prompt displays and right value is set", (done) => {
    (vscodeWrapper.showSaveDialog as sinon.SinonStub).resolves(fileUri);
    serverClient.sendRequest.callsFake(
      (type, details: SaveResultsAsCsvRequestParams) => {
        try {
  ...
  test("Save as with selection - test if selected range is passed in parameters", () => {
    ...
    configureSuccess();
    serverClient.sendRequest.callsFake(
      (type, params: SaveResultsAsCsvRequestParams) => {
  ...
  test("CSV configuration options are properly applied", (done) => {
    ...
    serverClient.sendRequest.callsFake(
      (_type, params: SaveResultsAsCsvRequestParams) => {
        try {
    ...
    serverClient.sendRequest.callsFake((type, params: SaveAsRequestParams) => {
      try {
        assert.equal(
          type.method,
          "query/saveInsert",
          "Should use INSERT request type",
        );
