...
suite("Runbook Studio generated Cities workload live smoke (gated)", function () {
    ...
    test("samples the explicit source database and completes the owned Docker workflow", async () => {
        ...
        try {
            ...
            expect(compile, compile?.errorCode).to.include({ ok: true, nodeCount: 18 });
            ...
            expect(run.nodeStates).to.have.length(18);
            ...
            expect(
                run.nodeStates?.find((node) => node.nodeId === "schema-fingerprint-before")
                    ?.outputCount,
            ).to.be.greaterThan(0);
            expect(
            ...
            expect(
                run.nodeStates?.find((node) => node.nodeId === "compare-snapshots")?.outputCount,
            ).to.be.greaterThan(0);
            expect(
