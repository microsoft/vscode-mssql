...
suite("Service Client tests", () => {
  ...
  function setupMocks(fixture: IFixture): void {
    if (fixture.downloadedServerPath === undefined) {
      ...
      testServiceProvider.downloadServerFiles.resolves(
  ...
  test.skip("initializeForPlatform should not install the service if already exists", async () => {
    ...
    const result = await serviceClient.initializeForPlatform(
  ...
  test("initializeForPlatform should fail given unsupported platform", async () => {
    ...
    try {
      await serviceClient.initializeForPlatform(
        fixture.platformInfo,
        undefined,
      );
      expect.fail(
        "Expected initializeForPlatform to throw for an invalid platform",
      );
    } catch (error) {
  ...
  test("handleLanguageServiceStatusNotification should change the UI status", () => {
    ...
    expect(
      testStatusView.languageServiceStatusChanged,
    ).to.have.been.calledOnceWithExactly(testFile, status);
  });
});
