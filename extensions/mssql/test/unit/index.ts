...
export async function run(): Promise<void> {
  ...
  const invert = /^true$/i.test(
    process.env.TEST_INVERT || process.env.MOCHA_INVERT || "",
  );
  if (rawPattern) {
    let rx: RegExp;
    try {
    ...
    console.log(
      `🔎 Filtering tests with pattern: ${rx}${invert ? " (inverted)" : ""}`,
    );
  }
  ...
  if (testDurations.length > 0) {
    const totalDuration = testDurations.reduce(
