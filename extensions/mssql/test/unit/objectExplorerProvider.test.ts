...
suite("Object Explorer Provider Tests", function () {
  ...
  setup(() => {
    ...
    connectionStore = {
      ...
      readAllConnections: sandbox
        .stub()
        .callsFake(async () => savedConnections),
      saveProfile: sandbox
        .stub()
        .callsFake(async (profile: IConnectionProfile) => {
          savedConnections = [
    ...
    (
      connectionManagerStub as unknown as { client: SqlToolsServiceClient }
    ).client = clientStub as unknown as SqlToolsServiceClient;
    (
      connectionManagerStub as unknown as { connectionStore: ConnectionStore }
    ).connectionStore = connectionStore;
    (
      connectionManagerStub as unknown as { vscodeWrapper: VscodeWrapper }
    ).vscodeWrapper = vscodeWrapperStub as unknown as VscodeWrapper;
    ...
    expect(
      ...
      "Object Explorer Provider is initialized properly",
    ).to.exist;

    objectExplorerServiceStub = sandbox.createStubInstance(
  ...
  test("Test Get Children from Object Explorer Provider with no children", async () => {
    ...
    const expandStub = sandbox
      .stub(testObjectExplorerService, "expandNode")
      .resolves([]);

    const children =
      await testObjectExplorerService.getChildren(parentTreeNode);
    ...
    const noItemsChildren =
      await testObjectExplorerService.getChildren(parentTreeNode);
    expect(
      noItemsChildren.length,
      "No items nodes should be returned",
    ).to.equal(1);
  ...
  test("Test server nodes sorting mechanism", () => {
    ...
    expect(
      sortedNodes[0].label,
      "First node should be the one with the lowest name",
    ).to.equal(connectionNode1.label);
    expect(
      sortedNodes[1].label,
    ...
    expect(
      sortedNodes[2].label,
      "Third node should be the one with the highest name",
    ).to.equal(connectionNode3.label);
  });
  ...
  test("Test expandNode function", async () => {
    ...
    expect(
      objectExplorerServiceStub.expandNode,
    ).to.have.been.calledOnceWithExactly(node, "test_session");
  ...
  test("Test removeConnectionNodes function", async () => {
    ...
    expect(
      objectExplorerServiceStub.removeConnectionNodes,
    ).to.have.been.calledOnceWithExactly(connections);
  });
  ...
  test("Test addDisconnectedNode function", () => {
    ...
    expect(
      objectExplorerServiceStub.addDisconnectedNode,
    ).to.have.been.calledOnceWithExactly(profile);
  });
  ...
  suite("Object Explorer Node Types Test", () => {
    test("Test Add Connection Tree Node", () => {
      ...
      expect(
        addConnectionTreeNode.label,
        "Label should be the same as constant",
      ).to.equal(LocalizedConstants.msgAddConnection);
      expect(
    ...
    test("Test Account Sign In Tree Node", () => {
      ...
      expect(
        accountSignInNode.label,
        "Label should be the same as constant",
      ).to.equal(LocalizedConstants.msgConnect);
      expect(
    ...
    test("Test Connect Tree Node", () => {
      ...
      expect(
        connectNode.command,
        "Connect Node has a dedicated command",
      ).to.not.equal(undefined);
      expect(
        connectNode.parentNode,
        "Connect Node should have a parent",
      ).to.equal(parentTreeNode);
      expect(
    ...
    test("Test getters and setters for Tree Node", () => {
      ...
      expect(
        treeNode.nodePath,
        "Node path should be equal to expected value",
      ).to.equal("test_path");
      treeNode.nodeStatus = treeNode.nodeStatus;
      expect(
        treeNode.nodeStatus,
        "Node status should be equal to expected value",
      ).to.equal("test_status");
      treeNode.nodeType = treeNode.nodeType;
      expect(
        treeNode.nodeType,
        "Node type should be equal to expected value",
      ).to.equal("Server");
      treeNode.sessionId = treeNode.sessionId;
      expect(
        treeNode.sessionId,
        "Session ID should be equal to expected value",
      ).to.equal("test_session");
    ...
    test("Test fromNodeInfo function", () => {
      ...
      expect(
        treeNodeInfo.nodePath,
        "Node path should be equal to expected value",
      ).to.equal(nodeInfo.nodePath);
      expect(
        treeNodeInfo.nodeStatus,
        "Node status should be equal to expected value",
      ).to.equal(nodeInfo.nodeStatus);
      expect(
        treeNodeInfo.nodeType,
        "Node type should be equal to expected value",
      ).to.equal(nodeInfo.nodeType);
      expect(
        treeNodeInfo.sessionId,
        "Session ID should be equal to expected value",
      ).to.equal("test_session");
      expect(
      ...
      expect(treeNodeInfo.isLeaf, "Node should not be a leaf").to.equal(
        nodeInfo.isLeaf,
      );
      expect(
