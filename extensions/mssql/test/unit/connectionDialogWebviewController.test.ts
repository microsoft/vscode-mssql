...
suite("ConnectionDialogWebviewController Tests", () => {
  ...
  setup(async () => {
    ...
    mockObjectExplorerProvider = sandbox.createStubInstance(
  ...
  suite("Initialization", () => {
    test("should initialize correctly for new connection", async () => {
      ...
      expect(controller.state.savedConnections[0]).to.deep.include(
        testSavedConnection,
      );

      expect(controller.state.recentConnections).to.have.lengthOf(1);
      expect(controller.state.recentConnections).to.deep.include(
        testMruConnection,
      );
      expect(
  ...
  suite("Reducers", () => {
    suite("setConnectionInputType", () => {
      ...
      test("should set connection input mode correctly and load server info for AzureBrowse", async () => {
        ...
        expect(sendErrorEvent.notCalled, "sendErrorEvent should not be called")
          .to.be.true;
    ...
    test("loadConnection", async () => {
      ...
      await controller["_reducerHandlers"].get("loadConnection")(
        controller.state,
        {
    ...
    suite("connect", () => {
      ...
      test("displays error when attempting to create OE session fails", async () => {
        ...
        mockObjectExplorerProvider.createSession.rejects(
    ...
    suite("filterAzureSubscriptions", () => {
      test("Filter change cancelled", async () => {
        ...
        const stub = (controller["loadAllAzureServers"] = sandbox
          .stub()
          .resolves());

        expect(stub.notCalled, "loadAllAzureServers should not be called").to.be
          .true;
      });
    ...
    suite("messageButtonClicked", () => {
      test("clearTokenCache", async () => {
        ...
        const azureControllerStub =
          sandbox.createStubInstance(MsalAzureController);

        connectionManager.azureController = azureControllerStub;

        await controller["_reducerHandlers"].get("messageButtonClicked")(
          controller.state,
          {
    ...
    suite("addFirewallRule", () => {
      test("displays error upon failure to create firewall rule", async () => {
        ...
        await controller["_reducerHandlers"].get("addFirewallRule")(
          controller.state,
          {
  ...
  test("getAzureActionButtons", async () => {
    ...
    expect(actionButtons.length).to.equal(
      1,
      "Should always have the Sign In button",
    );
    ...
    const isTokenValidStub = sandbox
      .stub(AzureController, "isTokenValid")
      .returns(false);
