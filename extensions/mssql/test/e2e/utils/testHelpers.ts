...
export async function addDatabaseConnection(
  ...
  const sqlServerTabContainer = vsCodePage.locator(
    '[role="tab"][aria-label^="SQL Server"]',
  );
  ...
  await iframe
    .getByRole("checkbox", { name: "Trust server certificate" })
    .click();
...
export async function waitForCommandPaletteToBeVisible(
  ...
  const commandPaletteInput = vsCodePage.locator(
