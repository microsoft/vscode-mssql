...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "Enforce import boundaries between extension, webview, and shared code",
      category: "Imports",
      recommended: false,
    },
    fixable: null,
    ...
    return {
      ImportDeclaration(node) {
        ...
        const isImportFromReactviews =
          /(^|\/)reactviews\//.test(importSource) ||
          importSource === "reactviews";

        const isFileInReactviews = /\/src\/reactviews\//.test(filePath);
        const isFileInSharedInterfaces = /\/src\/sharedInterfaces\//.test(
          filePath,
        );
        const isFileInTest = /\/test\//.test(filePath);
        const isImportFromVscodeClient = /vscode-languageclient/.test(
          importSource,
        );
        ...
        if (
          (isFileInReactviews || isFileInSharedInterfaces) &&
          isImportFromVscodeClient
        ) {
          context.report({
            node,
            message:
        ...
        if (isFileInReactviews) {
          ...
          if (importSource.startsWith("..")) {
            ...
            const isImportInReactviews = /\/src\/reactviews\//.test(
              resolvedImport,
            );

            if (
              isImportInSrc &&
              !isImportInSharedInterfaces &&
              !isImportInReactviews
            ) {
              context.report({
                node,
                message:
