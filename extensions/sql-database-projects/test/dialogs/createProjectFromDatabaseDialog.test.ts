...
suite.skip("Create Project From Database Dialog", () => {
    ...
    test("Should open dialog successfully", async function (): Promise<void> {
        ...
        sinon
            ...
            .resolves({ connected: true, connectionId: "0", errorMessage: "", errorCode: 0 });
    ...
    test("Should enable ok button correctly with a connection profile", async function (): Promise<void> {
        ...
        should(dialog.dialog.okButton.enabled).equal(
            false,
            "Ok button should not be enabled because project location is not filled",
        );
        ...
        should(dialog.dialog.okButton.enabled).equal(
            true,
            "Ok button should be enabled since all the required fields are filled",
        );
    });
    ...
    test("Should enable ok button correctly without a connection profile", async function (): Promise<void> {
        ...
        should(dialog.dialog.okButton.enabled).equal(
            false,
            "Ok button should not be enabled because all the required details are not filled",
        );
        ...
        should(dialog.dialog.okButton.enabled).equal(
            false,
            "Ok button should not be enabled because source database details and project location are not filled",
        );
        ...
        should(dialog.dialog.okButton.enabled).equal(
            false,
            "Ok button should not be enabled because source database details are not filled",
        );
        ...
        should(dialog.dialog.okButton.enabled).equal(
            false,
            "Ok button should not be enabled because source database is not filled",
        );
    ...
    test("Should create default project name correctly when database information is populated", async function (): Promise<void> {
        ...
        sinon
            ...
            .returns("DatabaseProjectMy Database");
        const dialog = new CreateProjectFromDatabaseDialog(mockConnectionProfile);
        await dialog.openDialog();
    ...
    test("Should include all info in import data model and connect to appropriate call back properties", async function (): Promise<void> {
        ...
        dialog.createProjectFromDatabaseCallback = (m) => {
            model = m;
        };
