...
suite("Create Project From Database Quickpick", () => {
  setup(function (): void {
    ...
    sinon
      ...
      .resolves(testUtils.vscodeMssqlIExtension.object); //set vscode mssql extension api
  ...
  test("Should exit when project location is not selected (test repeatedness for project location)", async function (): Promise<void> {
    ...
    quickPickStub
      .onSecondCall()
      .resolves(constants.browseEllipsisWithIcon as any);
  ...
  test("Should exit when folder structure is not selected and existing folder/file location is selected", async function (): Promise<void> {
    ...
    quickPickStub
      .onThirdCall()
      .resolves(path.join(projectFilePath, "test") as any);
    //user chooses to exit when prompted for folder structure
    quickPickStub.onCall(3).resolves(undefined);
  ...
  test("Should create project when all the information is provided", async function (): Promise<void> {
    ...
    should(
      createProjectFromDatabaseCallbackSpy.calledWithMatch(
        expectedImportDataModel,
      ),
    ).be.true(
