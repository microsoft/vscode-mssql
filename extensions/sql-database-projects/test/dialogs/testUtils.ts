...
export class MockVscodeMssqlIExtension implements vscodeMssql.IExtension {
  ...
  constructor() {
    ...
    this.sqlProjects =
      TypeMoq.Mock.ofType<vscodeMssql.ISqlProjectsService>().object;
    this.schemaCompare =
      TypeMoq.Mock.ofType<vscodeMssql.ISchemaCompareService>().object;
    this.azureAccountService =
      TypeMoq.Mock.ofType<vscodeMssql.IAzureAccountService>().object;
    this.azureResourceService =
      TypeMoq.Mock.ofType<vscodeMssql.IAzureResourceService>().object;
  }

  promptForFirewallRule(
    _: string,
    __: vscodeMssql.IConnectionInfo,
  ): Promise<boolean> {
  ...
  promptForConnection(
    _?: boolean,
  ): Promise<vscodeMssql.IConnectionInfo | undefined> {
    throw new Error("Method not implemented.");
  }
