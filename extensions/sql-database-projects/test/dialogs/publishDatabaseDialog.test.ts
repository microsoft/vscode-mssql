...
import {
  createContext,
  mockDacFxOptionsResult,
  TestContext,
} from "../testContext";
import {
  IPublishToDockerSettings,
  ISqlProjectPublishSettings,
} from "../../src/models/deploy/publishSettings";
...
suite.skip("Publish Database Dialog", () => {
  ...
  test("Should create default database name correctly ", async function (): Promise<void> {
    ...
    const publishDatabaseDialog = new PublishDatabaseDialog(project);
    should.equal(
      publishDatabaseDialog.getDefaultDatabaseName(),
  ...
  test("Should include all info in publish profile", async function (): Promise<void> {
    ...
    const dialog = TypeMoq.Mock.ofType(
    ...
    dialog
      .setup((x) => x.getConnectionUri())
      .returns(() => {
        return Promise.resolve("Mock|Connection|Uri");
      });
    dialog.setup((x) => x.targetDatabaseName).returns(() => "MockDatabaseName");
    dialog
      .setup((x) => x.getSqlCmdVariablesForPublish())
      .returns(() => proj.sqlCmdVariables);
    dialog
      .setup((x) => x.getDeploymentOptions())
      .returns(() => {
        return Promise.resolve(mockDacFxOptionsResult.deploymentOptions);
      });
    ...
    dialog.object.generateScript = (_, prof) => {
      profile = prof;
    };
    ...
    dialog.object.publishToContainer = (_, prof) => {
      deployProfile = prof;
    };
    await dialog.object.publishClick();
