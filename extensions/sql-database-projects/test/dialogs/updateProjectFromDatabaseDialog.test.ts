...
suite.skip("Update Project From Database Dialog", () => {
  ...
  test("Should populate endpoints correctly when no context passed", async function (): Promise<void> {
    const dialog = new UpdateProjectFromDatabaseDialog(
      undefined,
      undefined,
      [],
    );
    ...
    should.equal(
      dialog.serverDropdown!.value,
      undefined,
      `Server dropdown should not be populated, but instead was "${dialog.serverDropdown!.value}".`,
    );
    should.equal(
      dialog.databaseDropdown!.value,
      undefined,
      `Database dropdown should not be populated, but instead was "${dialog.databaseDropdown!.value}".`,
    );
    should.equal(
      dialog.projectFileDropdown!.value,
      "",
      `Project file dropdown should not be populated, but instead was "${dialog.projectFileDropdown!.value}".`,
    );
    should.equal(
      dialog.dialog.okButton.enabled,
      false,
      "Okay button should be disabled.",
    );
  });
  ...
  test("Should populate endpoints correctly when Project context is passed", async function (): Promise<void> {
    ...
    should.equal(
      ...
      `Project file dropdown should be the sqlproj path (${project.projectFilePath}), but instead was "${dialog.projectFileDropdown!.value}".`,
    );
    should.equal(
      dialog.dialog.okButton.enabled,
      false,
  ...
  test("Should populate endpoints correctly when Connection context is passed", async function (): Promise<void> {
    sinon
      .stub(azdata.connection, "getConnections")
      .resolves([
        <azdata.connection.ConnectionProfile>(<unknown>mockConnectionProfile),
      ]);
    sinon
      .stub(azdata.connection, "listDatabases")
      .resolves([mockConnectionProfile.databaseName!]);
    ...
    should.equal(
      (<any>dialog.serverDropdown!.value).displayName,
      profile.options["connectionName"],
      `Server dropdown should be "${profile.options["connectionName"]}", but instead was "${(<any>dialog.serverDropdown!.value).displayName}".`,
    );
    should.equal(
      dialog.databaseDropdown!.value,
      profile.databaseName,
      `Database dropdown should be "${profile.databaseName}", but instead was "${dialog.databaseDropdown!.value}".`,
    );
    should.equal(
      dialog.projectFileDropdown!.value,
      "",
  ...
  test("Should populate endpoints correctly when context is complete", async function (): Promise<void> {
    ...
    should.equal(
      dialog.databaseDropdown!.value,
      profile.databaseName,
      `Database dropdown should as "${profile.databaseName}", but instead was "${dialog.databaseDropdown!.value}".`,
    );
    should.equal(
      dialog.projectFileDropdown!.value,
      project.projectFilePath,
    ...
    should.deepEqual(
      dialog.projectFileDropdown!.values,
      uriList,
      `Project file dropdown list should be the sqlproj path (${mockURIList}), but instead was "${dialog.projectFileDropdown!.values}".`,
    );
    should.equal(
      dialog.dialog.okButton.enabled,
      true,
      "Okay button should be enabled when dialog is complete.",
    );
  });
  ...
  test("Should populate endpoints correctly when Connection context and workspace with projects is provided", async function (): Promise<void> {
    ...
    should.equal(
      dialog.projectFileDropdown!.value,
      uriList[0],
      `Project file dropdown should be the first project listed in the workspace URI list (${uriList[0]}), but instead was "${dialog.projectFileDropdown!.value}".`,
    );
    should.deepEqual(
      dialog.projectFileDropdown!.values,
      uriList,
      `Project file dropdown list should be the workspace URI list (${mockURIList}), but instead was "${dialog.projectFileDropdown!.values}".`,
    );
    should.equal(
      dialog.dialog.okButton.enabled,
      true,
  ...
  test("Should successfully complete the handleUpdateButtonClick method call and connect to appropriate call back properties when Connection context and workspace with projects is provided", async function (): Promise<void> {
    ...
    sinon
      .stub(azdata.connection, "getCredentials")
      .resolves({ ["credentials"]: "credentials" });
    ...
    const testProjectEndpointInfo: mssql.SchemaCompareEndpointInfo = {
      ...mockProjectEndpointInfo,
    };
    ...
    dialog.updateProjectFromDatabaseCallback = (m) => {
      model = m;
    };
