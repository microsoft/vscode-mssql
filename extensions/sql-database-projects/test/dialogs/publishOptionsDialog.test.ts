...
suite.skip("Publish Database Options Dialog", () => {
  ...
  test("Should open dialog successfully ", async function (): Promise<void> {
    ...
    const optionsDialog = new PublishOptionsDialog(
      testData.getDeploymentOptions(),
  ...
  test("Should deployment options gets initialized correctly with sample test project", async function (): Promise<void> {
    ...
    dialog
      ...
      .returns(() => {
        return Promise.resolve(
          testData.mockDacFxOptionsResult.deploymentOptions,
        );
      });
    ...
    const optionsDialog = new PublishOptionsDialog(
      options,
      new PublishDatabaseDialog(project),
    );
    ...
    Object.entries(
      optionsDialog.optionsModel.deploymentOptions.booleanOptionsDictionary,
    ).forEach((option) => {
