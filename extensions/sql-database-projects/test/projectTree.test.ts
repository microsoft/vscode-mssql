...
suite("Project Tree tests", function (): void {
  ...
  test("Should build tree from Project file correctly", function (): void {
    ...
    proj.sqlObjectScripts.push(
      proj.createFileProjectEntry(
        path.join("someFolder", "bNestedTest.sql"),
        EntryType.File,
      ),
    );
    proj.folders.push(
      proj.createFileProjectEntry(
        path.join("someFolder", "bNestedFolder"),
        EntryType.Folder,
      ),
    );
    proj.sqlObjectScripts.push(
      proj.createFileProjectEntry(
        path.join("someFolder", "aNestedTest.sql"),
        EntryType.File,
      ),
    );
    proj.folders.push(
      proj.createFileProjectEntry(
        path.join("someFolder", "aNestedFolder"),
        EntryType.Folder,
      ),
    );
    proj.folders.push(
      proj.createFileProjectEntry("someFolder", EntryType.Folder),
    );

    // duplicate files
    proj.sqlObjectScripts.push(
      proj.createFileProjectEntry("duplicate.sql", EntryType.File),
    );
    proj.sqlObjectScripts.push(
      proj.createFileProjectEntry("duplicate.sql", EntryType.File),
    );
    ...
    // duplicate folders
    proj.folders.push(
      proj.createFileProjectEntry("duplicateFolder", EntryType.Folder),
    );
    proj.folders.push(
      proj.createFileProjectEntry("duplicateFolder", EntryType.Folder),
    );
  ...
  test("Should be able to parse windows relative path as platform safe path", function (): void {
    ...
    proj.folders.push(
      proj.createFileProjectEntry(
        "someFolder1\\MyNestedFolder2",
        EntryType.Folder,
      ),
    );
    proj.folders.push(
      proj.createFileProjectEntry("someFolder1", EntryType.Folder),
    );
    proj.folders.push(
      proj.createFileProjectEntry(
        "someFolder1\\MyNestedFolder1",
        EntryType.Folder,
      ),
    );
    proj.sqlObjectScripts.push(
