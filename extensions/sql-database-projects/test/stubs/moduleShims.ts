...
function createAzdataShim(): AnyObject {
  ...
  const azdata =
    require("@microsoft/azdata-test/out/stubs/azdata") as AnyObject;

  azdata.TaskExecutionMode = azdata.TaskExecutionMode ?? {
    execute: "execute",
    script: "script",
  };
  ...
  azdata.connection.getConnections =
    azdata.connection.getConnections ?? (async () => []);
  ...
  azdata.connection.listDatabases =
    azdata.connection.listDatabases ?? (async () => []);
