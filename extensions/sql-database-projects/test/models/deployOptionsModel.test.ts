...
suite("Publish Dialog Deploy Options Model", () => {
  test("Should create model and set options successfully", function (): void {
    ...
    should.notEqual(
      ...
      undefined,
      "Options shouldn't be undefined",
    );
  ...
  test("Should get description", function (): void {
    const model = new DeployOptionsModel(testUtils.getDeploymentOptions());
    Object.entries(model.deploymentOptions.booleanOptionsDictionary).forEach(
      (option) => {
        // option[1] contains the value, description and displayName
        should(model.getOptionDescription(option[1].displayName)).not.equal(
          undefined,
          "publish option description should not be undefined",
        );
      },
    );
  });
  ...
  test("Should have no default exclude object types", function (): void {
    const model = new DeployOptionsModel(testUtils.getDeploymentOptions());
    should(model.deploymentOptions.excludeObjectTypes.value.length).be.equal(
      0,
      "There should be no object types excluded from excludeObjectTypes",
    );

    // should return true for all object type options as there are no default excludeObjectTypes in the deployment options
    Object.keys(model.deploymentOptions.objectTypesDictionary).forEach(
      (option) => {
        should(model.getExcludeObjectTypeOptionCheckStatus(option)).equal(
          false,
          "excludeObjectTypes property should be empty by default and return false",
        );
      },
    );
  });
  ...
  test("Should have atleast one default exclude object types", function (): void {
    ...
    should(model.deploymentOptions.excludeObjectTypes.value.length).be.equal(
      1,
      "There should be one excluded object",
    );
    ...
    Object.keys(model.deploymentOptions.objectTypesDictionary).forEach(
      (option) => {
        if (option === "SampleProperty1") {
          should(model.getExcludeObjectTypeOptionCheckStatus(option)).equal(
            true,
            "should return true for the excludeObjectTypes SampleProperty1 ",
          );
        } else {
          should(model.getExcludeObjectTypeOptionCheckStatus(option)).equal(
            false,
            "should return false for all excludeObjectTypes property as it is empty",
          );
        }
      },
    );
