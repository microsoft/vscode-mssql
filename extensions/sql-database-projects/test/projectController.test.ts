...
suite("ProjectsController", function (): void {
  ...
  suite("project controller operations", function (): void {
    suite("Project file operations and prompting", function (): void {
      ...
      test("Should return silently when no SQL object name provided in prompts", async function (): Promise<void> {
        for (const name of ["", "    ", undefined]) {
          ...
          const showErrorMessageSpy = sinon.spy(
            vscode.window,
            "showErrorMessage",
          );
          const projController = new ProjectsController(
      ...
      test("Should add existing item", async function (): Promise<void> {
        ...
        await projController.addExistingItemPrompt(
          createWorkspaceTreeItem(projTreeRoot),
        );
      ...
      test("Should show error if trying to add a folder that already exists", async function (): Promise<void> {
        ...
        should(project.folders.length).equal(
          0,
          "There should be no other folders",
        );
        await projController.addFolderPrompt(
          createWorkspaceTreeItem(projectRoot),
        );
        ...
        should(project.folders.length).equal(
          1,
          "Folder should be successfully added",
        );
        stub.restore();
      ...
      test("Should be able to add folder with reserved name as long as not at project root", async function (): Promise<void> {
        ...
        let node = projectRoot.children.find(
          (c) => c.friendlyName === "Tables",
        );
        sinon.restore();
        for (let i in constants.reservedProjectFolders) {
          // reload project
      ...
      test("Should create .vscode/tasks.json with isDefault=true when configureDefaultBuild is true", async function (): Promise<void> {
        ...
        should(exists).be.true(
          ".vscode/tasks.json should be created when configureDefaultBuild is true",
        );
        ...
        if (exists) {
          ...
          should(task.group.isDefault).equal(
            "true",
      ...
      test("Should delete database references", async function (): Promise<void> {
        ...
        await projController.delete(
          createWorkspaceTreeItem(
            databaseReferenceNodeChildren?.find(
              (x) => x.friendlyName === "master",
            )!,
          ),
        ); // system db reference
        await projController.delete(
      ...
      test("Should exclude nested ProjectEntry from node", async function (): Promise<void> {
        ...
        ).not.equal(undefined, "UpperFolder should still be there");
      ...
      test("Should exclude a folder", async function (): Promise<void> {
        ...
        await proj.addScriptItem(
          "SomeFolder/MyTable.sql",
          "CREATE TABLE [NotARealTable]",
        );
        ...
        proj.folders.length.should.equal(
          0,
          "Number of folders should not have changed",
        );
      });
      ...
      test.skip("Should delete folders with excluded items", async function (): Promise<void> {
        ...
        ).equal(false, "UpperFolder should not be part of proj file any more");
        should(await utils.exists(scriptEntry.fsUri.fsPath)).equal(
          false,
      ...
      test("Should reload correctly after changing sqlproj file", async function (): Promise<void> {
        ...
        let project = await Project.openProject(
          vscode.Uri.file(sqlProjPath).fsPath,
        );
        ...
        await fs.writeFile(
          sqlProjPath,
          baselines.newProjectFileWithScriptBaseline,
        );
        should(project.sqlObjectScripts.length).equal(0);

        // call reload project
        const projTreeRoot = new ProjectRootTreeItem(project);
        await projController.reloadProject(
          createWorkspaceTreeItem(projTreeRoot),
        );
      ...
      test("Should be able to add pre deploy and post deploy script", async function (): Promise<void> {
        ...
        sinon
          .stub(utils, "sanitizeStringForFilename")
          .returns(preDeployScriptName);
        ...
        sinon
          .stub(vscode.window, "showInputBox")
          .resolves(postDeployScriptName);
        sinon
          .stub(utils, "sanitizeStringForFilename")
          .returns(postDeployScriptName);
      ...
      test("Should change target platform", async function (): Promise<void> {
        ...
        should(project.getProjectTargetVersion()).equal(
          constants.targetPlatformToVersion.get(
            SqlTargetPlatform.sqlServer2019,
          ),
        );
    ...
    suite("Publishing and script generation", function (): void {
      test("Publish dialog should open from ProjectController", async function (): Promise<void> {
        ...
        projController
          .setup((x) => x.getPublishDialog(TypeMoq.It.isAny()))
          .returns(() => publishDialog.object);
      ...
      test("Callbacks are hooked up and called from Publish dialog", async function (): Promise<void> {
        const projectFile = await testUtils.createTestSqlProjFile(
          this.test,
          baselines.openProjectFileBaseline,
        );
        ...
        const setupPublishDialog = (): PublishDatabaseDialog => {
          ...
          sinon
            .stub(dialog, "getConnectionUri")
            .returns(Promise.resolve("fake|connection|uri"));
          return dialog;
        };
        ...
        should(holler).equal(
          publishHoller,
          "executionCallback() is supposed to have been setup and called for Publish scenario",
        );
        ...
        should(holler).equal(
          generateHoller,
          "executionCallback() is supposed to have been setup and called for GenerateScript scenario",
        );
      });
      ...
      test("Should copy dacpac to temp folder before publishing", async function (): Promise<void> {
        ...
        testContext.dacFxService
          ...
          .returns(async (p) => {
            ...
            postCopyContents = (
              await fs.readFile(publishedDacpacPath)
            ).toString();
        ...
        projController
          .setup((x) => x.buildProject(TypeMoq.It.isAny()))
          .returns(async () => {
            builtDacpacPath = await testUtils.createTestFile(
              this.test,
              fakeDacpacContents,
              "output.dacpac",
            );
        ...
        sinon
          .stub(utils, "getDacFxService")
          .resolves(testContext.dacFxService.object);
        ...
        await projController.object.publishOrScriptProject(
          proj,
          { connectionUri: "", databaseName: "", serverName: "" },
          false,
        );

        should(builtDacpacPath).not.equal(
          "",
          "built dacpac path should be set",
        );
        should(publishedDacpacPath).not.equal(
          "",
          "published dacpac path should be set",
        );
        should(builtDacpacPath).not.equal(
          publishedDacpacPath,
          "built and published dacpac paths should be different",
        );
        should(postCopyContents).equal(
          fakeDacpacContents,
          "contents of built and published dacpacs should match",
        );
  ...
  suite(
    "Create project from database operations and dialog",
    function (): void {
      teardown(() => {
        sinon.restore();
      });
      ...
      test("Should create list of all files and folders correctly", async function (): Promise<void> {
        ...
        fileList.length.should.equal(
          11,
          "number of files returned by generateScriptList()",
        );
        fileList
      ...
      test("Should error out for inaccessible path", async function (): Promise<void> {
        ...
        await projController.generateScriptList(testFolderPath);
        should(spy.calledOnce).be.true(
          "showErrorMessage should have been called",
        );
      ...
      test("Create project from Database dialog should open from ProjectController", async function (): Promise<void> {
        ...
        let createProjectFromDatabaseDialog = TypeMoq.Mock.ofType(
        ...
        projController
          .setup((x) =>
            x.getCreateProjectFromDatabaseDialog(TypeMoq.It.isAny()),
          )
          .returns(() => createProjectFromDatabaseDialog.object);

        await projController.object.createProjectFromDatabase(
          mockConnectionProfile,
        );
        should(opened).equal(true);
      });
      ...
      test.skip("Callbacks are hooked up and called from create project from database dialog", async function (): Promise<void> {
        const createProjectFromDbHoller =
          "hello from callback for createProjectFromDatabase()";

        let holler = "nothing";

        const createProjectFromDatabaseDialog = TypeMoq.Mock.ofType(
          CreateProjectFromDatabaseDialog,
          undefined,
          undefined,
          undefined,
        ...
        createProjectFromDatabaseDialog
          .setup((x) => x.handleCreateButtonClick())
          .returns(async () => {
        ...
        projController
          .setup((x) =>
            x.createProjectFromDatabaseCallback(
              TypeMoq.It.isAny(),
              TypeMoq.It.isAny(),
            ),
          )
        ...
        let dialog =
          await projController.object.createProjectFromDatabase(undefined);
        await dialog!.handleCreateButtonClick();

        should(holler).equal(
          createProjectFromDbHoller,
          "executionCallback() is supposed to have been setup and called for create project from database scenario",
        );
      });
      ...
      test("Should set model filePath correctly for ExtractType = File", async function (): Promise<void> {
        ...
        should(importPath.toUpperCase()).equal(
          vscode.Uri.file(
            path.join(folderPath, projectName + ".sql"),
          ).fsPath.toUpperCase(),
  ...
  suite("Add database reference", function (): void {
    test("Add database reference dialog should open from ProjectController", async function (): Promise<void> {
      ...
      let addDbReferenceDialog = TypeMoq.Mock.ofType(
        AddDatabaseReferenceDialog,
      );
      addDbReferenceDialog
        .setup((x) => x.openDialog())
        .returns(() => {
          opened = true;
          return Promise.resolve(undefined);
      ...
      projController.callBase = true;
      projController
        .setup((x) => x.getAddDatabaseReferenceDialog(TypeMoq.It.isAny()))
        .returns(() => addDbReferenceDialog.object);

      await projController.object.addDatabaseReference(new Project("FakePath"));
      should(opened).equal(true);
    });

    test("Callbacks are hooked up and called from Add database reference dialog", async function (): Promise<void> {
      const projPath = path.dirname(
        await testUtils.createTestSqlProjFile(
          this.test,
          baselines.openProjectFileBaseline,
        ),
      );
      ...
      const addDbReferenceDialog = TypeMoq.Mock.ofType(
        AddDatabaseReferenceDialog,
        undefined,
        undefined,
        proj,
      ...
      addDbReferenceDialog
        .setup((x) => x.addReferenceClick())
        .returns(() => {
          return projController.object.addDatabaseReferenceCallback(
            proj,
            {
              systemDb: SystemDatabase.Master,
              databaseName: "master",
              suppressMissingDependenciesErrors: false,
              systemDbReferenceType:
                mssql.SystemDbReferenceType.ArtifactReference,
            },
            {
      ...
      projController
        .setup((x) =>
          x.addDatabaseReferenceCallback(
            TypeMoq.It.isAny(),
            TypeMoq.It.is((_): _ is IDacpacReferenceSettings => true),
            TypeMoq.It.isAny(),
          ),
        )
        .returns(() => {
      ...
      should(holler).equal(
        addDbRefHoller,
        "executionCallback() is supposed to have been setup and called for add database reference scenario",
      );
    });
    ...
    test.skip("Should not allow adding circular project references", async function (): Promise<void> {
      ...
      const project1 = await Project.openProject(
        vscode.Uri.file(projPath1).fsPath,
      );
      const project2 = await Project.openProject(
        vscode.Uri.file(projPath2).fsPath,
      );
      const showErrorMessageSpy = sinon.spy(vscode.window, "showErrorMessage");
      const dataWorkspaceMock = TypeMoq.Mock.ofType<dataworkspace.IExtension>();
      dataWorkspaceMock
        .setup((x) =>
          x.getProjectsInWorkspace(TypeMoq.It.isAny(), TypeMoq.It.isAny()),
        )
        .returns(() =>
          Promise.resolve([
      ...
      should(showErrorMessageSpy.called).be.true(
        "showErrorMessage should have been called",
      );
    });
    ...
    test.skip("Should add dacpac references as relative paths", async function (): Promise<void> {
      ...
      const project1 = await Project.openProject(
        vscode.Uri.file(projFilePath).fsPath,
      );
      const showErrorMessageSpy = sinon.spy(vscode.window, "showErrorMessage");
      const dataWorkspaceMock = TypeMoq.Mock.ofType<dataworkspace.IExtension>();
      sinon
      ...
      should(project1.databaseReferences[0].referenceName).equal(
        "sameFolderTest",
      );
      should(project1.databaseReferences[0].pathForSqlProj()).equal(
      ...
      should(project1.databaseReferences[1].referenceName).equal(
        "nestedFolderTest",
      );
      ...
      should(project1.databaseReferences[2].referenceName).equal(
        "outsideFolderTest",
      );
      ...
      projFileText = (await fs.readFile(projFilePath)).toString();
      should(projFileText).containEql(
        "..\\someFolder\\outsideFolderTest.dacpac",
      );
    });
  });
  ...
  suite("AutoRest generation", function (): void {
    ...
    test.skip("Should create project from autorest-generated files", async function (): Promise<void> {
      ...
      projController
        .setup((x) => x.selectAutorestSpecFile())
        .returns(async () => specName);
      projController
        .setup((x) =>
          x.selectAutorestProjectLocation(TypeMoq.It.isAny(), undefined),
        )
        .returns(async () => {
      ...
      projController
        .setup((x) =>
          x.generateAutorestFiles(TypeMoq.It.isAny(), TypeMoq.It.isAny()),
        )
        .returns(async () => {
      ...
      const project =
        (await projController.object.generateProjectFromOpenApiSpec())!;
      ...
      const expectedScripts = fileList.filter(
        (f) => path.extname(f.fsPath) === ".sql",
      );
      should(
        project.sqlObjectScripts.filter((f) => f.type === EntryType.File)
          .length,
      ).equal(
      ...
      should(
        project.sqlObjectScripts.filter((f) => f.type === EntryType.Folder)
          .length,
      ).equal(
  ...
  suite("Move file", function (): void {
    test("Should move a file to another folder", async function (): Promise<void> {
      ...
      should(
        proj.sqlObjectScripts.find(
          (f) => f.relativePath === "UpperFolder\\script1.sql",
        ) !== undefined,
      ).be.true("The file path should have been updated");
      should(
        await utils.exists(
          path.join(proj.projectFolderPath, "UpperFolder", "script1.sql"),
        ),
    ...
    test("Should move a folder to another folder", async function (): Promise<void> {
      ...
      proj = await Project.openProject(proj.projectFilePath);
      should(
        proj.folders.find((f) => f.relativePath === "UpperFolder\\folder1") !==
          undefined,
    ...
    test("Should only allow moving files and folders", async function (): Promise<void> {
      ...
      const projectRootWorkspaceTreeItem =
        createWorkspaceTreeItem(projTreeRoot);
      await projController.moveFile(
        vscode.Uri.file(proj.projectFilePath),
        sqlcmdVarNode,
    ...
    test("Should not allow moving files between projects", async function (): Promise<void> {
      ...
      should(
        spy.calledWith(constants.movingFilesBetweenProjectsNotSupported),
      ).be.true(
      ...
      should(
        proj1.sqlObjectScripts.find((f) => f.relativePath === "script1.sql") !==
          undefined,
      ).be.true(
  ...
  suite("Rename file", function (): void {
    ...
    test("Should rename a sql object file", async function (): Promise<void> {
      ...
      proj = await Project.openProject(proj.projectFilePath);
      should(
        proj.sqlObjectScripts.find((f) => f.relativePath === "newName.sql") !==
          undefined,
      ).be.true("The file path should have been updated");
      should(
    ...
    test("Should rename a pre and post deploy script", async function (): Promise<void> {
      ...
      sinon
        .stub(vscode.window, "showInputBox")
        .resolves("predeployNewName.sql");
      const preDeployScriptNode = projTreeRoot.children.find(
        (x) => x.friendlyName === "Script.PreDeployment1.sql",
      );
      await projController.rename(
        createWorkspaceTreeItem(preDeployScriptNode!),
      );

      sinon.restore();
      sinon
        .stub(vscode.window, "showInputBox")
        .resolves("postdeployNewName.sql");
      const postDeployScriptNode = projTreeRoot.children.find(
        (x) => x.friendlyName === "Script.PostDeployment1.sql",
      );
      await projController.rename(
        createWorkspaceTreeItem(postDeployScriptNode!),
      );

      // reload project and verify files were renamed
      proj = await Project.openProject(proj.projectFilePath);
      should(
        proj.preDeployScripts.find(
          (f) => f.relativePath === "predeployNewName.sql",
        ) !== undefined,
      ).be.true("The pre deploy script file path should have been updated");
      should(
        await utils.exists(
          path.join(proj.projectFolderPath, "predeployNewName.sql"),
        ),
      ).be.true("The moved pre deploy script file should exist");

      should(
        proj.postDeployScripts.find(
          (f) => f.relativePath === "postdeployNewName.sql",
        ) !== undefined,
      ).be.true("The post deploy script file path should have been updated");
      should(
        await utils.exists(
          path.join(proj.projectFolderPath, "postdeployNewName.sql"),
        ),
    ...
    test("Should rename a folder", async function (): Promise<void> {
      ...
      should(
        proj.folders.find((f) => f.relativePath === "RenamedFolder") !==
          undefined,
      ).be.true("The folder path should have been updated");
      should(
        proj.sqlObjectScripts.find(
  ...
  suite("SqlCmd Variables", function (): void {
    test("Should delete sqlcmd variable", async function (): Promise<void> {
      ...
      sinon
        .stub(vscode.window, "showWarningMessage")
        .returns(<any>Promise.resolve("Yes"));
      await projController.delete(
        createWorkspaceTreeItem(
          projRoot.children.find(
            (x) => x.friendlyName === constants.sqlcmdVariablesNodeName,
    ...
    test("Should add sqlcmd variable without DefaultValue", async function (): Promise<void> {
      ...
      const infoMessageStub = sinon.stub(
        vscode.window,
    ...
    test("Should update sqlcmd variable", async function (): Promise<void> {
      ...
      const originalValue = project.sqlCmdVariables.get(
        sqlcmdVarToUpdate.friendlyName,
      );
      await projController.editSqlCmdVariable(
        createWorkspaceTreeItem(sqlcmdVarToUpdate),
      );
    ...
    test("Should remove file extensions from user input when creating files", async function (): Promise<void> {
      ...
      for (const testCase of testCases) {
        ...
        should(
          await utils.exists(
            path.join(project.projectFolderPath, expectedFileName),
          ),
        ).be.true(
...
> {
  ...
  should(proj.preDeployScripts.length).equal(
    1,
    "number of pre-deployment script entries",
  );
  should(proj.postDeployScripts.length).equal(
    1,
    "number of post-deployment script entries",
  );
  should(proj.noneDeployScripts.length).equal(
    1,
    "number of none script entries",
  );
  should(path.parse(scriptEntry.fsUri.fsPath).base).equal("someScript.sql");
  should((await fs.readFile(scriptEntry.fsUri.fsPath)).toString()).equal(
    "not a real script",
  );
...
function createWorkspaceTreeItem(
  node: BaseProjectTreeItem,
