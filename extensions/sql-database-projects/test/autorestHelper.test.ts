...
suite("Autorest tests", function (): void {
  ...
  test("Should detect autorest", async function (): Promise<void> {
    sinon
      ...
      .returns(<any>Promise.resolve(runViaNpx)); // stub a selection in case test runner doesn't have autorest installed
  ...
  test.skip("Should run an autorest command successfully", async function (): Promise<void> {
    ...
    sinon
      ...
      .returns(
        `${await autorestHelper.detectInstallation()} --version > ${dummyFile}`,
      );
