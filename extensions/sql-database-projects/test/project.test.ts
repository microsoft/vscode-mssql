...
projectSuite("Project: sqlproj content operations", function (): void {
  ...
  test("Should read Project from sqlproj", async function (): Promise<void> {
    ...
    should(project.sqlCmdVariables.get("BackupDatabaseName")).equal(
      "MyBackupDatabase",
    );
    ...
    should(project.databaseReferences[0].referenceName).containEql(
      constants.master,
    );
    should(
      project.databaseReferences[0] instanceof
        SystemDatabaseReferenceProjectEntry,
    ).equal(true);
    ...
    should(
      project.preDeployScripts.find(
        (f) => f.relativePath === "Script.PreDeployment1.sql",
      ),
    ).not.equal(undefined, "File Script.PreDeployment1.sql not read");
    should(
      project.postDeployScripts.find(
        (f) => f.relativePath === "Script.PostDeployment1.sql",
      ),
    ).not.equal(undefined, "File Script.PostDeployment1.sql not read");
    should(
      project.noneDeployScripts.find(
        (f) => f.relativePath === "Script.PreDeployment2.sql",
      ),
    ...
    should(
      project.publishProfiles.find(
        (f) => f.relativePath === "TestProjectName_1.publish.xml",
      ),
    ).not.equal(undefined, "Profile TestProjectName_1.publish.xml not read");
    should(
      project.publishProfiles.find(
        (f) => f.relativePath === "TestProjectName_2.publish.xml",
      ),
    ).not.equal(undefined, "Profile TestProjectName_2.publish.xml not read");
    should(
      project.publishProfiles.find(
        (f) => f.relativePath === "TestProjectName_3.publish.xml",
      ),
  ...
  test("Should read Project with Project reference from sqlproj", async function (): Promise<void> {
    ...
    project.databaseReferences[0].referenceName.should.containEql(
      "ReferencedTestProject",
    );
    (
      project.databaseReferences[0] instanceof SqlProjectReferenceProjectEntry
    ).should.be.true();
    project.databaseReferences[1].referenceName.should.containEql(
      constants.master,
    );
    (
      project.databaseReferences[1] instanceof
      SystemDatabaseReferenceProjectEntry
    ).should.be.true();
  });
  ...
  test("Should throw warning message while reading Project with more than 1 pre-deploy script from sqlproj", async function (): Promise<void> {
    ...
    should(stub.calledOnce).be.true(
      "showWarningMessage should have been called exactly once",
    );
  ...
  test("Should perform Folder and SQL object script operations", async function (): Promise<void> {
    ...
    const scriptPathTagged = path.join(
      folderPath,
      "Fake External Streaming Job.sql",
    );
    const scriptContentsTagged =
      "EXEC sys.sp_create_streaming_job 'job', 'SELECT 7'";
    ...
    should(
      project.folders.find(
        (f) => f.relativePath === convertSlashesForSqlProj(folderPath),
      ),
    ).not.equal(undefined);
    should(
  ...
  test("Should bulk-add scripts to sqlproj with pre-existing scripts on disk", async function (): Promise<void> {
    ...
    project.sqlObjectScripts.length.should.equal(
      2,
      "Number of scripts after adding",
    );
  });
  ...
  test("Should throw error while adding folders and SQL object scripts to sqlproj when a file does not exist on disk", async function (): Promise<void> {
    ...
    const relativePaths = list.map((f) =>
  ...
  test("Should show information messages when adding more than one pre/post deployment scripts to sqlproj", async function (): Promise<void> {
    ...
    stub
      .calledWith(constants.deployScriptExists(constants.PreDeploy))
      .should.be.true(
  ...
  test("Should not overwrite existing files", async function (): Promise<void> {
    ...
    const fileList = await testUtils.createListOfFiles(
      this.test,
      path.dirname(projFilePath),
    );
    ...
    should(fileStats.isFile()).equal(
      true,
      "Fourth entry in fileList should be a file",
    );
  ...
  test.skip("Should not add folders outside of the project folder", async function (): Promise<void> {
    ...
    should.equal(
      ...
      0,
      "Nothing should be added to the project",
    );

    // Try adding a parent of the project folder
    await testUtils.shouldThrowSpecificError(
      async () =>
        await project.addFolder(path.dirname(path.dirname(projFilePath))),
...
projectSuite(
  "Project: sdk style project content operations",
  function (): void {
    suiteSetup(async function (): Promise<void> {
      await baselines.loadBaselines();
    });
    ...
    test("Should handle excluding glob included folders", async function (): Promise<void> {
      ...
      should(project.folders.find((f) => f.relativePath === "folder1")).equal(
        undefined,
      );
    });
    ...
    test("Should handle excluding explicitly included folders", async function (): Promise<void> {
      ...
      should(project.folders.find((f) => f.relativePath === "folder2")).equal(
        undefined,
      );
    });
...
projectSuite("Project: database references", function (): void {
  ...
  test("Should read database references correctly", async function (): Promise<void> {
    ...
    let projRef: SqlProjectReferenceProjectEntry | undefined =
      project.databaseReferences.find(
        (r) =>
    ...
    projRef!
      .pathForSqlProj()
      .should.equal("..\\ReferencedProject\\ReferencedProject.sqlproj");
    ...
    projRef = project.databaseReferences.find(
      (r) =>
        r instanceof SqlProjectReferenceProjectEntry &&
        r.referenceName === "OtherProject",
    ) as SqlProjectReferenceProjectEntry;
    should(projRef).not.equal(undefined, "OtherProject reference");
    projRef!
      .pathForSqlProj()
      .should.equal("..\\OtherProject\\OtherProject.sqlproj");
    ...
    let dacpacRef: DacpacReferenceProjectEntry | undefined =
      project.databaseReferences.find(
        (r) =>
          r instanceof DacpacReferenceProjectEntry &&
          r.referenceName === "ReferencedDacpac",
      ) as DacpacReferenceProjectEntry;
    should(dacpacRef).not.equal(
      undefined,
      "dacpac reference for ReferencedDacpac",
    );
    dacpacRef!
      .pathForSqlProj()
      .should.equal("..\\ReferencedDacpac\\ReferencedDacpac.dacpac");
    ...
    dacpacRef = project.databaseReferences.find(
      (r) =>
        r instanceof DacpacReferenceProjectEntry &&
        r.referenceName === "OtherDacpac",
    ) as DacpacReferenceProjectEntry;
    should(dacpacRef).not.equal(undefined, "dacpac reference for OtherDacpac");
    dacpacRef!
      .pathForSqlProj()
      .should.equal("..\\OtherDacpac\\OtherDacpac.dacpac");
  ...
  test("Should delete database references correctly", async function (): Promise<void> {
    ...
    let ref = project.databaseReferences.find(
      (r) => r.referenceName === constants.msdb,
    );
  ...
  test("Should add system database artifact reference correctly", async function (): Promise<void> {
    ...
    const projFileText = (
      await fs.readFile(project.projectFilePath)
    ).toString();
    projFileText.should.containEql(
      '<ArtifactReference Include="$(SystemDacpacsLocation)',
    );
  });
  ...
  test("Should not allow adding duplicate system database references", async function (): Promise<void> {
    ...
    const systemDbReference: ISystemDatabaseReferenceSettings = {
      databaseVariableLiteralValue: systemDatabaseToString(
  ...
  test("Should handle trying to add duplicate database references when slashes are different direction", async function (): Promise<void> {
    ...
    projectReference.projectRelativePath = Uri.file(
      "testFolder\\testProject.sqlproj",
    );
    await testUtils.shouldThrowSpecificError(
      async () => await project.addProjectReference(projectReference),
      constants.databaseReferenceAlreadyExists,
  ...
  test("Should update sqlcmd variable values if value changes", async function (): Promise<void> {
    ...
    should(projFileText).containEql(
      "<DefaultValue>otherServerName</DefaultValue>",
    );
...
projectSuite("Project: properties", function (): void {
  ...
  test("Should throw on invalid target database version", async function (): Promise<void> {
    ...
    try {
      ...
      e.message.should.startWith(
        "Error: Invalid value for Database Schema Provider:",
      );
  ...
  test("Should read default database collation", async function (): Promise<void> {
    ...
    should(project.getDatabaseDefaultCollation()).equal(
...
projectSuite("Project: round trip updates", function (): void {
  ...
  test.skip("Should update SSDT project with new system database references", async function (): Promise<void> {
    await testUpdateInRoundTrip(
      this.test,
      baselines.SSDTUpdatedProjectBaseline,
    );
  });
  ...
  test("Should update SSDT project to work in ADS handling pre-existing targets", async function (): Promise<void> {
    await testUpdateInRoundTrip(
      this.test,
      baselines.SSDTProjectBaselineWithBeforeBuildTarget,
    );
  });
