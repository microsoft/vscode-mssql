...
suite("Tests to verify utils functions", function (): void {
  test("Should determine existence of files/folders", async () => {
    ...
    should(await utils.exists(path.join(testFolderPath, "file1.sql"))).equal(
      true,
    );
    should(await utils.exists(path.join(testFolderPath, "folder2"))).equal(
      true,
    );
    should(await utils.exists(path.join(testFolderPath, "folder4"))).equal(
      false,
    );
    should(
  ...
  test("Should get correct relative paths of files/folders", async () => {
    ...
    let projectUri = Uri.file(
      path.join(root, "project", "folder", "project.sqlproj"),
    );
    ...
    should(utils.trimUri(projectUri, fileUri)).equal(
      "../../forked/from/top/file.sql",
    );
  });
  ...
  test("Should convert from milliseconds to hr min sec correctly", () => {
    ...
    ).equal("1 hr, 59 min, 59 sec");
    should(utils.timeConversion(60 * 60 * 1000 + 59 * 60 * 1000)).equal(
      "1 hr, 59 min",
    );
    should(utils.timeConversion(60 * 60 * 1000)).equal("1 hr");
    should(utils.timeConversion(60 * 60 * 1000 + 59 * 1000)).equal(
      "1 hr, 59 sec",
    );
    should(utils.timeConversion(59 * 60 * 1000 + 59 * 1000)).equal(
      "59 min, 59 sec",
    );
  ...
  test("Should validate SQL server password correctly", () => {
    ...
    should(utils.isValidSQLPassword("")).equals(
      false,
      "empty string is invalid password",
    );
    ...
    should(utils.isValidSQLPassword("dF6$30")).equals(
      false,
      "dF6$30 is invalid password",
    );
    should(utils.isValidSQLPassword("dF65$530")).equals(
      true,
      "dF65$530 is valid password",
    );
    should(utils.isValidSQLPassword("dFdf65$530")).equals(
      true,
      "dF65$530 is valid password",
    );
    should(utils.isValidSQLPassword("av1fgh533@")).equals(
      true,
      "dF65$530 is valid password",
    );
  });
  ...
  test("findSqlVersionInTargetPlatform should return the version correctly", () => {
    ...
    should(
      utils.findSqlVersionInTargetPlatform("Azure Synapse SQL Pool"),
    ).equals(undefined, "invalid number returned for Azure Synapse SQL Pool");
  });
});
