...
export function createContext(): TestContext {
  ...
  const subscriptions: azure.subscription.Subscription[] = [
    { subscriptionId: "id1" },
    { subscriptionId: "id2" },
  ];
  const locations: azure.subscription.Location[] = [
    { id: "id1" },
    { id: "id2" },
  ];
  const groups: azure.resources.ResourceGroup[] = [
    { id: "id1", location: "l1" },
    { id: "id2", location: "l2" },
  ];
...
suite("Azure SQL client", function (): void {
  test("Should return accounts successfully", async function (): Promise<void> {
    ...
    const azureSqlClient = new AzureSqlClient(() =>
      Promise.resolve(testContext.azureAccountService),
    );
  ...
  test("Should return subscriptions successfully", async function (): Promise<void> {
    ...
    should(result[0].subscription.id).deepEqual(
      testContext.subscriptions[0].id,
    );
  });
  ...
  test("Should return locations successfully", async function (): Promise<void> {
    ...
    const azureSqlClient = new AzureSqlClient(
      () => Promise.resolve(testContext.azureAccountService),
      () => Promise.resolve(testContext.azureResourceService),
    );
