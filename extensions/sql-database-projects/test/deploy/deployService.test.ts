...
import {
  DeployService,
  getDockerImageSpec,
} from "../../src/models/deploy/deployService";
...
suite.skip("deploy service", function (): void {
  ...
  test("Should deploy a database to docker container successfully", async function (): Promise<void> {
    ...
    const deployService = new DeployService(
      testContext.azureSqlClient.object,
      testContext.outputChannel,
      shellExecutionHelper.object,
    );
    sandbox
      .stub(azdata.connection, "connect")
      .returns(Promise.resolve(mockConnectionResult));
    sandbox
      .stub(azdata.connection, "getUriForConnection")
    ...
    let connection = await deployService.deployToContainer(
      deployProfile,
      project1,
    );
  ...
  test("Should fail the deploy if docker is not running", async function (): Promise<void> {
    ...
    await should(
      deployService.deployToContainer(deployProfile, project1),
    ).rejected();
  });
  ...
  test("Should retry connecting to the server", async function (): Promise<void> {
    ...
    connectionStub
      .onFirstCall()
      .returns(Promise.resolve(mockFailedConnectionResult));
    connectionStub
      .onSecondCall()
      .returns(Promise.resolve(mockConnectionResult));
    sandbox
  ...
  test("Should create docker image info correctly", () => {
    ...
    const reallyLongName = new Array(128 + 1)
      .join("a")
      .replace(/[^a-zA-Z0-9_,\-]/g, "");
    const imageProjectName = reallyLongName.substring(
      0,
      128 - (constants.dockerImageNamePrefix.length + tag.length + 2),
    );
  ...
  test("Should create a new Azure SQL server successfully", async function (): Promise<void> {
    ...
    if (deployProfile?.sqlDbSetting?.session && session) {
      testContext.azureSqlClient
        .setup((x) =>
          x.createOrUpdateServer(
            session,
    ...
    should(deployProfile.sqlDbSetting?.serverName).equal(
