...
import {
  getDefaultPublishDeploymentOptions,
  getVscodeMssqlApi,
} from "../common/utils";
 ...
 */
export async function getPublishDatabaseSettings(
  project: ISqlProject,
  promptForConnection: boolean = true,
): Promise<ISqlProjectPublishSettings | undefined> {
  ...
  quickPick.items = [
    { label: constants.dontUseProfile },
    { label: constants.browseForProfileWithIcon },
  ];
  quickPick.ignoreFocusOut = true;
  quickPick.title = constants.selectProfileToUse;
  const profilePicked = new Promise<PublishProfile | undefined>(
    (resolve, reject) => {
      quickPick.onDidHide(() => {
      ...
      quickPick.onDidChangeSelection(async (items) => {
        if (items[0].label === constants.browseForProfileWithIcon) {
          const locations = await promptForPublishProfile(
      ...
      try {
        try {
          ...
          if (firewallRuleError?.connectionUri) {
            await vscodeMssqlApi.promptForFirewallRule(
              azureErr.connectionUri,
              connectionProfile,
            );
            connectionUri = await vscodeMssqlApi.connect(connectionProfile);
          } else {
  ...
  dbQuickpicks.unshift({
    label: `$(add) ${constants.createNew}`,
    isCreateNew: true,
  });
  ...
  if (projectNameIndex === -1) {
    ...
    // ...or move it to the top if it does
  ...
  while (!databaseName) {
    const selectedDatabase = await vscode.window.showQuickPick(dbQuickpicks, {
    ...
    if (selectedDatabase.isCreateNew) {
      databaseName =
        (await vscode.window.showInputBox({
          ...
          ignoreFocusOut: true,
          validateInput: (input) =>
            input ? undefined : constants.nameMustNotBeEmpty,
        })) ?? "";
  ...
  let sqlCmdVariables: Map<string, string> = getInitialSqlCmdVariables(
    project,
    publishProfile,
  );
    ...
    while (true) {
      ...
      for (const key of sqlCmdVariables.keys()) {
        quickPickItems.push({
          ...
          key?: string;
          isResetAllVars?: boolean;
          isDone?: boolean;
        });
      }
      quickPickItems.push({
        label: `$(refresh) ${constants.resetAllVars}`,
        isResetAllVars: true,
      });
      quickPickItems.unshift({
        label: `$(check) ${constants.done}`,
        isDone: true,
      });
      const sqlCmd = await vscode.window.showQuickPick(quickPickItems, {
        title: constants.chooseSqlcmdVarsToModify,
        ignoreFocusOut: true,
      });
      if (!sqlCmd) {
  ...
  let settings: ISqlProjectPublishSettings = {
    ...
    deploymentOptions:
      ...
      (await getDefaultPublishDeploymentOptions(project)),
 ...
 */
function getInitialSqlCmdVariables(
  project: ISqlProject,
...
export async function launchPublishTargetOption(
  project: Project,
): Promise<constants.PublishTargetType | undefined> {
  ...
  const logicalServerName =
    target === constants.targetPlatformToVersion.get(SqlTargetPlatform.sqlAzure)
      ? constants.AzureSqlLogicalServerName
  ...
  if (
    ...
    const enablePreviewFeatures = vscode.workspace
      ...
      .get(constants.enablePreviewFeaturesKey);
  ...
  const publishOption = await vscode.window.showQuickPick(options, {
    title: constants.selectPublishOption,
    ignoreFocusOut: true,
  });
