...
export class CreateProjectFromDatabaseDialog {
  ...
  public createProjectFromDatabaseCallback:
    | ((model: ImportDataModel, connectionId?: string) => any)
    | undefined;

  constructor(private profile: azdataType.IConnectionProfile | undefined) {
    this.dialog = getAzdataApi()!.window.createModelViewDialog(
      constants.createProjectFromDatabaseDialogName,
      "createProjectFromDatabaseDialog",
    );
    this.createProjectFromDatabaseTab = getAzdataApi()!.window.createTab(
      constants.createProjectFromDatabaseDialogName,
    );
  ...
  public async openDialog(): Promise<void> {
    ...
    this.toDispose.push(
      this.dialog.okButton.onClick(
        async () => await this.handleCreateButtonClick(),
      ),
    );
    ...
    if (this.profile && this.profile?.serverName) {
      ...
      connected = !!connections.find(
        (c) => c.connectionId === this.profile!.id,
      );

      if (!connected) {
        // if the connection clicked on isn't currently connected, try to connect
        const result = await getAzdataApi()!.connection.connect(
          this.profile,
          true,
          false,
        );
        ...
        if (!result.connected) {
          ...
          const connection =
            await getAzdataApi()!.connection.openConnectionDialog(
              undefined,
              this.profile,
              {
    ...
    if (connected) {
      await this.updateConnectionComponents(
        getConnectionName(this.profile),
        this.profile!.id,
        this.profile!.databaseName,
      );
    }
  ...
  private initializeCreateProjectFromDatabaseTab(): void {
    this.createProjectFromDatabaseTab.registerContent(async (view) => {
      ...
      const sourceDatabaseFormSection = view.modelBuilder
        .groupContainer()
        .withLayout({
      ...
      const targetProjectFormSection = view.modelBuilder
        ...
        .withItems([projectNameRow, projectLocationRow])
        .component();

      const folderStructureRow = this.createFolderStructureRow(view);

      this.includePermissionsCheckbox = view.modelBuilder
        .checkBox()
        .withProps({
      ...
      this.sdkStyleCheckbox = view.modelBuilder
        .checkBox()
        .withProps({
          checked: true,
      ...
      const sdkLearnMore = view.modelBuilder
        .hyperlink()
        .withProps({
      ...
      const sdkFormComponentGroup = view.modelBuilder
        .flexContainer()
        .withLayout({ flexFlow: "row", alignItems: "baseline" })
        .withItems([this.sdkStyleCheckbox, sdkLearnMore], {
          CSSStyles: { flex: "0 0 auto", "margin-right": "10px" },
        })
        .component();

      const settingsFormSection = view.modelBuilder
        .groupContainer()
        .withLayout({
        ...
        .withItems([
          folderStructureRow,
          this.includePermissionsCheckbox,
          sdkFormComponentGroup,
        ])
        .component();
  ...
  private createConnectionRow(
    ...
    const connectionRow = view.modelBuilder
      .flexContainer()
      .withItems([serverLabel, sourceConnectionTextBox], {
        flex: "0 0 auto",
        CSSStyles: {
          "margin-right": "10px",
          "margin-bottom": "-5px",
      ...
      .component();
    connectionRow.addItem(selectConnectionButton, {
      CSSStyles: {
        "margin-right": "0px",
        "margin-bottom": "-5px",
        "margin-top": "-10px",
  ...
  private createDatabaseRow(
    ...
    this.sourceDatabaseDropDown = view.modelBuilder
      .dropDown()
      .withProps({
    ...
    const databaseRow = view.modelBuilder
      .flexContainer()
      .withItems(
        [
          databaseLabel,
          <azdataType.DropDownComponent>this.sourceDatabaseDropDown,
        ],
        {
          flex: "0 0 auto",
          CSSStyles: { "margin-right": "10px", "margin-bottom": "-10px" },
        },
      )
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
  ...
  public setProjectName() {
    this.projectNameTextBox!.value = newProjectTool.defaultProjectNameFromDb(
      sanitizeStringForFilename(<string>this.sourceDatabaseDropDown!.value),
    );
  }

  private createSourceConnectionComponent(
    view: azdataType.ModelView,
  ): azdataType.InputBoxComponent {
    this.sourceConnectionTextBox = view.modelBuilder
      .inputBox()
      .withProps({
        value: "",
  ...
  private async updateConnectionComponents(
    ...
    void this.sourceConnectionTextBox!.updateProperty(
      "title",
      connectionTextboxValue,
    );
    ...
    if (connectionId) {
      ...
      try {
        databaseValues = (
          await getAzdataApi()!.connection.listDatabases(connectionId)
        )
  ...
  private createProjectNameRow(
    view: azdataType.ModelView,
  ): azdataType.FlexContainer {
    this.projectNameTextBox = view.modelBuilder
      .inputBox()
      .withValidation((component) => isValidBasename(component.value))
      .withProps({
    ...
    this.projectNameTextBox.onTextChanged((text) => {
      ...
      if (errorMessage !== undefined) {
        ...
        void this.projectNameTextBox!.updateProperty(
        ...
        void this.projectNameTextBox!.updateProperty(
          "title",
          this.projectNameTextBox!.value,
        );
        this.tryEnableCreateButton();
      }
    });

    const projectNameLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    const projectNameRow = view.modelBuilder
      .flexContainer()
      .withItems([projectNameLabel, this.projectNameTextBox], {
        flex: "0 0 auto",
        CSSStyles: {
          "margin-right": "10px",
          "margin-bottom": "-5px",
  ...
  private createProjectLocationRow(
    view: azdataType.ModelView,
  ): azdataType.FlexContainer {
    const browseFolderButton: azdataType.Component =
      this.createBrowseFolderButton(view);

    this.projectLocationTextBox = view.modelBuilder
      .inputBox()
      .withProps({
        value: "",
    ...
    this.projectLocationTextBox.onTextChanged(() => {
      void this.projectLocationTextBox!.updateProperty(
        "title",
    ...
    const projectLocationLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    const projectLocationRow = view.modelBuilder
      .flexContainer()
      .withItems([projectLocationLabel, this.projectLocationTextBox], {
        flex: "0 0 auto",
        CSSStyles: { "margin-right": "10px", "margin-bottom": "-10px" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
  ...
  private createBrowseFolderButton(
    view: azdataType.ModelView,
  ): azdataType.ButtonComponent {
    const browseFolderButton = view.modelBuilder
      .button()
      .withProps({
  ...
  private createFolderStructureRow(
    view: azdataType.ModelView,
  ): azdataType.FlexContainer {
    this.folderStructureDropDown = view.modelBuilder
      .dropDown()
      .withProps({
        values: [
          constants.file,
    ...
    const folderStructureLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    const folderStructureRow = view.modelBuilder
      .flexContainer()
      .withItems(
        [
          folderStructureLabel,
          <azdataType.DropDownComponent>this.folderStructureDropDown,
        ],
        {
          flex: "0 0 auto",
          CSSStyles: { "margin-right": "10px", "margin-top": "-10px" },
        },
      )
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
  ...
  public tryEnableCreateButton(): void {
    if (
      this.sourceConnectionTextBox!.value &&
      this.sourceDatabaseDropDown!.value &&
      this.projectNameTextBox!.value &&
      this.projectLocationTextBox!.value
    ) {
  ...
  public async handleCreateButtonClick(): Promise<void> {
    ...
    const connectionUri = await azdataApi!.connection.getUriForConnection(
      this.connectionId!,
    );
    const model: ImportDataModel = {
      ...
      version: "1.0.0.0",
      extractTarget: mapExtractTargetEnum(
        <string>this.folderStructureDropDown!.value,
      ),
  ...
  async validate(): Promise<boolean> {
    try {
      ...
      const parentDirectoryExists = await exists(
        this.projectLocationTextBox!.value!,
      );
      if (!parentDirectoryExists) {
        this.showErrorMessage(
          constants.ProjectParentDirectoryNotExistError(
            this.projectLocationTextBox!.value!,
          ),
        );
        return false;
      ...
      const projectDirectoryExists = await exists(
        path.join(
          this.projectLocationTextBox!.value!,
          this.projectNameTextBox!.value!,
        ),
      ...
      if (projectDirectoryExists) {
        this.showErrorMessage(
          constants.ProjectDirectoryAlreadyExistError(
            this.projectNameTextBox!.value!,
            this.projectLocationTextBox!.value!,
          ),
        );
      ...
      if (
        (await getDataWorkspaceExtensionApi().validateWorkspace()) === false
      ) {
        return false;
      }
