...
export class CreateProjectFromDatabaseDialog {
    ...
    public async openDialog(): Promise<void> {
        ...
        this.toDispose.push(
            this.dialog.okButton.onClick(async () => await this.handleCreateButtonClick()),
        );
        ...
        if (this.profile && this.profile?.serverName) {
            ...
            if (!connected) {
                ...
                if (!result.connected) {
                    ...
                    const connection = await getAzdataApi()!.connection.openConnectionDialog(
                        undefined,
                        this.profile,
                        {
    ...
    private createConnectionRow(view: azdataType.ModelView): azdataType.FlexContainer {
        ...
        connectionRow.addItem(selectConnectionButton, {
            CSSStyles: { "margin-right": "0px", "margin-bottom": "-5px", "margin-top": "-10px" },
        });
    ...
    private createDatabaseRow(view: azdataType.ModelView): azdataType.FlexContainer {
        ...
        const databaseRow = view.modelBuilder
            .flexContainer()
            .withItems([databaseLabel, <azdataType.DropDownComponent>this.sourceDatabaseDropDown], {
                flex: "0 0 auto",
    ...
    private createFolderStructureRow(view: azdataType.ModelView): azdataType.FlexContainer {
        ...
        const folderStructureRow = view.modelBuilder
            .flexContainer()
            .withItems(
                [folderStructureLabel, <azdataType.DropDownComponent>this.folderStructureDropDown],
                { flex: "0 0 auto", CSSStyles: { "margin-right": "10px", "margin-top": "-10px" } },
            )
            .withLayout({ flexFlow: "row", alignItems: "center" })
            .component();
    ...
    async validate(): Promise<boolean> {
        try {
            ...
            const projectDirectoryExists = await exists(
                path.join(this.projectLocationTextBox!.value!, this.projectNameTextBox!.value!),
            );
