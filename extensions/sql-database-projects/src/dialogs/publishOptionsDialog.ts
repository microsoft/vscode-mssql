...
export class PublishOptionsDialog {
  ...
  private excludeObjectTypesOptionsTable:
    | azdataType.DeclarativeTableComponent
    | undefined;
  private excludeObjectTypesOptionsFlexBuilder:
    | azdataType.FlexContainer
    | undefined;

  constructor(
    defaultOptions: mssql.DeploymentOptions,
    private publish: PublishDatabaseDialog,
  ) {
    this.optionsModel = new DeployOptionsModel(defaultOptions);
  }

  protected initializeDialog(): void {
    this.optionsTab = utils
      .getAzdataApi()!
      .window.createTab(constants.PublishOptions);
    this.excludeObjectTypesOptionsTab = utils
      .getAzdataApi()!
      .window.createTab(constants.ExcludeObjectTypeTab);
  ...
  public openDialog(): void {
    this.dialog = utils
      .getAzdataApi()!
      .window.createModelViewDialog(constants.AdvancedPublishOptions);
    ...
    let resetButton = utils
      .getAzdataApi()!
      .window.createButton(constants.ResetButton);
  ...
  private initializeDeploymentOptionsDialogTab(): void {
    this.optionsTab?.registerContent(async (view) => {
      ...
      const loader = view.modelBuilder
        .loadingComponent()
        .withProps({
          CSSStyles: {
            "margin-top": "50%",
      ...
      this.optionsFlexBuilder = view.modelBuilder
        .flexContainer()
        .withLayout({
          flexFlow: "column",
        })
      ...
      this.descriptionHeading = view.modelBuilder
        .table()
        .withProps({
          data: [],
          columns: [
      ...
      this.descriptionText = view.modelBuilder
        .text()
        .withProps({
          value: " ",
        })
      ...
      this.optionsTable = view.modelBuilder
        .declarativeTable()
        .withProps({
          width: "480px",
      ...
      this.disposableListeners.push(
        this.optionsTable.onRowSelected(async (selectedRow) => {
          // data[row][1] contains the option display name
          const displayName = <string>(
            this.optionsTable!.dataValues![selectedRow.row!][1].value
          );
      ...
      this.disposableListeners.push(
        this.optionsTable.onDataChanged!((changedData) => {
          if (changedData.row !== undefined) {
            // dataValues[row][1] contains the option display name
            const displayName = <string>(
              this.optionsTable?.dataValues![changedData.row][1].value
            );
      ...
      this.optionsFlexBuilder.addItem(this.optionsTable, {
        CSSStyles: { overflow: "scroll", height: "65vh", "padding-top": "2px" },
      });
      this.optionsFlexBuilder.addItem(this.descriptionHeading, {
        CSSStyles: { "font-weight": "bold", height: "30px" },
      });
      this.optionsFlexBuilder.addItem(this.descriptionText, {
        CSSStyles: {
          padding: "4px",
          "margin-right": "10px",
          overflow: "scroll",
  ...
  private initializeExcludeObjectTypesOptionsDialogTab(): void {
    this.excludeObjectTypesOptionsTab?.registerContent(async (view) => {
      this.excludeObjectTypesOptionsTable = view.modelBuilder
        .declarativeTable()
        .withProps({
          width: "480px",
          columns: [
      ...
      this.disposableListeners.push(
        this.excludeObjectTypesOptionsTable.onDataChanged((changedData) => {
          const displayName = <string>(
            this.excludeObjectTypesOptionsTable?.dataValues![changedData.row][1]
              .value
          );
          const checkboxValue = <boolean>changedData.value;
          this.optionsModel.setExcludeObjectTypesOptionValue(
            displayName,
            checkboxValue,
          );
          this.optionsChanged = true;
      ...
      this.excludeObjectTypesOptionsFlexBuilder = view.modelBuilder
        .flexContainer()
        .withLayout({
          flexFlow: "column",
        })
      ...
      this.excludeObjectTypesOptionsFlexBuilder.addItem(
        this.excludeObjectTypesOptionsTable,
        {
          CSSStyles: {
            overflow: "scroll",
            height: "80vh",
  ...
  protected execute(): void {
    ...
    this.publish.publishOptionsModified =
      this.isResetOptionsClicked && !this.optionsChanged
        ? false
        : this.optionsChanged || this.publish.publishOptionsModified;
  }
  ...
  private async reset(): Promise<void> {
    ...
    this.optionsFlexBuilder?.insertItem(this.optionsTable!, 0, {
      CSSStyles: { overflow: "scroll", height: "65vh", "padding-top": "2px" },
    });
    TelemetryReporter.sendActionEvent(
      TelemetryViews.PublishOptionsDialog,
    ...
    await this.updateExcludeObjectsTable();
    this.excludeObjectTypesOptionsFlexBuilder?.removeItem(
      this.excludeObjectTypesOptionsTable!,
    );
    this.excludeObjectTypesOptionsFlexBuilder?.addItem(
      this.excludeObjectTypesOptionsTable!,
      {
        CSSStyles: { overflow: "scroll", height: "80vh", "padding-top": "2px" },
      },
    );
