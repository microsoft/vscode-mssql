...
export class UpdateProjectFromDatabaseDialog {
  ...
  public updateProjectFromDatabaseCallback:
    | ((model: UpdateProjectDataModel) => any)
    | undefined;
  ...
  constructor(
    ...
    private workspaceProjects: vscode.Uri[],
  ) {
    ...
    this.dialog = getAzdataApi()!.window.createModelViewDialog(
      constants.updateProjectFromDatabaseDialogName,
      "updateProjectFromDatabaseDialog",
    );
    this.updateProjectFromDatabaseTab = getAzdataApi()!.window.createTab(
      constants.updateProjectFromDatabaseDialogName,
    );
    ...
    this.toDispose.push(
      this.dialog.onClosed((_) => this.initDialogPromise.resolve()),
    );
  }
  ...
  public async openDialog(): Promise<void> {
    ...
    this.toDispose.push(
      this.dialog.okButton.onClick(
        async () => await this.handleUpdateButtonClick(),
      ),
    );
  ...
  private initializeUpdateProjectFromDatabaseTab(): void {
    this.updateProjectFromDatabaseTab.registerContent(async (view) => {
      ...
      const targetProjectFormSection = view.modelBuilder
        ...
        .withItems([projectLocationRow, folderStructureRow])
        .component();

      const actionRow = await this.createActionRow(view);
      const actionFormSection = view.modelBuilder
        .groupContainer()
        .withLayout({
      ...
      this.formBuilder = <azdata.FormBuilder>view.modelBuilder
        .formContainer()
        .withFormItems(
          [
            {
  ...
  private createServerRow(view: azdata.ModelView): azdata.FlexContainer {
    ...
    const connectionRow = view.modelBuilder
      .flexContainer()
      .withItems([serverLabel, this.serverDropdown!], {
        flex: "0 0 auto",
        CSSStyles: {
          "margin-right": "10px",
          "margin-bottom": "-5px",
      ...
      .component();
    connectionRow.addItem(this.connectionButton!, {
      CSSStyles: {
        "margin-right": "0px",
        "margin-bottom": "-5px",
        "margin-top": "-10px",
  ...
  private createDatabaseRow(view: azdata.ModelView): azdata.FlexContainer {
    ...
    const databaseRow = view.modelBuilder
      .flexContainer()
      .withItems(
        [databaseLabel, <azdata.DropDownComponent>this.databaseDropdown!],
        {
          flex: "0 0 auto",
  ...
  private createServerComponent(view: azdata.ModelView) {
    this.serverDropdown = view.modelBuilder
      .dropDown()
      .withProps({
        editable: true,
  ...
  private createDatabaseComponent(view: azdata.ModelView) {
    this.databaseDropdown = view.modelBuilder
      .dropDown()
      .withProps({
        editable: true,
        fireOnTextChange: true,
  ...
  protected async populateDatabaseDropdown() {
    const connectionProfile = (
      this.serverDropdown!.value as ConnectionDropdownValue
    ).connection;
    ...
    if (values && values.length > 0) {
      ...
      if (
        connectionProfile.options.database &&
        connectionProfile.options.database !== constants.master
      ) {
  ...
  private async getServerValues() {
    let cons = await getAzdataApi()!.connection.getConnections(
      /* activeConnectionsOnly */ true,
    );
    ...
    values = values.reduce(
      (
        ...
        let exists = uniqueValues.find(
          (x) => x.displayName === conn.displayName,
        );
        if (!exists) {
  ...
  protected async getDatabaseValues(connectionId: string) {
    ...
    let values = (await getAzdataApi()!.connection.listDatabases(connectionId))
  ...
  private createConnectionButton(view: azdata.ModelView) {
    this.connectionButton = view.modelBuilder
      .button()
      .withProps({
  ...
  private createProjectLocationRow(
    ...
    const browseFolderButton: azdata.Component =
      this.createBrowseFileButton(view);
    ...
    this.projectFileDropdown = view.modelBuilder
      .dropDown()
      .withProps({
        editable: true,
        fireOnTextChange: true,
    ...
    this.projectFileDropdown.onValueChanged(async () => {
      await this.projectFileDropdown!.updateProperty(
        "title",
    ...
    const projectLocationRow = view.modelBuilder
      .flexContainer()
      .withItems([projectLocationLabel], {
        flex: "0 0 auto",
        CSSStyles: {
          "margin-right": "10px",
          "margin-bottom": "-5px",
      ...
      .component();
    projectLocationRow.addItem(this.projectFileDropdown, {
      CSSStyles: { "margin-right": "10px" },
    });
    projectLocationRow.addItem(browseFolderButton, {
      CSSStyles: { "margin-top": "2px" },
    });

    return projectLocationRow;
  }

  private createBrowseFileButton(
    view: azdata.ModelView,
    ...
    const folderStructureRow = view.modelBuilder
      ...
      .withItems(
        [
          ...
          <azdata.DropDownComponent>this.folderStructureDropDown,
        ],
        {
          flex: "0 0 auto",
          CSSStyles: { "margin-right": "10px", "margin-bottom": "-10px" },
  ...
  private async createActionRow(
    view: azdata.ModelView,
  ): Promise<azdata.FlexContainer> {
    this.compareActionRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "action",
    ...
    this.updateActionRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "action",
    ...
    let radioButtons = view.modelBuilder
      .flexContainer()
    ...
    const actionLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    const actionRow = view.modelBuilder
      .flexContainer()
      .withItems([actionLabel], {
        flex: "0 0 auto",
        CSSStyles: { "margin-right": "10px", "margin-top": "-17px" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
  ...
  public async handleUpdateButtonClick(): Promise<void> {
    const serverDropdownValue = this.serverDropdown!
      .value! as azdata.CategoryValue as ConnectionDropdownValue;
    const ownerUri = await getAzdataApi()!.connection.getUriForConnection(
      serverDropdownValue.connection.connectionId,
    );

    let connection = (
      await getAzdataApi()!.connection.getConnections(true)
    ).filter(
      (con) => con.connectionId === serverDropdownValue.connection.connectionId,
    )[0];
    connection.databaseName = this.databaseDropdown!.value! as string;

    const credentials = await getAzdataApi()!.connection.getCredentials(
  ...
  async validate(): Promise<boolean> {
    try {
      ...
      const parentDirectoryExists = await exists(
        path.dirname(this.projectFileDropdown!.value! as string),
      );
      if (!parentDirectoryExists) {
        this.showErrorMessage(
          constants.ProjectParentDirectoryNotExistError(
            this.projectFileDropdown!.value! as string,
          ),
        );
        return false;
      }
