...
export class AddDatabaseReferenceDialog {
  ...
  public suppressMissingDependenciesErrorsCheckbox:
    | azdataType.CheckBoxComponent
    | undefined;
  ...
  private systemDatabaseRadioButton:
    | azdataType.RadioButtonComponent
    | undefined;
  private systemDatabaseArtifactRefRadioButton:
    | azdataType.RadioButtonComponent
    | undefined;
  private systemDatabasePackageRefRadioButton:
    | azdataType.RadioButtonComponent
    | undefined;
  private systemDbRefRadioButtonsComponent:
    | azdataType.FormComponent
    | undefined;
  private systemDbRefType: SystemDbReferenceType =
  ...
  constructor(private project: Project) {
    this.dialog = utils
      ...
      .window.createModelViewDialog(
        constants.addDatabaseReferenceDialogName,
        "addDatabaseReferencesDialog",
      );
    this.addDatabaseReferenceTab = utils
      .getAzdataApi()!
      .window.createTab(constants.addDatabaseReferenceDialogName);
  ...
  public async openDialog(): Promise<void> {
    ...
    this.toDispose.push(
      this.dialog.okButton.onClick(async () => await this.addReferenceClick()),
    );
  ...
  private initializeTab(): void {
    this.addDatabaseReferenceTab.registerContent(async (view) => {
      ...
      this.systemDbRefRadioButtonsComponent =
        this.createSystemDbReferenceTypeRadioButtons();

      this.suppressMissingDependenciesErrorsCheckbox = view.modelBuilder
        .checkBox()
        .withProps({
          ...
          [
            {
              ...
              components: [
                radioButtonGroup,
                this.currentReferencedDatabaseType ===
                ReferencedDatabaseType.project
                  ? this.projectFormComponent
  ...
  public async addReferenceClick(): Promise<void> {
    ...
    if (
      ...
      const systemDbRef: ISystemDatabaseReferenceSettings = {
        ...
        systemDb: utils.getSystemDatabase(
          <string>this.systemDatabaseDropdown?.value,
        ),
        suppressMissingDependenciesErrors: <boolean>(
          this.suppressMissingDependenciesErrorsCheckbox?.checked
        ),
      ...
      if (
        ...
        const dacpacRef: IDacpacReferenceSettings = {
          ...
          dacpacFileLocation: vscode.Uri.file(
            <string>this.dacpacTextbox?.value,
          ),
          suppressMissingDependenciesErrors: <boolean>(
            this.suppressMissingDependenciesErrorsCheckbox?.checked
          ),
    ...
    TelemetryReporter.createActionEvent(
      ...
      .withAdditionalProperties({
        referencedDatabaseType: this.currentReferencedDatabaseType!.toString(),
      })
      .send();
  ...
  private createReferenceTypeRadioButtons(): azdataType.FormComponent {
    ...
    const radioButtons = [
      this.projectRadioButton,
      this.systemDatabaseRadioButton,
      dacpacRadioButton,
    ];
    ...
    let flexRadioButtonsModel: azdataType.FlexContainer =
      this.view!.modelBuilder.flexContainer()
        .withLayout({ flexFlow: "column" })
        .withItems(radioButtons)
        .withProps({
          ariaRole: "radiogroup",
          ariaLabel: constants.referenceRadioButtonsGroupTitle,
        })
        .component();
  ...
  private createSystemDbReferenceTypeRadioButtons(): azdataType.FormComponent {
    this.systemDatabasePackageRefRadioButton =
      this.view!.modelBuilder.radioButton()
        .withProps({
    ...
    this.systemDatabasePackageRefRadioButton.onDidChangeCheckedState(
      (checked) => {
        if (checked) {
    ...
    this.systemDatabaseArtifactRefRadioButton =
      this.view!.modelBuilder.radioButton()
        .withProps({
          name: "systemDbRefType",
    ...
    this.systemDatabaseArtifactRefRadioButton.onDidChangeCheckedState(
      (checked) => {
        if (checked) {
    ...
    const flexRadioButtonsModel: azdataType.FlexContainer =
      this.view!.modelBuilder.flexContainer()
        .withLayout({ flexFlow: "column" })
        .withItems(radioButtons)
        .withProps({
          ariaRole: "radiogroup",
          ariaLabel: constants.referenceTypeRadioButtonsGroupTitle,
        })
        .component();
  ...
  public projectRadioButtonClick(): void {
    this.formBuilder!.removeFormItem(
      <azdataType.FormComponent>this.dacpacFormComponent,
    );
    this.formBuilder!.removeFormItem(
      <azdataType.FormComponent>this.systemDatabaseFormComponent,
    );
    this.formBuilder!.removeFormItem(
      <azdataType.FormComponentGroup>this.nupkgFormComponent,
    );
    this.formBuilder!.removeFormItem(
      <azdataType.FormComponent>this.systemDbRefRadioButtonsComponent,
    );
    this.formBuilder!.insertFormItem(
      <azdataType.FormComponent>this.projectFormComponent,
      2,
    );
  ...
  private async createProjectDropdown(): Promise<azdataType.FormComponent> {
    this.projectDropdown = this.view!.modelBuilder.dropDown()
      .withProps({
    ...
    projectFiles = projectFiles.filter(
      (p) => p.fsPath !== this.project.projectFilePath,
    );
    this.projectDropdown.values = projectFiles.map(
      (p) => path.parse(p.fsPath).name,
    );
  ...
  private createSystemDatabaseDropdown(): azdataType.FormComponent {
    this.systemDatabaseDropdown = this.view!.modelBuilder.dropDown()
      .withProps({
  ...
  private createDacpacTextbox(): azdataType.FormComponent {
    this.dacpacTextbox = this.view!.modelBuilder.inputBox()
      .withProps({
    ...
    const databaseRow = this.view!.modelBuilder.flexContainer()
      .withItems([this.dacpacTextbox], {
        flex: "0 0 auto",
  ...
  private createNupkgFormComponentGroup(): azdataType.FormComponentGroup {
    this.nupkgNameTextbox = this.view!.modelBuilder.inputBox()
      .withProps({
    ...
    this.nupkgVersionTextbox = this.view!.modelBuilder.inputBox()
      .withProps({
  ...
  private createLoadDacpacButton(): azdataType.ButtonComponent {
    const loadDacpacButton = this.view!.modelBuilder.button()
      .withProps({
  ...
  private createLocationDropdown(): azdataType.FormComponent {
    this.locationDropdown = this.view!.modelBuilder.dropDown()
      .withProps({
        ariaLabel: constants.location,
        values:
          this.currentReferencedDatabaseType === ReferencedDatabaseType.systemDb
            ? constants.systemDbLocationDropdownValues
  ...
  public updateEnabledInputBoxes(): void {
    ...
    if (this.locationDropdown?.value === constants.sameDatabase) {
      ...
      // clear values in disabled fields
      this.databaseVariableTextbox!.value = isSystemDb
        ? ""
        : this.databaseVariableTextbox!.value;
      this.serverNameTextbox!.value = "";
      this.serverVariableTextbox!.value = "";
      ...
      this.locationDropdown?.value === constants.differentDbDifferentServer
    ) {
      this.databaseNameTextbox!.enabled = true;
    ...
    if (this.databaseNameTextbox!.enabled) {
      switch (this.currentReferencedDatabaseType) {
        ...
        case ReferencedDatabaseType.systemDb: {
          this.databaseNameTextbox!.value = <string>(
            this.systemDatabaseDropdown?.value
          );
          break;
        }
        case ReferencedDatabaseType.dacpac: {
          const dacpacName = this.dacpacTextbox!.value
            ? path.parse(this.dacpacTextbox!.value!).name
            : "";
          this.databaseNameTextbox!.value = dacpacName;
          this.databaseVariableTextbox!.value = dacpacName
            ? `${dacpacName}`
            : "";
          break;
        }
        case ReferencedDatabaseType.nupkg: {
          const nupkgName = this.nupkgNameTextbox!.value
            ? path.parse(this.nupkgNameTextbox!.value!).name
            : "";
  ...
  private createVariableSection(): azdataType.FormComponent {
    // database name row
    this.databaseNameTextboxLabel = this.createLabel(
    ...
    const databaseNameRow = this.view!.modelBuilder.flexContainer()
      .withItems([this.databaseNameTextboxLabel, this.databaseNameTextbox], {
        flex: "0 0 auto",
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
    ...
    this.databaseVariableTextbox = this.createInputBox(
      constants.databaseVariable,
      false,
      false,
    );
    const databaseVariableRow = this.view!.modelBuilder.flexContainer()
      .withItems(
        [
          this.createLabel(constants.databaseVariable),
          this.databaseVariableTextbox,
        ],
        { flex: "0 0 auto" },
      )
      .withLayout({ flexFlow: "row", alignItems: "center" })
    ...
    this.serverNameTextbox = this.createInputBox(
      constants.serverName,
      false,
      false,
    );
    const serverNameRow = this.view!.modelBuilder.flexContainer()
      .withItems([this.serverNameTextboxLabel, this.serverNameTextbox], {
        flex: "0 0 auto",
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
    ...
    // server variable row
    this.serverVariableTextboxLabel = this.createLabel(
      constants.serverVariable,
      false,
    );
    this.serverVariableTextbox = this.createInputBox(
      constants.serverVariable,
      false,
      false,
    );
    const serverVariableRow = this.view!.modelBuilder.flexContainer()
      .withItems(
        [this.serverVariableTextboxLabel, this.serverVariableTextbox],
        { flex: "0 0 auto" },
      )
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();

    const variableSection = this.view!.modelBuilder.flexContainer()
      .withItems([
        databaseNameRow,
        databaseVariableRow,
        serverNameRow,
      ...
      .withLayout({ flexFlow: "column" })
      .withProps({ CSSStyles: { "margin-bottom": "25px" } })
      .component();
    this.setDefaultDatabaseValues();
  ...
  ): azdataType.TextComponent {
    const label = this.view!.modelBuilder.text()
      .withProps({
        value: value,
  ...
  private createInputBox(
    ...
    const inputBox = this.view!.modelBuilder.inputBox()
      .withProps({
        ariaLabel: ariaLabel,
        enabled: enabled,
  ...
  private createExampleUsage(): azdataType.FormComponent {
    this.exampleUsage = this.view!.modelBuilder.text()
      .withProps({
        value:
          this.currentReferencedDatabaseType === ReferencedDatabaseType.project
            ? constants.databaseNameRequiredVariableOptional
            : constants.systemDatabaseReferenceRequired,
        CSSStyles: { "user-select": "text" },
      })
      .component();

    const exampleUsageWrapper = this.view!.modelBuilder.flexContainer()
      .withItems([this.exampleUsage], {
        CSSStyles: {
          width: "415px",
          height: "80px",
          padding: "0 10px",
  ...
  private updateExampleUsage(): void {
    ...
    switch (this.locationDropdown!.value) {
      ...
      case constants.differentDbSameServer: {
        if (!this.databaseNameTextbox?.value) {
          newText =
            this.currentReferencedDatabaseType ===
            ReferencedDatabaseType.systemDb
              ? constants.enterSystemDbName
              : constants.databaseNameRequiredVariableOptional;
          fontStyle = cssStyles.fontStyle.italics;
        } else {
          const db = this.databaseVariableTextbox?.value
            ? utils.formatSqlCmdVariable(this.databaseVariableTextbox?.value)
            : this.databaseNameTextbox.value;
      ...
      case constants.differentDbDifferentServer: {
        if (
          ...
          const server = utils.formatSqlCmdVariable(
            this.serverVariableTextbox.value,
          );
          const db = this.databaseVariableTextbox?.value
            ? utils.formatSqlCmdVariable(this.databaseVariableTextbox?.value)
            : this.databaseNameTextbox.value;
          newText = constants.differentDbDifferentServerExampleUsage(
            server,
            db,
          );
        }
    ...
    if (!this.validSqlCmdVariables()) {
      newText =
        utils.validateSqlCmdVariableName(this.databaseVariableTextbox?.value) ??
        utils.validateSqlCmdVariableName(this.serverVariableTextbox!.value!)!;
    }
  ...
  private validSqlCmdVariables(): boolean {
    if (
      (this.databaseVariableTextbox?.enabled &&
        this.databaseVariableTextbox?.value &&
        utils.validateSqlCmdVariableName(
          this.databaseVariableTextbox?.value,
        )) ||
      (this.serverVariableTextbox?.enabled &&
        this.serverVariableTextbox?.value &&
        utils.validateSqlCmdVariableName(this.serverVariableTextbox?.value))
    ) {
      return false;
  ...
  private dacpacRequiredFieldsFilled(): boolean {
    return (
      !!this.dacpacTextbox?.value &&
      this.validSqlCmdVariables() &&
      (this.locationDropdown?.value === constants.sameDatabase ||
        (this.locationDropdown?.value === constants.differentDbSameServer &&
          this.differentDatabaseSameServerRequiredFieldsFilled()) ||
        (this.locationDropdown?.value ===
          constants.differentDbDifferentServer &&
          this.differentDatabaseDifferentServerRequiredFieldsFilled()))
    );
  }
...
export function getSystemDbOptions(project: Project): string[] {
  ...
  if (
    projectTargetVersion.includes("azure") ||
    projectTargetVersion.includes("dw")
  ) {
...
export async function promptDacpacLocation(): Promise<
