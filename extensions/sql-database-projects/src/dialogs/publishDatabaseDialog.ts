...
export class PublishDatabaseDialog {
  ...
  private existingServerRadioButton:
    | azdataType.RadioButtonComponent
    | undefined;
  ...
  private sqlCmdVariablesTable:
    | azdataType.DeclarativeTableComponent
    | undefined;
  private sqlCmdVariablesFormComponentGroup:
  ...
  private serverConfigAdminPasswordTextBox:
    | azdataType.InputBoxComponent
    | undefined;
  ...
  public publish:
    | ((proj: Project, profile: ISqlProjectPublishSettings) => any)
    | undefined;
  public publishToContainer:
    | ((proj: Project, profile: IPublishToDockerSettings) => any)
    | undefined;
  public generateScript:
    | ((proj: Project, profile: ISqlProjectPublishSettings) => any)
    | undefined;
  public readPublishProfile:
    | ((profileUri: vscode.Uri) => Promise<PublishProfile>)
    | undefined;
  ...
  constructor(private project: Project) {
    this.dialog = utils
      .getAzdataApi()!
      .window.createModelViewDialog(
        constants.publishDialogName,
        "sqlProjectPublishDialog",
      );
    this.toDispose.push(
      this.dialog.onClosed((_) => this.completionPromise.resolve()),
    );
    this.publishTab = utils
      .getAzdataApi()!
      .window.createTab(constants.publishDialogName);
  }
  ...
  public openDialog(): void {
    ...
    this.toDispose.push(
      this.dialog.okButton.onClick(async () => await this.publishClick()),
    );

    this.dialog.cancelButton.label = constants.cancelButtonText;

    let generateScriptButton: azdataType.window.Button = utils
      .getAzdataApi()!
      .window.createButton(constants.generateScriptButtonText);
    this.toDispose.push(
      generateScriptButton.onClick(
        async () => await this.generateScriptClick(),
      ),
    );
    ...
    this.publishTab.registerContent(async (view) => {
      ...
      this.sqlCmdVariablesFormComponentGroup = {
        components: [
          ...
          {
            title: "",
            component: <azdataType.DeclarativeTableComponent>(
              this.sqlCmdVariablesTable
            ),
          },
        ],
      ...
      const horizontalFormSection = view.modelBuilder
        .flexContainer()
        .withLayout({ flexFlow: "column" })
        .component();
      horizontalFormSection.addItems([this.databaseRow]);

      this.formBuilder = <azdataType.FormBuilder>view.modelBuilder
        .formContainer()
        .withFormItems(
          [
            {
  ...
  public async getConnectionUri(): Promise<string> {
    try {
      ...
      if (this.connectionIsDataSource) {
        const dataSource = (
          this.dataSourcesDropDown!.value! as DataSourceDropdownValue
        ).dataSource;
        const connProfile: azdataType.IConnectionProfile =
          dataSource.getConnectionProfile();

        if (dataSource.integratedSecurity) {
          const connResult = await utils
            .getAzdataApi()!
            .connection.connect(connProfile, false, false);
          ...
          connId = (
            await utils
              .getAzdataApi()!
              .connection.openConnectionDialog(undefined, connProfile, {
      ...
      throw new Error(
        constants.unableToCreatePublishConnection +
          ": " +
  ...
  public async publishClick(): Promise<void> {
    if (this.existingServerSelected) {
      ...
      const imageInfo = getDockerBaseImage(
        this.project.getProjectTargetVersion(),
      );
  ...
  public async generateScriptClick(): Promise<void> {
    TelemetryReporter.sendActionEvent(
      ...
      TelemetryActions.generateScriptClicked,
    );
  ...
  public async getDeploymentOptions(): Promise<DeploymentOptions> {
    if (!this.deploymentOptions) {
      ...
      this.deploymentOptions = (await utils.getDefaultPublishDeploymentOptions(
        this.project,
  ...
  private createPublishTypeRadioButtons(
    view: azdataType.ModelView,
  ): azdataType.Component {
    const name = getPublishServerName(this.project.getProjectTargetVersion());
    const publishToLabel = view.modelBuilder
      .text()
      .withProps({
      ...
      .component();
    this.existingServerRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "publishType",
    ...
    this.dockerServerRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "publishType",
        label:
          name === constants.AzureSqlServerName
            ? constants.publishToDockerContainerPreview(name)
            : constants.publishToDockerContainer(name),
      })
      .component();
    ...
    const radioButtonContainer = view.modelBuilder
      .flexContainer()
      .withLayout({ flexFlow: "column" })
    ...
    let flexRadioButtonsModel: azdataType.FlexContainer = view.modelBuilder
      .flexContainer()
      .withLayout({ flexFlow: "row", alignItems: "baseline" })
      .withItems([publishToLabel, radioButtonContainer], {
        CSSStyles: { flex: "0 0 auto", "margin-right": "10px" },
      })
      .component();
  ...
  private createTargetConnectionComponent(
    view: azdataType.ModelView,
  ): azdataType.InputBoxComponent {
    this.targetConnectionTextBox = view.modelBuilder
      .inputBox()
      .withProps({
        value: "",
  ...
  private createProfileSection(
    ...
    this.loadProfileTextBox = view.modelBuilder
      .inputBox()
      .withProps({
    ...
    const buttonsList = view.modelBuilder
      .flexContainer()
      .withItems([selectProfileButton, saveProfileAsButton], {
        flex: "0 0 auto",
        CSSStyles: { "margin-right": "5px", "text-align": "justify" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
    const profileRow = view.modelBuilder
      .flexContainer()
      .withItems([this.loadProfileTextBox, buttonsList], {
        flex: "0 0 auto",
        CSSStyles: { "margin-right": "15px", "text-align": "justify" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
  ...
  private createConnectionRow(
    ...
    const selectConnectionButton: azdataType.Component =
      this.createSelectConnectionButton(view);

    const serverLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    const connectionRow = view.modelBuilder
      .flexContainer()
      .withItems([serverLabel, this.targetConnectionTextBox], {
        flex: "0 0 auto",
        CSSStyles: { margin: "-8px 10px -15px 0" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
    connectionRow.insertItem(selectConnectionButton, 2, {
      CSSStyles: { "margin-right": "0px" },
    });

    return connectionRow;
  }

  private async createLocalDbInfoRow(
    view: azdataType.ModelView,
  ): Promise<azdataType.FlexContainer> {
    const name = getPublishServerName(this.project.getProjectTargetVersion());
    this.serverPortTextBox = view.modelBuilder
      .inputBox()
      .withProps({
      ...
      .withValidation((component) =>
        utils.validateSqlServerPortNumber(component.value),
      )
      .component();
    ...
    const serverPortRow = this.createFormRow(
      view,
      constants.serverPortNumber(name),
      this.serverPortTextBox,
    );
    this.serverAdminPasswordTextBox = view.modelBuilder
      .inputBox()
      .withProps({
        value: "",
      ...
      .withValidation(
        (component) =>
          !utils.isEmptyString(component.value) &&
          utils.isValidSQLPassword(component.value || ""),
      )
      .component();

    const serverPasswordRow = this.createFormRow(
      view,
      constants.serverPassword(name),
      this.serverAdminPasswordTextBox,
    );
    this.serverConfigAdminPasswordTextBox = view.modelBuilder
      .inputBox()
      .withProps({
        value: "",
      ...
      .withValidation(
        (component) =>
          component.value === this.serverAdminPasswordTextBox?.value,
      )
      .component();
    ...
    const serverConfirmPasswordRow = this.createFormRow(
      view,
      constants.confirmServerPassword(name),
      this.serverConfigAdminPasswordTextBox,
    );

    const imageInfo = getDockerBaseImage(
      this.project.getProjectTargetVersion(),
    );
    const imageTags = await uiUtils.getImageTags(
      imageInfo!,
      this.project.getProjectTargetVersion(),
      true,
    );

    this.imageTagDropDown = view.modelBuilder
      .dropDown()
      .withProps({
        values: imageTags,
        value: imageTags[0],
    ...
    const imageTagDropDownRow = this.createFormRow(
      view,
      constants.imageTag,
      this.imageTagDropDown,
    );

    this.eulaCheckBox = view.modelBuilder
      .checkBox()
      .withProps({
    ...
    const eulaRow = view.modelBuilder
      .flexContainer()
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();

    this.localDbSection = view.modelBuilder
      .flexContainer()
      .withLayout({ flexFlow: "column" })
      .component();
    this.localDbSection.addItems([
      serverPortRow,
      serverPasswordRow,
      serverConfirmPasswordRow,
      imageTagDropDownRow,
      eulaRow,
    ]);
    ...
    if (imageInfo?.agreementInfo.link) {
      ...
      if (eulaRow && this.eulaCheckBox) {
        eulaRow?.clearItems();
        eulaRow?.addItems([this.eulaCheckBox, text], {
          flex: "0 0 auto",
  ...
  private createFormRow(
    view: azdataType.ModelView,
    label: string,
    component: azdataType.Component,
  ): azdataType.FlexContainer {
    const labelComponent = view.modelBuilder
      .text()
      .withProps({
        value: label,
        requiredIndicator: true,
    ...
    return view.modelBuilder
      .flexContainer()
      .withItems([labelComponent, component], {
        flex: "0 0 auto",
        CSSStyles: { "margin-right": "10px" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
  ...
  private createDatabaseRow(
    ...
    if (!this.existingServerSelected) {
      if (this.targetDatabaseTextBox === undefined) {
        this.targetDatabaseTextBox = view.modelBuilder
          .inputBox()
          .withProps({
      ...
      if (this.targetDatabaseDropDown === undefined) {
        this.targetDatabaseDropDown = view.modelBuilder
          .dropDown()
          .withProps({
    ...
    const databaseLabel = view.modelBuilder
      .text()
      .withProps({
    ...
    if (this.databaseRow === undefined) {
      this.databaseRow = view.modelBuilder
        .flexContainer()
        .withItems(
          [databaseLabel, <azdataType.Component>databaseComponent],
          itemLayout,
        )
        .withLayout({ flexFlow: "row", alignItems: "center" })
        .component();
  ...
  private createSqlCmdTable(
    view: azdataType.ModelView,
  ): azdataType.DeclarativeTableComponent {
    this.sqlCmdVars = this.project.sqlCmdVariables;

    const table = view.modelBuilder
      .declarativeTable()
      .withProps({
  ...
  private createRevertSqlCmdVarsButton(
    view: azdataType.ModelView,
  ): azdataType.ButtonComponent {
    let loadSqlCmdVarsButton: azdataType.ButtonComponent = view.modelBuilder
      .button()
      .withProps({
    ...
    loadSqlCmdVarsButton.onDidClick(async () => {
      ...
      await (<azdataType.DeclarativeTableComponent>(
        this.sqlCmdVariablesTable
      ))!.updateProperties({
        dataValues: data,
      });
  ...
  private createSelectConnectionButton(
    view: azdataType.ModelView,
  ): azdataType.Component {
    this.selectConnectionButton = view.modelBuilder
      .button()
      .withProps({
    ...
    this.selectConnectionButton.onDidClick(async () => {
      let connection = await utils
        .getAzdataApi()!
        .connection.openConnectionDialog(undefined, undefined, {
          saveConnection: false,
          showDashboard: false,
          showConnectionDialogOnError: true,
          showFirewallRuleOnError: true,
      ...
      await this.updateConnectionComponents(
  ...
  private async updateConnectionComponents(
    connectionTextboxValue: string,
    connectionId: string,
    database: string,
  ) {
    this.targetConnectionTextBox!.value = connectionTextboxValue;
    await this.targetConnectionTextBox!.updateProperty(
      "title",
      connectionTextboxValue,
    );
    ...
    if (connectionId) {
      const databaseValues = (
        await utils.getAzdataApi()!.connection.listDatabases(connectionId)
      )
  ...
  private createSelectProfileButton(
    view: azdataType.ModelView,
  ): azdataType.ButtonComponent {
    let loadProfileButton: azdataType.ButtonComponent = view.modelBuilder
      .button()
      .withProps({
    ...
    loadProfileButton.onDidClick(async () => {
      const fileUris = await promptForPublishProfile(
        this.project.projectFolderPath,
      );
      ...
      if (this.readPublishProfile) {
        ...
        if ((<Map<string, string>>result.sqlCmdVariables).size) {
          ...
          if (
            this.project.sqlCmdVariables.size === 0 &&
            this.sqlCmdVars?.size === 0
          ) {
        ...
        const data = this.convertSqlCmdVarsToTableFormat(
          this.getSqlCmdVariablesForPublish(),
        );
        await (<azdataType.DeclarativeTableComponent>(
          this.sqlCmdVariablesTable
        )).updateProperties({
        ...
        await this.loadProfileTextBox!.updateProperty(
          "title",
          fileUris[0].fsPath,
        );
  ...
  private createSaveProfileAsButton(
    view: azdataType.ModelView,
  ): azdataType.ButtonComponent {
    let saveProfileAsButton: azdataType.ButtonComponent = view.modelBuilder
      .button()
      .withProps({
    ...
    saveProfileAsButton.onDidClick(async () => {
      const filePath = await promptToSaveProfile(
        this.project,
        this.publishProfileUri,
      );
      ...
      if (this.savePublishProfile) {
        const targetConnectionString = this.connectionId
          ? await utils
              .getAzdataApi()!
              .connection.getConnectionString(this.connectionId, false)
          : "";
        const targetDatabaseName = this.targetDatabaseName ?? "";
        const deploymentOptions = await this.getDeploymentOptions();
        await this.savePublishProfile(
          filePath.fsPath,
          targetDatabaseName,
        ...
        TelemetryReporter.sendActionEvent(
          TelemetryViews.SqlProjectPublishDialog,
          TelemetryActions.profileSaved,
        );
      }

      this.publishProfileUri = filePath;

      await this.project.addNoneItem(
        path.relative(this.project.projectFolderPath, filePath.fsPath),
      );
      void vscode.commands.executeCommand(
        constants.refreshDataWorkspaceCommand,
      ); //refresh data workspace to load the newly added profile to the tree
    });

    return saveProfileAsButton;
  }

  private convertSqlCmdVarsToTableFormat(
    sqlCmdVars: Map<string, string>,
  ): azdataType.DeclarativeTableCellValue[][] {
    let data = [];
    ...
    if (this.existingServerRadioButton?.checked) {
      if (
        ((this.targetConnectionTextBox!.value &&
          this.targetDatabaseDropDown!.value) ||
          (this.connectionIsDataSource &&
            this.targetDatabaseDropDown!.value)) &&
      ...
      utils.validateSqlServerPortNumber(this.serverPortTextBox?.value) &&
      !utils.isEmptyString(this.serverAdminPasswordTextBox?.value) &&
      utils.isValidSQLPassword(
        this.serverAdminPasswordTextBox?.value || "",
        constants.defaultLocalServerAdminName,
      ) &&
      this.serverAdminPasswordTextBox?.value ===
        this.serverConfigAdminPasswordTextBox?.value &&
      this.imageTagDropDown!.value &&
  ...
  private allSqlCmdVariablesFilled(): boolean {
    for (let key in this.sqlCmdVars) {
      if (
        this.sqlCmdVars.get(key) === "" ||
        this.sqlCmdVars.get(key) === undefined
      ) {
        return false;
      }
    }
  ...
  private createOptionsButton(
    ...
    this.optionsButton = view.modelBuilder
      .button()
      .withProps({
    ...
    const optionsRow = view.modelBuilder
      .flexContainer()
      .withItems([this.optionsButton], {
        CSSStyles: { flex: "0 0 auto", margin: "-8px 0 0 307px" },
      })
      .withLayout({ flexFlow: "row", alignItems: "center" })
      .component();
  ...
  public async getDefaultDeploymentOptions(): Promise<DeploymentOptions> {
    const defaultDeploymentOptions =
      (await utils.getDefaultPublishDeploymentOptions(
        this.project,
      )) as DeploymentOptions;
    if (
      defaultDeploymentOptions &&
      defaultDeploymentOptions.excludeObjectTypes !== undefined
    ) {
...
export function promptForPublishProfile(
  defaultPath: string,
