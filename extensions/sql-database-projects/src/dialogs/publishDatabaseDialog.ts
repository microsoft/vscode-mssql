...
export class PublishDatabaseDialog {
    ...
    constructor(private project: Project) {
        this.dialog = utils
            .getAzdataApi()!
            .window.createModelViewDialog(constants.publishDialogName, "sqlProjectPublishDialog");
    ...
    public async getConnectionUri(): Promise<string> {
        try {
            ...
            if (this.connectionIsDataSource) {
                const dataSource = (this.dataSourcesDropDown!.value! as DataSourceDropdownValue)
                    .dataSource;
            ...
            throw new Error(
                constants.unableToCreatePublishConnection + ": " + utils.getErrorMessage(err),
            );
        }
    }
    ...
    private async createLocalDbInfoRow(
        ...
        this.serverPortTextBox = view.modelBuilder
            ...
            .withValidation((component) => utils.validateSqlServerPortNumber(component.value))
            .component();
        ...
        this.serverConfigAdminPasswordTextBox = view.modelBuilder
            ...
            .withValidation(
                (component) => component.value === this.serverAdminPasswordTextBox?.value,
            )
            .component();
    ...
    private createDatabaseRow(view: azdataType.ModelView): azdataType.FlexContainer {
        ...
        if (this.databaseRow === undefined) {
            this.databaseRow = view.modelBuilder
                .flexContainer()
                .withItems([databaseLabel, <azdataType.Component>databaseComponent], itemLayout)
                .withLayout({ flexFlow: "row", alignItems: "center" })
                .component();
        } else {
                ...
                if ((<Map<string, string>>result.sqlCmdVariables).size) {
                    ...
                    if (this.project.sqlCmdVariables.size === 0 && this.sqlCmdVars?.size === 0) {
                        this.formBuilder?.addFormItem(
                            <azdataType.FormComponentGroup>this.sqlCmdVariablesFormComponentGroup,
                        );
                    }
    ...
    private tryEnableGenerateScriptAndPublishButtons(): void {
        ...
        if (this.existingServerRadioButton?.checked) {
            if (
                ((this.targetConnectionTextBox!.value && this.targetDatabaseDropDown!.value) ||
    ...
    public async getDefaultDeploymentOptions(): Promise<DeploymentOptions> {
        const defaultDeploymentOptions = (await utils.getDefaultPublishDeploymentOptions(
            this.project,
        )) as DeploymentOptions;
