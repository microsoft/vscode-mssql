...
import {
  getSystemDbOptions,
  promptDacpacLocation,
} from "./addDatabaseReferenceDialog";
import {
  TelemetryActions,
  TelemetryReporter,
  TelemetryViews,
...
async function addProjectReference(
  ...
  const otherProjectQuickpickItems: (vscode.QuickPickItem & {
    uri: vscode.Uri;
  ...
  TelemetryReporter.createActionEvent(
    ...
    .withAdditionalProperties({
      referencedDatabaseType: constants.projectLabel,
    })
    .send();
...
async function addSystemDatabaseReference(
  ...
  TelemetryReporter.createActionEvent(
    ...
    .withAdditionalProperties({
      referencedDatabaseType: constants.systemDatabase,
    })
    .send();
...
async function addDacpacReference(
  project: Project,
): Promise<IDacpacReferenceSettings | undefined> {
  // (steps continued from addDatabaseReferenceQuickpick)
...
async function addNupkgReference(): Promise<
  INugetPackageReferenceSettings | undefined
> {
  // (steps continued from addDatabaseReferenceQuickpick)
  // 2. Prompt for location
  const location = await promptLocation();
...
async function promptSuppressUnresolvedRefErrors(): Promise<boolean> {
  const selectedOption = await vscode.window.showQuickPick(
    ...
    {
      title: constants.suppressMissingDependenciesErrors,
      ignoreFocusOut: true,
    },
  );
...
async function promptReferenceType(
  project: Project,
): Promise<SystemDbReferenceType | undefined> {
