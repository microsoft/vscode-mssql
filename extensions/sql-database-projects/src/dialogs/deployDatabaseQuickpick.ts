...
import {
  AppSettingType,
  IDeployAppIntegrationProfile,
  ISqlDbDeployProfile,
} from "../models/deploy/deployProfile";
 ...
 */
export async function launchDeployAppIntegrationQuickpick(
  project: Project,
  ...
  const localSettings = path.join(
    ...
    constants.azureFunctionLocalSettingsFileName,
  );
  ...
  if (integrateWithAzureFunctions && settingExist) {
    ...
    if ((result !== undefined && choices[result.label]) || false) {
      envVarName = await vscode.window.showInputBox({
        ...
        validateInput: (input) =>
          utils.isEmptyString(input) ? constants.valueCannotBeEmpty : undefined,
  ...
  return {
    ...
    appSettingType: settingExist
      ? AppSettingType.AzureFunction
      : AppSettingType.None,
  };
}

export async function launchCreateAzureServerQuickPick(
  project: Project,
  azureSqlClient: AzureSqlClient,
): Promise<ISqlDbDeployProfile | undefined> {
  ...
  let accountOption = await vscode.window.showQuickPick(accountOptions, {
    title: constants.azureAccounts,
    ignoreFocusOut: true,
  });
  ...
  const session = sessions.find(
    (x) => x.subscription.displayName === subscriptionName,
  );
  ...
  const resourceGroups: azure.resources.ResourceGroup[] =
    await azureSqlClient.getResourceGroups(session);
  ...
  if (resourceGroup.location) {
    const defaultLocation = locations.find(
      (x) => x.name === resourceGroup.location,
    );
    if (defaultLocation) {
  ...
  password = await vscode.window.showInputBox({
    ...
    value: password,
    validateInput: (input) =>
      !utils.isValidSQLPassword(input)
        ? constants.invalidSQLPasswordMessage(name)
        : undefined,
    password: true,
  });
  ...
  confirmPassword = await vscode.window.showInputBox({
    ...
    validateInput: (input) =>
      input !== password ? constants.passwordNotMatch(name) : undefined,
    password: true,
  });
  ...
  let settings: ISqlProjectPublishSettings | undefined =
    await getPublishDatabaseSettings(project, false);
