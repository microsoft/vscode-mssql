 ...
 */
export async function getPublishToDockerSettings(
  project: ISqlProject,
): Promise<IPublishToDockerSettings | undefined> {
  ...
  let portNumber = await vscode.window.showInputBox({
    ...
    validateInput: (input) =>
      !utils.validateSqlServerPortNumber(input)
        ? constants.portMustBeNumber
        : undefined,
  });
  ...
  let imageTagsItems: vscode.QuickPickItem[] = imageTags.map((tag) => {
    return { label: tag };
  });

  if (imageInfo.defaultTag) {
    // move the default to be the first one in the list
    const defaultIndex = imageTagsItems.findIndex(
      (i) => i.label === imageInfo.defaultTag,
    );
    if (defaultIndex > -1) {
      imageTagsItems.splice(defaultIndex, 1);
    }
    // add default next to the default value
    imageTagsItems.unshift({
      label: imageInfo.defaultTag,
      description: constants.defaultQuickPickItem,
    });
  }
  const imageTag = await vscode.window.showQuickPick(imageTagsItems, {
    title: constants.selectImageTag(name),
    ignoreFocusOut: true,
  });
...
async function launchEulaQuickPick(
  imageInfo: DockerImageInfo | undefined,
): Promise<boolean> {
        ...
        disposables.push(
          ...
          quickPick.onDidTriggerButton(async () => {
            await vscode.env.openExternal(
              vscode.Uri.parse(agreementInfo.link.url),
            );
          }),
