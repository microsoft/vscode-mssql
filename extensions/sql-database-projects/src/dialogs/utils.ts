...
export async function getImageTags(
  ...
  try {
    const imageTagsFromUrl = await HttpClient.getRequest(
      imageInfo?.tagsUrl,
      true,
    );
    if (imageTagsFromUrl?.tags) {
      ...
      (imageTagsFromUrl.tags as string[]).forEach((imageTag) => {
        const version =
          utils.findSqlVersionInImageName(imageTag) || defaultVersion;
        let tags = versionToImageTags.has(version)
          ? versionToImageTags.get(version)
          : [];
        tags = tags ?? [];
        tags = tags?.concat(imageTag);
        versionToImageTags.set(version, tags);
        maxVersionNumber =
          version && version > maxVersionNumber ? version : maxVersionNumber;
      });

      // Find the version maps to the target framework and default to max version in the tags
      const targetVersion =
        utils.findSqlVersionInTargetPlatform(
          constants.getTargetPlatformFromVersion(target),
        ) || maxVersionNumber;
      ...
      imageTags = imageTags.sort((a, b) =>
        a.indexOf(constants.dockerImageDefaultTag) > 0
          ? -1
          : a.localeCompare(b),
      );

      if (defaultTagFirst) {
        const defaultIndex = imageTags.findIndex(
          (i) => i === imageInfo.defaultTag,
        );
        if (defaultIndex > -1) {
 ...
 */
export function getDefaultDockerImageWithTag(
  ...
  if (imageInfo?.displayName === constants.SqlServerDockerImageName) {
    switch (projectTargetVersion) {
      case constants.targetPlatformToVersion.get(
        SqlTargetPlatform.sqlServer2022,
      ):
      ...
      case constants.targetPlatformToVersion.get(
        ...
        break;
      case constants.targetPlatformToVersion.get(
        SqlTargetPlatform.sqlServer2017,
      ):
...
export function populateResultWithVars(
  ...
  if (dbServerValues.dbVariable) {
    ...
    referenceSettings.serverName = ensureSetOrDefined(
    ...
    referenceSettings.databaseVariableLiteralValue = ensureSetOrDefined(
      dbServerValues.dbName,
    );
  }
}
