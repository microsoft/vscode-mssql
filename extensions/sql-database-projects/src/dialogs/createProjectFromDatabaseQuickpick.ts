...
import {
  defaultProjectNameFromDb,
  defaultProjectSaveLocation,
} from "../tools/newProjectTool";
...
export async function createNewProjectFromDatabaseWithQuickpick(
  ...
  createProjectFromDatabaseCallback?: (
    ...
    connectionInfo?: string | IConnectionInfo,
    serverName?: string,
  ) => Promise<void>,
): Promise<void> {
  ...
  let connectionProfile: IConnectionInfo | undefined =
  ...
  while (!dbs) {
    ...
    if (isValidProfile) {
      try {
        ...
        dbs = (await vscodeMssqlApi.listDatabases(connectionUri)).filter(
          (db) => !constants.systemDbs.includes(db),
  ...
  if (
    ...
    connectionProfile.database !== constants.master
  ) {
  ...
  const projectName = await vscode.window.showInputBox({
    ...
    value: defaultProjectNameFromDb(
      sanitizeStringForFilename(selectedDatabase),
    ),
    validateInput: (value) => {
  ...
  const browseProjectLocationOptions: string[] = [
  ...
  while (true) {
    ...
    const locationExists = await exists(
    ...
    browseProjectLocationTitle =
      constants.folderAlreadyExistsChooseNewLocation(projectName);
    continue;
  }
  ...
  if (createProjectFromDatabaseCallback) {
    await createProjectFromDatabaseCallback(
      model,
      connectionProfile,
