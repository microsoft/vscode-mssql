...
export class BuildHelper {
   ...
   * @param outputChannel
   */
  public async createBuildDirFolder(
    ...
    const scriptDomExists =
      await this.ensureScriptDomDllPresence(outputChannel);
  ...
  public async ensureDacFxDllsPresence(
    outputChannel: vscode.OutputChannel,
  ): Promise<boolean> {
    ...
    const microsoftBuildSqlVersionConfig = vscode.workspace.getConfiguration(
  ...
  public async ensureScriptDomDllPresence(
    outputChannel: vscode.OutputChannel,
  ): Promise<boolean> {
  ...
  public async ensureNugetAndFilesPresence(
    ...
    if (await utils.exists(fullNugetPath)) {
      ...
      for (const fileName of expectedFiles) {
        if (
          !(await utils.exists(path.join(this.extensionBuildDir, fileName)))
        ) {
          missingNuget = true;
          break;
        }
  ...
  public async downloadAndExtractNuget(
    ...
    try {
      ...
      outputChannel.appendLine(
        constants.downloadingFromTo(downloadUrl, nugetPath),
      );
      await httpClient.download(downloadUrl, nugetPath, outputChannel);
    } catch (e) {
      throw constants.errorDownloading(
        extractFolderPath,
   ...
   */
  public constructBuildArguments(
    buildDirPath: string,
