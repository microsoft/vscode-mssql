...
export class NetCoreTool extends ShellExecutionHelper {
  ...
  private netCoreInstallState: netCoreInstallState =
    netCoreInstallState.netCoreVersionSupported;
   ...
   */
  public async findOrInstallNetCore(
  ...
  private getDotnetPathIfPresent(
    folderPath: string | undefined,
  ): string | undefined {
    if (
      !isNullOrUndefined(folderPath) &&
   ...
   */
  private async isNetCoreVersionSupportedForBuild(): Promise<
    boolean | undefined
  > {
    try {
      ...
      if (isSupported) {
        ...
        this.netCoreInstallState =
          netCoreInstallState.netCoreVersionNotSupported;
      }
  ...
  public async runDotnetCommand(
    ...
    if (
      options &&
      options.commandTitle !== undefined &&
      options.commandTitle !== null
    ) {
    ...
    const dotnetPath = utils.getQuotedPath(
      path.join(this.netcoreInstallLocation, dotnet),
    );
   ...
   */
  public async verifyNetCoreInstallation(
    skipVersionSupportedCheck = false,
  ): Promise<void> {
