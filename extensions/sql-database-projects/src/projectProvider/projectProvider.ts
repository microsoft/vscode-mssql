  ...
  implements dataworkspace.IProjectProvider, sqldbproj.IExtension
{
   ...
   */
  public async getProjectTreeDataProvider(
    projectFilePath: vscode.Uri,
  ): Promise<vscode.TreeDataProvider<BaseProjectTreeItem>> {
    const provider = new SqlDatabaseProjectTreeViewProvider();
    const project = await Project.openProject(
      projectFilePath.fsPath,
      true,
      true,
    );
  ...
  public async createProject(
    ...
    targetPlatform?: sqldbproj.SqlTargetPlatform,
    sdkStyle: boolean = false,
    configureDefaultBuild: boolean = false,
  ): Promise<vscode.Uri> {
    if (!targetPlatform) {
      const projectType = this.supportedProjectTypes.find(
        (x) => x.id === projectTypeId,
      );
      if (projectType && projectType.defaultTargetPlatform) {
        targetPlatform =
          projectType.defaultTargetPlatform as sqldbproj.SqlTargetPlatform;
      }
    }
  ...
  public addItemPrompt(
    project: sqldbproj.ISqlProject,
    relativeFilePath: string,
    options?: sqldbproj.AddItemOptions,
  ): Promise<void> {
    return this.projectController.addItemPrompt(
      project,
      relativeFilePath,
      options,
    );
   ...
   */
  public get projectToolbarActions(): (
    | dataworkspace.IProjectAction
    ...
    let group: dataworkspace.IProjectActionGroup = {
   ...
   */
  public getDashboardComponents(
    projectFile: string,
  ): dataworkspace.IDashboardTable[] {
    const width = 200;
  ...
  public openSqlNewProjectDialog(
    ...
    const projectUri =
      getDataWorkspaceExtensionApi().openSpecificProjectNewProjectDialog(
        projectType,
      );
    return projectUri;
  }
   ...
   */
  public getProjectDatabaseSchemaProvider(
    projectFilePath: string,
  ): Promise<string> {
    return this.projectController.getProjectDatabaseSchemaProvider(
      projectFilePath,
    );
  }
  ...
  public cleanDockerObjectsIfNeeded(imageLabel: string): Promise<void> {
    return this.projectController.deployService.cleanDockerObjectsIfNeeded(
      imageLabel,
    );
  }
}
