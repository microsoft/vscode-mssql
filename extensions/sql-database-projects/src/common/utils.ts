...
export function ensureTrailingSlash(
  path: string,
  slashCharacter: string = constants.SqlProjPathSeparator,
): string {
...
export function systemDatabaseToString(systemDb: SystemDatabase): string {
  if (
    ...
    systemDb === vscodeMssql.SystemDatabase.Master
  ) {
...
export function getSystemDatabase(name: string): SystemDatabase {
  if (getAzdataApi()) {
    return name === constants.master
      ? mssql.SystemDatabase.Master
      : mssql.SystemDatabase.MSDB;
  } else {
    return name === constants.master
 ...
 */
export function readSqlCmdVariables(
  xmlDoc: Document,
  ...
  for (
    let i = 0;
    i <
    xmlDoc.documentElement.getElementsByTagName(constants.SqlCmdVariable)
      ?.length;
    i++
  ) {
    const sqlCmdVar = xmlDoc.documentElement.getElementsByTagName(
      constants.SqlCmdVariable,
    )[i];
    ...
    if (
      ...
      sqlCmdVariables.set(
        varName,
        sqlCmdVar.getElementsByTagName(constants.DefaultValue)[0].childNodes[0]
          .nodeValue!,
      );
    } else {
      // profile path
      sqlCmdVariables.set(
        varName,
        sqlCmdVar.getElementsByTagName(constants.Value)[0].childNodes[0]
          .nodeValue!,
      );
    }
  }
 ...
 * @param name
 */
export function removeSqlCmdVariableFormatting(
 ...
 */
export function validateSqlCmdVariableName(
  name: string | undefined,
): string | null {
 ...
 * @param folderPath
 */
export async function getSqlProjectFilesInFolder(
  folderPath: string,
): Promise<string[]> {
...
export function getDataWorkspaceExtensionApi(): dataworkspace.IExtension {
  const dataworkspaceExtName = getAzdataApi()
    ? dataworkspace.extension.name
    : dataworkspace.extension.vscodeName;
...
export type ISchemaCompareService =
  | mssql.ISchemaCompareService
  | vscodeMssql.ISchemaCompareService;
export type ISqlProjectsService =
  | mssql.ISqlProjectsService
  | vscodeMssql.ISqlProjectsService;

export async function getDacFxService(): Promise<IDacFxService> {
  if (getAzdataApi()) {
    const ext = vscode.extensions.getExtension(
      mssql.extension.name,
...
export type AzureResourceServiceFactory =
  () => Promise<vscodeMssql.IAzureResourceService>;
...
export type AzureAccountServiceFactory =
  () => Promise<vscodeMssql.IAzureAccountService>;
 ...
 */
export async function getDefaultPublishDeploymentOptions(
  project: ISqlProject,
): Promise<mssql.DeploymentOptions | vscodeMssql.DeploymentOptions> {
...
export async function retry<T>(
  ...
  for (let count = 0; count < numberOfAttempts; count++) {
    ...
    outputChannel.appendLine(
      constants.retryRunMessage(count, numberOfAttempts, name),
    );
    ...
    try {
      ...
      if (validationResult.validated) {
        outputChannel.appendLine(
          constants.retrySucceedMessage(name, formattedResult),
        );
        return result;
      } else {
        outputChannel.appendLine(
          constants.retryFailedMessage(
            name,
            formattedResult,
      ...
      outputChannel.appendLine(
        constants.retryMessage(name, getErrorMessage(err)),
      );
    }
  }
 ...
 */
export async function detectCommandInstallation(
  command: string,
): Promise<boolean> {
  try {
...
export function isValidSQLPassword(
  password: string,
  userName: string = "sa",
): boolean {
...
export async function showErrorMessageWithOutputChannel(
  errorMessageFunc: (error: string) => string,
  error: any,
  outputChannel: vscode.OutputChannel,
): Promise<void> {
...
export async function showInfoMessageWithOutputChannel(
  message: string,
  outputChannel: vscode.OutputChannel,
): Promise<void> {
 ...
 */
export async function getSqlFilesInFolder(
  folderPath: string,
...
export async function getFoldersInFolder(
  ...
  if (ignoreBinObj) {
    ...
    return await glob(folderFilter, {
      onlyDirectories: true,
      ignore: [binIgnore, objIgnore],
    });
  } else {
 ...
 */
export function getFoldersToFile(
  startFolder: string,
  endFile: string,
): string[] {
  let folders: string[] = [];
  ...
  const relativePath = convertSlashesForSqlProj(
    endFolderPath.substring(startFolder.length),
  );
  const pathSegments = trimChars(relativePath, " \\").split(
    constants.SqlProjPathSeparator,
  );
  let folderPath =
    convertSlashesForSqlProj(startFolder) + constants.SqlProjPathSeparator;
 ...
 */
export function getFoldersAlongPath(
  startFolder: string,
  endFolder: string,
): string[] {
  let folders: string[] = [];
  ...
  const relativePath = convertSlashesForSqlProj(
    endFolder.substring(startFolder.length),
  );
  const pathSegments = trimChars(relativePath, " \\").split(
    constants.SqlProjPathSeparator,
  );
 ...
 */
export function findSqlVersionInImageName(
  imageName: string,
): number | undefined {
 ...
 * @returns SQL server version
 */
export function findSqlVersionInTargetPlatform(
...
export function throwIfNotConnected(
  connectionResult: azdataType.ConnectionResult,
): void {
  if (!connectionResult.connected) {
    throw new Error(
      `${connectionResult.errorMessage} (${connectionResult.errorCode})`,
    );
  }
}
 ...
 */
export async function fileContainsCreateTableStatement(
  fullPath: string,
  ...
  if (getAzdataApi() && (await exists(fullPath))) {
    ...
    try {
      const result = await dacFxService.parseTSqlScript(
        fullPath,
 ...
 */
export async function getTargetPlatformFromServerVersion(
  serverInfo: azdataType.ServerInfo | vscodeMssql.IServerInfo,
  serverUrl?: string,
): Promise<SqlTargetPlatform | undefined> {
  ...
  if (isCloud) {
    ...
    if (azdataApi) {
      if (isSqlDwUnifiedServer(serverUrl)) {
        ...
        engineEdition === azdataApi.DatabaseEngineEdition.SqlOnDemand
      ) {
        targetPlatform = SqlTargetPlatform.sqlDwServerless;
      } else if (
        engineEdition === azdataApi.DatabaseEngineEdition.SqlDbFabric
      ) {
        targetPlatform = SqlTargetPlatform.sqlDbFabric;
      } else if (
        engineEdition === azdataApi.DatabaseEngineEdition.SqlDataWarehouse
      ) {
      ...
      if (isSqlDwUnifiedServer(serverUrl)) {
        ...
        engineEdition === vscodeMssql.DatabaseEngineEdition.SqlOnDemand
      ) {
        targetPlatform = SqlTargetPlatform.sqlDwServerless;
      } else if (
        engineEdition === vscodeMssql.DatabaseEngineEdition.SqlDbFabric
      ) {
        targetPlatform = SqlTargetPlatform.sqlDbFabric;
      } else if (
        engineEdition === vscodeMssql.DatabaseEngineEdition.SqlDataWarehouse
      ) {
        targetPlatform = SqlTargetPlatform.sqlDW;
      } else {
        targetPlatform = SqlTargetPlatform.sqlAzure;
    ...
    serverInfo.engineEditionId ===
      vscodeMssql.DatabaseEngineEdition.SqlDbFabric ||
    serverInfo.engineEditionId ===
      getAzdataApi()?.DatabaseEngineEdition.SqlDbFabric
  ) {
...
export function isSqlDwUnifiedServer(server?: string): boolean | undefined {
  ...
  return (
    serverLowerCase?.includes("datawarehouse.pbidedicated.windows.net") ||
  ...
  if (contents) {
    ...
    try {
      ...
      if (error.code === "EEXIST") {
        ...
        throw new Error(
          constants.fileAlreadyExists(path.parse(absoluteFilePath).name),
        );
      }
...
export function throwIfFailed(
  result: azdataType.ResultStatus | vscodeMssql.ResultStatus,
): void {
  if (!result.success) {
