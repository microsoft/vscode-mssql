...
export function readSqlCmdVariables(
    ...
    for (
        let i = 0;
        i < xmlDoc.documentElement.getElementsByTagName(constants.SqlCmdVariable)?.length;
        i++
    ) {
        ...
        if (
            ...
            sqlCmdVariables.set(
                varName,
                sqlCmdVar.getElementsByTagName(constants.DefaultValue)[0].childNodes[0].nodeValue!,
            );
        } else {
            // profile path
            sqlCmdVariables.set(
                varName,
                sqlCmdVar.getElementsByTagName(constants.Value)[0].childNodes[0].nodeValue!,
            );
        }
    }
...
export async function getDefaultPublishDeploymentOptions(
    ...
    const result = await (dacFxService as vscodeMssql.IDacFxService).getDeploymentOptions(
        DeploymentScenario.Deployment as unknown as vscodeMssql.DeploymentScenario,
    );
...
export async function getTargetPlatformFromServerVersion(
    ...
    if (isCloud) {
        ...
        serverInfo.engineEditionId === vscodeMssql.DatabaseEngineEdition.SqlDbFabric ||
        serverInfo.engineEditionId === getAzdataApi()?.DatabaseEngineEdition.SqlDbFabric
    ) {
