...
export const postDeploymentScriptName = "PostDeploymentScript.sql";
...
export const newSqlProjectName = l10n.t("New SQL project name");
export function generatingProjectFromSpec(specName: string) {
    return l10n.t(
...
export function generatingProjectFailed(errorMessage: string) {
    return l10n.t(
        "Generating SQL project from OpenAPI spec failed. Check output pane for more details. Error: {0}",
        errorMessage,
    );
}
export const noSqlFilesGenerated = l10n.t(
    "No .sql files were generated from the spec. Please confirm that your spec contains schema definitions, or check the output log for details.",
);
export function multipleMostDeploymentScripts(count: number) {
    return l10n.t("Unexpected number of {0} files: {1}", postDeploymentScriptName, count);
}
//#endregion
