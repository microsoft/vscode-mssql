...
import {
  readPublishProfile,
  promptForSavingProfile,
  savePublishProfile,
} from "../models/publishProfile/publishProfile";
...
import {
  ...
  PublishData,
  Status,
} from "../models/dashboardData/dashboardData";
import {
  getPublishDatabaseSettings,
  launchPublishTargetOption,
} from "../dialogs/publishDatabaseQuickpick";
...
} from "../models/projectEntry";
import {
  UpdateProjectAction,
  UpdateProjectDataModel,
} from "../models/api/updateProject";
...
import {
  ...
  ISqlProjectPublishSettings,
} from "../models/deploy/publishSettings";
...
export class ProjectsController {
  ...
  //#region Dashboard
  public getDashboardPublishData(
    projectFile: string,
  ): (string | dataworkspace.IconCellValue)[][] {
    ...
    for (let i = this.publishInfo.length - 1; i >= 0; i--) {
      if (this.publishInfo[i].projectFile === projectFile) {
        ...
        let infoRow: (string | dataworkspace.IconCellValue)[] = [
          { text: text, icon: icon },
  ...
  public async createNewProject(
    creationParams: NewProjectParams,
  ): Promise<string> {
    ...
    if (
      ...
      !UUID.isUUID(creationParams.projectGuid)
    ) {
    ...
    if (await utils.exists(newProjFilePath)) {
      throw new Error(
        constants.projectAlreadyExists(
          newProjFileName,
          path.parse(newProjFilePath).dir,
        ),
      );
    }
    ...
    if (utils.getAzdataApi()) {
      ...
      result = await (
        sqlProjectsService as mssql.ISqlProjectsService
      ).createProject(
        newProjFilePath,
        projectStyle,
        targetPlatform,
      ...
      result = await (
        sqlProjectsService as mssqlVscode.ISqlProjectsService
      ).createProject(
        newProjFilePath,
        projectStyle,
        targetPlatform,
  ...
  private async addTasksJsonFile(
    ...
    await this.addFileToProjectFromTemplate(
      project,
      templates.get(ItemType.tasks),
      ".vscode/tasks.json",
      new Map([["ConfigureDefaultBuild", configureDefaultBuild.toString()]]),
    );
  }
  ...
  private async addFileToProjectFromTemplate(
    ...
    const newFileText = templates.macroExpansion(
      itemType.templateScript,
      expansionMacros,
    );
  ...
  public async buildProject(
    ...
    let buildInfoNew = new DashboardData(
      project.projectFilePath,
      Status.inProgress,
      project.getProjectTargetVersion(),
      currentBuildTimeInfo,
    );
    ...
    if (project.sqlProjStyle === mssql.ProjectType.LegacyStyle) {
      const result = await this.buildHelper.createBuildDirFolder(
        this._outputChannel,
      );

      if (!result) {
        void vscode.window.showErrorMessage(
          constants.errorRetrievingBuildFiles,
        );
        return "";
      }
    }
    ...
    try {
      const crossPlatCompatible: boolean =
        await Project.checkPromptCrossPlatStatus(
          project,
    ...
    try {
      ...
      const currentBuildIndex = this.buildInfo.findIndex(
        (b) => b.startDate === currentBuildTimeInfo,
      );
      this.buildInfo[currentBuildIndex].status = Status.success;
      this.buildInfo[currentBuildIndex].timeToCompleteAction =
        utils.timeConversion(timeToBuild);
      ...
      this.buildInfo[currentBuildIndex].timeToCompleteAction =
        utils.timeConversion(timeToFailureBuild);
  ...
  private async createVsCodeTask(
    ...
    vscodeTask = new vscode.Task(
      ...
      taskDefinition.type,
      new vscode.ShellExecution(taskDefinition.command, args, {
   ...
   */
  public async publishToNewAzureServer(
    context: Project | dataworkspace.WorkspaceTreeItem,
    deployProfile: ISqlDbDeployProfile,
  ): Promise<void> {
    try {
      TelemetryReporter.sendActionEvent(
        TelemetryViews.ProjectController,
        TelemetryActions.publishToNewAzureServer,
      );
      const project: Project = await this.getProjectFromContext(context);
      if (deployProfile?.deploySettings && deployProfile?.sqlDbSetting) {
        void utils.showInfoMessageWithOutputChannel(
          constants.creatingAzureSqlServer(
            deployProfile?.sqlDbSetting?.serverName,
          ),
          this._outputChannel,
        );
        const connectionUri =
          await this.deployService.createNewAzureSqlServer(deployProfile);
        if (connectionUri) {
          deployProfile.deploySettings.connectionUri = connectionUri;
          const publishResult = await this.publishOrScriptProject(
            project,
            deployProfile.deploySettings,
            true,
          );
          if (publishResult && publishResult.success) {
            if (deployProfile.sqlDbSetting) {
              ...
              await this.connectionService.getConnection(
                deployProfile.sqlDbSetting,
                true,
                deployProfile.sqlDbSetting.dbName,
              );
            }
            void vscode.window.showInformationMessage(
              constants.publishProjectSucceed,
            );
          } else {
            void utils.showErrorMessageWithOutputChannel(
              constants.publishToNewAzureServerFailed,
          ...
          void utils.showErrorMessageWithOutputChannel(
            constants.publishToNewAzureServerFailed,
            constants.deployProjectFailedMessage,
            this._outputChannel,
          );
        }
      }
   ...
   * @param deployProfile
   */
  public async publishToDockerContainer(
    context: Project | dataworkspace.WorkspaceTreeItem,
    deployProfile: IPublishToDockerSettings,
  ): Promise<void> {
    const project: Project = await this.getProjectFromContext(context);
    ...
    const dockerImageNameForTelemetry =
      deployProfile.dockerSettings.dockerBaseImage.replace(/\//gi, "_");
    try {
      TelemetryReporter.createActionEvent(
        TelemetryViews.ProjectController,
        TelemetryActions.publishToContainer,
      )
        .withAdditionalProperties({
          dockerBaseImage: dockerImageNameForTelemetry,
        })
        .send();

      void utils.showInfoMessageWithOutputChannel(
        constants.publishingProjectMessage,
        this._outputChannel,
      );
      const connectionUri = await this.deployService.deployToContainer(
        deployProfile,
        project,
      );
      if (connectionUri) {
      ...
      if (deployProfile.sqlProjectPublishSettings.connectionUri) {
        const publishResult = await this.publishOrScriptProject(
          project,
          deployProfile.sqlProjectPublishSettings,
          true,
        );
        if (publishResult && publishResult.success) {
          await this.connectionService.getConnection(
            deployProfile.dockerSettings,
            true,
            deployProfile.dockerSettings.dbName,
          );
          void vscode.window.showInformationMessage(
            constants.publishProjectSucceed,
          );
          ...
          void utils.showErrorMessageWithOutputChannel(
            constants.publishToContainerFailed,
            publishResult?.errorMessage || "",
            this._outputChannel,
          );
        }
  ...
  public async publishProject(
    ...
    if (utils.getAzdataApi()) {
      ...
      publishDatabaseDialog.publish = async (proj, prof) =>
        this.publishOrScriptProject(proj, prof, true);
      publishDatabaseDialog.publishToContainer = async (proj, prof) =>
        this.publishToDockerContainer(proj, prof);
      publishDatabaseDialog.generateScript = async (proj, prof) =>
        this.publishOrScriptProject(proj, prof, false);
      publishDatabaseDialog.readPublishProfile = async (profileUri) =>
        readPublishProfile(profileUri);
      publishDatabaseDialog.savePublishProfile = async (
        profilePath,
        databaseName,
        connectionString,
      ...
      const shouldUsePreview =
        vscode.workspace
          ...
          .get<boolean>(constants.enablePreviewFeaturesKey) ||
        vscode.workspace
          .getConfiguration(constants.mssqlConfigSectionKey)
          .get<boolean>(constants.mssqlEnableExperimentalFeaturesKey);
  ...
  private async publishDatabase(project: Project): Promise<void> {
    ...
    if (publishTarget === constants.PublishTargetType.docker) {
      ...
      try {
        const settings = await launchCreateAzureServerQuickPick(
          project,
          this.azureSqlClient,
        );
      ...
      let settings: ISqlProjectPublishSettings | undefined =
        await getPublishDatabaseSettings(project);
      ...
      if (settings) {
        ...
        await this.publishOrScriptProject(
          project,
          settings,
   ...
   */
  public async publishOrScriptProject(
    project: Project,
    settings: ISqlProjectPublishSettings,
    publish: boolean,
  ): Promise<mssql.DacFxResult | undefined> {
    ...
    const tempDir = path.join(
      os.tmpdir(),
      `${path.parse(dacpacPath).name}_${new Date().getTime()}`,
    );
    ...
    telemetryProps.profileUsed = (
      settings.publishProfileUri !== undefined ? true : false
    ).toString();
    ...
    let publishInfoNew = new PublishData(
      project.projectFilePath,
      Status.inProgress,
      project.getProjectTargetVersion(),
      currentPublishTimeInfo,
    ...
    try {
      ...
      if (publish) {
        ...
        if (azdataApi) {
          result = await (dacFxService as mssql.IDacFxService).deployDacpac(
            tempDacpacPath,
            settings.databaseName,
            true,
            settings.connectionUri,
            azdataApi.TaskExecutionMode.execute,
            settings.sqlCmdVariables,
            settings.deploymentOptions as mssql.DeploymentOptions,
          );
        } else {
          ...
          result = await (
            ...
            settings.sqlCmdVariables,
            settings.deploymentOptions as mssqlVscode.DeploymentOptions,
          );
        }
      } else {
        ...
        if (azdataApi) {
          result = await (
            dacFxService as mssql.IDacFxService
          ).generateDeployScript(
            tempDacpacPath,
            settings.databaseName,
            settings.connectionUri,
            azdataApi.TaskExecutionMode.script,
            settings.sqlCmdVariables,
            settings.deploymentOptions as mssql.DeploymentOptions,
          );
        } else {
          ...
          result = await (
            ...
            settings.databaseName,
            settings.connectionUri,
            TaskExecutionMode.script as unknown as mssqlVscode.TaskExecutionMode,
            settings.sqlCmdVariables,
            settings.deploymentOptions as mssqlVscode.DeploymentOptions,
          );
        }
      ...
      telemetryProps.totalDuration = (
        actionEndTime - buildStartTime
      ).toString();
      ...
      const currentPublishIndex = this.publishInfo.findIndex(
        (d) => d.startDate === currentPublishTimeInfo,
      );
      this.publishInfo[currentPublishIndex].status = Status.failed;
      this.publishInfo[currentPublishIndex].timeToCompleteAction =
        utils.timeConversion(timeToFailurePublish);
      throw err;
    }
    const actionEndTime = new Date().getTime();
    ...
    telemetryProps.sqlcmdVariablesCount =
      project.sqlCmdVariables.size.toString();
    telemetryProps.projectTargetPLatform = project.getProjectTargetVersion();

    const currentPublishIndex = this.publishInfo.findIndex(
      (d) => d.startDate === currentPublishTimeInfo,
    );
    this.publishInfo[currentPublishIndex].status = result.success
      ? Status.success
      : Status.failed;
    this.publishInfo[currentPublishIndex].timeToCompleteAction =
      utils.timeConversion(timeToPublish);
  ...
  public async schemaCompare(
    source: dataworkspace.WorkspaceTreeItem | azdataType.IConnectionProfile,
    ...
    try {
      ...
      if (service) {
        ...
        if (source as dataworkspace.WorkspaceTreeItem) {
          sourceParam = (
            await this.getProjectFromContext(
              source as dataworkspace.WorkspaceTreeItem,
            )
          ).projectFilePath;
        } else {
        ...
        try {
          ...
          if (utils.getAzdataApi()) {
            ...
            await vscode.commands.executeCommand(
              constants.schemaCompareStartCommand,
              sourceParam,
              targetParam,
              undefined,
            );
  ...
  public async addFolderPrompt(
    ...
    const relativePathToParent = this.getRelativePath(
      projectRelativeUri,
      treeNode.element,
    );
    const absolutePathToParent = path.join(
      project.projectFolderPath,
      relativePathToParent,
    );
    const newFolderName = await this.promptForNewObjectName(
      new templates.ProjectScriptType(
        ItemType.folder,
        constants.folderFriendlyName,
        "",
      ),
      project,
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          constants.folderAlreadyExists(path.parse(absoluteFolderPath).name),
        );
      }
  ...
  private async promptForNewObjectName(
    ...
    do {
      ...
      (await utils.exists(
        path.join(
          folderPath,
          `${suggestedName}${counter}${fileExtension ?? ""}`,
        ),
      ))
    );
  ...
  public isReservedFolder(
    ...
    const sameLocation =
      path.parse(absoluteFolderPath).dir === projectFolderPath;
  ...
  public async addItemPrompt(
    ...
    if (itemType.type === ItemType.preDeployScript) {
      telemetryMeasurements.numPredeployScripts =
        project.preDeployScripts.length;
    } else if (itemType.type === ItemType.postDeployScript) {
      telemetryMeasurements.numPostdeployScripts =
  ...
  public async addExistingItemPrompt(
    treeNode: dataworkspace.WorkspaceTreeItem,
  ): Promise<void> {
    const project = await this.getProjectFromContext(treeNode);
  ...
  public async delete(context: dataworkspace.WorkspaceTreeItem): Promise<void> {
    ...
    if (node instanceof DatabaseReferenceTreeItem) {
      confirmationPrompt = constants.deleteReferenceConfirmation(
        node.friendlyName,
      );
    } else if (node instanceof SqlCmdVariableTreeItem) {
      confirmationPrompt = constants.deleteSqlCmdVariableConfirmation(
        node.friendlyName,
      );
    } else if (node instanceof FolderNode) {
      confirmationPrompt = constants.deleteConfirmationContents(
        node.friendlyName,
      );
    } else {
  ...
  public async rename(context: dataworkspace.WorkspaceTreeItem): Promise<void> {
    ...
    const originalAbsolutePath = utils.getPlatformSafeFileEntryPath(
      node.projectFileUri.fsPath,
    );
    ...
    const newFilePath = path.join(
      path.dirname(
        utils.getPlatformSafeFileEntryPath(node.relativeProjectUri.fsPath!),
      ),
      newFileName,
    );
   ...
   * @param context
   */
  public async editSqlCmdVariable(
    context: dataworkspace.WorkspaceTreeItem,
  ): Promise<void> {
   ...
   */
  public async openContainingFolder(
    context: dataworkspace.WorkspaceTreeItem,
  ): Promise<void> {
    const project = await this.getProjectFromContext(context);
    await vscode.commands.executeCommand(
   ...
   */
  public async openReferencedSqlProject(
    context: dataworkspace.WorkspaceTreeItem,
  ): Promise<void> {
    const node = context.element as BaseProjectTreeItem;
    const project = await this.getProjectFromContext(node);
   ...
   */
  public async openFileWithWatcher(
    ...
    const initiallyContainsCreateTableStatement =
    ...
    const fileWatcher: vscode.FileSystemWatcher =
      vscode.workspace.createFileSystemWatcher(fileSystemUri.fsPath);
    this.fileWatchers.set(fileSystemUri.fsPath, {
      fileWatcher: fileWatcher,
      containsCreateTableStatement: initiallyContainsCreateTableStatement,
    });

    fileWatcher.onDidChange(async (uri: vscode.Uri) => {
      const afterContainsCreateTableStatement =
        await utils.fileContainsCreateTableStatement(
          fileSystemUri.fsPath,
          projectTargetVersion,
        );
      const previousStatus = this.fileWatchers.get(
        uri.fsPath,
      ...
      if (previousStatus !== afterContainsCreateTableStatement) {
        ...
        this.fileWatchers.get(uri.fsPath)!.containsCreateTableStatement =
      ...
      await vscode.window.showQuickPick(
        ...
        }),
        {
          ...
          placeHolder: constants.selectTargetPlatform(
            constants.getTargetPlatformFromVersion(
              project.getProjectTargetVersion(),
            ),
          ),
        },
  ...
  public async addDatabaseReference(
    context: Project | dataworkspace.WorkspaceTreeItem,
  ): Promise<AddDatabaseReferenceDialog | undefined> {
    const project = await this.getProjectFromContext(context);

    if (utils.getAzdataApi()) {
      const addDatabaseReferenceDialog =
        this.getAddDatabaseReferenceDialog(project);
      addDatabaseReferenceDialog.addReference = async (proj, settings) =>
  ...
  public getAddDatabaseReferenceDialog(
    project: Project,
      ...
      if ((<IProjectReferenceSettings>settings).projectName !== undefined) {
        ...
        const referencedProject = await Project.openProject(
          workspaceProjects.filter(
            (p) =>
              path.parse(p.fsPath).name ===
              projectReferenceSettings.projectName,
          )[0].fsPath,
        );
        ...
        projectReferenceSettings.projectRelativePath =
        ...
        for (let r of projectReferences) {
          if (
            (<SqlProjectReferenceProjectEntry>r).projectName ===
            project.projectFileName
          ) {
            void vscode.window.showErrorMessage(
        ...
        (<ISystemDatabaseReferenceSettings>settings).systemDb !== undefined
      ) {
        await project.addSystemDatabaseReference(
          <ISystemDatabaseReferenceSettings>settings,
        );
      } else if (
        (<IDacpacReferenceSettings>settings).dacpacFileLocation !== undefined
      ) {
        ...
        await project.addNugetPackageReference(
          <INugetPackageReferenceSettings>settings,
        );
      }
  ...
  public async validateExternalStreamingJob(
  ...
  ): Promise<mssql.ValidateStreamingJobResult> {
    ...
    const result: mssql.ValidateStreamingJobResult =
      await dacFxService.validateStreamingJob(
        dacpacPath,
        streamingJobDefinition,
      );
  ...
  public async selectAutorestProjectLocation(
    ...
    | { newProjectFolder: string; outputFolder: string; projectName: string }
    | undefined
  > {
    ...
    while (true) {
      ...
      if (newProjectFolder && (await utils.exists(newProjectFolder))) {
        ...
        quickPickTitle =
          constants.folderAlreadyExistsChooseNewLocation(newProjectFolder);
      ...
      const folders = await vscode.window.showOpenDialog({
        ...
        openLabel: constants.selectString,
        defaultUri:
          defaultOutputLocation ?? vscode.workspace.workspaceFolders?.[0]?.uri,
  ...
  public async generateAutorestFiles(
    ...
    return vscode.window.withProgress(
      ...
      async (_progress, _token) => {
        return this.autorestHelper.generateAutorestFiles(
          specPath,
          newProjectFolder,
        );
      },
  ...
  public async openProjectInWorkspace(projectFilePath: string): Promise<void> {
    ...
    await workspaceApi.addProjectsToWorkspace([
  ...
  public async promptForAutorestProjectName(
    defaultName?: string,
  ): Promise<string | undefined> {
  ...
  public async generateProjectFromOpenApiSpec(
    ...
    try {
      ...
      const specPath: string | undefined =
        options?.openApiSpecFile?.fsPath ||
        (await this.selectAutorestSpecFile());
      if (!specPath) {
        return;
      }
      ...
      const result = await this.generateAutorestFiles(
        specPath,
  ...
  private findPostDeploymentScript(
    files: vscode.Uri[],
    ...
    switch (results.length) {
      ...
      default:
        throw new Error(
          constants.multipleMostDeploymentScripts(results.length),
        );
    }
  }

  private async getSqlFileList(
    folder: string,
    ...
    const files = entries
      .filter(
        (file) =>
          !file.isDirectory() &&
          path.extname(file.name) === constants.sqlFileExtension,
      )
  ...
  private async getProjectFromContext(
    ...
    if (context instanceof BaseProjectTreeItem) {
      ...
      throw new Error(
        constants.unexpectedProjectContext(JSON.stringify(context)),
      );
    }
  }
  ...
  private getConnectionProfileFromContext(
    context:
      ...
      | mssqlVscode.ITreeNodeInfo
      | undefined,
  ): azdataType.IConnectionProfile | mssqlVscode.IConnectionInfo | undefined {
    if (!context) {
      return undefined;
    }
  ...
  private refreshProjectsTree(
    workspaceTreeItem: dataworkspace.WorkspaceTreeItem,
  ): void {
    (
  ...
  public async createProjectFromDatabase(
    context:
      ...
      | mssqlVscode.ITreeNodeInfo
      | undefined,
  ): Promise<CreateProjectFromDatabaseDialog | undefined> {
    const profile = this.getConnectionProfileFromContext(context);
    if (utils.getAzdataApi()) {
      let createProjectFromDatabaseDialog =
        this.getCreateProjectFromDatabaseDialog(
          profile as azdataType.IConnectionProfile,
        );

      createProjectFromDatabaseDialog.createProjectFromDatabaseCallback =
        async (model, connectionId) =>
          await this.createProjectFromDatabaseCallback(
            model,
            connectionId,
            (profile as azdataType.IConnectionProfile)?.serverName,
          );
      ...
      if (context) {
        ...
        const databaseName = (
          await utils.getVscodeMssqlApi()
        ).getDatabaseNameFromTreeNode(treeNodeContext);
  ...
  public getCreateProjectFromDatabaseDialog(
    profile: azdataType.IConnectionProfile | undefined,
  ): CreateProjectFromDatabaseDialog {
    ...
    try {
      ...
      if (connectionInfo) {
        if (typeof connectionInfo === "string") {
          serverInfo = await utils
            .getAzdataApi()!
            .connection.getServerInfo(connectionInfo);
        } else {
          serverInfo = (await utils.getVscodeMssqlApi()).getServerInfo(
            connectionInfo,
          );
        }
      }
      ...
      const scriptList: vscode.Uri[] =
        model.extractTarget === mssql.ExtractTarget.file
      ...
      workspaceApi.showProjectsView();
      await workspaceApi.addProjectsToWorkspace([
        vscode.Uri.file(newProjFilePath),
      ]);
    } catch (err) {
      void vscode.window.showErrorMessage(utils.getErrorMessage(err));
      TelemetryReporter.sendErrorEvent2(
  ...
  public async createProjectFromDatabaseApiCall(
    model: ImportDataModel,
  ): Promise<void> {
    ...
    if (azdataApi) {
      await (service as mssql.IDacFxService).createProjectFromDatabase(
        model.database,
        model.filePath,
        model.projName,
        model.version,
        model.connectionUri,
        model.extractTarget as mssql.ExtractTarget,
        azdataApi.TaskExecutionMode.execute,
  ...
  public async generateScriptList(absolutePath: string): Promise<vscode.Uri[]> {
    ...
    if (!(await utils.exists(absolutePath))) {
      if (await utils.exists(absolutePath + constants.sqlFileExtension)) {
        ...
        void vscode.window.showErrorMessage(
          constants.cannotResolvePath(absolutePath),
        );
        return fileList;
      }
    }
    ...
    do {
      ...
      if (filepath) {
        ...
        if (stat.isDirectory()) {
          ...
          stat.isFile() &&
          path.extname(filepath) === constants.sqlFileExtension
        ) {
  ...
  public async updateProjectFromDatabase(
    context:
      | azdataType.IConnectionProfile
      | mssqlVscode.ITreeNodeInfo
      | dataworkspace.WorkspaceTreeItem,
  ): Promise<UpdateProjectFromDatabaseDialog | undefined> {
    let connection:
      | azdataType.IConnectionProfile
      | mssqlVscode.IConnectionInfo
      | undefined;
    ...
    try {
      if ("connectionProfile" in context) {
        connection = this.getConnectionProfileFromContext(
          context as azdataType.IConnectionProfile | mssqlVscode.ITreeNodeInfo,
        );
      }
    } catch {}
    ...
    try {
      if ("treeDataProvider" in context) {
        project = await this.getProjectFromContext(
          context as dataworkspace.WorkspaceTreeItem,
        );
      }
    } catch {}
    ...
    const workspaceProjects = await utils.getSqlProjectsInWorkspace();
    if (utils.getAzdataApi()) {
      const updateProjectFromDatabaseDialog =
        this.getUpdateProjectFromDatabaseDialog(
          connection,
          project,
          workspaceProjects,
        );
      updateProjectFromDatabaseDialog.updateProjectFromDatabaseCallback =
        async (model) => await this.updateProjectFromDatabaseCallback(model);
      ...
      await UpdateProjectFromDatabaseWithQuickpick(
        ...
        (model: UpdateProjectDataModel) =>
          this.updateProjectFromDatabaseCallback(model),
      );
      return undefined;
    }
  }

  public getUpdateProjectFromDatabaseDialog(
    connection:
      | azdataType.IConnectionProfile
      | mssqlVscode.IConnectionInfo
      | undefined,
    project: Project | undefined,
    workspaceProjects: vscode.Uri[],
  ): UpdateProjectFromDatabaseDialog {
    return new UpdateProjectFromDatabaseDialog(
      connection,
      project,
      workspaceProjects,
    );
  ...
  public async updateProjectFromDatabaseCallback(
    model: UpdateProjectDataModel,
  ) {
    try {
      const startTime = new Date();
   ...
   */
  public async updateProjectFromDatabaseApiCall(
    model: UpdateProjectDataModel,
  ): Promise<void> {
    if (model.action === UpdateProjectAction.Compare) {
      if (utils.getAzdataApi()) {
        // ADS environment
        await vscode.commands.executeCommand(
          constants.schemaCompareRunComparisonCommand,
        ...
        .then(async (result) => {
          if (result === constants.yesString) {
            ...
            void vscode.commands.executeCommand(
              constants.refreshDataWorkspaceCommand,
            );
  ...
  private async schemaCompareAndUpdateProject(
    source:
      ...
      | mssqlVscode.SchemaCompareEndpointInfo,
    target:
      | mssql.SchemaCompareEndpointInfo
      | mssqlVscode.SchemaCompareEndpointInfo,
  ): Promise<void> {
    ...
    target.targetScripts = await this.getProjectScriptFiles(
    ...
    let comparisonResult:
      ...
      | mssqlVscode.SchemaCompareResult;

    if (utils.getAzdataApi()) {
      // Azure Data Studio environment
      comparisonResult = await (
        service as mssql.ISchemaCompareService
      ).schemaCompare(
        operationId,
        source as mssql.SchemaCompareEndpointInfo,
        target as mssql.SchemaCompareEndpointInfo,
        utils.getAzdataApi()!.TaskExecutionMode.execute,
        deploymentOptions.defaultDeploymentOptions,
      );
    } else {
      // VS Code environment
      comparisonResult = await (
        service as mssqlVscode.ISchemaCompareService
      ).compare(
        operationId,
    ...
    const publishResult = await this.schemaComparePublishProjectChanges(
      operationId,
      target.projectFilePath,
      target.extractTarget as mssql.ExtractTarget,
    );
    ...
    if (publishResult.success) {
      ...
      void vscode.window.showErrorMessage(
        constants.applyError(publishResult.errorMessage),
      );
    }
  }

  public async getProjectScriptFiles(
  ...
  public async getProjectDatabaseSchemaProvider(
    projectFilePath: string,
  ): Promise<string> {
    const project = await Project.openProject(projectFilePath);
  ...
  public async schemaComparePublishProjectChanges(
    operationId: string,
    projectFilePath: string,
    folderStructure: mssql.ExtractTarget | mssqlVscode.ExtractTarget,
    ...
    if (utils.getAzdataApi()) {
      ...
      result = await (
        ...
        operationId,
        projectPath,
        folderStructure as mssql.ExtractTarget,
        utils.getAzdataApi()!.TaskExecutionMode.execute,
      );
    } else {
      // VS Code environment
    ...
    if (!result.errorMessage) {
      ...
      result.deletedFiles.forEach((f: any) =>
        toRemove.push(vscode.Uri.file(f)),
      );
  ...
  public async moveFile(
    ...
    if (
      !sourceFileNode ||
      !(
        sourceFileNode instanceof FileNode ||
        sourceFileNode instanceof FolderNode
      )
    ) {
      void vscode.window.showErrorMessage(
        constants.onlyMoveFilesFoldersSupported,
      );
      return;
    }
    ...
    if (projectUri.fsPath !== target.element.projectFileUri.fsPath) {
      void vscode.window.showErrorMessage(
        constants.movingFilesBetweenProjectsNotSupported,
      );
      return;
    }
    ...
    if (
      ...
      target.element.fileSystemUri.fsPath
    ) {
      folderPath = path.basename(
        path.dirname(target.element.projectFileUri.fsPath!),
      );
    } else {
