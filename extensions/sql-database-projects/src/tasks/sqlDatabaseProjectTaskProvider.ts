...
export class SqlDatabaseProjectTaskProvider implements vscode.TaskProvider {
   ...
   */
  public readonly onDidChangeTasks: vscode.Event<void> =
    this._onDidChangeTasks.event;
  ...
  public async createTasks(): Promise<vscode.Task[]> {
    ...
    for (const workspaceFolder of workspaceFolders) {
      ...
      const sqlProjPaths = await vscode.workspace.findFiles(
        new vscode.RelativePattern(folderPath, "**/*.sqlproj"),
      );
   ...
   */
  public getTask(
    definition: SqlprojTaskDefinition,
    runCodeAnalysis: boolean = false,
  ): vscode.Task {
