...
export class DeployOptionsModel {
  ...
  public setOptionsToValueNameLookup(): void {
    Object.entries(this.deploymentOptions.booleanOptionsDictionary).forEach(
      (option) => {
  ...
  public setDeploymentOptions(): void {
    Object.entries(this.optionsValueNameLookup).forEach((option) => {
      ...
      this.deploymentOptions.booleanOptionsDictionary[
        option[1].optionName
  ...
  public getOptionDescription(displayName: string): string {
    ...
    if (optionName === undefined) {
      void vscode.window.showWarningMessage(
        constants.OptionNotFoundWarningMessage(displayName),
      );
    }
    return optionName !== undefined
      ? this.deploymentOptions.booleanOptionsDictionary[optionName.optionName]
          .description
      : "";
  }
  ...
  public setExcludeObjectTypesLookup(): void {
    Object.entries(this.deploymentOptions.objectTypesDictionary).forEach(
      (option) => {
        const optionValue: mssql.IOptionWithValue = {
          optionName: option[0],
  ...
  public getExcludeObjectTypeOptionCheckStatus(optionName: string): boolean {
    return this.deploymentOptions.excludeObjectTypes.value?.find(
   ...
   */
  public setExcludeObjectTypesOptionValue(
    displayName: string,
    checked: boolean,
  ): void {
