...
import {
  ...
  IUserDatabaseReferenceSettings,
} from "./IDatabaseReferenceSettings";
import {
  TelemetryActions,
  TelemetryReporter,
  TelemetryViews,
...
import {
  ...
  FileProjectEntry,
  NugetPackageReferenceProjectEntry,
...
export class Project implements ISqlProject {
  ...
  public static async openProject(
    ...
    promptIfNeedsUpdating: boolean = false,
    ...
    if (promptIfNeedsUpdating) {
      await this.checkPromptCrossPlatStatus(
        proj,
   ...
   */
  public static async checkPromptCrossPlatStatus(
    project: Project,
          ...
          if (result === constants.yesString) {
            try {
              ...
              void window.showErrorMessage(
                utils.getErrorMessage(utils.getErrorMessage(error)),
              );
            }
          }
        });
  ...
  private async readProjectProperties(): Promise<void> {
    ...
    const result = await sqlProjService.getProjectProperties(
    ...
    this._outputPath = path.isAbsolute(result.outputPath)
      ...
      : path.join(
          ...
          utils.getPlatformSafeFileEntryPath(result.outputPath),
        );
  ...
  private async readCrossPlatformCompatibility(): Promise<void> {
    const result = await this.sqlProjService.getCrossPlatformCompatibility(
      this.projectFilePath,
    );
    utils.throwIfFailed(result);
  ...
  private async readSqlCmdVariables(): Promise<void> {
    const sqlcmdVariablesResult = await this.sqlProjService.getSqlCmdVariables(
      this.projectFilePath,
    );
  ...
  private async readSqlObjectScripts(): Promise<void> {
    ...
    var result: GetScriptsResult =
      await this.sqlProjService.getSqlObjectScripts(this.projectFilePath);
  ...
  private async readFolders(): Promise<void> {
    var result: GetFoldersResult = await this.sqlProjService.getFolders(
      this.projectFilePath,
    );
    utils.throwIfFailed(result);
    ...
    if (result.folders?.length > 0) {
      ...
      for (var folderPath of result.folders) {
        ...
        folderEntries.push(
          this.createFileProjectEntry(folderPath, EntryType.Folder),
        );
      }
    }
  ...
  private async readPreDeployScripts(
    warnIfMultiple: boolean = false,
  ): Promise<void> {
    var result: GetScriptsResult =
      await this.sqlProjService.getPreDeploymentScripts(this.projectFilePath);
    utils.throwIfFailed(result);
  ...
  private async readPostDeployScripts(
    warnIfMultiple: boolean = false,
  ): Promise<void> {
    var result: GetScriptsResult =
      await this.sqlProjService.getPostDeploymentScripts(this.projectFilePath);
    utils.throwIfFailed(result);
  ...
  private async readNoneItems(): Promise<void> {
    ...
    if (utils.getAzdataApi()) {
      sqlProjService =
        (await utils.getSqlProjectsService()) as mssql.ISqlProjectsService;
    } else {
      sqlProjService =
        (await utils.getSqlProjectsService()) as vscodeMssql.ISqlProjectsService;
    }

    var result: GetScriptsResult = await sqlProjService.getNoneItems(
      this.projectFilePath,
    );
    utils.throwIfFailed(result);
  ...
  private async readDatabaseReferences(): Promise<void> {
    ...
    const databaseReferencesResult =
      ...
      this._databaseReferences.push(
        new DacpacReferenceProjectEntry({
          ...
          suppressMissingDependenciesErrors:
            dacpacReference.suppressMissingDependencies,

          databaseVariableLiteralValue:
            dacpacReference.databaseVariableLiteralName,
      ...
      this._databaseReferences.push(
        new SqlProjectReferenceProjectEntry({
          projectName: path.basename(
            utils.getPlatformSafeFileEntryPath(projectReference.projectPath),
            constants.sqlprojExtension,
          ),
          projectGuid: projectReference.projectGuid ?? "",
          suppressMissingDependenciesErrors:
            projectReference.suppressMissingDependencies,
          projectRelativePath: Uri.file(
            utils.getPlatformSafeFileEntryPath(projectReference.projectPath),
          ),

          databaseVariableLiteralValue:
            projectReference.databaseVariableLiteralName,
    ...
    for (const nupkgReference of databaseReferencesResult.nugetPackageReferences) {
      this._databaseReferences.push(
        new NugetPackageReferenceProjectEntry({
          ...
          suppressMissingDependenciesErrors:
            nupkgReference.suppressMissingDependencies,

          databaseVariableLiteralValue:
            nupkgReference.databaseVariableLiteralName,
  ...
  public async updateProjectForCrossPlatform(): Promise<void> {
    ...
    if (await utils.exists(this.projectFilePath + "_backup")) {
      ...
      await fs.rename(
        this.projectFilePath + "_backup",
        this.projectFilePath + "_backup" + counter,
      );
    }

    const result = await this.sqlProjService.updateProjectForCrossPlatform(
      this.projectFilePath,
    );
    utils.throwIfFailed(result);
  ...
  public async addFolder(relativeFolderPath: string): Promise<void> {
    ...
    const result = await this.sqlProjService.addFolder(
      this.projectFilePath,
      relativeFolderPath,
    );
    utils.throwIfFailed(result);
  ...
  public async deleteFolder(relativeFolderPath: string): Promise<void> {
    const result = await this.sqlProjService.deleteFolder(
      this.projectFilePath,
      relativeFolderPath,
    );
    utils.throwIfFailed(result);
  ...
  public async excludeFolder(relativeFolderPath: string): Promise<void> {
    const result = await this.sqlProjService.excludeFolder(
      this.projectFilePath,
      relativeFolderPath,
    );
    utils.throwIfFailed(result);
  ...
  public async moveFolder(
    ...
    const result = await this.sqlProjService.moveFolder(
      this.projectFilePath,
      relativeSourcePath,
  ...
  public async addSqlObjectScript(
    ...
    const result = await this.sqlProjService.addSqlObjectScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async deleteSqlObjectScript(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.deleteSqlObjectScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async excludeSqlObjectScript(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.excludeSqlObjectScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async addPreDeploymentScript(relativePath: string): Promise<void> {
    ...
    const result = await this.sqlProjService.addPreDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async deletePreDeploymentScript(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.deletePreDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async excludePreDeploymentScript(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.excludePreDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async addPostDeploymentScript(relativePath: string): Promise<void> {
    ...
    const result = await this.sqlProjService.addPostDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async deletePostDeploymentScript(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.deletePostDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async excludePostDeploymentScript(
    ...
    const result = await this.sqlProjService.excludePostDeploymentScript(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async addNoneItem(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.addNoneItem(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async deleteNoneItem(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.deleteNoneItem(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
  ...
  public async excludeNoneItem(relativePath: string): Promise<void> {
    const result = await this.sqlProjService.excludeNoneItem(
      this.projectFilePath,
      relativePath,
    );
    utils.throwIfFailed(result);
   ...
   */
  public async addScriptItem(
    relativeFilePath: string,
    contents?: string,
    itemType?: string,
  ): Promise<FileProjectEntry> {
    // Check if file already has been added to sqlproj
    const normalizedRelativeFilePath =
      utils.convertSlashesForSqlProj(relativeFilePath);

    const existingEntry = this.sqlObjectScripts.find(
      (f) =>
        f.relativePath.toUpperCase() ===
        normalizedRelativeFilePath.toUpperCase(),
    );
    if (existingEntry) {
    ...
    return this.createFileProjectEntry(
  ...
  public async changeTargetPlatform(compatLevel: string): Promise<void> {
    ...
    TelemetryReporter.createActionEvent(
      ...
      TelemetryActions.changePlatformType,
    )
    ...
    const result = await this.sqlProjService.setDatabaseSchemaProvider(
      this.projectFilePath,
  ...
  public getProjectTargetVersion(): string {
    ...
    const version = this._databaseSchemaProvider.substring(
      constants.MicrosoftDatatoolsSchemaSqlSql.length,
      this._databaseSchemaProvider.length -
        constants.databaseSchemaProvider.length,
    );

    // make sure version is valid
    if (
      !Array.from(constants.targetPlatformToVersion.values()).includes(version)
    ) {
  ...
  public async addSystemDatabaseReference(
    settings: ISystemDatabaseReferenceSettings,
  ): Promise<void> {
    // check if reference to this database already exists
    if (
      this.databaseReferences.find(
    ...
    if (utils.getAzdataApi()) {
      ...
      referenceType =
        settings.systemDbReferenceType as mssql.SystemDbReferenceType;
      sqlProjService = this.sqlProjService as mssql.ISqlProjectsService;
      ...
      referenceType =
        settings.systemDbReferenceType as vscodeMssql.SystemDbReferenceType;
      sqlProjService = this.sqlProjService as vscodeMssql.ISqlProjectsService;
      result = await sqlProjService.addSystemDatabaseReference(
        this.projectFilePath,
        systemDb,
  ...
  public async addDatabaseReference(
    settings: IDacpacReferenceSettings,
  ): Promise<void> {
   ...
   */
  public async addProjectReference(
    settings: IProjectReferenceSettings,
  ): Promise<void> {
  ...
  public async addNugetPackageReference(
    settings: INugetPackageReferenceSettings,
  ): Promise<void> {
  ...
  private async addUserDatabaseReference(
    settings: IUserDatabaseReferenceSettings,
    reference:
      | SqlProjectReferenceProjectEntry
      | DacpacReferenceProjectEntry
      | NugetPackageReferenceProjectEntry,
    ...
    if (settings.databaseVariable && settings.databaseName) {
      await this.sqlProjService.addSqlCmdVariable(
        this.projectFilePath,
        settings.databaseVariable,
    ...
    const databaseLiteral = settings.databaseVariable
      ? undefined
      : settings.databaseName;
    ...
    if (reference instanceof SqlProjectReferenceProjectEntry) {
      ...
      result = await this.sqlProjService.addSqlProjectReference(
        this.projectFilePath,
        reference.pathForSqlProj(),
        reference.projectGuid,
        settings.suppressMissingDependenciesErrors,
        settings.databaseVariable,
        settings.serverVariable,
        databaseLiteral,
      ...
      referenceName = (<IDacpacReferenceSettings>settings).dacpacFileLocation
        .fsPath;
      result = await this.sqlProjService.addDacpacReference(
        this.projectFilePath,
        reference.pathForSqlProj(),
        settings.suppressMissingDependenciesErrors,
        settings.databaseVariable,
      ...
      result = await this.sqlProjService.addNugetPackageReference(
        this.projectFilePath,
        reference.packageName,
        (<INugetPackageReferenceSettings>settings).packageVersion,
        settings.suppressMissingDependenciesErrors,
        settings.databaseVariable,
        settings.serverVariable,
        databaseLiteral,
  ...
  private databaseReferenceExists(
    entry: IDatabaseReferenceProjectEntry,
  ): boolean {
    const found =
      this._databaseReferences.find(
        (reference) => reference.pathForSqlProj() === entry.pathForSqlProj(),
      ) !== undefined;
    return found;
  }

  public async deleteDatabaseReferenceByEntry(
    entry: IDatabaseReferenceProjectEntry,
  ): Promise<void> {
  ...
  public async deleteDatabaseReference(name: string): Promise<void> {
    const result = await this.sqlProjService.deleteDatabaseReference(
      this.projectFilePath,
      name,
    );
    utils.throwIfFailed(result);
  ...
  public async addSqlCmdVariable(
    ...
    const result = await this.sqlProjService.addSqlCmdVariable(
      this.projectFilePath,
      name,
      defaultValue,
    );
  ...
  public async updateSqlCmdVariable(
    ...
    const result = await this.sqlProjService.updateSqlCmdVariable(
      this.projectFilePath,
      name,
      defaultValue,
    );
  ...
  public async deleteSqlCmdVariable(variableName: string): Promise<void> {
    const result = await this.sqlProjService.deleteSqlCmdVariable(
      this.projectFilePath,
      variableName,
    );
    utils.throwIfFailed(result);
  ...
  public getDatabaseSourceValues(): string[] {
    return this._databaseSource.trim() === ""
      ? []
      : this._databaseSource.split(";");
  }

  public createFileProjectEntry(
    relativePath: string,
    entryType: EntryType,
    sqlObjectType?: string,
    containsCreateTableStatement?: boolean,
  ): FileProjectEntry {
    let platformSafeRelativePath =
  ...
  public async move(
    ...
    const originalRelativePath = utils.trimUri(
      projectRelativeUri,
      node.relativeProjectUri,
    );
    destinationRelativePath = utils.trimUri(
      projectRelativeUri,
      vscode.Uri.file(destinationRelativePath),
    );
    ...
    if (node instanceof SqlObjectFileNode) {
      result = await this.sqlProjService.moveSqlObjectScript(
        this.projectFilePath,
        originalRelativePath,
        destinationRelativePath,
      );
    } else if (node instanceof PreDeployNode) {
      result = await this.sqlProjService.movePreDeploymentScript(
        this.projectFilePath,
        originalRelativePath,
        destinationRelativePath,
      );
    } else if (node instanceof PostDeployNode) {
      result = await this.sqlProjService.movePostDeploymentScript(
        this.projectFilePath,
        originalRelativePath,
        destinationRelativePath,
      );
    } else if (node instanceof NoneNode || node instanceof PublishProfileNode) {
      result = await this.sqlProjService.moveNoneItem(
        this.projectFilePath,
        originalRelativePath,
        destinationRelativePath,
      );
    } else if (node instanceof FolderNode) {
      result = await this.sqlProjService.moveFolder(
        this.projectFilePath,
        originalRelativePath,
        destinationRelativePath,
      );
