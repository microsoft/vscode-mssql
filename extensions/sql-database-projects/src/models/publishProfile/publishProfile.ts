...
export async function readPublishProfile(
...
export async function load(
  ...
  const profileXmlDoc: Document = new xmldom.DOMParser().parseFromString(
    profileText.toString(),
  );

  // read target database name
  let targetDbName: string = "";
  let targetDatabaseNameCount =
  ...
  const optionsResult = await dacfxService.getOptionsFromProfile(
    profileUri.fsPath,
  );
  ...
  if (
    ...
    try {
      ...
      if (dataSource.integratedSecurity) {
        if (azdataApi) {
          const connectionResult = await utils
            .getAzdataApi()!
            .connection.connect(connectionProfile, false, false);
          if (!connectionResult.connected) {
            const connection = await utils
              .getAzdataApi()!
              .connection.openConnectionDialog(undefined, connectionProfile, {
...
export async function savePublishProfile(
  ...
  deploymentOptions?: mssql.DeploymentOptions | vscodeMssql.DeploymentOptions,
): Promise<void> {
  const dacFxService = await utils.getDacFxService();
  if (utils.getAzdataApi()) {
    await (dacFxService as mssql.IDacFxService).savePublishProfile(
      profilePath,
      databaseName,
      connectionString,
      sqlCommandVariableValues,
      deploymentOptions as mssql.DeploymentOptions,
    );
  } else {
...
export function promptToSaveProfile(
  project: Project,
    ...
    defaultUri:
      ...
      vscode.Uri.file(
        path.join(
          ...
          `${project.projectFileName}_1.publish.xml`,
        ),
      ),
 ...
 * @returns
 */
export async function promptForSavingProfile(
  project: Project,
  settings:
    | ISqlProjectPublishSettings
    | ISqlDbDeployProfile
  ...
  if (result === constants.yesString) {
    ...
    if (settings) {
      if (isISqlProjectPublishSettings(settings)) {
        ...
        publishProfileUri =
          settings.sqlProjectPublishSettings.publishProfileUri;
      }
    }
    ...
    await project.addNoneItem(
      path.relative(project.projectFolderPath, filePath.fsPath),
    );
 ...
 */
function isISqlProjectPublishSettings(
  settings:
    | ISqlProjectPublishSettings
    | ISqlDbDeployProfile
    | IPublishToDockerSettings
 ...
 */
function isIPublishToDockerSettings(
  settings:
    | ISqlProjectPublishSettings
    | ISqlDbDeployProfile
    | IPublishToDockerSettings
    | undefined,
): settings is IPublishToDockerSettings {
...
async function getConnectionString(
  ...
  if (connectionUri) {
    ...
    ).getConnectionString(connectionUri, false);
  }
...
async function getDeploymentOptions(
  settings:
    | ISqlProjectPublishSettings
    | ISqlDbDeployProfile
    | IPublishToDockerSettings
    | undefined,
  project: Project,
): Promise<vscodeMssql.DeploymentOptions | undefined> {
