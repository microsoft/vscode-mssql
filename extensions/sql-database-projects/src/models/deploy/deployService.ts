...
export class DeployService {
  constructor(
    private _azureSqlClient = new AzureSqlClient(),
    private _outputChannel: vscode.OutputChannel,
    shellExecutionHelper: ShellExecutionHelper | undefined = undefined,
  ) {
    this._shellExecutionHelper =
      shellExecutionHelper ?? new ShellExecutionHelper(this._outputChannel);
  ...
  private async verifyDocker(): Promise<void> {
    try {
      ...
      throw Error(
        constants.dockerNotRunningError(utils.getErrorMessage(error)),
      );
    }
  }
   ...
   */
  public async createNewAzureSqlServer(
    profile: ISqlDbDeployProfile | undefined,
    ...
    this.logToOutput(
      constants.creatingAzureSqlServer(profile?.sqlDbSetting?.serverName),
    );

    // Create the server
    const server = await this._azureSqlClient.createOrUpdateServer(
      profile.sqlDbSetting.session,
      profile?.sqlDbSetting.resourceGroupName,
      profile?.sqlDbSetting.serverName,
      {
    ...
    if (server) {
      ...
      this.logToOutput(
        constants.azureSqlServerCreated(profile?.sqlDbSetting?.serverName),
      );

      // Connect to the server
      return await this._connectionService.getConnection(
        profile.sqlDbSetting,
        false,
        constants.master,
      );
    }
  ...
  public async deployToContainer(
    profile: IPublishToDockerSettings,
    project: Project,
  ): Promise<string | undefined> {
    return await this.executeTask(constants.deployDbTaskName, async () => {
      ...
      const imageSpec = getDockerImageSpec(
        project.projectFileName,
        profile.dockerSettings.dockerBaseImage,
      );
      ...
      //
      const createdDockerId: string | undefined = await this.runDockerContainer(
        imageSpec,
        profile.dockerSettings,
      );
      this.logToOutput(`Docker container created. Id: ${createdDockerId}`);

      // Waiting a bit to make sure docker container doesn't crash
      //
      const runningDockerId = await utils.retry(
        "Validating the docker container",
        async () => {
          return this.executeCommand(
            `docker ps -q -a --filter label=${imageSpec.label} -q`,
          );
        },
        (dockerId) => {
          return Promise.resolve({
            validated: dockerId !== undefined,
            errorMessage: constants.dockerContainerNotRunningErrorMessage,
          });
        },
        (dockerId) => {
      ...
      if (runningDockerId) {
        this.logToOutput(
          constants.dockerContainerCreatedMessage(runningDockerId),
        );
        return await this._connectionService.getConnection(
          profile.dockerSettings,
          false,
        ...
        if (createdDockerId) {
          ...
          await this.executeCommand(
            constants.dockerLogMessage(createdDockerId),
          );
        }
      }
  ...
  private async runDockerContainer(
    ...
    await this.executeCommand(
      `docker run -p ${profile.port}:1433 -e "MSSQL_SA_PASSWORD=${profile.password}" -e "ACCEPT_EULA=Y" -e "MSSQL_PID=Developer" --label ${dockerImageSpec.label} -d --name ${dockerImageSpec.containerName} ${profile.dockerBaseImage} `,
      sensitiveData,
    );
    return await this.executeCommand(
      `docker ps -q -a --filter label=${dockerImageSpec.label} -q`,
    );
  }
        ...
        azdataApi!.tasks.startBackgroundOperation({
          ...
          operation: async (op) => {
            try {
              ...
              let errorMsg = constants.taskFailedError(
                taskName,
                error ? error.message : "",
              );
  ...
  public async executeCommand(
    ...
    return await this._shellExecutionHelper.runStreamedCommand(
      cmd,
      undefined,
      sensitiveData,
      timeout,
  ...
  public async getCurrentDockerContainer(
    imageLabel: string,
  ): Promise<string[]> {
    const currentIds = await this.executeCommand(
      `docker ps -q -a --filter label=${imageLabel}`,
    );
  ...
  public async cleanDockerObjectsIfNeeded(imageLabel: string): Promise<void> {
    ...
    if (containerIds.length > 0) {
      ...
      if (result === constants.yesString) {
        this.logToOutput(constants.cleaningDockerImagesMessage);
        await this.cleanDockerObjects(containerIds, [
          "docker stop",
          "docker rm",
        ]);
      }
  ...
  public async cleanDockerObjects(
    ids: string[],
    commandsToClean: string[],
  ): Promise<void> {
    for (let index = 0; index < ids.length; index++) {
      ...
      if (id) {
        for (
          let commandId = 0;
          commandId < commandsToClean.length;
          commandId++
        ) {
...
export function getDockerImageSpec(
  projectName: string,
  baseImage: string,
  ...
  const tag = baseImage
    .replace(":", "-")
    .replace(constants.sqlServerDockerRegistry, "")
    .replace(regexForDockerImageName, "");

  // cut the name if it's too long
  //
  imageProjectName = imageProjectName.substring(
    0,
    tagMaxLength - (constants.dockerImageNamePrefix.length + tag.length + 2),
  );
  const imageLabel =
    `${constants.dockerImageLabelPrefix}-${imageProjectName}`.toLocaleLowerCase();
  const imageTag =
    `${constants.dockerImageNamePrefix}-${imageProjectName}-${tag}`.toLocaleLowerCase();
  const dockerName =
    `${constants.dockerImageNamePrefix}-${imageProjectName}-${imageUniqueId}`.toLocaleLowerCase();
