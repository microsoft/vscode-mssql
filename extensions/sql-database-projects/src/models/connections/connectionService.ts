...
export class ConnectionService {
  ...
  private async connectToDatabase(
    profile: ISqlConnectionProperties,
    saveConnectionAndPassword: boolean,
    database: string,
  ): Promise<azdataType.ConnectionResult | string | undefined> {
    const azdataApi = utils.getAzdataApi();
    const vscodeMssqlApi = azdataApi
      ? undefined
    ...
    if (azdataApi) {
      ...
      return await azdataApi.connection.connect(
        connectionProfile,
        saveConnectionAndPassword,
        false,
      );
      ...
      let connectionUrl = "";
      try {
        connectionUrl = await vscodeMssqlApi.connect(
          connectionProfile,
          saveConnectionAndPassword,
        );
      } catch (err) {
  ...
  private async validateConnection(
    connection: azdataType.ConnectionResult | string | undefined,
  ): Promise<utils.ValidationResult> {
    const azdataApi = utils.getAzdataApi();
    if (!connection) {
      return {
        validated: false,
        errorMessage: constants.connectionFailedError("No result returned"),
      };
    } else if (azdataApi) {
      ...
      if (connectionResult) {
        const connected =
          ...
          connectionResult.connectionId !== undefined;
        return {
          validated: connected,
          errorMessage: connected
            ? ""
            : constants.connectionFailedError(connectionResult?.errorMessage!),
        };
      } else {
        return {
          validated: false,
          errorMessage: constants.connectionFailedError(""),
        };
      }
    } else {
      return {
        validated: connection !== undefined,
        errorMessage: constants.connectionFailedError(""),
      };
    }
  }
   ...
   */
  private async formatConnectionResult(
    connection: azdataType.ConnectionResult | string | undefined,
  ): Promise<string> {
    const azdataApi = utils.getAzdataApi();
    const connectionResult =
      connection !== undefined && azdataApi
        ? <azdataType.ConnectionResult>connection
        : undefined;
  ...
  public async getConnection(
    ...
    let connection = await utils.retry(
      ...
      this._outputChannel,
      this.defaultSqlNumberOfRetries,
      profile.connectionRetryTimeout || this.defaultSqlRetryTimeoutInSec,
    );
    ...
    if (connection) {
      ...
      if (azdataApi) {
        ...
        return azdataApi.connection.getUriForConnection(
          connectionResult.connectionId!,
        );
      } else {
