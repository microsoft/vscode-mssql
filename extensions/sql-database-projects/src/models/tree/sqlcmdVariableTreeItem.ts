...
export class SqlCmdVariablesTreeItem extends BaseProjectTreeItem {
  ...
  constructor(
    ...
    super(
      vscode.Uri.file(
        path.join(projectNodeName, constants.sqlcmdVariablesNodeName),
      ),
      sqlprojUri,
    );
  ...
  private construct(sqlCmdVariables: Map<string, string>) {
    ...
    for (const sqlCmdVariable of sqlCmdVariables.keys()) {
      this.sqlcmdVariableTreeItems.push(
        new SqlCmdVariableTreeItem(
          sqlCmdVariable,
  ...
  public get treeItem(): vscode.TreeItem {
    const sqlCmdVariableFolderItem = new vscode.TreeItem(
      this.relativeProjectUri,
...
export class SqlCmdVariableTreeItem extends BaseProjectTreeItem {
  constructor(
    private sqlcmdVar: string,
    sqlCmdNodeRelativeProjectUri: vscode.Uri,
    sqlprojUri: vscode.Uri,
  ) {
    super(
      vscode.Uri.file(
        path.join(sqlCmdNodeRelativeProjectUri.fsPath, sqlcmdVar),
      ),
      sqlprojUri,
    );
  ...
  public get treeItem(): vscode.TreeItem {
    const sqlcmdVariableItem = new vscode.TreeItem(
      this.relativeProjectUri,
