...
export class DatabaseReferencesTreeItem extends BaseProjectTreeItem {
  ...
  constructor(
    ...
    super(
      vscode.Uri.file(
        path.join(projectNodeName, constants.databaseReferencesNodeName),
      ),
      sqlprojUri,
    );
  ...
  private construct(databaseReferences: IDatabaseReferenceProjectEntry[]) {
    ...
    for (const reference of databaseReferences) {
      this.references.push(
        reference instanceof SqlProjectReferenceProjectEntry
          ? new SqlProjectReferenceTreeItem(
              reference,
              this.relativeProjectUri,
              this.projectFileUri,
            )
          : new DatabaseReferenceTreeItem(
              reference,
              this.relativeProjectUri,
              this.projectFileUri,
            ),
  ...
  public get treeItem(): vscode.TreeItem {
    const refFolderItem = new vscode.TreeItem(
      this.relativeProjectUri,
...
export class DatabaseReferenceTreeItem extends BaseProjectTreeItem {
  constructor(
    public readonly reference: IDatabaseReferenceProjectEntry,
    referencesNodeRelativeProjectUri: vscode.Uri,
    sqlprojUri: vscode.Uri,
  ) {
    super(
      vscode.Uri.file(
        path.join(
          referencesNodeRelativeProjectUri.fsPath,
  ...
  public get treeItem(): vscode.TreeItem {
    const refItem = new vscode.TreeItem(
      this.relativeProjectUri,
