...
export class ProjectRootTreeItem extends BaseProjectTreeItem {
  ...
  fileChildren: {
    [childName: string]: fileTree.FolderNode | fileTree.FileNode;
  } = {};
  project: Project;
  ...
  constructor(project: Project) {
    super(
      vscode.Uri.parse(
        path.basename(project.projectFilePath, sqlprojExtension),
      ),
    ...
    this.projectNodeName = path.basename(
      project.projectFilePath,
      sqlprojExtension,
    );

    this.databaseReferencesNode = new DatabaseReferencesTreeItem(
      this.projectNodeName,
      this.projectFileUri,
      project.databaseReferences,
    );
    this.sqlCmdVariablesNode = new SqlCmdVariablesTreeItem(
      this.projectNodeName,
      this.projectFileUri,
      project.sqlCmdVariables,
    );
    this.construct();
  }
  ...
  public get children(): BaseProjectTreeItem[] {
    ...
    return output.concat(
      Object.values(this.fileChildren).sort(fileTree.sortFileFolderNodes),
    );
  }

  public get treeItem(): vscode.TreeItem {
    const collapsibleState = vscode.workspace.getConfiguration(
  ...
  public get type(): DatabaseProjectItemType {
    ...
    if (utils.getAzdataApi()) {
      projectType =
        this.project.sqlProjStyle === mssql.ProjectType.SdkStyle
  ...
  private construct() {
    ...
    for (const folder of this.project.folders.sort((a, b) =>
      ...
      const newNode = new fileTree.FolderNode(
        folder.fsUri,
        this.projectFileUri,
        folder.relativePath,
      );
    ...
    for (const preDeployEntry of this.project.preDeployScripts) {
      const newNode = new fileTree.PreDeployNode(
        preDeployEntry.fsUri,
        this.projectFileUri,
        preDeployEntry.relativePath,
      );
    ...
    for (const postDeployEntry of this.project.postDeployScripts) {
      const newNode = new fileTree.PostDeployNode(
        postDeployEntry.fsUri,
        this.projectFileUri,
        postDeployEntry.relativePath,
      );
    ...
    for (const noneEntry of this.project.noneDeployScripts) {
      const newNode = new fileTree.NoneNode(
        noneEntry.fsUri,
        this.projectFileUri,
        noneEntry.relativePath,
      );
    ...
    for (const publishProfile of this.project.publishProfiles) {
      const newNode = new fileTree.PublishProfileNode(
        publishProfile.fsUri,
        this.projectFileUri,
        publishProfile.relativePath,
      );
    ...
    for (const entry of this.project.sqlObjectScripts) {
      ...
      if (entry.sqlObjectType === ExternalStreamingJob) {
        newNode = new fileTree.ExternalStreamingJobFileNode(
          entry.fsUri,
          this.projectFileUri,
          entry.relativePath,
        );
  ...
  private addNode(
    newNode: fileTree.FileNode | fileTree.FolderNode,
    entry: FileProjectEntry,
  ): void {
    // Don't add external folders
    if (
      entry.type !== EntryType.File &&
      entry.relativePath.startsWith(RelativeOuterPath)
    ) {
      return;
    }
    ...
    if (
      Object.keys(parentNode.fileChildren).includes(
        path.basename(entry.fsUri.path),
      )
    ) {
   ...
   */
  private getEntryParentNode(
    entry: FileProjectEntry,
  ): fileTree.FolderNode | ProjectRootTreeItem {
