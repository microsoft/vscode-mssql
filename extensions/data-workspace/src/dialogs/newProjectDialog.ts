...
export class NewProjectDialog extends DialogBase {
    ...
    public newDialogPromise: Promise<void> = new Promise<void>(
        (resolve, reject) => (this.newProjectDialogComplete = { resolve, reject }),
    );
    public projectUri: vscode.Uri | undefined;

    constructor(
        private workspaceService: IWorkspaceService,
        private specificProjectType?: IProjectType,
    ) {
        ...
        TelemetryReporter.createActionEvent(
            TelemetryViews.NewProjectDialog,
            TelemetryActions.NewProjectDialogLaunched,
        )
            .withAdditionalProperties({
                isWorkspaceOpen: (vscode.workspace.workspaceFile !== undefined).toString(),
            })
            .send();
    }
        ...
        try {
            ...
            if (!parentDirectoryExists) {
                this.showErrorMessage(
                    constants.ProjectParentDirectoryNotExistError(this.model.location),
                );
                return false;
            }

            // there shouldn't be an existing sub directory with the same name as the project in the selected location
            const projectDirectoryExists = await directoryExist(
                path.join(this.model.location, this.model.name),
            );
    ...
    override async onComplete(): Promise<void> {
        try {
            TelemetryReporter.createActionEvent(
                TelemetryViews.NewProjectDialog,
                TelemetryActions.NewProjectDialogCompleted,
            )
                .withAdditionalProperties({
                    projectFileExtension: this.model.projectFileExtension,
                    projectTemplateId: this.model.projectTypeId,
                })
                .send();

            this.projectUri = await this.workspaceService.createProject(
                this.model.name,
                vscode.Uri.file(this.model.location),
                this.model.projectTypeId,
                this.model.targetPlatform,
                this.model.sdkStyleProject,
                this.model.configureDefaultBuild,
            );
            this.newProjectDialogComplete?.resolve();
        } catch (err) {
    ...
    protected async initialize(view: azdataType.ModelView): Promise<void> {
        ...
        if (
            this.specificProjectType &&
            allProjectTypes.find((p) => p.id === this.specificProjectType?.id)
        ) {
            allProjectTypes = [this.specificProjectType];
        }

        const projectTypeRadioCardGroup = view.modelBuilder
            .radioCardGroup()
            .withProps({
        ...
        this.register(
            projectTypeRadioCardGroup.onSelectionChanged(async (e) => {
                ...
                if (selectedProject?.sdkStyleOption) {
                    ...
                    if (selectedProject.sdkStyleLearnMoreUrl) {
                        await sdkLearnMore.updateProperty(
                            "url",
                            selectedProject.sdkStyleLearnMoreUrl,
                        );
        ...
        const projectNameTextBox = view.modelBuilder
            .inputBox()
        ...
        const locationTextBox = view.modelBuilder
            .inputBox()
            .withProps({
        ...
        locationTextBox.value =
            defaultSaveLocation?.fsPath ?? vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;

        this.register(
            locationTextBox.onTextChanged(() => {
        ...
        this.register(
            browseFolderButton.onDidClick(async () => {
                let folderUris = await vscode.window.showOpenDialog({
                    ...
                    canSelectMany: false,
                    defaultUri: locationTextBox.value
                        ? vscode.Uri.file(locationTextBox.value)
                        : undefined,
                });
        ...
        const targetPlatformDropdown = view.modelBuilder
            .dropDown()
            .withProps({
        ...
        this.register(
            targetPlatformDropdown.onValueChanged(() => {
        ...
        const sdkProjectCheckbox = view.modelBuilder
            .checkBox()
            .withProps({
                checked: true,
        ...
        this.formBuilder = view.modelBuilder.formContainer().withFormItems([
            ...
            },
            {
                ...
                required: true,
                component: this.createHorizontalContainer(view, [
                    locationTextBox,
                    browseFolderButton,
                ]),
            },
    ...
    getDefaultTargetPlatform(projectType: IProjectType): string | undefined {
        ...
        if (
            projectType.defaultTargetPlatform &&
            projectType.targetPlatforms?.includes(projectType.defaultTargetPlatform)
        ) {
