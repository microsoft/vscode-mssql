...
export async function createNewProjectWithQuickpick(
  ...
  while (true) {
    const browseProjectLocation = await vscode.window.showQuickPick(browseProjectLocationOptions, {
      title: browseProjectLocationTitle,
      ignoreFocusOut: true,
    });
  ...
  if (projectType.targetPlatforms) {
    ...
    let targetPlatforms: vscode.QuickPickItem[] = projectType.targetPlatforms.map(
      (targetPlatform) => {
        return { label: targetPlatform };
      },
    );

    if (projectType.defaultTargetPlatform) {
      // move the default target platform to be the first one in the list
      const defaultIndex = targetPlatforms.findIndex(
        (i) => i.label === projectType.defaultTargetPlatform,
      );
      if (defaultIndex > -1) {
