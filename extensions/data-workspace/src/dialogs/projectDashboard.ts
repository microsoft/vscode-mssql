...
export class ProjectDashboard {
  ...
  constructor(
    ...
    private _treeItem: WorkspaceTreeItem,
  ) {}
  ...
  public async showDashboard(): Promise<void> {
    ...
    this.projectProvider = await this._workspaceService.getProjectProvider(
  ...
  private async createDashboard(title: string, projectFilePath: string): Promise<void> {
    this.dashboard = getAzdataApi()!.window.createModelViewDashboard(title, "ProjectDashboard", {
  ...
  private createToolbarContainer(projectFilePath: string): azdataType.ToolbarContainer {
    const projectActions: (IProjectAction | IProjectActionGroup)[] =
      this.projectProvider!.projectToolbarActions;
    ...
    projectActions.forEach((action, actionIndex) => {
      if (this.isProjectAction(action)) {
        ...
        buttons.push({
          component: button,
          toolbarSeparatorAfter: projectActionsLength - 1 === actionIndex,
        });
      } else {
        ...
        action.actions.forEach((groupAction, index) => {
          ...
          buttons.push({
            ...
            toolbarSeparatorAfter:
              groupLength - 1 === index || projectActionsLength - 1 === actionIndex,
          }); // Add toolbar separator at the end of the group
        });
      }
    });
    ...
    return this.modelView!.modelBuilder.toolbarContainer().withToolbarItems(buttons).component();
  }
  ...
  private createContainer(title: string, projectFilePath: string): azdataType.FlexContainer {
    this.rootContainer = this.modelView!.modelBuilder.flexContainer()
      .withLayout({
        flexFlow: "column",
        width: "100%",
        height: "100%",
  ...
  private createHeader(title: string, location: string): azdataType.Component {
    const headerContainer = this.modelView!.modelBuilder.flexContainer()
      .withLayout({
        flexFlow: "column",
        width: "100%",
        height: "30%",
    ...
    const header = this.modelView!.modelBuilder.flexContainer()
      .withLayout({
        flexFlow: "column",
        width: "100%",
        height: "30%",
    ...
    const titleLabel = this.modelView!.modelBuilder.text()
      .withProps({
        value: title,
        CSSStyles: { "margin-block-start": "20px", "margin-block-end": "0px" },
      })
      .component();
    header.addItem(titleLabel, {
      CSSStyles: {
        "padding-left": "34px",
        "padding-top": "15px",
        "font-size": "36px",
    ...
    const backgroundUri = vscode.Uri.file(backgroundImage!.light.toString()).with({
      scheme: "vscode-file",
    });
  ...
  private createTables(projectFile: string): azdataType.Component {
    const dashboardData: IDashboardTable[] =
      this.projectProvider!.getDashboardComponents(projectFile);

    const tableContainer = this.modelView!.modelBuilder.flexContainer()
      .withLayout({
        flexFlow: "column",
        width: "100%",
        height: "auto",
    ...
    dashboardData.forEach((info) => {
      const tableNameLabel = this.modelView!.modelBuilder.text()
        .withProps({
          value: info.name,
          CSSStyles: { "margin-block-start": "30px", "margin-block-end": "0px" },
        })
        .component();
      tableContainer.addItem(tableNameLabel, {
        CSSStyles: {
          "padding-left": "25px",
          "padding-bottom": "20px",
          ...constants.cssStyles.title,
        },
      });
      ...
      if (info.data.length === 0) {
        ...
        const noDataLabel = this.modelView!.modelBuilder.text()
          ...
          .component();
        tableContainer.addItem(noDataLabel, {
          CSSStyles: { "padding-left": "25px", "padding-bottom": "20px" },
        });
      } else {
        ...
        info.columns.forEach((column: IDashboardColumnInfo) => {
          let col = {
            ...
            valueType:
              column.type === "icon"
                ? getAzdataApi()!.DeclarativeDataType.component
                : getAzdataApi()!.DeclarativeDataType.string,
            isReadOnly: true,
            width: column.width,
            headerCssStyles: {
              border: "none",
        ...
        info.data.forEach((values) => {
          ...
          values.forEach((val) => {
            if (typeof val === "string") {
              ...
              const iconComponent = this.modelView!.modelBuilder.image()
                .withProps({
                  iconPath: val.icon,
                  width: "15px",
                  height: "15px",
                ...
                .component();
              const stringComponent = this.modelView!.modelBuilder.text()
                .withProps({
                  value: val.text,
                  CSSStyles: {
              ...
              const columnData = this.modelView!.modelBuilder.flexContainer()
                .withItems([iconComponent, stringComponent], {
                  flex: "0 0 auto",
