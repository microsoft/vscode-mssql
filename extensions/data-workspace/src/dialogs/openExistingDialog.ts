...
export class OpenExistingDialog extends DialogBase {
  ...
  constructor(private workspaceService: IWorkspaceService) {
    ...
    TelemetryReporter.createActionEvent(
      TelemetryViews.OpenExistingDialog,
      TelemetryActions.OpenExistingDialogLaunched,
    )
  ...
  override async onComplete(): Promise<void> {
    try {
      ...
      const telemetryProps: any = {
        hasWorkspaceOpen: (vscode.workspace.workspaceFile !== undefined).toString(),
      };
      ...
      if (this.remoteGitRepoRadioButton!.checked) {
        TelemetryReporter.createActionEvent(
          TelemetryViews.OpenExistingDialog,
          TelemetryActions.GitClone,
        )
          .withAdditionalProperties({ selectedTarget: "project" })
          .send();

        addProjectsPromise = this.workspaceService.gitCloneProject(
          (<azdataType.InputBoxComponent>this.gitRepoTextBoxComponent?.component).value!,
          this.localClonePathTextBox!.value!,
        );
      } else {
        telemetryProps.cancelled = "false";
        addProjectsPromise = this.workspaceService.addProjectsToWorkspace([
          vscode.Uri.file(this.filePathTextBox!.value!),
        ]);
      }

      TelemetryReporter.createActionEvent(
        TelemetryViews.OpenExistingDialog,
        TelemetryActions.OpeningProject,
      )
  ...
  protected async initialize(view: azdataType.ModelView): Promise<void> {
    this.localRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "location",
    ...
    this.register(
      this.localRadioButton.onDidChangeCheckedState((checked) => {
        if (checked) {
          ...
          this.formBuilder?.insertFormItem(
            <azdataType.FormComponent>this.filePathAndButtonComponent,
            1,
          );
        }
      }),
    ...
    this.remoteGitRepoRadioButton = view.modelBuilder
      .radioButton()
      .withProps({
        name: "location",
    ...
    this.locationRadioButtonFormComponent = {
      ...
      component: view.modelBuilder
        .flexContainer()
        .withItems([this.localRadioButton, this.remoteGitRepoRadioButton], {
          flex: "0 0 auto",
          CSSStyles: { "margin-right": "15px" },
        })
    ...
    this.register(
      this.remoteGitRepoRadioButton.onDidChangeCheckedState((checked) => {
        if (checked) {
          this.formBuilder?.removeFormItem(
            <azdataType.FormComponent>this.filePathAndButtonComponent,
          );
          this.formBuilder?.insertFormItem(
            <azdataType.FormComponent>this.gitRepoTextBoxComponent,
            1,
          );
          this.formBuilder?.insertFormItem(
            <azdataType.FormComponent>this.localClonePathComponent,
            2,
          );
        }
      }),
    ...
    const gitRepoTextBox = view.modelBuilder
      .inputBox()
      .withProps({
    ...
    this.localClonePathTextBox = view.modelBuilder
      .inputBox()
      .withProps({
    ...
    this.register(
      this.localClonePathTextBox.onTextChanged(() => {
        return this.localClonePathTextBox!.updateProperty(
          "title",
          this.localClonePathTextBox!.value!,
        );
      }),
    ...
    const localClonePathBrowseFolderButton = view.modelBuilder
      .button()
      .withProps({
    ...
    this.register(
      localClonePathBrowseFolderButton.onDidClick(async () => {
    ...
    this.localClonePathComponent = {
      ...
      component: this.createHorizontalContainer(view, [
        this.localClonePathTextBox,
        localClonePathBrowseFolderButton,
      ]),
      required: true,
    };

    this.filePathTextBox = view.modelBuilder
      .inputBox()
      .withProps({
    ...
    this.register(
      this.filePathTextBox.onTextChanged(() => {
        return this.filePathTextBox!.updateProperty("title", this.filePathTextBox!.value!);
      }),
    );

    const localProjectBrowseFolderButton = view.modelBuilder
      .button()
      .withProps({
        ariaLabel: constants.BrowseButtonText,
        title: constants.BrowseButtonText,
    ...
    const flexContainer = this.createHorizontalContainer(view, [
      this.filePathTextBox,
      localProjectBrowseFolderButton,
    ]);
    ...
    this.formBuilder = view.modelBuilder
      .formContainer()
      .withFormItems([this.locationRadioButtonFormComponent, this.filePathAndButtonComponent]);
