...
export const ProjectProviderRegistry: IProjectProviderRegistry =
    new (class implements IProjectProviderRegistry {
        ...
        registerProvider(provider: IProjectProvider, providerId: string): vscode.Disposable {
            ...
            provider.supportedProjectTypes.forEach((projectType) => {
                this._providerFileExtensionMapping[projectType.projectFileExtension.toUpperCase()] =
                    provider;
                this._providerProjectTypeMapping[projectType.id.toUpperCase()] = provider;
            });

            TelemetryReporter.createActionEvent(
                TelemetryViews.ProviderRegistration,
                TelemetryActions.ProviderRegistered,
            )
                .withAdditionalProperties({
                    providerId: providerId,
                    extensions: provider.supportedProjectTypes
            ...
            return new vscode.Disposable(() => {
                ...
                if (idx >= 0) {
                    ...
                    provider.supportedProjectTypes.forEach((projectType) => {
                        delete this._providerFileExtensionMapping[
                            projectType.projectFileExtension.toUpperCase()
                        ];
        ...
        getProviderByProjectExtension(extension: string): IProjectProvider | undefined {
            return extension
                ? this._providerFileExtensionMapping[extension.toUpperCase()]
                : undefined;
        }

        getProviderByProjectType(projectType: string): IProjectProvider | undefined {
            return projectType
                ? this._providerProjectTypeMapping[projectType.toUpperCase()]
                : undefined;
        }
    })();
