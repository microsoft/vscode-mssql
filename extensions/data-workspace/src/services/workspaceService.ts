...
export class WorkspaceService implements IWorkspaceService {
  ...
  public async addProjectsToWorkspace(projectFiles: vscode.Uri[]): Promise<void> {
    ...
    const previousProjects: string[] = (await this.getProjectsInWorkspace(undefined, false)).map(
      (p) => p.path,
    );
    ...
    const updatedExcludedProjects = excludedProjects.filter(
      (excludedProj) =>
        !projectFiles.find((newProj) => vscode.workspace.asRelativePath(newProj) === excludedProj),
    );
    ...
    this.openedProjects = this.openedProjects.filter(
      (project) =>
        !this.excludedProjects?.find(
          (excludedProject) => excludedProject === vscode.workspace.asRelativePath(project),
        ),
    );
  ...
  async gitCloneProject(url: string, localClonePath: string): Promise<void> {
    ...
    const gitApi: any = (vscode.extensions.getExtension("vscode.git")!.exports as any).getAPI(1);
    const opts = {
    ...
    try {
      ...
      const repositoryPath = (await vscode.window.withProgress(opts, (progress, token) =>
        gitApi.clone(url!, { parentPath: localClonePath!, progress, recursive: true }, token),
      )) as string;
