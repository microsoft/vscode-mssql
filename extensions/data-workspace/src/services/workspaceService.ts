...
export class WorkspaceService implements IWorkspaceService {
    private _onDidWorkspaceProjectsChange: vscode.EventEmitter<void> =
        new vscode.EventEmitter<void>();
    readonly onDidWorkspaceProjectsChange: vscode.Event<void> =
        this._onDidWorkspaceProjectsChange?.event;
    ...
    constructor() {
        ...
        this.getProjectsInWorkspace(undefined, true).catch((err) =>
            Logger.error(`Error initializing projects in workspace ${err}`),
        );
    }
    ...
    public updateIfProjectProviderAvailable(): void {
        Logger.log(
            `Checking ${vscode.extensions.all.length} extensions to see if there is a project provider is available`,
        );
        const startTime = new Date().getTime();

        this._isProjectProviderAvailable = vscode.extensions.all.some(
            (e) => e.packageJSON.contributes?.projects?.length > 0,
        );
        Logger.log(
            `isProjectProviderAvailable is ${this._isProjectProviderAvailable}. Total time = ${new Date().getTime() - startTime}ms`,
        );
    }
    ...
    public async addProjectsToWorkspace(projectFiles: vscode.Uri[]): Promise<void> {
        ...
        for (const projectFile of projectFiles) {
            ...
            if (
                ...
                vscode.Uri.file(relativePath).fsPath === projectFile.fsPath
            ) {
        ...
        if (newWorkspaceFolders.length > 0) {
            ...
            vscode.workspace.updateWorkspaceFolders(
                ...
                undefined,
                ...newWorkspaceFolders.map((folder) => ({ uri: vscode.Uri.file(folder) })),
            );
        }
        ...
        const previousProjects: string[] = (
            await this.getProjectsInWorkspace(undefined, false)
        ).map((p) => p.path);
        ...
        for (const projectFile of projectFiles) {
            if (previousProjects.includes(projectFile.path)) {
                ...
                TelemetryReporter.createActionEvent(
                    TelemetryViews.WorkspaceTreePane,
        ...
        const excludedProjects = this.getWorkspaceConfigurationValue<string[]>(
            ExcludedProjectsConfigurationName,
        );
        const updatedExcludedProjects = excludedProjects.filter(
            (excludedProj) =>
                !projectFiles.find(
                    (newProj) => vscode.workspace.asRelativePath(newProj) === excludedProj,
                ),
        );
        if (excludedProjects.length !== updatedExcludedProjects.length) {
            await this.setWorkspaceConfigurationValue(
                ExcludedProjectsConfigurationName,
                updatedExcludedProjects,
            );
        }
     ...
     */
    public async getProjectsInWorkspace(
        ext?: string,
        refreshFromDisk: boolean = false,
    ): Promise<vscode.Uri[]> {
        Logger.log(`Getting projects in workspace`);
        const startTime = new Date().getTime();

        if (refreshFromDisk || this.openedProjects === undefined) {
            // always check if nothing cached
        ...
        this.excludedProjects = this.getWorkspaceConfigurationValue<string[]>(
            ExcludedProjectsConfigurationName,
        );
        this.openedProjects = this.openedProjects.filter(
            (project) =>
                !this.excludedProjects?.find(
                    (excludedProject) =>
                        excludedProject === vscode.workspace.asRelativePath(project),
                ),
        );
        ...
        Logger.log(
            `Finished looking for projects in workspace. Opened: ${this.openedProjects.length}. Excluded: ${this.excludedProjects.length}. Total time = ${new Date().getTime() - startTime}ms`,
        );

        // filter by specified extension
        if (ext) {
            return this.openedProjects.filter((p) =>
                p.fsPath.toLowerCase().endsWith(ext.toLowerCase()),
            );
        } else {
    ...
    private async refreshProjectsFromDisk(): Promise<void> {
        ...
        try {
            const projectPromises =
                vscode.workspace.workspaceFolders?.map((f) => this.getAllProjectsInFolder(f.uri)) ??
                [];
            const allProjects = (await Promise.all(projectPromises)).reduce(
    ...
    async getAllProjectsInFolder(folder: vscode.Uri): Promise<vscode.Uri[]> {
        ...
        const supportedProjectExtensions = [
            ...new Set(
                (await this.getAllProjectTypes()).map((p) => {
                    return p.projectFileExtension;
                }),
            ),
        ];
    ...
    ): Promise<dataworkspace.IProjectProvider | undefined> {
    ...
    async createProject(
        ...
        if (provider) {
            const projectFile = await provider.createProject(
                name,
                location,
                projectTypeId,
    ...
    async gitCloneProject(url: string, localClonePath: string): Promise<void> {
        // TODO: Fix git API types - Phase 2
        const gitApi: any = (vscode.extensions.getExtension("vscode.git")!.exports as any).getAPI(
            1,
        );
        const opts = {
        ...
        try {
            ...
            const repositoryPath = (await vscode.window.withProgress(opts, (progress, token) =>
                gitApi.clone(
                    url!,
                    { parentPath: localClonePath!, progress, recursive: true },
                    token,
                ),
            )) as string;
     ...
     */
    public async ensureProviderExtensionLoaded(
        projectType: string | undefined = undefined,
     ...
     */
    private async handleProjectProviderExtension(
        ...
        try {
            if (!extension.isActive) {
                if (forceActivate) {
                    ...
                    const projectTypes = extension.packageJSON.contributes?.projects as
                        | string[]
                        | undefined;
                    if (!projectTypes) {
                        return;
                    ...
                    for (const projType of projectTypes) {
                        const projFilesInWorkspace = await vscode.workspace.findFiles(
                            `**/*.${projType}`,
                        );
        ...
        if (
            extension.isActive &&
            extension.exports &&
            !ProjectProviderRegistry.providers.includes(extension.exports)
        ) {
    ...
    async removeProject(projectFile: vscode.Uri): Promise<void> {
        TelemetryReporter.createActionEvent(
            TelemetryViews.WorkspaceTreePane,
            TelemetryActions.ProjectRemovedFromWorkspace,
        )
            .withAdditionalProperties({
                projectType: path.extname(projectFile.fsPath),
            })
        ...
        await this.setWorkspaceConfigurationValue(ExcludedProjectsConfigurationName, [
            ...new Set(excludedProjects),
        ]);
        this._onDidWorkspaceProjectsChange.fire();
    }
    ...
    getWorkspaceConfigurationValue<T>(configurationName: string): T {
        return vscode.workspace
            .getConfiguration(WorkspaceConfigurationName)
            .get(configurationName) as T;
    }
