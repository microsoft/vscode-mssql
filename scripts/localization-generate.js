 ...
 */
async function generateRuntimeLocalizationForExtension(
  xliffPrefix,
  extensionPath,
) {
  ...
  try {
    ...
    logger.info(
      `Found ${xliffFiles.length} XLIFF files to process for ${xliffPrefix}`,
    );
      ...
      try {
        ...
        for (const fileContent of l10nDetailsArrayFromXlf) {
          if (fileContent.name === "bundle") {
            ...
            const formatted = await writeJsonAndFormat(
              filePath,
            ...
            if (formatted) {
              ...
              logger.warning(
                `Created bundle file: ${fileName} (formatting failed)`,
              );
            }
            generatedFiles++;
            ...
            if (fileContent.language === "enu") {
              logger.debug(
                "Skipping English package file (manually maintained)",
              );
              continue;
            }
            ...
            if (formatted) {
              ...
              logger.warning(
                `Created package file: ${fileName} (formatting failed)`,
              );
            }
            generatedFiles++;
          }
    ...
    logger.success(
      `Runtime localization generation for ${xliffPrefix} completed!`,
    );
    logger.info(
...
if (require.main === module) {
  ...
  if (!xliffPrefix) {
    logger.error(
      "Usage: node localization-generate.js <xliffPrefix> [extensionPath]",
    );
    logger.error(
      "Example: node localization-generate.js vscode-mssql ./extensions/mssql",
    );
    process.exit(1);
  }
