...
export interface CompileBatchOptions {
    /** Key columns to return so the caller can re-read server-generated row values. */
    readonly observationColumns?: readonly string[];
    /** Internal target used by compileBatch to collect all observations in one result set. */
...
function observationClause(
    ...
    const projection = columns
        .map((name) => {
            ...
            return `${source}.${quoteName(name)} AS ${quoteName(name)}`;
        })
        .join(", ");
    return target
        ? ` OUTPUT ${stringLiteral(String(edit.rowId))}, ${stringLiteral(edit.kind)}, ${columns
              .map((name) => `${source}.${quoteName(name)}`)
              .join(", ")} INTO ${target}`
        : ` OUTPUT ${stringLiteral(String(edit.rowId))} AS [__sql_edit_row_id], ${stringLiteral(edit.kind)} AS [__sql_edit_kind], ${projection}`;
}
...
export function compileInsert(
    ...
    const output = observationClause(
        table,
        edit,
        options?.observationColumns,
        "INSERTED",
        options?.observationTarget,
    );
    ...
    if (names.length === 0) {
        ...
        return {
            rowId: edit.rowId,
            kind: "insert",
            sql: guard(`INSERT INTO ${target}${output} DEFAULT VALUES`, edit, "insert"),
            guarded: true,
        };
    }
    ...
    return {
        ...
        kind: "insert",
        sql: guard(
            `INSERT INTO ${target} (${columns})${output} VALUES (${literals})`,
            edit,
            "insert",
        ),
        guarded: true,
...
export function compileUpdate(
    ...
    return {
        ...
        kind: "update",
        sql: guard(
            `UPDATE ${target} SET ${assignments}${output} WHERE ${predicate}`,
            edit,
            "update",
        ),
        guarded: true,
...
export function compileDelete(
    ...
    return {
        rowId: edit.rowId,
        kind: "delete",
        sql: guard(`DELETE FROM ${target}${output} WHERE ${predicate}`, edit, "delete"),
        guarded: true,
    };
}
...
export function compileEdit(
    ...
    switch (edit.kind) {
        case "insert":
            return compileInsert(table, edit, options);
        case "update":
            return compileUpdate(table, key, edit, options);
        case "delete":
            return compileDelete(table, key, edit, options);
    }
}
