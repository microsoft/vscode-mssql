...
import { compileBatch, parseFailure, scriptBatch } from "../../src/edit/dml/commit";
import {
    compileDelete,
    compileInsert,
    compileUpdate,
    orderEdits,
} from "../../src/edit/dml/compile";
import { compilePageQuery, compileSeek, resolvePageOrder } from "../../src/edit/query/select";
import { DEFAULT_VALUE } from "../../src/edit/types/valueCodec";
...
void test("an explicit default is compiled identically for insert and update", () => {
    const meta = table({
        columns: [
    ...
    const key = chooseKeyStrategy(meta);
    const insert = compileInsert(meta, {
        rowId: 1,
        kind: "insert",
        values: { Name: DEFAULT_VALUE },
    });
    const update = compileUpdate(meta, key, {
        rowId: 2,
        kind: "update",
        values: { Name: DEFAULT_VALUE },
        original: { Id: "7", Name: "old" },
    });
    assert.match(insert.sql, /VALUES \(DEFAULT\)/);
    assert.match(update.sql, /\[Name\] = DEFAULT/);
});
...
void test("an observed commit returns the server key for generated-row reconciliation", () => {
    const meta = table();
    const key = chooseKeyStrategy(meta);
    const batch = compileBatch(meta, key, [{ rowId: 9, kind: "insert", values: { Name: "new" } }], {
        observationColumns: key.columns,
    });
    assert.match(batch.sql, /DECLARE @sql_edit_observations TABLE/);
    assert.match(batch.sql, /OUTPUT N'9', N'insert', INSERTED\.\[Id\] INTO @sql_edit_observations/);
    assert.match(
        batch.sql,
        /SELECT \[__sql_edit_row_id\], \[__sql_edit_kind\], \[__sql_edit_key_0\]/,
    );
    assert.match(batch.sql, /FROM @sql_edit_observations/);
});
