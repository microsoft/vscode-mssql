...
export async function runCatalogQuery(
    ...
    if (!gateResult.allowed) {
        ...
        return { query, gate: gateResult, durationMs: 0 };
    }
    ...
    try {
        ...
        return { query, result, gate: gateResult, durationMs };
    } catch (error) {
        span.fail(error);
        const message = describeError(error, query);
        diag.emit({
            type: SqlDiagEvents.queryRun,
            status: "error",
            fields: { queryId: sys(query.id), error: userText(message) },
        });
        return { query, gate: gateResult, error: message, durationMs: Date.now() - started };
    }
}
 ...
 */
export function describeError(error: unknown, query: CatalogQuery): string {
    const raw = error instanceof Error ? error.message : String(error);
    const denied = /permission|principal|not have|denied/i.test(raw);

    if (denied && query.requiresPermission) {
        return `${query.title} needs the ${query.requiresPermission} permission. ${raw}`;
    }
    return raw;
}
