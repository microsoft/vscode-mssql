...
suite("T-SQL scanner and parser diagnostic coverage", () => {
    ...
    test("reports numeric precision overflow", () => {
        ...
        assert.deepEqual(parse(`SELECT ${value};`).diagnostics.map(({ message }) => message), [
    ...
    test("reports a decimal value where an integer is required", () => {
        assert.deepEqual(parse("SET TEXTSIZE 1.5;").diagnostics.map(({ message }) => message), [
    ...
    test("reports an invalid ODBC datetime option", () => {
        assert.deepEqual(parse("SELECT {xx '2020-01-01'};").diagnostics.map(({ message }) => message), [
...
SELECT 1 FOR JSON AUTO, TYPE;`;
        assert.deepEqual(parse(sql).diagnostics.map(({ message }) => message), [
...
function parse(sql) {
    return new LezerSyntaxService().parse(
        new ImmutableTextSnapshot("file:///diagnostic-coverage.sql", 1, sql),
    );
}
