...
async function hoverOn(sql, needle) {
...
suite("hover for names no symbol is bound to", () => {
    ...
    test("keeps quiet where there is nothing to say", async () => {
        assert.equal(await hoverOn("SELECT 1;", "1"), undefined);
        assert.equal(await hoverOn("SELECT 'text';", "'text'"), undefined);
        assert.equal(await hoverOn("SELECT 1;", "SELECT"), undefined);
    });
