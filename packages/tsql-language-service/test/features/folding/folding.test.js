...
const { fold, script } = require("../../support/foldingHarness.js");
...
suite("folding ranges", () => {
    ...
    test("ranges are sorted, uniquely started, and properly nested", () => {
        ...
        for (const range of ranges) {
            while (open.length > 0 && open.at(-1).end < range.start) open.pop();
            if (open.length > 0) assert.ok(range.end <= open.at(-1).end, "ranges must nest");
            open.push(range);
        }
    });
    ...
    test("ends stop at the last line of content, not on the blank lines after it", () => {
        ...
        assert.equal(syntax.document.positionAt(ranges[0].end).line, 1);
        const remainderOfLine = syntax.document.text.slice(ranges[0].end).split(/\r?\n/u)[0];
