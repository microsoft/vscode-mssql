...
`);
    ...
    // ScriptDOM audit-specification fixtures use action groups and object permission actions.
...
`);
    ...
    // ScriptDOM server-configuration fixtures cover bounded paths and affinity ranges.
...
`);
    ...
    // ScriptDOM Always Encrypted fixtures contain master-key and multi-value encryption-key DDL.
...
function parse(sql) {
    return new LezerSyntaxService().parse(
        new ImmutableTextSnapshot("file:///administrative-platform.sql", 1, sql),
    );
}
