...
`);
        ...
        assert.equal((snapshot.tree.toString().match(/BrokerPriorityStatement\(/gu) ?? []).length, 4);
    });
    ...
    test("reports a missing broker priority option value", () => {
        ...
        assert.ok(snapshot.diagnostics.some(({ message }) => /Incorrect syntax near '\)'\./u.test(message)));
    });
});

function parse(sql) {
    return new LezerSyntaxService().parse(
        new ImmutableTextSnapshot("file:///broker-priority.sql", 1, sql),
    );
}
