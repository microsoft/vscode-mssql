...
function runtime(deploymentMode) {
...
async function build(sql, options = {}) {
...
suite("T-SQL build-mode incremental equivalence", () => {
    const document = (text) => new ImmutableTextSnapshot("file:///build-incremental.sql", 1, text);
    ...
    test("matches a fresh analysis after an edit", async () => {
        ...
        const profile = { deploymentMode: "build" };
        ...
        const change = {
            range: { start: first.indexOf("int"), end: first.indexOf("int") + 3 },
            text: "geography",
        };
