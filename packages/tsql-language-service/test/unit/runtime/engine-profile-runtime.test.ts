...
suite("runtime engine profile", () => {
    ...
    test("reprofiles open documents without reparsing unchanged text", async () => {
        ...
        const countingSyntax: ProfileAwareSyntaxService = {
            get profile() {
        ...
        assert.deepEqual(
            republished.syntax.diagnostics.map(
                (diagnostic) =>
                    defined(diagnostic.availability, "expected availability details").featureId,
            ),
            ["statement.backup"],
        );
    });
