...
async function open(sql: string, provider = metadata()) {
...
suite("routine call matrix", () => {
    ...
    test("agrees between a fresh bind and an incremental one", async () => {
        ...
        assertDefined(incrementalCall, "expected the incrementally bound call");
        assertDefined(freshCall, "expected the freshly bound call");
    ...
    test("keeps unrelated batches stable across an argument-list edit", async () => {
        ...
        const editedCall = edited.semantics.model.callAt(untouched);
        const freshCall = fresh.semantics.model.callAt(untouched);
        assertDefined(editedCall, "expected the untouched incremental call");
        assertDefined(freshCall, "expected the untouched fresh call");
        assert.deepEqual(editedCall.target, freshCall.target);
    });
});
