...
async function open(sql: string, metadata = provider(), capabilities?: EngineCapabilities) {
...
function typesByText(snapshot: DocumentAnalysisSnapshot, sql: string): Map<string, ExpressionType> {
...
function requiredType(types: ReadonlyMap<string, ExpressionType>, text: string): ExpressionType {
    const type = types.get(text);
    assertDefined(type, `expected a type for ${text}`);
    return type;
}

function callName(call: ResolvedCall): string | undefined {
    if (call.target.kind === "catalog") return call.name?.object;
    if ("name" in call.target) return call.target.name;
    return undefined;
}
...
suite("shared semantic model", () => {
    ...
    test("publishes one relation for an aliased table source", async () => {
        ...
        assertDefined(relation.name, "expected the catalog relation name");
    ...
    test("keeps an unhydrated relation shape distinct from an empty one", async () => {
        ...
        assertDefined(relation, "expected the pending relation");
    ...
    test("publishes one call model for scalar, table-valued, conversion, and operator calls", async () => {
        ...
        const byName = new Map<string, ResolvedCall>();
        for (const call of calls) {
            const name = callName(call);
            if (name) byName.set(name, call);
        }

        const topCall = byName.get("TOP");
        const castCall = byName.get("CAST");
        const leftCall = byName.get("LEFT");
        assertDefined(topCall, "expected the TOP call");
        assertDefined(castCall, "expected the CAST call");
        assertDefined(leftCall, "expected the LEFT call");
        assert.equal(topCall.target.kind, "operator");
        assert.equal(topCall.shape, "keywordSeparated");
        assert.equal(castCall.target.kind, "builtin");
        assert.equal(castCall.shape, "keywordSeparated");
        assert.equal(leftCall.shape, "parenthesized");
    ...
    test("records one conversion role for CAST", async () => {
        ...
        assertDefined(call, "expected the CAST call");
        ...
        assertDefined(help, "expected CAST signature help");
        assertDefined(help.signatures[0], "expected a CAST signature");
    ...
    test("resolves a document-local object through the timeline", async () => {
        ...
        const afterDrop = timeline.resolve(parts, sql.lastIndexOf("dbo.Staging"));
        assertDefined(afterDrop, "expected the dropped object state");
        assert.equal(afterDrop.exists, false);
        const afterCreate = timeline.resolve(parts, sql.indexOf("DROP TABLE") - 1);
        assertDefined(afterCreate, "expected the created object state");
    ...
    test("binds literal, variable, column, conversion, and call types", async () => {
        ...
        const { snapshot } = await open(sql);
        const model = snapshot.semantics.model;
        const typeAt = (text: string): ExpressionType => {
            const type = model.typeAt(sql.indexOf(text));
            assertDefined(type, `expected a type for ${text}`);
            return type;
        };
    ...
    test("attributes a column type to the relation that exposed it", async () => {
        ...
        const relation = model.relationFor("c", sql.indexOf("c.Name"));
        assertDefined(type, "expected the column type");
        assertDefined(relation, "expected the source relation");
        assert.equal(type.sourceRelation, relation.id);
    });
    ...
    test("types operators by data type precedence", async () => {
        ...
        assert.equal(requiredType(typed, "1 + 2").displayName, "int");
        assert.equal(requiredType(typed, "1 + 1.5").displayName, "numeric");
        assert.equal(requiredType(typed, "c.Id + c.Amount").displayName, "money");
        assert.equal(requiredType(typed, "c.Id + c.Amount").nullable, true);
        assert.equal(requiredType(typed, "'a' + 'b'").displayName, "varchar");
        assert.equal(requiredType(typed, "N'a' + 'b'").displayName, "nvarchar");
    });
    ...
    test("types CASE from its result branches", async () => {
        ...
        assert.equal(requiredType(typed, "CASE WHEN 1 = 1 THEN 1 ELSE 2 END").displayName, "int");
        assert.equal(requiredType(typed, "CASE WHEN 1 = 1 THEN 1 ELSE 2 END").nullable, false);
        assert.equal(requiredType(typed, "CASE WHEN 1 = 1 THEN 1 END").nullable, true);
    });
    ...
    test("types a scalar subquery through its projected column", async () => {
        ...
        assertDefined(type, "expected the scalar subquery type");
    ...
    test("types a catalog scalar call from reported metadata", async () => {
        ...
        const { snapshot } = await open(sql);
        assert.equal(
            requiredType(typesByText(snapshot, sql), "dbo.Total()").displayName,
            "decimal(18,2)",
        );
    });
    ...
    test("binds references in the scopes the model publishes", async () => {
        ...
        const outer = model.scopeAt(sql.indexOf("o.Id"));
        assertDefined(outer, "expected the outer query scope");
    ...
    test("resolves local DDL identically for diagnostics and features", async () => {
        ...
        const timeline = snapshot.semantics.model.timeline;
        const beforeDrop = timeline.resolve(["dbo", "Staging"], sql.indexOf("DROP") - 1);
        const afterDrop = timeline.resolve(["dbo", "Staging"], sql.length);
        assertDefined(beforeDrop, "expected the pre-drop object state");
        assertDefined(afterDrop, "expected the post-drop object state");
        assert.equal(beforeDrop.exists, true);
        assert.equal(afterDrop.exists, false);
    });
    ...
    test("keeps object namespaces apart in the shared timeline", async () => {
        ...
        const table = timeline.resolve(["dbo", "Shared"], end, ["table"]);
        const procedure = timeline.resolve(["dbo", "Shared"], end, ["procedure"]);
        assertDefined(table, "expected the local table");
        assertDefined(procedure, "expected the local procedure");
...
suite("identifier handling", () => {
    ...
    test("round-trips bracketed, quoted, and omitted-part names", () => {
        ...
        assertDefined(quoted.parts[0], "expected the quoted identifier component");
