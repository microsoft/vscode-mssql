...
import type { DocumentAnalysisSnapshot, ExpressionType } from "../../../../src/index.ts";
...
suite("member expression types", () => {
    ...
    test("types XML methods from the method and its written result type", async () => {
        ...
        assert.equal(requiredType(typed, "@body.value('(/a)[1]', 'int')").displayName, "int");
        assert.equal(requiredType(typed, "@body.query('/a')").displayName, "xml");
        assert.equal(requiredType(typed, "@body.exist('/a')").displayName, "bit");
        // A column receiver reaches the same conclusion as a variable receiver: `xml` is `xml`
        // however it was declared.
        assert.equal(
            requiredType(typed, "d.Body.value('(/a)[1]', 'nvarchar(20)')").displayName,
            "nvarchar(20)",
        );
    });
    ...
    test("types a CLR member from its reported type", async () => {
        ...
        assert.equal(requiredType(typed, "@location.Latitude").displayName, "float");
    ...
    test("does not treat a non-XML receiver as XML", async () => {
        ...
        const { typed } = await open(sql);
        assert.equal(requiredType(typed, "dbo.Total(@count)").displayName, "decimal(18,2)");
    });
    ...
    test("reads a scalar routine's return value without demanding it as an argument", async () => {
        ...
        assert.equal(requiredType(typed, "dbo.Total(1)").displayName, "decimal(18,2)");
...
suite("rowset-producing operators", () => {
    ...
    test("publishes an XML nodes() rowset as a relation", async () => {
        ...
        assert.deepEqual(
            relation.columns.map(({ name, type }) => {
                assertDefined(type, `expected a type for ${name}`);
                return { name, type: type.displayName };
            }),
...
suite("member completion", () => {
    async function membersAt(sql: string, marker: string): Promise<string[]> {
