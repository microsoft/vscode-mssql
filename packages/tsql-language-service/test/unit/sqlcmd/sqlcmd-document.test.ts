...
function parse(text: string, host?: SqlCmdHost): SqlCmdDocumentSnapshot {
...
suite("SQLCMD directives", () => {
    ...
    test("projects plain SQL unchanged", () => {
        ...
        assert.equal(defined(snapshot.toSource(3), "expected a source position").offset, 3);
    ...
    test("recognizes every documented directive", () => {
        ...
        assert.equal(
            defined(snapshot.directives.at(-1), "expected the final directive").batchCount,
            5,
        );
    ...
    test("never executes a shell command", () => {
        ...
        const diagnostic = defined(allowed.diagnostics[0], "expected a shell diagnostic");
        assert.equal(diagnostic.code, "SqlCmdShellCommandNotExecuted");
...
suite("SQLCMD variables", () => {
    ...
    test("applies seed, assignment, and removal in order", () => {
        ...
        assert.equal((snapshot.variables as { readonly set?: unknown }).set, undefined);
    });
    ...
    test("keeps unresolved references verbatim", () => {
        ...
        assert.equal(
            defined(snapshot.diagnostics[0], "expected an unresolved variable diagnostic").code,
            "SqlCmdUnresolvedVariable",
        );
    });
    ...
    test("maps a length-changing substitution back to its reference", () => {
        ...
        const source = defined(
            snapshot.toSource(projectedName + 4),
            "expected a mapped source position",
        );
...
suite("SQLCMD includes", () => {
    function includeHost(files: Record<string, string>, policy?: SqlCmdPolicy): SqlCmdHost {
        return {
    ...
    test("splices an include and maps ranges into it", () => {
        ...
        const source = defined(
            snapshot.toSource(childOffset),
            "expected an included source position",
        );
    ...
    test("threads variable state through nested includes", () => {
        ...
        assert.equal(defined(snapshot.includes[1], "expected the nested include").depth, 1);
    });
    ...
    test("detects an include cycle", () => {
        ...
        assert.equal(
            defined(snapshot.diagnostics[0], "expected an include cycle diagnostic").code,
            "SqlCmdIncludeCycle",
        );
    });
    ...
    test("enforces depth, count, and size limits", () => {
        ...
        assert.equal(
            defined(
                parse(":r a.sql\n", deep).diagnostics[0],
                "expected an include depth diagnostic",
            ).code,
        ...
        assert.equal(
            defined(parse(":r a.sql\n", big).diagnostics[0], "expected an include size diagnostic")
                .code,
        ...
        assert.equal(
            defined(
                parse(":r a.sql\n:r a.sql\n", many).diagnostics[0],
                "expected an include count diagnostic",
            ).code,
    ...
    test("refuses an include whose name is still unresolved", () => {
        ...
        assert.equal(
            defined(snapshot.diagnostics[0], "expected an unresolved include diagnostic").code,
            "SqlCmdUnresolvedInclude",
        );
        assert.equal(
            defined(snapshot.includes[0], "expected an unresolved include").state,
            "unresolvedReference",
        );
    });
});
...
suite("SQLCMD connection regions", () => {
    ...
    test("opens a region per connect and keeps earlier regions", () => {
        ...
        assert.equal(
            defined(snapshot.connectionRegionAt(first), "expected the first region").index,
            0,
        );
        const secondRegion = defined(
            snapshot.connectionRegionAt(second),
            "expected the second region",
        );
        assert.equal(secondRegion.index, 1);
        assert.equal(secondRegion.connectionId, "conn-1");
        const boundary = defined(snapshot.connectionRegions[0], "expected the initial region").range
            .end;
        assert.equal(
            defined(snapshot.connectionRegionAt(boundary), "expected the boundary region").index,
            1,
        );
    });
    ...
    test("never carries a password into an argument value", () => {
        ...
        const directive = defined(snapshot.directives[0], "expected a connect directive");
    ...
    test("reports a connection the host does not know", () => {
        ...
        assert.equal(
            defined(snapshot.diagnostics[0], "expected an unknown connection diagnostic").code,
            "SqlCmdUnknownConnection",
        );
        assert.equal(
            defined(snapshot.connectionRegions.at(-1), "expected the final connection region")
                .connectionId,
            undefined,
        );
    });
...
suite("SQLCMD incremental updates", () => {
    const service = (): SqlCmdDocumentService => new SqlCmdDocumentService(fullHost());

    function fullHost(): SqlCmdHost {
        return {
            includes: new MemorySqlCmdIncludeStore([
                ["file:///c:/scripts/child.sql", "SELECT child;\n"],
            ]),
    ...
    function applyEdit(text: string, change: TextChange): string {
        return text.slice(0, change.start) + change.text + text.slice(change.end);
    }

    function assertEquivalent(before: string, change: TextChange): SqlCmdDocumentSnapshot {
...
suite("SQLCMD completion", () => {
    ...
    test("completes directive names", () => {
        ...
        assert.deepEqual(defined(result.items[0], "expected the first completion item").edit, {
            start: 0,
            end: 3,
            newText: ":serverlist",
        });
