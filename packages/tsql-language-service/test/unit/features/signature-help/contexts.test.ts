...
async function optionalHelpAt(template: string) {
...
async function helpAt(template: string) {
    return requiredSignatureHelp(await optionalHelpAt(template));
}
...
suite("signature help contexts", () => {
    ...
    test("answers while the target columns of an INSERT are being named", async () => {
        ...
        assert.match(defined(first.signatures[0].documentation), /Name the target columns/u);
    ...
    test("keeps naming columns apart from supplying values", async () => {
        ...
        assert.match(defined(values.signatures[0].documentation), /Each VALUES expression/u);
    });

    test("stops answering once the column list is closed", async () => {
        assert.equal(
            await optionalHelpAt("INSERT INTO dbo.Customers (Id) ^VALUES (1);"),
            undefined,
        );
    });
    ...
    test("says nothing for a name it cannot describe", async () => {
        assert.equal(await optionalHelpAt("SELECT dbo.not_a_routine(^);"), undefined);
    });
});
