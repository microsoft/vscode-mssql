...
import { defined, requiredSignatureHelp } from "../../support/assertions.ts";
...
suite("T-SQL signature help", () => {
    ...
    test("describes catalog procedure parameters and follows named arguments", async () => {
        ...
        const help = requiredSignatureHelp(
            features.signatureHelp("file:///procedure-signature.sql", 1, sql.length),
        );
        ...
        assert.match(defined(help.signatures[0].parameters[1]).documentation ?? "", /optional/u);
        assert.match(
            defined(help.signatures[0].parameters[2]).documentation ?? "",
            /Input\/output/u,
        );
    });
    ...
    test("states the return contract of the routine being called", async () => {
        ...
        assert.equal(
            requiredSignatureHelp(
                features.signatureHelp("file:///return-contract.sql", 1, procedure.length),
    ...
    test("describes catalog function parameters", async () => {
        ...
        const help = requiredSignatureHelp(
            features.signatureHelp("file:///function-signature.sql", 1, sql.length),
        );
        assert.equal(help.activeParameter, 1);
        assert.deepEqual(
    ...
    test("uses explicit INSERT column order for value hints", async () => {
        ...
        const help = requiredSignatureHelp(
            features.signatureHelp("file:///insert-explicit-signature.sql", 1, sql.length),
        );
        assert.equal(help.activeParameter, 1);
        assert.deepEqual(
            help.signatures[0].parameters.map(({ label }) => label),
    ...
    test("uses default insertable table order for INSERT value hints", async () => {
        ...
        const help = requiredSignatureHelp(
            features.signatureHelp("file:///insert-default-signature.sql", 1, sql.length),
        );
        assert.deepEqual(
            help.signatures[0].parameters.map(({ label }) => label),
    ...
    test("describes built-in JSON function parameters", async () => {
        ...
        const help = requiredSignatureHelp(
            features.signatureHelp("file:///json-signature.sql", 1, sql.length),
        );
        assert.equal(help.activeParameter, 1);
...
SELECT dbo.LocalFunction(1, `;
        ...
        const procedure = requiredSignatureHelp(
            features.signatureHelp("file:///local-signatures.sql", 1, procedureOffset),
        );
        const routine = requiredSignatureHelp(
            features.signatureHelp("file:///local-signatures.sql", 1, sql.length),
        );
...
function object(id: string, name: string, kind: SqlObjectKind): ObjectMetadata {
    return {
        ref: { id, database: "db" },
        database: "db",
        schema: "dbo",
