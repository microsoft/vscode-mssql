...
async function open(sql: string, metadata = catalog()) {
...
suite("object definition providers", () => {
    const descriptor: ObjectDefinitionDescriptor = {
        database: "db",
        schema: "dbo",
        name: "Customers",
        kind: "table",
    ...
    test("the offline provider answers nothing", async () => {
        assert.equal(await new NullObjectDefinitionProvider().getDefinition(), undefined);
    });
    ...
    test("one caller giving up leaves the fetch running for the others", async () => {
        ...
        const givingUp = cached.getDefinition(request, abandoned.signal);
        const waiting = cached.getDefinition(request);
        abandoned.abort();

        await assert.rejects(() => givingUp, /cancelled/u);
        release();
