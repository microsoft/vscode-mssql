...
suite("T-SQL lexical and query grammar foundation", () => {
    // Verifies the checked-in SqlParser vocabulary includes global, contextual, and SQL 2025 words.
...
`);
    ...
    // Verifies malformed token diagnostics match SqlParser's message, severity, and exact span.
    test("reports SqlParser-compatible syntax diagnostics at a bad token", () => {
    ...
    // Verifies an incomplete multipart name uses SqlParser's EOF message and zero-width range.
    test("reports the SqlParser expectation for a multipart identifier at EOF", () => {
...
function parse(sql) {
    return new LezerSyntaxService().parse(new ImmutableTextSnapshot("file:///grammar.sql", 1, sql));
}
