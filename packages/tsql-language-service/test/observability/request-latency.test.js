...
suite("request latency recorder", () => {
    test("summarises only the methods that were called", () => {
        ...
        assert.equal(summary.completion.count, 2);
        assert.equal(summary.hover.count, 1);
        assert.equal(
            summary.definition,
            undefined,
    ...
    test("reports a percentile that is a sample which actually occurred", () => {
        ...
        const { p50Ms, p95Ms, maximumMs } = recorder.summary().completion;
    ...
    test("counts a stale request without counting it as an answer", () => {
        ...
        const summary = recorder.summary();
        assert.equal(summary.completion.staleDiscarded, 1);
        assert.equal(summary.completion.count, 0, "a discarded result is not a served request");
    ...
    test("still times a request that failed for any other reason", () => {
        ...
        assert.equal(recorder.summary().hover.count, 1, "a slow error path must not look fast");
        assert.equal(recorder.summary().hover.staleDiscarded, 0);
    });
    ...
    test("reports cancellation as zero because it cannot be seen from here", () => {
        ...
        assert.equal(recorder.summary().completion.cancelled, 0);
    });
});
...
suite("feature latency reaches the published statistics", () => {
    test("a completion request is measured and reported", async () => {
        ...
        const { latency } = runtime.getStats("file:///latency.sql").requests;
        assert.equal(latency.completion.count, 1);
        assert.equal(latency.hover.count, 1);
        assert.ok(latency.completion.p95Ms >= 0);
    });
    ...
    test("a request against a superseded version is discarded and counted", async () => {
        ...
        assert.equal(runtime.getStats("file:///stale.sql").requests.staleResultsDiscarded, 1);
    });
});
