...
/**
 * Finds redundant recovery tokens around a line-leading parenthesized SELECT. Keeping this bounded
 * scanner outside the LR grammar avoids making every parenthesized expression a statement starter.
 */
export function findGroupedSelectWrapperOffsets(text: string): ReadonlySet<number> {
    const result = new Set<number>();
    let lineStart = 0;
    while (lineStart < text.length) {
        let start = lineStart;
        while (isHorizontalSpace(text.charCodeAt(start))) start++;
        if (text.charCodeAt(start) === 40) collectGroupedSelectAt(text, start, result);
...
function collectGroupedSelectAt(text: string, start: number, result: Set<number>): void {
    const wrapperStarts: number[] = [];
    let offset = start;
    while (text.charCodeAt(offset) === 40) {
        wrapperStarts.push(offset++);
        while (isSqlWhitespace(text.charCodeAt(offset))) offset++;
    }
    if (!matchesWord(text, offset, "select")) return;

    const wrapperSet = new Set(wrapperStarts);
    const stack: boolean[] = [];
    ...
    while (offset < text.length) {
        ...
        if (current === 40) {
            stack.push(wrapperSet.has(offset));
        } else if (current === 41) {
            const wrapper = stack.pop();
            if (wrapper === undefined) return;
            if (wrapper) wrapperEnds.push(offset);
            if (stack.length === 0) break;
        }
        offset++;
    }
    if (wrapperEnds.length !== wrapperStarts.length) return;
    for (const wrapper of wrapperStarts) result.add(wrapper);
    for (const wrapper of wrapperEnds) result.add(wrapper);
}

function matchesWord(text: string, start: number, expected: string): boolean {
    if (text.slice(start, start + expected.length).toLocaleLowerCase() !== expected) return false;
    const follower = text.charCodeAt(start + expected.length);
    return !isIdentifierCode(follower);
}
