...
const parserLocalContextWords = new Set(
    `abort action always ansidefaults ansinulldfltoff ansinulldflton ansinulls ansipadding cast
     ...
     filestreamon filetable fanin first fmtonly following forceplan forwardonly function generated global graph
     fulltextstoplist grouping groups hidden high identityinsert filestream columnset allsparsecolumns implicittransactions include increment io isolation json keyset
     ...
     maxdop numericroundabort notification off offset only optimistic out output parameters parseonly partition partitions pause
     ...
     policy pool privileges raw readtext reconfigure reset rename resource role rule scheduler security secret secondary selective sent service session singleton softnuma spatial specification sql state statisticalsemantics stop stoplist symmetric system schemabinding timestamp updatetext writetext
