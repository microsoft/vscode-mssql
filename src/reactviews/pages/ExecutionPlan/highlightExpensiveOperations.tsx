...
export const HighlightExpensiveOperations: React.FC<HighlightExpensiveOperationsProps> = ({
	...
	const handleHighlightExpensiveOperation = async () => {
		if (executionPlanView) {
			const enumSelected = highlightMetricOptionsEnum[highlightMetricOptions.indexOf(highlightMetricSelected)];
			const expensiveOperationDelegate: (cell: ep.AzDataGraphCell) => number | undefined = getExpensiveOperationDelegate(enumSelected)!;
			executionPlanView.clearExpensiveOperatorHighlighting();
			const elementId = executionPlanView.highlightExpensiveOperator(expensiveOperationDelegate);
			if (elementId) {
				executionPlanView.centerElement(executionPlanView.getElementById(elementId)!);
			}
	...
	return (
		<div id="highlightExpensiveOpsContainer" className={classes.inputContainer} style={{background:utils.iconBackground(executionPlanState!.theme!)}}>
			...
			<Button onClick={handleHighlightExpensiveOperation} icon={<Checkmark20Regular />} />
			<Button icon={<Dismiss20Regular />} onClick={handleHighlightClose}/>
		</div>
...
function getExpensiveOperationDelegate(selectedExpensiveOperationType: ep.ExpensiveMetricType): (cell: ep.AzDataGraphCell) => number | undefined {
	const getElapsedTimeInMs = (cell: ep.AzDataGraphCell): number | undefined => cell.elapsedTimeInMs;
