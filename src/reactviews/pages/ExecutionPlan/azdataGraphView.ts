...
import { badgeIconPaths, collapseExpandNodeIconPaths, executionPlanNodeIconPaths } from './constants';
import { v4 as generateUuid } from 'uuid';
const azdataGraph = require('azdata')();
...
export class AzdataGraphView {
	...
	constructor(
		...
		private _executionPlan: ep.ExecutionPlanGraph,
		...
		const temp = azdataGraph;
		temp;
		// Cannot read properties of undefined (reading azdataQueryPlan)
		this._diagram = azdataGraph.azdataQueryPlan(queryPlanConfiguration);
		(<any>this._parentContainer.firstChild).ariaLabel = `${executionPlanDiagramName}, use arrow keys to navigate between nodes`;
	 ...
	 */
	public searchNodes(searchQuery: SearchQuery): ep.ExecutionPlanNode[] {
	...
	private populate(node: ep.ExecutionPlanNode): AzDataGraphCell {
		let diagramNode: AzDataGraphCell = <AzDataGraphCell>{};
		diagramNode.label = 'TBD';
	...
	private populateProperties(props: ep.ExecutionPlanGraphElementProperty[] | undefined): AzDataGraphCellMetric[] {
		if (!props) {
			return [];
		}
...
export interface InternalExecutionPlanEdge extends ep.ExecutionPlanEdge {
	/**
...
export type InternalExecutionPlanElement = InternalExecutionPlanEdge | ep.ExecutionPlanNode;
