...
export let releaseNotesPromptDescription = l10n.t('View mssql for Visual Studio Code release notes?');
...
export let msgRunQueryInProgress = l10n.t('A query is already running for this editor session. Please cancel this query or wait for its completion.');
...
export let msgChooseDatabaseNotConnected = l10n.t('No connection was found. Please connect to a server first.');
...
export let azureServerCouldNotStart = l10n.t('Server could not start. This could be a permissions error or an incompatibility on your system. You can try enabling device code authentication from settings.');
export let azureAuthNonceError = l10n.t('Authentication failed due to a nonce mismatch, please close Azure Data Studio and try again.');
export let azureAuthStateError = l10n.t('Authentication failed due to a state mismatch, please close ADS and try again.');
...
export let accountRemovedSuccessfully = l10n.t('Selected Microsoft Entra account removed successfully.');
...
export let noAzureAccountForRemoval = l10n.t('No Microsoft Entra account can be found for removal.');
export let clearedAzureTokenCache = l10n.t('Azure token cache cleared successfully.');
export let cannotConnect = l10n.t('Cannot connect due to expired tokens. Please re-authenticate and try again.');
...
export let msgSavePassword = l10n.t('Save Password? If \'No\', password will be required each time you connect');
export let profileNamePrompt = l10n.t('Profile Name');
export let profileNamePlaceholder = l10n.t('[Optional] Enter a display name for this connection profile');
...
export let extensionNotInitializedError = l10n.t('Unable to execute the command while the extension is initializing. Please try again later.');
export let untitledScheme = l10n.t('untitled');
export let msgChangeLanguageMode = l10n.t('To use this command, you must set the language to "SQL". Confirm to change language mode.');
...
export let msgPromptRetryCreateProfile = l10n.t('Error: Unable to connect using the connection information provided. Retry profile creation?');
export let refreshTokenLabel = l10n.t('Refresh Credentials');
export let msgGetTokenFail = l10n.t('Failed to fetch user tokens.');
export let msgPromptRetryConnectionDifferentCredentials = l10n.t('Error: Login failed. Retry using different credentials?');
export let msgPromptSSLCertificateValidationFailed = l10n.t('Encryption was enabled on this connection, review your SSL and certificate configuration for the target SQL Server, or set \'Trust server certificate\' to \'true\' in the settings file. Note: A self-signed certificate offers only limited protection and is not a recommended practice for production environments. Do you want to enable \'Trust server certificate\' on this connection and retry?');
export let msgPromptRetryFirewallRuleNotSignedIn = l10n.t('Your client IP address does not have access to the server. Add a Microsoft Entra account and create a new firewall rule to enable access.');
...
export let msgPromptRetryFirewallRuleAdded = l10n.t('Firewall rule successfully added. Retry profile creation? ');
export let msgAccountRefreshFailed = l10n.t('Credential Error: An error occurred while attempting to refresh account credentials. Please re-authenticate.');
export let msgPromptProfileUpdateFailed = l10n.t('Connection Profile could not be updated. Please modify the connection details manually in settings.json and try again.');
...
export let msgAuthTypeNotFound = l10n.t('Failed to get authentication method, please remove and re-add the account.');
...
export function msgConnectedServerInfo(serverName: string, documentName: string, serverInfo: string) {
	return l10n.t({
...
export function msgChangingDatabase(databaseName: string, serverName: string, documentName: string) {
	return l10n.t({
...
export let macSierraRequiredErrorMessage = l10n.t('macOS Sierra or newer is required to use this feature.');
...
export let testalizationConstant = l10n.t('test');
...
export let msgCannotSaveMultipleSelections = l10n.t('Save results command cannot be used with multiple selections.');
...
export let flavorDescriptionMssql = l10n.t('Use T-SQL intellisense and syntax error checking on current document');
export let flavorDescriptionNone = l10n.t('Disable intellisense and syntax error checking on current document');
...
export let azureSignInWithDeviceCodeDescription = l10n.t('Sign in to your Azure subscription with a device code. Use this in setups where the Sign In command does not work');
export let azureSignInToAzureCloud = l10n.t('Azure: Sign In to Azure Cloud');
export let azureSignInToAzureCloudDescription = l10n.t('Sign in to your Azure subscription in one of the sovereign clouds.');
...
export let msgClearedRecentConnectionsWithErrors = l10n.t('The recent connections list has been cleared but there were errors while deleting some associated credentials. View the errors in the MSSQL output channel.');
export let connectProgressNoticationTitle = l10n.t('Testing connection profile...');
export let msgMultipleSelectionModeNotSupported = l10n.t('Running query is not supported when the editor is in multiple selection mode.');
...
export let objectExplorerNodeRefreshError = l10n.t('An error occurred refreshing nodes. See the MSSQL output channel for more details.');
export let showOutputChannelActionButtonText = l10n.t('Show MSSQL output');
export let reloadPrompt = l10n.t('Authentication Library has changed, please reload Visual Studio Code.');
export let reloadPromptGeneric = l10n.t('This setting requires Visual Studio Code to be relaunched, please reload Visual Studio Code.');
