    ...
    constructor(private protocol: IMessageProtocol, private handler: any, isClient: boolean = false) {
        if (!isClient) {
            const first = protocol.onMessage(message => {
                ...
                if (message === 'ready') {
                    // sanity check
                    protocol.onMessage(this.onRecieve);
                    first.dispose();
                    this.ready.resolve();
                }
            });
...
export function createProxy(protocol: IMessageProtocol, handler: IServerProxy): IWebviewProxy;
export function createProxy(protocol: IMessageProtocol, handler: IWebviewProxy): IServerProxy;
export function createProxy(protocol: IMessageProtocol, handler: any): any {
    const messageProxy = new MessageProxy(protocol, handler);
    let proxy = {
