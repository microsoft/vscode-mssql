	...
	return new Promise<NetworkResponse<T>>(((resolve, reject) => {
		...
		request.on('connect', (response, socket) => {
			...
			socket.on('end', () => {
				...
				const httpStatusCode = parseInt(dataStringArray[0].split(' ')[1], 2);
				...
				const entryHeaders = entries.entries as Object as Record<string, string>;
				const networkResponse = NetworkUtils.getNetworkResponse(
					entryHeaders,
					parseBody(httpStatusCode, statusMessage, entryHeaders, body) as T,
					httpStatusCode
				);
	...
	return new Promise<NetworkResponse<T>>((resolve, reject) => {
		...
		request.on('response', (response) => {
			...
			response.on('end', () => {
				...
				const networkResponse = NetworkUtils.getNetworkResponse(
					parsedHeaders,
					parseBody(statusCode, statusMessage, parsedHeaders, Buffer.concat([...data]).toString()) as T,
					statusCode
				);
