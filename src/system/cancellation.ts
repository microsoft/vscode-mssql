		...
		if (results.some(([, r]) => r.paused)) {
			async function getContinuationValue() {
				...
				for (const [id, result] of results) {
					completed.set(id, { value: <any>(result.paused ? await result.value : result.value), paused: false });
				}
...
export async function pauseOnCancelOrTimeoutMapPromise<Id, T>(
	...
	const mapPromise = source.then(m =>
		m == null ? <any>m : pauseOnCancelOrTimeoutMap(m, ignoreErrors, cancellation, timeout, continuation),
	);
...
export async function pauseOnCancelOrTimeoutMapTuplePromise<Id, T, U extends unknown[]>(
	...
	const mapPromise = source.then(m =>
		m == null ? <any>m : pauseOnCancelOrTimeoutMapTuple(m, cancellation, timeout, continuation),
	);
