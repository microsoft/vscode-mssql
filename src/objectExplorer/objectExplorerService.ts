  ...
  private handleSessionCreatedNotification(): NotificationHandler<SessionCreatedParameters> {
    ...
    const handler = async (result: SessionCreatedParameters) => {
      ...
      if (result.success) {
        ...
        let nodeConnection = this._sessionIdToConnectionCredentialsMap.get(
          result.sessionId,
        );
        for (let connection of savedConnections) {
          if (
            Utils.isSameConnection(connection.connectionCreds, nodeConnection)
          ) {
        ...
        if (
          ...
          nodeLabel = !nodeLabel
            ? getConnectionDisplayName(self._currentNode.connectionInfo)
            : nodeLabel;
  ...
  private handleExpandSessionNotification(): NotificationHandler<ExpandResponse> {
    ...
    const handler = (result: ExpandResponse) => {
      if (result && result.nodes) {
        const credentials = self._sessionIdToConnectionCredentialsMap.get(
          result.sessionId,
        );
  ...
  public async createSession(
    ...
    if (connectionCredentials) {
      ...
      if (connectionCredentials.connectionString) {
        ...
        if (
          ConnectionCredentials.isPasswordBasedCredential(connectionCredentials)
        ) {
          ...
          if (Utils.isEmpty(password)) {
            ...
            if (!(<IConnectionProfile>connectionCredentials).savePassword) {
              ...
              if (
                connectionCredentials.authenticationType !== Constants.azureMfa
              ) {
