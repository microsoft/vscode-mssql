				...
				if (result.messageType === MessageType.Complete || result.messageType === MessageType.Fragment) {
					...
					for await (const part of chatResponse.stream) {
						if (part instanceof vscode.LanguageModelChatResponseTextPart) {
							...
							// for (const message of result.requestMessages) {
							// 	if (message.text.indexOf(tool.functionName) >= 0) {
							...
							// // IMPORTANT
							// messages.push(vscode.LanguageModelChatMessage.User(`Above is the result of calling the function ${tool.functionName}. The user cannot see this result, so you should explain it to the user if referencing it in your answer.`));
