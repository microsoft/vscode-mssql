...
import { getNonce } from '../env/node/crypto';
	...
	implements WebviewHost<Descriptor>, Disposable
{
	...
	dispose() {
		this._disposed = true;
		resetContextKeys(<any>this.descriptor.contextKeyPrefix);
...
export function resetContextKeys(
	contextKeyPrefix: `mssql:webview:${WebviewTypes | CustomEditorTypes}` | `mssql:webviewView:${WebviewViewTypes}`,
): void {
