...
import {
    ...
    IApiClientResponse,
    IFabricApiClient,
} from "../api/fabricApiClient";
import { doFabricAction } from "../util/fabricError";
import { FabricEnvironmentName } from "../util/fabricEnvironments";
...
export class FabricApiClient implements IFabricApiClient {
    ...
    constructor(private vscodeWrapper: VscodeWrapper) {
        this.logger = Logger.create(vscodeWrapper.outputChannel, "FabricApiClient");
    }
    ...
    public async sendRequest(options: IApiClientRequestOptions): Promise<IApiClientResponse> {
        ...
        return await doFabricAction(
            // { fabricLogger: this.logger, telemetryActivity: activity },
            async () => {
                ...
                if (!token) {
                    // if we're using a pathTemplate, then the base is from Fabric Environment and we need the token. Also needed for rundebug, where we use the invocation endpoint
                    token = await this.getToken();
                }
                if (token) {
                    // no token means we're not using the Fabric Environment or from a test
                    tokenToUse = `${tokenType} ${token}`;
                    // eslint-disable-next-line @typescript-eslint/naming-convention
                    curHeaders["Authorization"] = tokenToUse;
                }
                if (options.url === undefined) {
                    // if we're using one of the fabric endpoints, add the originating app header so back end can add telemetry
                    // eslint-disable-next-line @typescript-eslint/naming-convention
                ...
                if (options.formData) {
                    // For more details, see Task 1330509: Determine why deployments consistently fail on 1st try, and require retry
                    for (const key in options.formData) {
                        if (Object.prototype.hasOwnProperty.call(options.formData, key)) {
                            ...
                            if (typeof element === "string" && element.startsWith("File:")) {
                                ...
                                options.formData[key] = azApi.createFileFromStream(
                                    () => strm,
                                    baseFilename,
                                );
                            }
                ...
                if (azApiresponse.status >= 300) {
                    activity.addOrUpdateProperties({
                        bodyAsText: azApiresponse.bodyAsText ?? "No body",
                    });
                }
                activity.addOrUpdateProperties({
                    statusCode: azApiresponse.status.toString(),
                    apiTimeout: req.timeout.toString(),
                ...
                if (debugLogging) {
                    ...
                    this.logger.log(
                        `    API Call Status (${elapsedms}ms) = ${azApiresponse.status}  RootActivityId = ${rootActivityId} RequestId = ${requestId}`,
                    );
                ...
                if (azApiresponse.bodyAsText) {
                    ...
                    if (azApiresponse.headers.get("Content-Type")?.startsWith("application/json")) {
                        // if it says we have JSON, then parse it
                        try {
                        ...
                        if (
                            ...
                            // ONEBOX, EDOG, or EDOGONEBOX
                            try {
                                response.parsedBody = JSON.parse(azApiresponse.bodyAsText);
                                this.logger.log(
                                    `ONEBOX: Parsed body as JSON ContentType = ${azApiresponse.headers.get("Content-Type")}: ${azApiresponse.bodyAsText}`,
                                );
                            } catch (error) {
                                this.logger.log(
                                    `ONEBOX: exception parsing bodyAsText as JSON. ContentType = ${azApiresponse.headers.get("Content-Type")}: ${azApiresponse.bodyAsText}`,
                                );
                                // don't throw!!
                            }
                        }
