...
> {
  ...
  private async initializeDialog() {
    ...
    this.state.connectionFormComponents =
      await this.generateConnectionFormComponents();
    this.state.connectionStringComponents =
      await this.generateConnectionStringComponents();
  ...
  private getFormComponent(
    ...
    return this.getActiveFormComponents().find(
  ...
  private async completeFormComponents(
    components: Map<
      string,
      {
        ...
        component: FormItemSpec<IConnectionDialogProfile>;
      }
    >,
  ) {
  ...
  private async generateConnectionFormComponents(): Promise<{
    ...
    advancedComponents: {
      [category: string]: FormItemSpec<IConnectionDialogProfile>[];
    };
  }> {
    ...
    const allConnectionFormComponents = new Map<
      string,
      {
        option: ConnectionOption;
        component: FormItemSpec<IConnectionDialogProfile>;
    ...
    for (const option of connectionOptions) {
      allConnectionFormComponents.set(option.name, {
        option,
        component: this.convertToFormComponent(option),
      });
    }
    ...
    for (const [
      optionName,
      { option, component },
    ] of allConnectionFormComponents) {
  ...
  private async generateConnectionStringComponents(): Promise<
    FormItemSpec<IConnectionDialogProfile>[]
  > {
    return [
      {
        ...
        validate: (value: string) => {
          if (
            this.state.selectedFormTab === FormTabType.ConnectionString &&
            !value
          ) {
            return {
              isValid: false,
  ...
  private registerRpcHandlers() {
    ...
    this.registerReducer("formAction", async (state, payload) => {
      if (payload.event.isAction) {
        ...
        (this.state.connectionProfile[payload.event.propertyName] as any) =
          payload.event.value;
    ...
    this.registerReducer("connect", async (state) => {
      ...
      const usedFields = new Set<keyof IConnectionDialogProfile>(
      ...
      Object.keys(
        this.state.connectionFormComponents.advancedComponents,
      ).forEach((group) => {
        this.state.connectionFormComponents.advancedComponents[group].forEach(
          (c) => {
            if (!c.hidden) {
      ...
      Object.keys(this.state.connectionProfile).forEach((optionName) => {
        if (!usedFields.has(optionName as keyof IConnectionDialogProfile)) {
          (this.state.connectionProfile[
            optionName as keyof IConnectionDialogProfile
          ] as any) = undefined;
        }
      });
