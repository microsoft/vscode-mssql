...
gulp.task("ext:compile-view", (done) => {
    return gulp
        ...
        .on("error", function () {
            if (process.env.BUILDMACHINE) {
                done("Extension view failed to build. See Above.");
                process.exit(1);
            }
        })
        .pipe(srcmap.write(".", { includeContent: false, sourceRoot: "../src" }))
...
gulp.task(
    ...
    gulp.series(async function () {
        try {
            await generateReactWebviewsBundle();
        } catch (err) {
            console.log("Build failed, continuing...", err.message);
            // Error is swallowed here - task will complete successfully
        }
    }),
);
...
gulp.task("ext:copy-systemjs-config", (done) => {
    return gulp
        .src([config.paths.project.root + "/src/oldViews/htmlcontent/*.js"])
        .pipe(gulp.dest("out/src/oldViews/htmlcontent"));
});
...
gulp.task("ext:copy-html", (done) => {
    return gulp
        .src([config.paths.project.root + "/src/extension/controllers/sqlOutput.ejs"])
        .pipe(gulp.dest("out/src/extension/controllers/"));
});

// Copy css
gulp.task("ext:copy-css", (done) => {
    return gulp
        .src([config.paths.project.root + "/src/oldViews/htmlcontent/src/css/*.css"])
...
gulp.task("ext:copy-images", (done) => {
    return gulp
        .src([config.paths.project.root + "/src/oldViews/htmlcontent/src/images/**/*"])
...
gulp.task("ext:copy-dependencies", (done) => {
    gulp.src([config.paths.project.root + "/node_modules/rxjs/**/*"]).pipe(
        gulp.dest("out/src/oldViews/htmlcontent/src/js/lib/rxjs"),
    );

    gulp.src([config.paths.project.root + "/node_modules/angular-in-memory-web-api/**/*"]).pipe(
        gulp.dest("out/src/oldViews/htmlcontent/src/js/lib/angular-in-memory-web-api"),
    );
    ...
    if (min) {
        ...
        gulp.src([config.paths.project.root + "/node_modules/zone.js/**/*"]).pipe(
            gulp.dest("out/src/oldViews/htmlcontent/src/js/lib/zone.js"),
        );
    }
    ...
    gulp.src([config.paths.project.root + "/node_modules/angular2-slickgrid/out/**/*.js"], {
        base: config.paths.project.root + "/node_modules/angular2-slickgrid",
    }).pipe(gulp.dest("out/src/views/htmlcontent/src/js/lib/angular2-slickgrid"));
...
gulp.task("ext:copy-config", () => {
    return gulp
        .src(config.paths.project.root + "/src/extension/configurations/config.json")
        .pipe(gulp.dest(config.paths.project.root + "/out/src/extension"));
});
...
gulp.task("watch-views", function () {
    return gulp.watch(
        ["./src/views/**/*", "./typings/**/*", "./src/shared/**/*"],
        gulp.series("ext:compile-views"),
    );
});
