...
| `stsEnvelopeJournal` | diagnostic               | privacy-safe STS2-derived metrics; raw journals under `artifacts/sts2/` only when `captureSqlText` is explicitly enabled for a diagnostic pass                                                           | local STS build + `STS_ENABLE_STS2=1` (see STS_INSTRUMENTATION.md)                                                                       |
