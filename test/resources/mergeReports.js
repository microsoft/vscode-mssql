...
async function mergeCoverage(e2eFile, unitFile) {
    ...
    try {
        ...
        const linesCovered = calculateMergedValue(unitRoot, e2eRoot, "lines-covered");
        const branchesValid = calculateMergedValue(unitRoot, e2eRoot, "branches-valid");
        const branchesCovered = calculateMergedValue(unitRoot, e2eRoot, "branches-covered");

        // Calculate the line-rate and branch-rate as a weighted average
        const lineRate = calculateWeightedRate(unitRoot, e2eRoot, "line-rate", linesValid);
        const branchRate = calculateWeightedRate(unitRoot, e2eRoot, "branch-rate", branchesValid);
        ...
        const reactPackagesElement = e2eRoot.packages[0]["package"];
        let newPackageElement = undefined;

        if (packages && reactPackagesElement) {
            for (const reactPackageElement of reactPackagesElement) {
                // if the new package element is not found, create a new one
                if (!newPackageElement) {
                    newPackageElement = reactPackageElement;
                    newPackageElement["$"]["name"] = "src.reactviews.pages"; // Update the name attribute
                }
                // add the classes of the current package to the new package element
                else {
                    newPackageElement.classes[0].class.push(
                        ...reactPackageElement.classes[0].class,
                    );
                }
            }
            packages.push(newPackageElement); // Append the updated package element
        }
...
function calculateWeightedRate(unitRoot, e2eRoot, rateKey, validLines) {
    return (
        (parseFloat(unitRoot["$"][rateKey]) * parseInt(unitRoot["$"]["lines-valid"], 10) +
            parseFloat(e2eRoot["$"][rateKey]) * parseInt(e2eRoot["$"]["lines-valid"], 10)) /
        validLines
    );
}
