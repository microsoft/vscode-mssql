...
async function mergeCoverage(e2eFile, unitFile) {
  ...
  try {
    ...
    setMergedValues(unitRoot, linesValid, linesCovered, lineRate, branchesValid, branchesCovered, branchRate);
    ...
    if (packages && reactPackageElement) {
      reactPackageElement["$"]["name"] = "src.views.pages"; // Update the name attribute
...
function setMergedValues(unitRoot, linesValid, linesCovered, lineRate, branchesValid, branchesCovered, branchRate) {
