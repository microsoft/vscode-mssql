...
- **FR-017**: System MUST display each filter popover with: a header "Filter: {ColumnName}", a divider, the filter input controls, Apply and Clear buttons, and another divider. Numeric and Text data filter popovers additionally include an example hint line.
- **FR-018**: System MUST display an example hint at the bottom of numeric filter popovers only (e.g., "Example: Find queries with Duration > 100"), text data shows hint at the bottom: 'Search within TextData text content'. Categorical filters do not display example hints.
- **FR-018a**: System MUST display a placeholder text inside the text filter input field (e.g., "Enter text...") to guide users on the expected input.
- **FR-018b**: System MUST display a "Search values..." placeholder text in the categorical filter search input to help users find values in the checkbox list.
...
- **FR-029**: System MUST display the filtered event count and total event count in the toolbar (e.g., "150 / 1,000 events" when filtered, or "1,000 events" when no filter is active). This provides immediate feedback on how many events match the current filter criteria.
...
- **ColumnFilterCriteria**: Represents a filter applied to a single column, containing the column field, filter type, and filter criteria (selected values for categorical, or operator + value for numeric/date/text).
- **ProfilerColumnDef**: Extended column definition that includes the data type (string, number, datetime) and optional filterMode setting ("categorical" or "text") for string columns.
